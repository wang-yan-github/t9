...
/*
 ...
 @version 1.0
 */
public class T9ReceiveMail {
  /*
   * 方法说明：主方法，接收用户输入的邮箱服务器、用户名和密码 输入参数： 返回类型：
   */
  public static void main(String args[]) {
    try {
      String popServer = "tongda2000.com";   //邮件服务器
      String popUser = "yzq";              //根据自已的用户名修改
      String popPassword = "tongdayzq";          //根据自已的密码修改
  ...
  public static void receive(String popServer, String popUser, String popPassword) {
    ...
    try {
      ...
      for (int msgNum = 0; msgNum < msgs.length; msgNum++) {
        Message msrg = msgs[msgNum];
        Flags flags = msrg.getFlags();
        Flags.Flag[] flagArray = flags.getSystemFlags();
        for (int i = 0; i < flagArray.length; i++) {
          if (flagArray[i] == Flags.Flag.SEEN) {
            System.out.println("the mail has been seen");
          }
        }
  ...
  public static void printMessage(Message message) {
    try {
      ...
      if (content instanceof Multipart) {
        ...
        Multipart mulPart = (Multipart)content;
//        mulPart.
