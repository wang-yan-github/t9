...
public class T9CodeUtil{
  ...
  public static void autoCode(Connection conn, String tableNo, Map req)
    ...
    T9velocityUtil.velocity(result, pojoOutPath, pojoTemplateName, pojoTemlateUrl);
    ...
    request.put("tableNameReal", T9StringFormat.format(tableName));//
    
    T9velocityUtil.velocity(request, actOutPath, actTemplateName, actTemlateUrl);
    //增删改页面index
  ...
  public static Map<String, String> dsTableList(Connection conn)
    ...
    T9ORM orm = new T9ORM();
    Map m =null;
    List<T9DsTable> dsts = orm.loadListSingle(conn, T9DsTable.class, m);
    for (T9DsTable dst : dsts){
