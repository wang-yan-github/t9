package t9.rad.dbexputil.mssql;
...
public class T9ExpDataMsSql {
  private static final String increment = "SEQ_ID";
  private static final String isNull = "YES";
  private static String selfConfigFile = "D:\\project\\t9\\webroot\\t9\\WEB-INF\\config\\selfconfig.properties";
  private static String sysConfigFile = "D:\\project\\t9\\webroot\\t9\\WEB-INF\\config\\sysconfig.properties";
  
  static {
    T9SysProps.setProps(T9ConfigLoader.loadSysProps(sysConfigFile));
    if(new File(selfConfigFile).exists()){
      T9SysProps.addProps(T9ConfigLoader.loadSysProps(selfConfigFile));
    }
  }
  ...
  public static Connection getDbConn(String dbType) throws Exception{
    if("mysql".equals(dbType)){
      return TestDbUtil.getConnection(false, "t9");
    }else {
      return TestDbUtil.getConnection(false, "TEST");
    }
  }
  ...
  public static Connection getDbConn(String dbType,String name) throws Exception{
    if("mysql".equals(dbType)){
      T9DBUtility dUtility = new T9DBUtility("mysql");
      return dUtility.getConnection(false, name);
    }else if("sqlserver".equals(dbType)) {
      T9DBUtility dUtility = new T9DBUtility("sqlserver");
      return dUtility.getConnection(false, name);
    }else {
      T9DBUtility dUtility = new T9DBUtility("oracle");
      return dUtility.getConnection(false, name);
    }
  }
  /**
  ...
  public static void createTableSQL(Connection conn,String  schemaPattern,ArrayList<String> tables,FileOutputStream fio,boolean debug) throws Exception{
    ...
    for (int i = 0; i < tables.size(); i++) {
      ...
      byte[] createBytes = createSql.getBytes(T9Const.DEFAULT_CODE);
  ...
  public static void createDataBase(Connection conn2,String dbName,boolean debug) throws Exception{
    ...
    try {
      ...
      T9DBUtility.close(smt, null, null);
    }
  }
  /**
   * 建表语句
  ...
  public static void createInsertSql(Connection conn,String  schemaPattern, ArrayList<String> tables,FileOutputStream fio,boolean debug) throws Exception{
    for (int i = 0; i < tables.size(); i++) {
      ...
      for (int j = 0; j < temp.size(); j++) {
        byte[] createBytes = temp.get(j).getBytes(T9Const.DEFAULT_CODE);
        fio.write(createBytes);
      }
      fio.flush();
      ...
      while( rs.next() ){
        ...
        for (int j = 1; j <= columnCount; j++) {
          ...
          if(Types.TIMESTAMP == typeInt){
            ...
            String dateTime = (date ==  null) ?null : T9Utility.getDateTimeStr(date);
...
//       }
        ...
        for (int i = 0; i < param.length; i++) {
          ...
          if (Boolean.class.isInstance(pa)) { // 映射boolean类型
            ...
            try {
              ...
              java.sql.Timestamp sqlDate = T9Utility.parseTimeStamp(((java.util.Date)pa).getTime());
  ...
  public static void main(String[] args) throws Exception {
   /* Connection conn = getDbConn();
    ...
    expMySqlData("t9", tables, fio, true);*/
  ...
  public void test(){
    ...
    try {
      String scm = T9SysProps.getProp("db.jdbc.olddbms.dbname");;
      String schemaPattern2 = T9SysProps.getProp("db.jdbc.newdbms.dbname");
      String olddbms = T9SysProps.getProp("db.jdbc.olddbms");;
      String newdbms = T9SysProps.getProp("db.jdbc.newdbms");
