 ...
 *
 */
public class T9OAKnowAnswerLogic{
  /**
   * 查找问题状态
   * @param dbConn
   * @param flag 传进来的问题id
   ...
   * @throws Exception
   */
  public T9OAAsk findAskStatus(Connection dbConn, int askId)throws Exception{
   ...
   * @throws Exception
   */
   public List<T9OAAsk> findRefAsk(Connection dbConn, int  flag)throws Exception{
     PreparedStatement ps = null;
     ResultSet rs = null;      
     ...
     String sql = "select a.SEQ_ID"
       ...
       +",a.CREATE_TIME"
           ...
           +",a.ASK" 
           +",a.ASK_STATUS"
         +" from wiki_ask a, wiki_ask b"
         +" where ( a.RELATED_KEYWOED like " + T9StringUtil.dbLike("b.ASK");
     String key = findKeyWord(dbConn, flag);
     if(T9StringUtil.isNotEmpty(key)){
       for(int i=0; i<key.split(" ").length; i++){//concat(concat('%','"+ key.split(" ")[i] +"'),'%')
         sql += "or a.RELATED_KEYWOED like " + T9StringUtil.dbLike("'"+ T9DBUtility.escapeLike(key.split(" ")[i])+"'") + T9DBUtility.escapeLike();
       }                 
     }    
  ...
  * @throws Exception
  */
  public List<T9AskAnswer> findOtherAnswer(Connection dbConn, int askId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;      
    List<T9AskAnswer> answers= new ArrayList<T9AskAnswer>();
    try{
      String sql = "select answer.ANSWER_CONTENT " +
          ...
          "               ,p.USER_NAME " +
          "               ,answer.ASK_ID " +
          "        from  person p, wiki_ask_answer answer, wiki_ask ask" +
   ...
   * @throws Exception
   */
  public T9AskAnswer findBetterAnswer(Connection dbConn, int askId) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;   
    T9AskAnswer answer = new T9AskAnswer(); 
    String sql = "select answer.ANSWER_CONTENT " +
  ...
  public String findKeyWord(Connection dbConn, int askId)throws Exception{
    PreparedStatement ps = null;
    String sql = "select RELATED_KEYWOED from wiki_ask where seq_id = " + askId;
    ResultSet rs = null;
    //T9Out.println(sql);
    ps = dbConn.prepareStatement(sql);
    rs = ps.executeQuery();
   ...
   * @throws Exception
   */
  public List<T9OAComment> findBetterAnswerPingLun(Connection dbConn, int answerId) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    List<T9OAComment>  comments = new ArrayList<T9OAComment>();
     try{
      String sql = " select p.user_name" +
       		"                 ,comm.MEMBER" +
       		"                 ,comm.COMMENT_TIME" +
       		...
       		"                 ,comm.ASK_ID" +
       		"                 ,comm.SEQ_ID" +
       		"           from person p ,wiki_comment comm ,wiki_ask_answer ans" +
       ...
       rs = ps.executeQuery();
       while(rs.next()){
         T9OAComment comm = new T9OAComment(); 
  ...
  public int  insertAnswer(Connection dbConn, T9AskAnswer ans) throws Exception{
    PreparedStatement ps = null;
    int id = 0;
    try{      
      String sql = "insert into wiki_ask_answer(ASK_ID, ANSWER_USER, ANSWER_TIME, ANSWER_CONTENT, GOOD_ANSWER) values(?,?,"+ T9DBUtility.currDateTime()+",?,?)" ;
  ...
  public int getReplyCount(Connection dbConn, int askId) throws Exception{
    ...
    ResultSet rs = null; 
    try{
      String sql = "select ASK_REPLY_COUNT from wiki_ask where seq_id = " + askId;
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      if(rs.next()){
       return rs.getInt(1);
  ...
  public int updateReplyCount(Connection dbConn, int askId, int count)throws Exception{
    ...
    int row = 0;
    try{
      String sql  = "update wiki_ask set ASK_REPLY_COUNT =  " + (count++) +" where seq_id ="+askId;
      //T9Out.println(sql);
      ps = dbConn.prepareStatement(sql);
   ...
   * @throws Exception
   */
  public int goodAnswerPingLun(Connection dbConn, T9OAComment comm)throws Exception{
    PreparedStatement ps = null;
    int id = 0;
    try{     
      String sql = "insert into wiki_comment(MEMBER, ASK_ID, COMMENTS, COMMENT_TIME) values(?,?,?,"+ T9DBUtility.currDateTime()+")";
  ...
  public int goodAnswer(Connection dbConn,  int answeId)throws Exception{
    ...
    int id = 0;
    try{
      String sql = "update wiki_ask_answer" +              //把answeId的纪录设置为最佳答案
      		"         set GOOD_ANSWER =1" +
  ...
  public int changeStatus(Connection dbConn, int askId)throws Exception{
    ...
    int id = 0;
    try{
      String sql = "update wiki_ask" +
      		"         set ASK_STATUS =1," +
      		"         RESOLUTION_TIME = " + T9DBUtility.currDateTime()  +
      		"         where SEQ_ID = "+askId ;
      //T9Out.println(sql);
      ps = dbConn.prepareStatement(sql);
      id = ps.executeUpdate();
  ...
  */
  public int addFen(Connection dbConn, int userId)throws Exception{
    ...
    try{
      ...
      //T9Out.println(sql);
      ps = dbConn.prepareStatement(sql);
      //T9Out.println(dbConn.getAutoCommit());
      id = ps.executeUpdate();     
    }catch(Exception e){
      throw e;
    }finally{
  ...
  */
  ...
  public int deleteAnswer(Connection dbConn, int answerId ) throws Exception{
    ...
    int id = 0;
    try{
      String sql = "delete from wiki_ask_answer where SEQ_ID = " + answerId;
      ps = dbConn.prepareStatement(sql);     
      id = ps.executeUpdate();
    } catch (Exception e){
      throw e;
  ...
  public int deleteCommet(Connection dbConn, int askId) throws Exception{
    ...
    int id = 0;
    try{
      String sql = "delete from wiki_comment where ASK_ID = " + askId;
      ps = dbConn.prepareStatement(sql);     
      id = ps.executeUpdate();
    }catch(Exception e){
      throw e;
  ...
  public int deleteComment(Connection dbConn,  int commentId)throws Exception{
    ...
    int id = 0;
    try{
      String sql = "delete from wiki_comment where SEQ_ID = " + commentId;
      ps = dbConn.prepareStatement(sql);     
      id = ps.executeUpdate();
    }catch(Exception e){
      throw e;
  ...
  public int changeAnswer(Connection dbConn,  int commentId, String answer) throws Exception{
    PreparedStatement ps = null;
    int id = 0;   
    String sql = "update wiki_ask_answer"
   ...
   * @throws Exception
   */
  public int changeAsk(Connection dbConn, T9OAAsk ask) throws Exception {
    PreparedStatement ps = null;
    int id = 0;
    String sql = "update wiki_ask"
      + " set ASK =?"
  ...
  public int deleteAskById(Connection dbConn, int askId) throws Exception {
    PreparedStatement ps = null;
    int id = 0;
    String sql = "delete from wiki_ask where SEQ_ID = " + askId;
    //T9Out.println(sql);
    try{
      ps = dbConn.prepareStatement(sql);
      id = ps.executeUpdate();
  ...
  public int deleteAnswerById(Connection dbConn, int askId) throws Exception{
    PreparedStatement ps = null;
    int id = 0;
    String sql = "delete from wiki_ask_answer where ASK_ID =" + askId;
    //T9Out.println(sql);
    try{
      ps = dbConn.prepareStatement(sql);
      id = ps.executeUpdate();
