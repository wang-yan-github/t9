 ...
 *
 */
public class T9OAKnowMyPanelLogic{
  /**
   * 查找oa知道的标题
   * @param dbConn
   * @return
   ...
   * @throws Exception
   */
  public int findMyAskCount(Connection dbConn, T9Person user) throws Exception{
    String sql = "select COUNT(*) from wiki_ask where CREATOR = " + user.getSeqId();
    PreparedStatement ps = null;
    ResultSet rs = null; 
    try{
      ps = dbConn.prepareStatement(sql);
   ...
   * @throws Exception
   */
  public List<T9OAAsk> findMyAsks(Connection dbConn, T9Person user, T9PageUtil pu) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    ...
    try{      
      ...
      while(rs.next()){
        ...
        ask.setAsk(T9StringUtil.subString(30, rs.getString(2)));
   ...
   * @throws Exception
   */
  public List<T9OAAsk> findMyReferenceAsks (Connection dbConn, T9Person user, T9PageUtil pu) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    List<T9OAAsk> asks = new ArrayList<T9OAAsk>();   
    ...
    String sql = "select SEQ_ID,CREATE_TIME,ASK_STATUS,ASK from wiki_ask where SEQ_ID in (" + ids +")";
    //T9Out.println(sql);
    try{      
      ps = dbConn.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);    
      ps.setMaxRows(pu.getCurrentPage() * pu.getPageSize());
      ...
      while(rs.next()){
        ...
        ask.setAsk(T9StringUtil.subString(30, rs.getString(4)));
        asks.add(ask);
      }
    }catch(Exception e){
      throw e;
   ...
   * @throws Exception
   */
  public String findMyAskIds(Connection dbConn, T9Person user)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    String ids = "";
    String sql = " select ASK_ID from wiki_ask_answer where ANSWER_USER = "+ user.getSeqId() 
                 +" union "
                 +" select ASK_ID  from WIKI_COMMENT where MEMBER =  " + user.getSeqId();
    try{
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      while(rs.next()){
   ...
   * @throws Exception
   */
  public int findMyReferenceAsksCount(Connection dbConn, T9Person user) throws Exception{  
    String ids = findMyAskIds(dbConn, user);
    int len = 0;
    if(!T9Utility.isNullorEmpty(ids)){
  ...
  public int updateOAName(Connection dbConn, String  oaName) throws Exception{
    PreparedStatement ps = null;
    try{
      String sql = "update wiki_info set SYS_NAME = ?";
      ps = dbConn.prepareStatement(sql);
  ...
  */
  public boolean isNull(Connection dbConn)throws Exception{
    ...
    ResultSet rs = null;
    try{
      String sql = "select  SYS_NAME from wiki_info";
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      if(rs.next()){
        return false;
  ...
  public int insertOaName(Connection dbConn, String  oaName)throws Exception{
    PreparedStatement ps = null;
    try{
      String sql = "insert into wiki_info(SYS_NAME) values(?)";
      ps = dbConn.prepareStatement(sql);
      ps.setString(1, oaName);
      //T9Out.println(sql);
      int k = ps.executeUpdate();
   ...
   * @throws Exception
   */
  public List<T9Person> findPersons(Connection dbConn, String  userKey, T9PageUtil pu) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    ...
    try{
      String sql =    
              ...
              if(T9StringUtil.isNotEmpty(userKey)){
                sql += " and (USER_NAME like '%"+ T9DBUtility.escapeLike(userKey) +"%' or USER_ID like '%"+ T9DBUtility.escapeLike(userKey) +"%')";
              }              
  ...
  public int findPersonsCount(Connection dbConn, String nameKey) throws Exception{
    ...
    String sql = 
        ...
        + "from person "
        +" where NOT_LOGIN!= '1' and DEPT_ID!='0' ";    
    if(T9StringUtil.isNotEmpty(nameKey)){
      sql += "and (USER_NAME like '%"+ T9DBUtility.escapeLike(nameKey) +"%' or USER_ID like '%"+ T9DBUtility.escapeLike(nameKey) +"%')";
    }
    //T9Out.println(sql);
    ps = dbConn.prepareStatement(sql);
    rs = ps.executeQuery();
  ...
  public T9Person findPerson(Connection dbConn, int userId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;  
    String sql = 
   ...
   * @throws Exception
   */
  public int updatePerson(Connection dbConn, T9Person user)throws Exception{
    PreparedStatement ps = null;
    int id =0;
    try{
      String sql = 
...
///////////////////////////////////////////////////////begin////////////////////////// 
  ...
  public int deleteAskAnsOfOther(Connection dbConn, int userId) throws Exception{
    PreparedStatement ps = null;
    int id = 0; 
    String sql = "delete from wiki_ask_answer where ASK_ID in ("
            +"select distinct SEQ_ID from wiki_ask where CREATOR = '"+ userId 
            +"')";
    //T9Out.println(sql);
  try {
    ps = dbConn.prepareStatement(sql);
  ...
  public int deleteAskComment(Connection dbConn, int userId)throws Exception{
    ...
    int id = 0; 
    String sql = 
      "delete from wiki_comment where ASK_ID in ("
      +" select ASK_ID from wiki_ask_answer where ANSWER_USER = '"+ userId +"' and GOOD_ANSWER = '1'"
      +" )";
    try {
      ps = dbConn.prepareStatement(sql);
      id = ps.executeUpdate();
  ...
  public int deleteUserAsk(Connection dbConn, int userId) throws Exception{
    ...
    try {
    String sql = 
      "delete from WIKI_ASK where CREATOR = '" + userId +"'";
    ps = dbConn.prepareStatement(sql);
    id = ps.executeUpdate();
  } catch (Exception e) {
    throw e;
  ...
  public int deleteUsersComment(Connection dbConn, int userId)throws Exception{
    PreparedStatement ps = null;
    int id = 0; 
    String sql = "delete from wiki_comment where MEMBER = '" + userId +"'";
    try {
      ps = dbConn.prepareStatement(sql);
      id = ps.executeUpdate();
    } catch (Exception e) {
   ...
   */
    public int deleteUserAnswes(Connection dbConn, int userId)throws Exception{
      PreparedStatement ps = null;
      int id = 0; 
      String sql = "delete from wiki_ask_answer where ANSWER_USER = '" + userId +"'";
      try {
        ps = dbConn.prepareStatement(sql);
        id = ps.executeUpdate();
      } catch (Exception e) {
