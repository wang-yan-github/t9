 ...
 *
 */
public class T9OAKnowTypeLogic{
   ...
   * @throws Exception
   */
  public  String findAskByType(Connection dbConn, int typeId, int flag, T9PageUtil pu) throws Exception{
    ...
    return T9OAToJsonUtil.toJson(askList);
  }
 ...
 * @throws Exception
 */
  public T9CategoriesType findTypeByTypeId(Connection dbConn, int typeId, int parentId) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    int temp = (parentId == 0)? typeId: parentId;//取父类 
    
    List<T9CategoriesType> typesList = new ArrayList<T9CategoriesType>();
   ...
   * @throws Exception
   */
  public List<T9OAAsk> findAllByTypeId(Connection dbConn, int typeId, int flag) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    List<T9OAAsk> typesList = new ArrayList<T9OAAsk>();    
    try{
      String sql = "select ask.SEQ_ID" +
      		...
      		"               ,ask.ASK" +
      		"               ,ask.CATEGORIE_ID" +
      		"         from  categories_type typ, wiki_ask ask" +
   ...
   * @throws Exception
   */
  public List<T9OAAsk> findAllByTypeIdPage(Connection dbConn, int typeId, int flag, T9PageUtil pu) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    List<T9OAAsk> typesList = new ArrayList<T9OAAsk>();    
    try{
   ...
   * @throws Exception
   */
  public List<T9CategoriesType> findTypseUtil(Connection dbConn, int askId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    List<T9CategoriesType> types = new ArrayList<T9CategoriesType>();    
    String sql = 
      "select SEQ_ID,CATEGORIE_NAME,PEARENT_ID from categories_type"
      +" connect by prior PEARENT_ID = seq_id"
      +" start with SEQ_ID = ("
      +" select distinct T.SEQ_ID"
      +" from categories_type t , wiki_ask q "
   ...
   * @return
   */
  public  T9CategoriesType  findTypeUtil2(Connection dbConn, int askId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    String sql = "select type.SEQ_ID,type.CATEGORIE_NAME,type.PEARENT_ID from categories_type type, wiki_ask q where type.SEQ_ID = q.CATEGORIE_ID and q.SEQ_ID ="+ askId;
    try{
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      if(rs.next()){
   ...
   * @throws Exception
   */
  public List<T9CategoriesType> findTypseUtil3(Connection dbConn, int askId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null; 
    T9CategoriesType sub = findTypeUtil2(dbConn, askId);
    List<T9CategoriesType> types = new ArrayList<T9CategoriesType>();   
    if(sub != null){
      types.add(sub);
      String sql = "select SEQ_ID,CATEGORIE_NAME,PEARENT_ID from categories_type type where SEQ_ID="+ sub.getPearentId(); 
      try{
        ps = dbConn.prepareStatement(sql);
        rs = ps.executeQuery();
        if(rs.next()){
