...
public class T9ServletUtility {
  ...
  public static void setNeedNoCheckUriList(String uriStr) {
    if (T9Utility.isNullorEmpty(uriStr)) {
      return;
    }
  ...
  public static boolean isLoginAction(HttpServletRequest request) {
    ...
    if (qUri.equals(contextPath + "/login.jsp")
        ...
        || qUri.startsWith(contextPath + "/t9/core/funcs/system/act/T9SystemAct/")
        || qUri.startsWith(contextPath + "/t9/core/funcs/system/info/act/T9InfoAct/")
        || qUri.startsWith(contextPath + "/core/funcs/system/info/")) {
      
      return true;
    }
    if (!T9Utility.isNullorEmpty(T9SysProps.getString("NEED_NOT_CHECKED_URI"))) {
      if (needNoCheckUriList.size() < 1) {
        setNeedNoCheckUriList(T9SysProps.getString("NEED_NOT_CHECKED_URI"));
      }
  ...
  public static boolean isGbkCode(HttpServletRequest request) {
    ...
    if (gbkCodeUriList.size() > 0 || !T9Utility.isNullorEmpty(T9SysProps.getString("NEED_GBKCODE_URI"))) {
      if (gbkCodeUriList.size() < 1) {
        setNeedGbkCodeUriList(T9SysProps.getString("NEED_GBKCODE_URI"));
      }
  ...
  public static String getUrlParams(HttpServletRequest request,
    ...
    while (eParamName.hasMoreElements()) {
      ...
      String paramValue = T9Utility.iso88591ToGbk(
  ...
  public static Class applicationClass(String className) throws ClassNotFoundException {
      ...
      if (classLoader == null) {
        classLoader = T9ServletUtility.class.getClassLoader();
      }
...
//    if (T9Utility.isNullorEmpty(msrg)) {
//      return "";
//    }
//    MessageResources messages = null;
//    if (T9Utility.isNullorEmpty(rscId)) {
...
//    return localeMsrg;
  ...
  public static String getParam(HttpServletRequest request, String paramName, String defalutValue) {
    ...
    Map paramMapInner = (Map)request.getAttribute(T9BeanKeys.PARAM_MAP_INNER);
    if (paramMapInner != null) {
      paramValue = (String)paramMapInner.get(paramName);
      if (!T9Utility.isNullorEmpty(paramValue)) {
        return paramValue;
      }
    }
