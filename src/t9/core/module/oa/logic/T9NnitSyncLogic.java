...
public class T9NnitSyncLogic {
  public void editNuit(T9Organization o, Connection conn) throws Exception {
    String cols = "update unit set UNIT_NAME=?, TEL_NO=?, FAX_NO=?, POST_NO=?, ADDRESS=?, URL=?, EMAIL=?, BANK_NAME=?, BANK_NO=? where SEQ_ID = ?";
  ...
  public void addNuit(T9Organization o , Connection conn) throws Exception {
    String cols = "insert into unit (SEQ_ID, UNIT_NAME , TEL_NO, FAX_NO, POST_NO, ADDRESS, URL, EMAIL, BANK_NAME, BANK_NO) values (?,?,?,?,?,?,?,?,?,?)";
    PreparedStatement stm = null;
    try {
      stm = conn.prepareStatement(cols);
