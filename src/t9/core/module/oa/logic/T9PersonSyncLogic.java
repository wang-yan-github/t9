...
public class T9PersonSyncLogic {
  public void editPerson(T9Person o , Connection conn) throws Exception {
    String cols = "update USER SET  USER_ID=?, USER_NAME=?, BYNAME=?, USEING_KEY=?, PASSWORD=?, KEY_SN=?, SECURE_KEY_SN=?, USER_PRIV=?, POST_PRIV=?, POST_DEPT=?, DEPT_ID=?, DEPT_ID_OTHER=?, SEX=?, BIRTHDAY=?, IS_LUNAR=?, TEL_NO_DEPT=?, FAX_NO_DEPT=?, ADD_HOME=?, POST_NO_HOME=?, TEL_NO_HOME=?, MOBIL_NO=?, BP_NO=?, EMAIL=?, OICQ_NO=?, ICQ_NO=?, MSN=?, NICK_NAME=?, AUATAR=?, CALL_SOUND=?, BBS_SIGNATURE=?, BBS_COUNTER=?, DUTY_TYPE=?, LAST_VISIT_TIME=?, SMS_ON=?, MENU_TYPE=?, UIN=?, PIC_ID=?, AUTHORIZE=?, CANBROADCAST=?, DISABLED=?, MOBILE_SP=?, MOBILE_PS1=?, MOBILE_PS2=?, LAST_PASS_TIME=?, THEME=?, SHORTCUT=?, PANEL=?, ONLINE=?, ON_STATUS=?, USER_DEFINE=?, MOBIL_NO_HIDDEN=?, MYTABLE_LEFT=?, MYTABLE_RIGHT=?, EMAIL_CAPACITY=?, FOLDER_CAPACITY=?, USER_PRIV_OTHER=?, USER_NO=?, NOT_LOGIN=?, NOT_VIEW_USER=?, NOT_VIEW_TABLE=?, BKGROUND=?, BIND_IP=?, LAST_VISIT_IP=?, MENU_IMAGE=?, WEATHER_CITY=?, SHOW_RSS=?, MY_RSS=?, REMARK=?, MENU_EXPAND=?, WEBMAIL_CAPACITY=?, WEBMAIL_NUM=?, MY_STATUS=?, SCORE=?, TDER_FLAG=?, LIMIT_LOGIN=?, CONCERN_USER=?, IM_RANGE=?, PHOTO=? WHERE UID=?";
    ...
    try {
      ...
      stm.setString(45,o.getTheme() != null ? o.getTheme() : "");
      ...
      stm.setInt(77,o.getImRange());
      stm.setString(78,o.getPhoto());
      stm.setInt(79,o.getSeqId());
      stm.executeUpdate();
    } catch(Exception ex) {
      throw ex;
    } finally {
  ...
  //  UID, USER_ID, USER_NAME, USER_NAME_INDEX, BYNAME, USEING_KEY, USING_FINGER, PASSWORD, KEY_SN, SECURE_KEY_SN, USER_PRIV, POST_PRIV, POST_DEPT, DEPT_ID, DEPT_ID_OTHER, SEX, BIRTHDAY, IS_LUNAR, TEL_NO_DEPT, FAX_NO_DEPT, ADD_HOME, POST_NO_HOME, TEL_NO_HOME, MOBIL_NO, BP_NO, EMAIL, OICQ_NO, ICQ_NO, MSN, NICK_NAME, AUATAR, CALL_SOUND, BBS_SIGNATURE, BBS_COUNTER, DUTY_TYPE, LAST_VISIT_TIME, SMS_ON, MENU_TYPE, UIN, PIC_ID, AUTHORIZE, CANBROADCAST, DISABLED, MOBILE_SP, MOBILE_PS1, MOBILE_PS2, LAST_PASS_TIME, THEME, SHORTCUT, PORTAL, PANEL, ONLINE, ON_STATUS, USER_DEFINE, MOBIL_NO_HIDDEN, MYTABLE_LEFT, MYTABLE_RIGHT, EMAIL_CAPACITY, FOLDER_CAPACITY, USER_PRIV_OTHER, USER_NO, NOT_LOGIN, NOT_VIEW_USER, NOT_VIEW_TABLE, NOT_SEARCH, BKGROUND, BIND_IP, LAST_VISIT_IP, MENU_IMAGE, WEATHER_CITY, SHOW_RSS, MY_RSS, REMARK, MENU_EXPAND, WEBMAIL_CAPACITY, WEBMAIL_NUM, MY_STATUS, SCORE, TDER_FLAG, LIMIT_LOGIN, CONCERN_USER, PHOTO, IM_RANGE, LEAVE_TIME
    String cols = "insert into USER (UID, USER_ID, USER_NAME, BYNAME, USEING_KEY, PASSWORD, KEY_SN, SECURE_KEY_SN, "
        ...
        +", OICQ_NO, ICQ_NO, MSN, NICK_NAME, AUATAR, CALL_SOUND, BBS_SIGNATURE, BBS_COUNTER"
        ...
        +", THEME, SHORTCUT, PANEL, ONLINE, ON_STATUS, USER_DEFINE, MOBIL_NO_HIDDEN"
    ...
    try {
      ...
      stm.setInt(78,o.getImRange());
      stm.setString(79,o.getPhoto());
      stm.executeUpdate();
    } catch(Exception ex) {
      throw ex;
    } finally {
