...
public class T9DepartmentSyncLogic {
  public void editDepartment(T9Department o , Connection conn) throws Exception {
    String cols = "update DEPARTMENT set DEPT_NAME=?, TEL_NO=?, FAX_NO=?, DEPT_NO=?, DEPT_PARENT=?, MANAGER=?, LEADER1=?, LEADER2=?, DEPT_FUNC=? where DEPT_ID = ?";
  ...
  public void addDepartment(T9Department o , Connection conn) throws Exception {
    String cols = "insert into DEPARTMENT (DEPT_ID, DEPT_NAME, TEL_NO, FAX_NO, DEPT_NO, DEPT_PARENT, MANAGER, LEADER1, LEADER2, DEPT_FUNC ) values (?,?,?,?,?,?,?,?,?,?)";
    PreparedStatement stm = null;
    try {
      stm = conn.prepareStatement(cols);
