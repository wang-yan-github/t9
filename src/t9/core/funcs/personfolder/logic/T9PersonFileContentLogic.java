...
public class T9PersonFileContentLogic {
	private static Logger log = Logger.getLogger("t9.core.funcs.personfolder.logic.T9PersonFileContentLogic");
	 ...
	 * @throws Exception
	 */
	public String uploadFileLogic(Connection dbConn, T9FileContent content, T9FileUploadForm fileForm,T9Person loginUser,String seqId,String filePath) throws Exception {
		// 获取登录用户信息
	...
	public T9FileContent getFileInfoByMap(Connection dbConn, Map map) throws NumberFormatException, Exception {
		T9ORM orm = new T9ORM();
		return (T9FileContent) orm.loadObjSingle(dbConn, T9FileContent.class, map);
	}
	...
	// String filePath = T9SysProps.getAttachPath() + separator + "file_folder" +
	// separator + currDate;
	//
	// T9InnerEMailUtilLogic emut = new T9InnerEMailUtilLogic();
	// String rand = emut.getRandom();
	...
	public boolean updateSingle(Connection dbConn, T9FileContent fileContent) {
		boolean flag = false;
					...
					for (int i = 0; i < attIdArray.length; i++) {
						...
						if (map.size() != 0) {
							...
							for (String keySet : set) {
								...
								if (file.exists()) {
									T9FileUtility.copyFile(file.getAbsolutePath(), filePath + File.separator + currDate + File.separator + newAttaName);
									randFlag += currDate + "_" + rand + ",";
									newAttName += keyValue + "*";
									isHave = true;
								} else if (oldFile.exists()) {
									T9FileUtility.copyFile(oldFile.getAbsolutePath(), filePath + File.separator + currDate + File.separator + newAttaName);
									randFlag += currDate + "_" + rand + ",";
									newAttName += keyValue + "*";
									isHave = true;
								}
	...
	public void cutFile(Connection dbConn, String seqIdStrs, String sortId, String filePath, int loginUserSeqId) throws NumberFormatException,
			Exception {
		// T9FileUtility.xcopyFile(srcPath + fileName, desPath + fileName);
		Date date = new Date();
		SimpleDateFormat format = new SimpleDateFormat("yyMM");
		String currDate = format.format(date);
		T9InnerEMailUtilLogic emut = new T9InnerEMailUtilLogic();
					...
					for (int i = 0; i < attIdArray.length; i++) {
						...
						if (map.size() != 0) {
							...
							for (String keySet : set) {
								...
								if (file.exists()) {
									T9FileUtility.xcopyFile(file.getAbsolutePath(), filePath + File.separator + currDate + File.separator + newAttaName);

									randFlag += currDate + "_" + rand + ",";
									newAttName += keyValue + "*";
									isHave = true;
								} else if (oldFile.exists()) {
									T9FileUtility.xcopyFile(oldFile.getAbsolutePath(), filePath + File.separator + currDate + File.separator + newAttaName);
								...
								// T9FileUtility.xcopyFile(filePath + File.separator + fileFolder
