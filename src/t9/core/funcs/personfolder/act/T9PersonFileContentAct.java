...
import t9.core.funcs.personfolder.data.T9Page;
import t9.core.funcs.personfolder.logic.T9PersonFileContentLogic;
...
public class T9PersonFileContentAct {
...
//					contentStr = contentStr.replace("\"", "\\\"");
					...
					sb.append(",attachmentName:\"" + T9Utility.encodeSpecial(attachmentName) + "\"");
					sb.append(",attachmentDesc:\"" + T9Utility.encodeSpecial(attchmentDesc) + "\"");
					sb.append(",userId:\"" + T9Utility.null2Empty(content.getUserId()) + "\"");
					...
					sb.append(",readers:\"" + T9Utility.encodeSpecial(T9Utility.null2Empty(content.getReaders())) + "\"");
					sb.append(",creater:\"" + T9Utility.encodeSpecial(T9Utility.null2Empty(content.getCreater())) + "\"");

					String logStr = T9Utility.null2Empty(content.getLogs());
					logStr.replaceAll("[\n-\r]", "<br>");
		...
		try {
			...
			if (fileContents.size() > 0) {
				...
				for (int i = (int) first; i < last; i++) {
					...
					String YM = contentLogic.getFilePathFolder(T9Utility.null2Empty(content.getAttachmentId()));
					String attachmentNameStr = T9Utility.null2Empty(content.getAttachmentName());
	...
	public String getPersonContentsById(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// List<T9FileContent> contentList =
	...
	public String isHaveFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		T9PersonFileContentLogic logic = new T9PersonFileContentLogic();
	...
	public String getFileContentInfoById(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String createName = logic.getPersonNamesByIds(dbConn, T9Utility.null2Empty(fileContent.getCreater()));
...
//			contentStr = contentStr.replace("\"", "\\\"");
	...
	public String newFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			T9FileContent fileContent = logic.getFileInfoByMap(dbConn, map2);
	...
	public String updateFileInfoById(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Connection dbConn = null;
		T9PersonFileContentLogic fileContentLogic = new T9PersonFileContentLogic();
		String contentIdStr = request.getParameter("contentId");
		...
		try {
			T9RequestDbConn requestDbConn = (T9RequestDbConn) request.getAttribute(T9BeanKeys.REQUEST_DB_CONN_MGR);
			dbConn = requestDbConn.getSysDbConn();
			// T9FileContent content = fileContentLogic.getFileContentInfoById(dbConn,
			// contentId);
			Map map2 = new HashMap();
			map2.put("SEQ_ID", contentId);
			// map2.put("USER_ID", loginUserSeqId);
			T9FileContent content = fileContentLogic.getFileInfoByMap(dbConn, map2);
    ...
    if (T9Utility.isNullorEmpty(managerPriSeqId)) {
      managerPriSeqId= "0";
    }
		...
		try {
			...
			if (flag) {
				...
				if ("edit".equals(actionFlag.trim())) {
					...
					content.setAttachmentId(T9Utility.null2Empty(attIdString).trim() + T9Utility.null2Empty(attachmentId).trim() + T9Utility.null2Empty(newAttchId).trim());
	...
	public String shareEdiSingleUpload(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (flag) {
				...
				content.setAttachmentId(attIdString.trim() + T9Utility.null2Empty(attachmentId).trim());
				content.setAttachmentName(attNameString.trim() + T9Utility.null2Empty(attachmentName).trim());
				content.setSendTime(T9Utility.parseTimeStamp());
				content.setSubject(subject1);
				content.setContent(content1);
				content.setAttachmentDesc(attachmentDesc1);
	...
	public String createFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			request.setAttribute(T9ActionKeys.RET_STATE, T9Const.RETURN_OK);
			request.setAttribute(T9ActionKeys.RET_MSRG, "成功存取数据！");
	...
	public String resendTo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			T9RequestDbConn requesttDbConn = (T9RequestDbConn) request.getAttribute(T9BeanKeys.REQUEST_DB_CONN_MGR);
			dbConn = requesttDbConn.getSysDbConn();
			T9FileContent userContent = new T9FileContent();
			...
			if (seqIdArry.length != 0) {
				for (String seqIdStr : seqIdArry) {
					T9FileContent content = logic.getFileContentInfoById(dbConn, Integer.parseInt(seqIdStr));
					if (content!=null) {
						String newSubject = "";
						String subject = T9Utility.null2Empty(content.getSubject())+ "由(" + loginName + "转发)";
						...
						if (userIdArry.length != 0) {
							for (String userIdStr : userIdArry) {
								String[] attIdArray = {};
								String[] attNameArray = {};
								String attachmentId = T9Utility.null2Empty(content.getAttachmentId());
								String attachmentName = T9Utility.null2Empty(content.getAttachmentName());
								...
								for (int i = 0; i < attIdArray.length; i++) {
									...
									if (map.size() != 0) {
										...
										for (String keySet : set) {
											...
											if (file.exists()) {
												T9FileUtility.copyFile(file.getAbsolutePath(), filePath + currDate + File.separator + newAttaName);
												isHave = true;
											} else if (oldFile.exists()) {
												T9FileUtility.copyFile(oldFile.getAbsolutePath(), filePath + currDate + File.separator + newAttaName);
												isHave = true;
											}
											if (isHave) {
								...
								userContent.setSendTime(T9Utility.parseTimeStamp());
								...
								T9FileContent fileContent = logic.getMaxSeqId(dbConn);
								maxSeqId = fileContent.getSeqId();
								sentFlag = 1;
							}
							String smsContent = loginName + "给你转发文件:" + T9Utility.null2Empty(newSubject);
							// 短信提醒
							if ("smsResend".equals(smsResend.trim())) {

								T9PersonFolderLogic filFolderLogic = new T9PersonFolderLogic();
								T9FileContent newContent = logic.getFileContentInfoById(dbConn, maxSeqId);
								...
								sms.setSmsType(T9LogConst.PERSONAL_FOLD);
	...
	public String transferFolder(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		T9PersonFileContentLogic logic = new T9PersonFileContentLogic();
		String separator = File.separator;
		String filePath = T9SysProps.getAttachPath() + separator + module + separator;
