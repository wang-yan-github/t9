...
import t9.core.util.mail.T9MailAuthenticator;
 ...
 * 
 */
public class T9MailSmtpUtil {
  public static boolean sendWebMail(Properties p, String userName,
      String password, boolean isValidate, T9WebmailBody wmb,String attachPath) throws Exception {
    T9MailAuthenticator authenticator = null;
    Properties pro = p;
    p.put("mail.smtp.auth","true");
    // 如果需要身份认证，则创建一个密码验证器
    if (isValidate) {
      authenticator = new T9MailAuthenticator(userName, password);
    }
    ...
    try {
     ...
     // Address to = new InternetAddress(wmb.getToMail());
      ...
      wmb.setToMailCopy(T9Utility.null2Empty(wmb.getToMailCopy()));
      wmb.setToMailSecret(T9Utility.null2Empty(wmb.getToMailSecret()));
     ...
     // mailMessage.setSubject(wmb.getSubject());
      ...
      // 设置附件
      mainPart = addFileAffixs(mainPart, attachPath, wmb.getAttachmentName(), wmb.getAttachmentId(), T9EmailCont.MODULE);
  ...
  public static InternetAddress[] getIa(String toMail) throws Exception {
    if (T9Utility.isNullorEmpty(toMail)) {
      return null;
    }
 ...
 * @throws Exception 
 */
  public static boolean sendWebMail(T9Webmail wm, T9WebmailBody wmb,String attachPath) throws Exception {
