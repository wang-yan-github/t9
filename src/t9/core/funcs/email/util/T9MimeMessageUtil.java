 ...
 * 
 */
public class T9MimeMessageUtil {
  /**
   * 判断邮件中是否含有附件
   * 
   * @param msg
          ...
          for (int k = 1; count > 1 && k < count; k++) {
            ...
            if (attname != null) {
              ...
              if (Content_ID != null && Content_ID.length > 0) {
                ...
                String path = "/t9/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?attachmentName="+ attname+ "&attachmentId="+ attid +"&module=email";
    ...
    if (part.isMimeType("multipart/*")) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if ((disposition != null)
            && ((disposition.equals(Part.ATTACHMENT)) || (disposition
                .equals(Part.INLINE)))) {
          fileName = T9Utility.null2Empty(mpart.getFileName());
          ...
          if ((fileName != null)
            ...
            String[] re = saveFile(fileName, mpart.getInputStream(),
                attachPath);
            //if (T9Utility.isNullorEmpty(cid_name)) {
              //cid_name = re[0];
            //}
            attachMap.put(re[0], re);
          }
  ...
  public static String[] saveFile(String fileName, InputStream in,
    ...
    // 保存附件到email模块
    T9InnerEMailLogic iemu = new T9InnerEMailLogic();
