package t9.core.funcs.office.ntko.logic;
...
import t9.core.funcs.office.ntko.data.T9OcLog;
...
public class T9NtkoLogic {
   ...
   * @throws Exception
   */
  public String getWordModel(Connection conn,T9Person person) throws Exception{
    String[] filters = null;
    T9ORM orm = new T9ORM();
    String result = "";
    ArrayList<T9WordModel> wmlist = (ArrayList<T9WordModel>) orm.loadListSingle(conn, T9WordModel.class, filters);
    for (T9WordModel wm : wmlist) {
      String privStr = wm.getPrivStr();
      if(isRuleWordModel(privStr,person)){
        String filed = "{attachmentId:\"" + wm.getAttachmentId() + "\""
          + ",docName:\"" + T9Utility.encodeSpecial(wm.getModelName()) + "\""
          + ",attachmentName:\"" + T9Utility.encodeSpecial(wm.getAttachmentName()) + "\"}";
        if(!"".equals(result)){
          result += ",";
        }
        result += filed;
   ...
   * @return
   */
  public boolean isRuleWordModel(String privStr,T9Person person){
    boolean bool = false;
    if(privStr != null && !"".equals(privStr)){
      ...
      if(T9NtkoCont.ALL_DEPT.equals(privDept) 
          ...
          || !T9WorkFlowUtility.checkId(privRole, person.getUserPrivOther(), true).equals("")
          || !T9WorkFlowUtility.checkId(privDept, person.getDeptIdOther(), true).equals("")
          ){
        bool = true;
      }
  ...
  public String isCanEditLogic(Connection conn , int userId,String attrId) throws Exception{
    String result = "";
    String sysTimeStr = T9SysProps.getProp("$ATTACH_LOCK_REF_SEC");
    long timeTemp = 0l;
    try {
  ...
  public void saveAttrEdit(Connection conn, String attrId,int userId,Date lastVisit) throws Exception{
    String sql = "insert into ATTACHMENT_EIDT (ATTACHMENT_ID,USER_ID,LAST_VISIT) values(?,?,?)";
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
      ...
      ps.setInt(2, userId);
      ps.setTimestamp(3,T9Utility.parseTimeStamp(T9Utility.getDateTimeStr(lastVisit)));
      ps.executeUpdate();
    } catch (Exception e) {
      throw e;
    } finally {
  ...
  public void updateAttrEdit(Connection conn, String attrId,int userId,Date lastVisit) throws Exception{
    String sql = "update ATTACHMENT_EIDT set USER_ID = ? ,LAST_VISIT = ? where ATTACHMENT_ID ='" + attrId + "'";
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
      ps.setInt(1, userId);
      ps.setTimestamp(2,T9Utility.parseTimeStamp(lastVisit.getTime()));
      ps.executeUpdate();
    } catch (Exception e) {
      throw e;
    } finally {
  ...
  public String getLogList(Connection conn,Map request) throws Exception{
    ...
    String sql =  "select SEQ_ID,LOG_TIME,LOG_UID,LOG_TYPE,LOG_IP from OC_LOG where 1=1 AND ATTACH_ID='" + attachId + "'";
