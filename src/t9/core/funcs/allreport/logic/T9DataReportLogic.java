...
import t9.core.funcs.allreport.data.T9AllReport;
import t9.core.funcs.allreport.data.T9Field;
import t9.core.funcs.allreport.data.T9JionCondition;
import t9.core.funcs.allreport.data.T9LoadConfigFile;
...
public class T9DataReportLogic {
    ...
    String dd="";
    try{
     String sql="select * from all_report_priv where r_id="+id+" and "+T9DBUtility.findInSet(person.getSeqId()+"", "user_str");
     ...
     T9AllReport report=new T9AllReport();
     T9ORM orm=new T9ORM();
     report=(T9AllReport)orm.loadObjSingle(conn,T9AllReport.class,Integer.parseInt(id));
  ...
  public String getItemDataType(Connection conn,String mId,String item)throws Exception{
    ...
    try{
      ...
      String configFile=T9SysProps.getString("allreportconfig");
      configFile+=data;
      T9LoadConfigFile loadconfigfile=new T9LoadConfigFile(configFile);
      List<T9Field> fieldList= loadconfigfile.getFieldList();
      for(int i=0;i<fieldList.size();i++){
        T9Field field=fieldList.get(i);
  ...
  public String getTableListLogic(Connection conn,T9Person user,Map request)  throws Exception{
    ...
    try{
      ...
      T9LoadConfigFile configFile=this.getFieldListbyMid( conn, mId+"");
      List<T9Field> listFields=new ArrayList();
      List<T9JionCondition> listConditions=new ArrayList();
   ...
   //   String groupField= getGroupField(group_field,listFields);
    ...
    //  itemsAndFromStr+=","+groupField.substring(0, groupField.indexOf("."));
     ...
     //  dataList=this.getGroupCalListLgic(list_item,dataList);
      ...
      data="{table:'"+T9Utility.encodeSpecial(data)+"'}";
    }catch(Exception e){
      throw e;
    }finally{
      T9DBUtility.close(stmt, rs, null);
   ...
   * 
   * */
  public List<List<String>> getExcelListLogic(Connection conn,T9Person user,Map request)  throws Exception{
    Statement stmt=null;
    ResultSet rs=null;
    List<List<String>> dataList=new ArrayList();
   ...
   * @throws Exception
   */
  public List<String> getExcTableHead(String type,String items,List<T9Field> listFields)throws Exception{
...
//     }
  ...
  public String getItemData(Connection conn,ResultSet rs,int index,String itemStr,List<T9Field> listField)throws Exception{
    String itemData="";
    String itemOrg[]=itemStr.split(",");
    String item=itemOrg[index];
    for(int i=0;i<listField.size();i++){
      T9Field field=listField.get(i);
      if("1".equals(item))
      {
        item="*";
      }
      ...
      if(item.trim().equals(field.getItem().trim())){
        ...
        if(!"".equals(field.getSelectItem())){
           if("self".equals(field.getSelectItem())){
             ...
             try{
               String sql="select class_desc from code_item where class_no='"+selectItem+"' and sort_no='"+itemData+"' ";
               Statement stmt=conn.createStatement();
               ResultSet rs1=stmt.executeQuery(sql);
               if(rs1.next()){
                 itemData=rs1.getString("class_desc");
               }
                T9DBUtility.close(stmt, rs1, null);
             }catch(Exception ex){
               ex.printStackTrace();
             }finally{
  ...
  public String dealForignKey(Connection conn,T9Field field,String data)throws Exception{
    Statement stmt=null;
    ResultSet rs=null;
    String keyData="";
    try{
  ...
  public String getTableHead(String type,String items,List<T9Field> listFields)throws Exception{
    String headLine="<tr>";
    String itemOrg[]=items.split(",");
    for(int i=0;i<itemOrg.length;i++){
      if(!"".equals(itemOrg[i])){
  ...
  public String getQueryStr(List<T9Field> listFields,Map request,String query )throws Exception{
    String queryStr="";
    ...
    for(int i=0;i<que.length;i++){
      if(!"".equals(que[i])){
        ...
        if("DATE".equals(getDataType(item,listFields))){
          ...
          queryStr+=" and ";
          queryStr+=T9DBUtility.getDateFilter(this.getGroupField(item, listFields), value, ">=");  
          }
          ...
          if(!T9Utility.isNullorEmpty(value)){
          queryStr+=" and ";
          queryStr+=T9DBUtility.getDateFilter(this.getGroupField(item.substring(0, item.length()-1), listFields), value, "<=");  
           }
          }
        else {
          String value = request.get(item) == null ? null : ((String[]) request.get(item))[0];
  ...
  public String getDataType(String item,List<T9Field> listFields)throws Exception{
   String result="";
    for(int i=0;i<listFields.size();i++){
      T9Field field=listFields.get(i);
      if(item.equals(field.getItem()) ){
  ...
  public String getGroupField(String group_field,List<T9Field> listFields)throws Exception{
    String items="";
    for(int x=0;x<listFields.size();x++){
      T9Field field=listFields.get(x);
  ...
  public String getJionConditions(String itemsAndFromStr,List<T9JionCondition> listConditions)throws Exception{
   ...
   for(int i=0;i<listConditions.size();i++){
     T9JionCondition condition=listConditions.get(i);
  ...
  public String[] getSelectItems(String list_item,List<T9Field> listFields)throws Exception{
    String items="";
    int num=0;
    String itemNum="";
    ...
    for(int i=0;i<listItem.length;i++){
      ...
      if(!T9Utility.isNullorEmpty(itemOrg)){
  ...
  public T9LoadConfigFile getFieldListbyMid(Connection conn,String mId)throws Exception{
    Statement stmt=null;
    ResultSet rs=null;
    T9LoadConfigFile file=new T9LoadConfigFile();
    try{
      ...
      String configPath=T9SysProps.getString("allreportconfig");
      configPath+=configFile;
     file=new T9LoadConfigFile(configPath);
 ...
 * @throws Exception
 */
  public ArrayList<T9DbRecord> convertList( List<List<String>> list)throws Exception{
