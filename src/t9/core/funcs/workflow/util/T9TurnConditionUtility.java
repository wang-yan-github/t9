...
import t9.core.funcs.workflow.util.sort.T9UserSortComparatorUtility;
...
public class T9TurnConditionUtility {
    /**
     * 主要用于子流程返回父流程
     * 
     * @param user
     ...
     * @throws Exception
     */
    public String userSelect2(Connection conn, T9Person user, String prcsBack, int flowId, String autoUserOp,
            String ids, Map map) throws Exception {
        String prcsOpUser = "";
        ...
        String query2 = "SELECT * from FLOW_PROCESS where FLOW_SEQ_ID='" + flowId + "' and PRCS_ID='"
                + prcsBack + "'";
        Statement stm2 = null;
        ResultSet rs2 = null;
        ...
        if (!T9Utility.isNullorEmpty(ids) && !T9Utility.isNullorEmpty(autoUserOp)) {
            if (autoUserOp.endsWith(",")) {
                autoUserOp = prcsOpUser.substring(0, autoUserOp.length() - 1);
            }
            T9ORM orm = new T9ORM();
            T9Person person = (T9Person) orm
                    .loadObjSingle(conn, T9Person.class, Integer.parseInt(autoUserOp));
            if (person != null) {
                ...
                if ("ALL_DEPT".equals(prcsDept) || "0".equals(prcsDept)
                        || T9WorkFlowUtility.findId(prcsUser, prcsNewUserId)
                        || T9WorkFlowUtility.findId(prcsDept, String.valueOf(prcsNewDeptId))
                        || T9WorkFlowUtility.findId(prcsPriv, prcsNewUserPriv)
                        || T9WorkFlowUtility.privOther(prcsPriv, prcsNewUserPrivOther) == 1) {
        ...
        if (!"".equals(ids)) {
            String findSql = "SELECT SEQ_ID,DEPT_ID,USER_PRIV,USER_NAME,USER_PRIV_OTHER from PERSON where SEQ_ID IN ("
                    + ids + ")";
            Statement stm9 = null;
            ResultSet rs9 = null;
            try {
                ...
                while (rs9.next()) {
                    ...
                    if ("ALL_DEPT".equals(prcsDept) || "0".equals(prcsDept)
                            || T9WorkFlowUtility.findId(prcsUser, userId)
                            || T9WorkFlowUtility.findId(prcsDept, String.valueOf(deptId))
                            || T9WorkFlowUtility.findId(prcsPriv, userPriv)
                            || T9WorkFlowUtility.privOther(prcsPriv, userPrivOther) == 1) {
                        prcsUserAuto += userId + ",";
        ...
        sb.append("isAutoSelect:" + isAutoSelect + ",prcsOpUser:'" + prcsOpUser + "',prcsOpUserName:'"
                + prcsOpUserName + "',prcsUser:'" + prcsUserAuto + "',prcsUserName:'" + prcsUserName + "'");
        if (map != null) {
            map.put("isAutoSelect", isAutoSelect);
            map.put("prcsOpUser", prcsOpUser);
            map.put("prcsOpUserName", prcsOpUserName);
            map.put("prcsUser", prcsUserAuto);
            map.put("prcsUserName", prcsUserName);
        }
        return sb.toString();
    }
     ...
     * @throws Exception
     */
    public String userSelect(T9Person user, T9FlowProcess fp, T9FlowRun flowRun, Connection conn, Map map2)
            throws Exception {
        T9ORM orm = new T9ORM();
        StringBuffer sb = new StringBuffer();
        String prcsUser = fp.getPrcsUser() == null ? "" : fp.getPrcsUser();
        String prcsDept = T9OrgSelectLogic.changeDept(conn, fp.getPrcsDept());
        ...
        if (fp.getChildFlow() != 0) {
            ...
            if (prcs != null) {
                return this.userSelect(user, prcs, null, conn, map2);
            } else {
                sb.append("isAutoSelect:" + isAutoSelect + ",prcsOpUser:'" + prcsOpUser
                        + "',prcsOpUserName:'" + prcsOpUserName + "',prcsUser:'" + prcsUserAuto
                        + "',prcsUserName:'" + prcsUserName + "'");
                if (map2 != null) {
                    map2.put("isAutoSelect", isAutoSelect);
                    map2.put("prcsOpUser", prcsOpUser);
                    map2.put("prcsOpUserName", prcsOpUserName);
                    map2.put("prcsUser", prcsUserAuto);
                    map2.put("prcsUserName", prcsUserName);
                }
                return sb.toString();
            }
        }
        ...
        if (fp.getAutoType() != null && fp.getAutoType().equals("1")) {
            if (flowRun != null) {
                ...
                T9Person person = (T9Person) orm.loadObjSingle(conn, T9Person.class, userId);
                if (person != null) {
            ...
            // 是否有经办权限
            if ("ALL_DEPT".equals(prcsDept) || T9WorkFlowUtility.findId(prcsUser, prcsNewUserId)
                    || T9WorkFlowUtility.findId(prcsDept, String.valueOf(prcsNewDeptId))
                    || T9WorkFlowUtility.findId(prcsPriv, prcsNewUserPriv)
                    || T9WorkFlowUtility.privOther(prcsDept, prcsNewUserDeptOther) == 1
                    || T9WorkFlowUtility.privOther(prcsPriv, prcsNewUserPrivOther) == 1) {
                prcsOpUser = prcsNewUserId;
                prcsOpUserName = prcsNewUserName;
            ...
            if (autoUser != null && !"".equals(autoUser)) {
                if (autoUserOp != null && !"".equals(autoUserOp)) {
                    ...
                    T9Person person = (T9Person) orm.loadObjSingle(conn, T9Person.class,
                            Integer.parseInt(autoUserOp));
                    if (person != null) {
                        prcsNewUserId = autoUserOp;
                        prcsNewUserName = person.getUserName();
                        prcsNewDeptId = person.getDeptId();
                ...
                // String findSql = T9WorkFlowUtility.createFindSql("SEQ_ID",
                // fp.getAutoUser());
                String ids = fp.getAutoUser();
                if (ids.endsWith(",")) {
                    ids = ids.substring(0, ids.length() - 1);
                }
                String findSql = "SELECT SEQ_ID,DEPT_ID,USER_PRIV,USER_NAME,USER_PRIV_OTHER, DEPT_ID_OTHER from PERSON where SEQ_ID IN ("
                        + ids + ")";
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        if ("ALL_DEPT".equals(prcsDept) || T9WorkFlowUtility.findId(prcsUser, userId)
                                || T9WorkFlowUtility.findId(prcsDept, String.valueOf(deptId))
                                || T9WorkFlowUtility.findId(prcsPriv, userPriv)
                                || T9WorkFlowUtility.privOther(prcsDept, deptOther) == 1
                                || T9WorkFlowUtility.privOther(prcsPriv, userPrivOther) == 1) {
                            prcsUserAuto += userId + ",";
                            prcsUserName += userName + ",";
                        }
            ...
            // 自动选择部门
        } else if ("2".equals(fp.getAutoType()) || "4".equals(fp.getAutoType())
                || "5".equals(fp.getAutoType()) || "6".equals(fp.getAutoType())) {
            if (flowRun != null) {
                ...
                if (autoBaseUser != 0) {
                    // 查出争对步骤的主办人员
                    String query = "select " + " USER_ID" + " FROM FLOW_RUN_PRCS" + " WHERE RUN_ID ="
                            + flowRun.getRunId() + " AND FLOW_PRCS = '" + fp.getAutoBaseUser() + "'"
                            + " AND OP_FLAG = 1 " + " ORDER BY PRCS_ID ";
                    String baseUserId = "";
                    Statement stm = null;
                    ResultSet rs = null;
                    try {
                    ...
                    if (!T9Utility.isNullorEmpty(baseUserId)) {
                        query = "select " + " DEPT_ID " + " FROM PERSON WHERE " + " SEQ_ID=" + baseUserId;
                        Statement stm2 = null;
                        ResultSet rs2 = null;
                        try {
                            stm2 = conn.createStatement();
                ...
                if ("2".equals(fp.getAutoType())) {
                    ...
                    T9PrcsRoleUtility util = new T9PrcsRoleUtility();
                ...
                T9Department dept = deptLogic.getDepartmentById(tmpDeptId, conn);
                String manager = "";
                if (dept != null && dept.getManager() != null) {
                    manager = dept.getManager();
                }
                if ("4".equals(fp.getAutoType()) || "6".equals(fp.getAutoType())) {
                    String query = "SELECT LEADER1,LEADER2 FROM DEPARTMENT WHERE SEQ_ID='" + baseDeptId + "'";
                    Statement stm2 = null;
                    ResultSet rs2 = null;
                    String leader1 = "";
                    String leader2 = "";
                    ...
                    if (!T9Utility.isNullorEmpty(leader1) && "4".equals(fp.getAutoType())) {
                        manager = leader1;
                    }
                    if (!T9Utility.isNullorEmpty(leader2) && "6".equals(fp.getAutoType())) {
                        manager = leader2;
                    }
                }
                ...
                if (!"".equals(manager) && manager != null) {
                    // 以后做的
                    String query = T9WorkFlowUtility.createFindSql("SEQ_ID", manager);
                    query = "SELECT " + " SEQ_ID" + " ,DEPT_ID" + " ,USER_PRIV" + " ,USER_NAME "
                            + " ,USER_PRIV_OTHER" + ", DEPT_ID_OTHER" + " FROM PERSON  WHERE  " + query;
                    ...
                    try {
                        ...
                        while (rs5.next()) {
                            ...
                            if ("ALL_DEPT".equals(prcsDept)
                                    || T9WorkFlowUtility.findId(prcsUser, String.valueOf(userId))
                                    || T9WorkFlowUtility.findId(prcsDept, String.valueOf(deptId))
                                    || T9WorkFlowUtility.findId(prcsPriv, userPriv)
                                    || T9WorkFlowUtility.privOther(prcsDept, deptOther) == 1
                                    || T9WorkFlowUtility.privOther(prcsPriv, userPrivOther) == 1) {
                    ...
                    // 注意 tmpDeptId
                    String query = "SELECT " + " p.SEQ_ID as userId" + ",p.USER_NAME" + ",p.USER_PRIV_OTHER "
                            + ",u.SEQ_ID " + ", p.DEPT_ID_OTHER"

                            + " from PERSON p,USER_PRIV u where" + " p.USER_PRIV = u.SEQ_ID "
                            + " and DEPT_ID = " + tmpDeptId + " and p.SEQ_ID !=" + user.getSeqId()
                    ...
                    try {
                        ...
                        while (rs3.next()) {
                            ...
                            if ("ALL_DEPT".equals(prcsDept) || T9WorkFlowUtility.findId(prcsUser, userId)
                                    || T9WorkFlowUtility.findId(prcsDept, String.valueOf(user.getDeptId()))
                                    || T9WorkFlowUtility.findId(prcsPriv, userPriv)
                                    || T9WorkFlowUtility.privOther(prcsDept, deptOther) == 1
                                    || T9WorkFlowUtility.privOther(prcsPriv, userPrivOther) == 1) {
            ...
            if (flowRun != null) {
                String formUserStr = "";
                if (T9WorkFlowUtility.isNumeric(fp.getAutoUser())) {
                    String itemData = "";
                    if (!T9WorkFlowUtility.isSave2DataTable()) {
                        Map map = new HashMap();
                        map.put("RUN_ID", flowRun.getRunId());
                        map.put("ITEM_ID", Integer.parseInt(fp.getAutoUser()));
                        T9FlowRunData frd = (T9FlowRunData) orm.loadObjSingle(conn, T9FlowRunData.class, map);
                        if (frd != null) {
                        ...
                        String tableName = T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE
                                + flowRun.getFlowId() + "_" + formSeqId;

                        String query2 = "select DATA_" + Integer.parseInt(fp.getAutoUser()) + " from "
                                + tableName + " where RUN_ID =" + flowRun.getRunId();
                        Statement stm8 = null;
                        ResultSet rs8 = null;
                        try {
                    ...
                    if (itemData != null && !"".equals(itemData)) {
                        ...
                        // 按照名字查询
                        String query = "SELECT SEQ_ID " + " ,DEPT_ID " + " ,USER_PRIV " + " ,USER_NAME "
                                + ",DEPT_ID_OTHER" + " ,USER_PRIV_OTHER FROM PERSON WHERE "
                                + T9WorkFlowUtility.createFindSql("USER_NAME", itemData);
                    ...
                    if (list.size() > 0) {
                        ...
                        Collections.sort(list, new T9UserSortComparatorUtility(formUserStr));
                    }
                    prcsUserAuto = prcsUserName = "";
                    for (T9Person tmp : list) {
                        if ("ALL_DEPT".equals(prcsDept)
                                || T9WorkFlowUtility.findId(prcsUser, String.valueOf(tmp.getSeqId()))
                                || T9WorkFlowUtility.findId(prcsDept, String.valueOf(tmp.getDeptId()))
                                || T9WorkFlowUtility.findId(prcsPriv, tmp.getUserPriv())
                                || T9WorkFlowUtility.privOther(prcsDept, tmp.getDeptIdOther()) == 1
                                || T9WorkFlowUtility.privOther(prcsPriv, tmp.getUserPrivOther()) == 1) {
            ...
            // 按照流程步选择
        } else if (fp.getAutoType() != null && "8".equals(fp.getAutoType()) && fp.getAutoBaseUser() != 0) {
            if (flowRun != null) {
                String query = "select USER_ID FROM FLOW_RUN_PRCS WHERE RUN_ID='" + flowRun.getRunId()
                        + "' AND FLOW_PRCS='" + fp.getAutoBaseUser() + "' AND OP_FLAG=1";
                Statement stm8 = null;
                ResultSet rs8 = null;
                T9Person tmp = null;
                prcsUserAuto = prcsUserName = "";
                try {
                    ...
                    if (rs8.next()) {
                        int userId = rs8.getInt(1);
                        String query1 = "SELECT SEQ_ID " + " ,DEPT_ID " + " ,USER_PRIV " + " ,USER_NAME "
                                + ",DEPT_ID_OTHER" + " ,USER_PRIV_OTHER FROM PERSON WHERE SEQ_ID =" + userId;
            ...
            if (!"".equals(manager) && manager != null) {
                ...
                String query = T9WorkFlowUtility.createFindSql("SEQ_ID", manager);
                query = "SELECT " + " SEQ_ID" + " ,DEPT_ID" + " ,USER_PRIV" + " ,USER_NAME "
                        + " ,USER_PRIV_OTHER" + " FROM PERSON  WHERE  " + query;
                Statement stm5 = null;
                ResultSet rs5 = null;
                try {
                    stm5 = conn.createStatement();
                ...
                // 注意 tmpDeptId
                String query = "SELECT " + " p.SEQ_ID as userId" + ",p.USER_NAME" + ",p.USER_PRIV_OTHER "
                        + ",u.SEQ_ID " + " from PERSON p,USER_PRIV u where" + " p.USER_PRIV = u.SEQ_ID "
                        + " and DEPT_ID = " + tmpDeptId + " and p.SEQ_ID !=" + user.getSeqId()
                        + " order by PRIV_NO,USER_NO,USER_NAME";
            ...
            isAutoSelect = true;
        } else if ("22".equals(fp.getAutoType())) {
            String baseDeptId2 = fp.getAutoUser();

            baseDeptId2 = T9WorkFlowUtility.getOutOfTail(baseDeptId2);
            String query = "SELECT MANAGER FROM DEPARTMENT WHERE SEQ_ID in (" + baseDeptId2 + ")";
            Statement stm2 = null;
            ResultSet rs2 = null;
            String manager = "";
            try {
                ...
                rs2 = stm2.executeQuery(query);
                while (rs2.next()) {
                    String leader = T9Utility.null2Empty(rs2.getString("MANAGER"));
                    String[] ls = leader.split(",");
                    for (String s : ls) {
                        if (!T9Utility.isNullorEmpty(s) && !T9WorkFlowUtility.findId(manager, s)) {
                            manager += s + ",";
                        }
                    }
                }
            ...
            manager = T9WorkFlowUtility.getOutOfTail(manager);

            if (!"".equals(manager) && manager != null) {
                // 以后做的
                String query2 = T9WorkFlowUtility.createFindSql("SEQ_ID", manager);
                query = "SELECT " + " SEQ_ID" + " ,DEPT_ID" + " ,USER_PRIV" + " ,USER_NAME "
                        + " ,USER_PRIV_OTHER" + ", DEPT_ID_OTHER" + " FROM PERSON  WHERE  " + query2;
                Statement stm5 = null;
                ResultSet rs5 = null;
                try {
                    stm5 = conn.createStatement();
            ...
            if ("ALL_DEPT".equals(prcsDept) || T9WorkFlowUtility.findId(prcsUser, userId)
                    || T9WorkFlowUtility.findId(prcsDept, deptId)
                    || T9WorkFlowUtility.findId(prcsPriv, userPriv)
                    || T9WorkFlowUtility.privOther(prcsDept, deptOther) == 1
                    || T9WorkFlowUtility.privOther(prcsPriv, userPrivOther) == 1) {
                prcsUserAuto += userId + ",";
            ...
            if (aPrcsUser.length == 1) {
                ...
                T9Person tmp = (T9Person) orm.loadObjSingle(conn, T9Person.class,
                        Integer.parseInt(aPrcsUser[0]));
                if (tmp != null) {
                    prcsUserName = tmp.getUserName() + ",";
    ...
    public String checkCondition(Map formData, String condition, String conditionSet) {
        ...
        for (int i = 0; i < conArray.length; i++) {
            ...
            if (value != null && itemCon.indexOf("include") != -1) {
                ...
                if ("数值".equals(itemValue) && flag == T9Utility.isInteger(value)) {
                    checkPass = true;// 2010-12-08
                } else if ("日期".equals(itemValue) && flag == T9Utility.isDay(value)) {
                    checkPass = true;
                } else if ("日期+时间".equals(itemValue) && flag == T9Utility.isDayTime(value)) {
                    checkPass = true;
                }
            } else if (value != null && itemCon.indexOf(">=") != -1) {
                Double iValue = 0.0;
                itemConDesc = "大于等于";
                if (T9Utility.isNumber(itemValue)) {
                    if (T9Utility.isNumber(value)) {
                ...
                itemConDesc = "小于等于";
                Double iValue = 0.0;
                if (T9Utility.isInteger(itemValue)) {
                    if (T9Utility.isNumber(value)) {
                        iValue = Double.parseDouble(value);
                    }
                    Double value1 = Double.parseDouble(itemValue);
            ...
            // 形成条件公式值运算和条件公式描述
            count = i + 1;
            conditionSetValue = conditionSetValue
                    .replaceAll("\\[" + count + "\\]", String.valueOf(checkPass));
            conditionSetDesc = conditionSetDesc.replaceAll("\\[" + count + "\\]", setDesc);
        }
        if (!"".equals(conditionSet) && conditionSet != null) {
            // 条件公式值运算判断
    ...
    public String checkCondition(T9Person user, Map formData, T9FlowProcess fp, boolean isIn, int runId,
            int prcsId, int flowPrcs, Connection conn) throws Exception {
        // 是否验证转入条件
        String condition = "";
        String conditionSetValue = "";
        String conditionSetDesc = "";
        String conditionSet = "";
        String notPass = "";
        String conditionDesc = T9Utility.null2Empty(fp.getConditionDesc());
        String[] ss = conditionDesc.split("\n");
        String prcsInDesc = "";
        String prcsOutDesc = "";

        if (conditionDesc != null && !"".equals(conditionDesc) && ss.length >= 1) {
            prcsInDesc = ss[0];
            if (ss.length > 1) {
                prcsOutDesc = ss[1];
            }
        ...
        for (int i = 0; i < conArray.length; i++) {
             ...
             * 
             */
            if (itemTitle.indexOf("[主办人会签意见]") != -1 || itemTitle.indexOf("[从办人会签意见]") != -1) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        String query1 = "select CONTENT from FLOW_RUN_FEEDBACK where " + " RUN_ID = " + runId
                                + " and PRCS_ID=" + prcsId + " and USER_ID=" + prcsUser;
                        Statement stm2 = null;
                        ResultSet rs2 = null;
                        try {
                            stm2 = conn.createStatement();
                ...
                String query = "select ATTACHMENT_NAME " + " from FLOW_RUN where  " + " RUN_ID=" + runId;
                ...
                String query = "select " + " ATTACHMENT_ID" + " from FLOW_RUN where " + " RUN_ID=" + runId;
                ...
                String queryPriv = "select PRIV_NAME " + " from USER_PRIV where " + " SEQ_ID = "
                        + user.getUserPriv();
                String privName = "";
                Statement stm = null;
                ResultSet rs = null;
                ...
                String queryPriv = "select PRIV_NO " + " from USER_PRIV where " + " SEQ_ID = "
                        + user.getUserPriv();
                String privName = "";
                Statement stm = null;
                ResultSet rs = null;
                ...
                String queryDept = "select DEPT_NAME " + " from DEPARTMENT where " + " SEQ_ID = "
                        + user.getDeptId();
                String deptName = "";
                Statement stm = null;
                ResultSet rs = null;
                ...
                T9Department dp = logic.getDepartmentById(user.getDeptId(), conn);
                ...
                if (otherPriv != null && !"".equals(otherPriv.trim())) {
                    String conSql = T9WorkFlowUtility.createFindSql("SEQ_ID", otherPriv);
        ...
        if (!"".equals(conditionSet) && conditionSet != null) {
            ...
            if (result == null || !result) {
                ...
                if ("".equals(conditionDesc) || conditionDesc == null) {
    ...
    public Map getForm(int formId, int runId, int flowId, Connection conn) throws Exception {
        ...
        List<T9FlowFormItem> items = orm.loadListSingle(conn, T9FlowFormItem.class, map);
        // ----- 取表单数据 --------
        Map formData = new HashMap();
        Map tmpMap = new HashMap();

        if (!T9WorkFlowUtility.isSave2DataTable()) {
            String query1 = "select ITEM_ID" + " ,ITEM_DATA " + " from FLOW_RUN_DATA where " + " RUN_ID = "
                    + runId + " order by ITEM_ID";
            ...
            String query1 = "select * from " + tableName + " where " + " RUN_ID = " + runId;
            Statement stm2 = null;
            ResultSet rs2 = null;
            try {
                stm2 = conn.createStatement();
                ...
                if (rs2.next()) {
                    for (int b = 1; b <= metas.getColumnCount(); b++) {
                        ...
                        if (name != null && !"".equals(name) && name.startsWith("DATA_")) {
                            String it = name.replace("DATA_", "");
                            if (!T9Utility.isNullorEmpty(it) && T9Utility.isInteger(it)) {
        ...
        String query = "select PRINT_MODEL_SHORT" + " from FLOW_FORM_TYPE where " + " SEQ_ID = " + formId;
        String printModel = "";
        Statement stm = null;
        ResultSet rs = null;
        try {
     ...
     * @throws Exception
     */
    public String turnOther(String prcsUser, int flowId, int runId, int prcsId, String flowPrcs,
            String opUser, Map otherMap, Connection conn) throws Exception {
        Date curDate = new Date();
        String[] uArray = prcsUser.split(",");
        for (int i = 0; i < uArray.length; i++) {
            String tmp = uArray[i];
            if (tmp != null && !"".equals(tmp)) {
                String query = "select * from FLOW_RULE where " + " FLOW_ID=" + flowId + " and USER_ID="
                        + tmp + " and STATUS = 1";
                Statement stmt = null;
                ResultSet rs = null;
                try {
                    ...
                    while (rs.next()) {
                        ...
                        if (status == 1) {
                            if (!opUser.equals(tmp)) {
                                ...
                                logic.runLog(runId, prcsId + 1, iFlowPrcs, Integer.parseInt(tmp), 2, content,
                                        "", conn);
                            }
    ...
    public String getUserByDeptRole(int deptId, String roleId, Connection conn) throws Exception {
        String query = "select PERSON.SEQ_ID from PERSON , USER_PRIV where USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND (DEPT_ID ='"
                + deptId
                + "' OR "
                + T9DBUtility.findInSet(String.valueOf(deptId), "DEPT_ID_OTHER")
                + ")"
                + " and (USER_PRIV = '"
                + roleId
                + "' OR "
                + T9DBUtility.findInSet(roleId, "USER_PRIV_OTHER")
