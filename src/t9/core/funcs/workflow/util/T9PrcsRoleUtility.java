...
public class T9PrcsRoleUtility {
  ...
  public boolean prcsRole(int flowId  , int prcsId , int checkType , T9Person user , Connection conn) throws Exception{
    ...
    if(prcsId == 0){
      ...
      flowProcessList = orm.loadListSingle(conn, T9FlowProcess.class, filters);
      return this.prcsRole(flowType, flowProcessList, checkType, user , conn);
    }else{
      T9FlowProcessLogic flowPrcsLogic = new  T9FlowProcessLogic();
      //查出相关步骤
      T9FlowProcess flowProcess = flowPrcsLogic.getFlowProcessById(flowId, prcsId+ "" , conn);
   ...
   * @throws Exception 
   */
  public boolean prcsRole(T9FlowType flowType , T9FlowProcess flowProcess , int checkType  , T9Person user ,Connection conn) throws Exception{
    List<T9FlowProcess> flowProcessList = new ArrayList();
    flowProcessList.add(flowProcess);
    return this.prcsRole(flowType, flowProcessList, checkType, user , conn);
  }
  
  public boolean prcsRole(T9FlowType flowType , int checkType , T9Person user , Connection conn) throws Exception {
    boolean flag = false;
    List<T9FlowProcess> flowProcessList  =  null;
    if ("1".equals(flowType.getFlowType())) {
      String query = "select " 
        ...
        + " , PRCS_DEPT"
        + " , PRCS_PRIV"
        + " from FLOW_PROCESS where FLOW_SEQ_ID=" + flowType.getSeqId() + " and PRCS_ID=1";
 ...
 * @throws Exception 
 */
public boolean prcsRole(T9FlowType flowType , List<T9FlowProcess> flowProcessList , int checkType  , T9Person user , Connection conn) throws Exception{
  T9FlowProcess flowPrcs = null;
  ...
  if(checkType == 0){
    ...
    //固定流程
      for(T9FlowProcess tmp : flowProcessList){
 ...
 * @return true-有这个权限,false-没有这个权限
 */
public boolean prcsRoleByManager(T9FlowType flowType , List<T9FlowProcess> flowProcessList , int checkType  , T9Person user){
  T9FlowProcess flowPrcs = null;
  if(flowProcessList != null 
      && flowProcessList.size() == 1){
    flowPrcs = flowProcessList.get(0);
 ...
 * @throws Exception 
 */
public boolean prcsRoleByQuery(T9FlowType flowType  , T9Person user  , Connection conn) throws Exception{
  if(user == null){
    return false;
  }
  //如果是管理员
  ...
  String query  = "SELECT PRCS_USER , PRCS_DEPT, PRCS_PRIV  from FLOW_PROCESS where FLOW_SEQ_ID=" + flowType.getSeqId();
  Statement stm = null;
  ResultSet rs = null;
  try {
    stm = conn.createStatement();
  ...
  public String runRole(int flowRunId , int flowTypeId , int  prcsId , T9Person user , Connection conn) throws Exception{
    T9FlowRun flowRun = this.getFlowRun(flowRunId, conn);
    int flowId = 0 ;
    if (flowRun != null) {
      flowId = flowRun.getFlowId();
    }
    T9FlowType flowType = this.getFlowType(flowId, conn);
  ...
  public T9FlowType getFlowType(int flowId , Connection conn) throws Exception {
    T9FlowType flowType = null;
    String query = "select QUERY_USER, QUERY_USER_DEPT , MANAGE_USER , MANAGE_USER_DEPT  from FLOW_TYPE where SEQ_ID=" + flowId;
    Statement stm5 = null;
    ResultSet rs5 = null;
    try {
  ...
  public T9FlowRun getFlowRun(int runId , Connection conn) throws Exception {
    T9FlowRun flowRun = null;
    String query = "select BEGIN_USER,FLOW_ID from FLOW_RUN WHERE RUN_ID = " + runId;
    Statement stm4 = null;
    ResultSet rs4 = null;
    try {
   ...
   * @throws Exception
   */
  public String runRole(T9FlowRun flowRun , T9FlowType flowType , int  prcsId , T9Person user , Connection conn) throws Exception{
    ...
    String query = "select 1 from FLOW_RUN_PRCS WHERE user_id =" + user.getSeqId() + " and op_FLAG = 1 AND RUN_ID =" + flowRun.getRunId() ;
    if(prcsId != 0 ){
    ...
    query = "select 1 from FLOW_RUN_PRCS WHERE user_id =" + user.getSeqId() + " AND RUN_ID =" + flowRun.getRunId() ;
    if(prcsId != 0 ){
      query +=  " and prcs_ID =" + prcsId;
    }
    Statement stm5 = null;
    ...
    query = "select VIEW_USER from FLOW_RUN WHERE  RUN_ID =" + flowRun.getRunId() ;
    Statement stm9 = null;
    ResultSet rs9 = null;
    try {
      ...
      if(rs9.next()){
        String viewUser = rs9.getString("VIEW_USER");
        if (T9WorkFlowUtility.findId(viewUser, String.valueOf(user.getSeqId()))) {
          runRole += "8,";
        }
      }
    } catch(Exception ex) {
