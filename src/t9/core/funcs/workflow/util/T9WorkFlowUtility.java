...
public class T9WorkFlowUtility {
  ...
  public static String encodeSpecial(String srcStr) {
    return T9Utility.encodeSpecial(T9Utility.null2Empty(srcStr));
  }
  /**
   * 判段id是不是在str里面
   * @param str
   ...
   * @throws Exception 
   */
  public static boolean checkPriv(T9FlowType flowType , List<T9FlowProcess> list , T9Person loginUser ,Connection conn) throws Exception {
    //取得第一步
    T9PrcsRoleUtility roleUtility = new T9PrcsRoleUtility();
    boolean flag = false;
    //2是自由流程
    if ("1".equals(flowType.getFlowType())) {
      T9FlowProcess firstPrc = null;
   ...
   * @throws Exception
   */
  public String getSignInfo(String modelShort , T9FlowRun fr , Connection conn) throws Exception {
    // TODO Auto-generated method stub
    String query = "select PRCS_ID "  
      + " , FLOW_PRCS from FLOW_RUN_PRCS where "
      + "  RUN_ID = " + fr.getRunId();
    Map map = new HashMap();
    Statement stm2 = null;
    ResultSet rs2 = null ;
    try {
      ...
      while (rs2.next()) {
        ...
        String query1 = "select PRCS_NAME from FLOW_PROCESS where "
   ...
   * @throws Exception
   */
  public String signReplace(List<String[]> signMacro , Map prcsIdArray , T9FlowRun fr, int type , String modelShort, Connection conn) throws Exception {
    int count = 0 ;
    for (String[] tmp : signMacro) {
      ...
      if (type == 1) {
        query = "select * from FLOW_RUN_FEEDBACK a left join FLOW_RUN_PRCS b on " 
          + " a.RUN_ID = b.RUN_ID  " 
        ...
        query += "SELECT * from FLOW_RUN_FEEDBACK where RUN_ID=" + fr.getRunId();
      ...
      try {
        ...
        while (rs.next()) {
          ...
          String signData = T9WorkFlowUtility.clob2String(rs.getClob("SIGN_DATA"));
          String prcsName = "";
          String deptName = "";
          String longName = "";
          String privName = "";
  ...
  public String getAttach(String modelShort , T9FlowRun fr , Connection conn , String imgPath) {
  // TODO Auto-generated method stub
  ...
  public boolean isDeptParent(int childDept , int parentDept , Connection conn) throws Exception {
    String query = "select DEPT_PARENT from DEPARTMENT where SEQ_ID =" + childDept;
    Statement stmt = null;
    ResultSet rs = null;
    int parentId = 0;
    try {
 ...
 * @throws Exception 
 ...
 public final static String clob2String(Clob clob) throws Exception {
   ...
   * @throws Exception 
   */
  public boolean isHaveRight(int deptId , T9Person u , Connection conn) throws Exception {
   ...
   * @throws Exception 
   */
  public boolean isHaveSortRight(int deptId , T9Person u , Connection conn) throws Exception {
    String userPrivOther = u.getUserPrivOther();
    if (!u.isAdminRole() 
        && !T9WorkFlowUtility.findId(userPrivOther, "1")
        && deptId != 0
  ...
  public String getNotDisplaySort(Connection conn) throws Exception{
    String sortIds = T9SysProps.getString("FLOWSORT_NEED_NOT_DISPLAY");
    if (T9Utility.isNullorEmpty(sortIds)) {
      return null;
    }
    sortIds = getInStr(sortIds);
    String query = "select SEQ_ID from FLOW_SORT WHERE SORT_NAME in (" + sortIds + ")";
    Statement stm = null; 
    ResultSet rs = null; 
    sortIds = "";
    try { 
  ...
  public static String copyAttach(String attId , String attName , String moduleSrc , String moduleDesc) throws Exception {
    String newAttId = "";
    String[] attIds = T9Utility.null2Empty(attId).split(",");
    String[] attNames = T9Utility.null2Empty(attName).split("\\*");
  ...
  public static boolean isSave2DataTable() {
    String workflowSave2DataTable  = T9SysProps.getProp("WorkflowSave2DataTable");
  ...
  public void saveFormDataToTable(Connection conn ,int runId , Map dataMap , int formId) throws Exception {
    String tableName = "" +T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + formId;
        ...
        for(int   i = 1;i   <=   colNum;i++){ 
          ...
          if (!"RUN_ID".equalsIgnoreCase(name) 
            ...
            if (name.startsWith("DATA_")) {
              ...
              String update = "update FLOW_RUN_DATA set ITEM_DATA = '" + value + "' WHERE RUN_ID = " + runId + " and ITEM_ID = '"  + itemId + "'" ;
  ...
  public static void updateFormData(Connection conn, int runId , int flowId , int itemId , String value) throws Exception {
    String update = "update FLOW_RUN_DATA set ITEM_DATA = ? WHERE RUN_ID = " + runId + " and ITEM_ID = '"  + itemId + "'" ;
    if (isSave2DataTable()) {
      ...
      update = "update " + tableName + " set  "+ key +"=?  where RUN_ID = "+ runId;
    } 
  ...
  public static long getAttachSize(String name , String id , String module) throws Exception {
    long b = 0 ;
    T9WorkFlowUtility util = new T9WorkFlowUtility();
    String path = util.getAttachPath(id, name, module);
  ...
  public static long getAttachSizes(String name , String id , String module) throws Exception {
    long b = 0 ;
    String[] attIds = T9Utility.null2Empty(id).split(",");
    String[] attNames = T9Utility.null2Empty(name).split("\\*");
    for(int i = 0 ;i < attIds.length ;i ++){
      String tmp = attIds[i];
      if ("".equals(tmp)) {
        continue;
      ...
      String attN = attNames[i];
      b += T9WorkFlowUtility.getAttachSize(attN, tmp, module);
    }
    return b;
  }
}
