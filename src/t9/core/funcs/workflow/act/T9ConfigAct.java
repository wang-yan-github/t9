...
public class T9ConfigAct {
  private static Logger log = Logger
    .getLogger("t9.core.funcs.workflow.act.T9ConfigAct");
  ...
  public String saveConfig(HttpServletRequest request,
    ...
    try{
      ...
      String[] flowAction = request.getParameterValues("FLOW_ACTION");
      String flows = "";
      for (String f : flowAction) {
        if (T9Utility.isNullorEmpty(f)) continue;
        flows += f + ",";
      }
      flows = T9WorkFlowUtility.getOutOfTail(flows);
      ...
      logic.updateSysPar("FLOW_ACTION", flows, dbConn);
