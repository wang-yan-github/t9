...
import t9.core.funcs.workflow.data.T9FlowHook;
...
public class T9FlowHookLogic {
  ...
  public String getHook(Connection conn,int hid , String webrootPath)  throws Exception{
    String query = "select * from FLOW_HOOK where SEQ_ID =  " + hid;
    Statement stm = null;
    ResultSet rs = null;
    ...
    String query2 = "SELECT FLOW_TYPE.SEQ_ID,FLOW_NAME,FLOW_TYPE from FLOW_TYPE,FLOW_SORT where FLOW_TYPE.FLOW_SORT=FLOW_SORT.SEQ_ID order by SORT_NO,FLOW_NO";
    Statement stm2 = null;
    ResultSet rs2 = null;
    sb.append(",flows:[");
    int count = 0 ;
    ...
    String query3 = "select FORM_SEQ_ID FROM FLOW_TYPE WHERE SEQ_ID =" + flowId;
  ...
  public void addHookLogic(Connection conn,Map request,T9Person person)  throws Exception{
    ...
    try{
      T9FlowHook hook=new T9FlowHook();
      hook.setSystem("0");
  ...
  public void deleteHookLogic(Connection dbConn,String seqId) throws Exception {
    Statement stmt=null;
     try {
        String sql="delete from flow_hook where seq_id='"+seqId+"'";
        stmt=dbConn.createStatement();
