...
public class T9WorkLogLogic {
  ...
  public int deleteLog(Connection conn , String seqIds) throws Exception{
    int result = 0;
    String sql = "delete from FLOW_RUN_LOG WHERE SEQ_ID in(" + seqIds + ")";
    PreparedStatement ps = null;
    try {
      ps = conn.prepareStatement(sql);
  ...
  public String getPrcsName(Connection conn , String flowId,String flowPrcs) throws Exception{
    String result = "";
    String sql = "select PRCS_NAME from FLOW_PROCESS WHERE FLOW_SEQ_ID=" + flowId + " AND PRCS_ID=" + flowPrcs;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
