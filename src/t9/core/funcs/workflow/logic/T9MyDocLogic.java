...
public class T9MyDocLogic {
  public String getMyWork(Connection conn , T9Person user , int maxCount  , String sortId) throws Exception {
    ...
    String query = "SELECT FLOW_RUN_PRCS.PRCS_ID " 
      ...
      + " , PERSON.USER_NAME"
      + " from FLOW_RUN_PRCS,FLOW_RUN,FLOW_TYPE, PERSON WHERE  " 
      ...
      while (rs.next()) {
        ...
        if ("1".equals(flowType)) {
          if (!prcsNameMap.containsKey(sFlowPrcsI)) {
            String query3 = "select PRCS_NAME from FLOW_PROCESS WHERE FLOW_SEQ_ID="+ flowId +" AND PRCS_ID=" + flowPrcs;
            Statement stm3 = null;
            ResultSet rs3 = null;
            try {
        ...
        T9FlowRunData rd6 =  wf.getFlowRunData(conn, flowId, runId, "标题");
        String title = "";
        if (rd6 != null) {
          title = T9Utility.encodeSpecial(T9Utility.null2Empty(rd6.getItemData()));
        }
        sb.append("{");
  ...
  public String getFocusWork(Connection conn, T9Person user, int maxCount , String sortId) throws Exception {
    ...
    String query = "SELECT FLOW_RUN.RUN_ID " 
      ...
      + " ,FLOW_RUN.FOCUS_USER "
      + " ,PERSON.USER_NAME"
      + "  FROM FLOW_RUN ,FLOW_TYPE,PERSON  WHERE  " 
