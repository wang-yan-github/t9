...
public class T9WorkflowSave2DataTableLogic {
  private static Logger log = Logger
  .getLogger("t9.core.funcs.workflow.logic.T9WorkflowSave2DataTableLogic");
  ...
  public String getFields(Connection conn, String seqId) throws Exception {
    ...
    ResultSet rs = null;
    String str = "";
    String sql = "select ITEM_ID from FLOW_FORM_ITEM WHERE FORM_ID=" + seqId;
    try {
      stm = conn.createStatement();
      rs =  stm.executeQuery(sql);
      while (rs.next()) {
        int i = rs.getInt("ITEM_ID");
        if (!T9WorkFlowUtility.findId(str, i + "")) {
          str += i + ",";
        }
      } 
    } catch(Exception ex) {
    ...
    str = t9.core.funcs.workflow.util.T9WorkFlowUtility.getOutOfTail(str);
    return str;
  }
  public String getFlowTypeByFormId(Connection conn, String seqId) throws Exception {
    ...
    ResultSet rs = null;
    String str = "";
    String sql = "select SEQ_ID from FLOW_TYPE WHERE FORM_SEQ_ID=" + seqId;
    try {
      stm = conn.createStatement();
      rs =  stm.executeQuery(sql);
      while (rs.next()) {
  ...
  public void updateFlowTypeByFormId(Connection conn, String  old , int newId) throws Exception {
    // TODO Auto-generated method stub
    String sql = "UPDATE  FLOW_TYPE SET FORM_SEQ_ID = '"+ newId +"' WHERE FORM_SEQ_ID=" + old;
    this.exSql(conn, sql);
  }
  public void updateFormTypeByFormId(Connection conn, String  old , int newId) throws Exception {
    // TODO Auto-generated method stub
    String sql = "UPDATE  FLOW_FORM_TYPE SET FORM_ID = '"+ newId +"' WHERE SEQ_ID=" + old + " OR FORM_ID =" + old;
    this.exSql(conn, sql);
  }
  ...
  public void alterTable(Connection dbConn, String tableName, String seqStr) throws Exception {
    ...
    metaData = T9WorkFlowUtility.getOutOfTail(metaData);
  ...
  public String needAddField(String fields , String metaData) {
    ...
    for (String m : ss) {
      ...
      if(!T9WorkFlowUtility.findId(metaData , field)){
        idStr += field + ",";
      }
    }
    idStr = T9WorkFlowUtility.getOutOfTail(idStr);
    return idStr;
  }
  private void alterMysqlSql(Connection dbConn, String fields, String tableName) throws Exception {
    String[] ss  = fields.split(",");
    for (String m : ss) {
      if (!T9Utility.isNullorEmpty(m)) {
  ...
  private void alterMssqlSql(Connection dbConn, String fields, String tableName) throws Exception {
    ...
    for (String m : ss) {
      if (!T9Utility.isNullorEmpty(m)) {
      String sql = "ALTER TABLE "+ tableName +" ADD "+ m +" NTEXT";
      try {
        this.exSql(dbConn, sql);
      } catch (Exception ex) {
        log.error("为表"+ tableName + ",增加字段:"+ m +"时出错!");
  ...
  public void createSaveTable(Connection conn) throws Exception {
    ...
    String query = "select SEQ_ID , FORM_SEQ_ID from flow_type" ;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
      ...
      while (rs.next()) {
        ...
        String tableName =  "" +T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + rs.getInt("SEQ_ID") + "_" + formId;
  ...
  public void createSaveTableById(Connection conn , int id) throws Exception {
    ...
    String query = "select SEQ_ID , FORM_SEQ_ID from flow_type where SEQ_ID=" + id ;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
  ...
  public void updateFormItemMaxById(Connection conn , int formId) throws Exception {
    String query = "select SEQ_ID, ITEM_MAX from flow_form_type where SEQ_ID =" + formId ;
    Statement stm = null;
    ResultSet rs = null;
    int max = 0;
    try {
    ...
    Statement stm2 = null;
    ResultSet rs2 = null;
    query = "select max(ITEM_ID) FROM FLOW_FORM_ITEM WHERE FORM_ID=" + formId;
    try {
      ...
      if (rs2.next()) {
        int i = rs2.getInt(1);
        if (i > max) {
          String update = "update FLOW_FORM_TYPE SET ITEM_MAX = " + i + " where SEQ_ID =" + formId;
  ...
  public void updateFormItemMax(Connection conn) throws Exception {
    String query = "select SEQ_ID, ITEM_MAX from flow_form_type" ;
    Statement stm = null;
    ResultSet rs = null;
    String formIds = "";
    String itemMaxs = "";
    ...
    for (int b = 0 ; b < formIdss.length ; b++) {
      ...
      if (!T9Utility.isNullorEmpty(id)) {
        Statement stm2 = null;
        ResultSet rs2 = null;
        query = "select max(ITEM_ID) FROM FLOW_FORM_ITEM WHERE FORM_ID=" + id;
        try {
          stm2 = conn.createStatement();
          rs2 =  stm2.executeQuery(query);
          if (rs2.next()) {
            ...
            if (i > Integer.parseInt(im)) {
              String update = "update FLOW_FORM_TYPE SET ITEM_MAX = " + i + " where SEQ_ID =" + id;
              this.exSql(conn, update);
            }
          }
        } catch(Exception ex) {
  ...
  public void updateFlowFormTable(Connection dbConn, int seqId) throws Exception {
    if (T9WorkFlowUtility.isSave2DataTable()) {
      ...
      if (!T9Utility.isNullorEmpty(flowTypes)) {
        String[] flowType = flowTypes.split(",");
        for (String ss : flowType) {
          if (!T9Utility.isNullorEmpty(ss)) {
            String tableName = "" +T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + ss + "_" + seqId;
  ...
  public void updateItemMax(Connection conn , int id) throws Exception {
    Statement stm2 = null;
    ResultSet rs2 = null;
    String query = "select max(ITEM_ID) FROM FLOW_FORM_ITEM WHERE FORM_ID=" + id;
    try {
      stm2 = conn.createStatement();
      rs2 =  stm2.executeQuery(query);
      if (rs2.next()) {
  ...
  public void convertData2Table(Connection conn) throws Exception {
    // TODO Auto-generated method stub
    String sql = "select RUN_ID , RUN_NAME , BEGIN_USER, BEGIN_TIME , FLOW_TYPE.SEQ_ID , FLOW_TYPE.FORM_SEQ_ID FROM FLOW_RUN , FLOW_TYPE WHERE FLOW_ID = FLOW_TYPE.SEQ_ID";
    ...
    for (Map m : list) {
      ...
      String tableName = "" +T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + flowId + "_" + formId;
    }
