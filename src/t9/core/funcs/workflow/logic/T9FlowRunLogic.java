...
import t9.core.funcs.workflow.praser.T9PraseData2Form;
import t9.core.funcs.workflow.praser.T9PraseData2PrintForm;
...
public class T9FlowRunLogic {
     ...
     *             文号
     */
    public String getRunName(T9FlowType flowType, T9Person loginUser, Connection conn, boolean isUpdateAutoNum)
            throws Exception {
        // conn.setTransactionIsolation(Connection.TRANSACTION_READ_COMMITTED);
        T9DeptLogic deptLogic = new T9DeptLogic();
        T9UserPrivLogic userPrivLogic = new T9UserPrivLogic();
        T9FlowSortLogic fsLogic = new T9FlowSortLogic();

        T9FlowSort flowSort = fsLogic.getFlowSortById(flowType.getFlowSort(), conn);
        ...
        if (flowType.getAutoName() == null || "".equals(flowType.getAutoName())) {
            ...
            if (isUpdateAutoNum) {
                ...
                if (runName.indexOf("{N}") != -1) {
                    String query = "update FLOW_TYPE set AUTO_NUM=" + autoNum + " where SEQ_ID="
                            + flowType.getSeqId();
    ...
    /**
     * 为T9FlowRunAct/getNewMsg方法组装数据
     * 
     * @param flowType
     * @param loginUser
     * @param list
     * @return 字符串 形如
     *         "{formId:23,flowName:'ddd',runName:'请假申请(2010-01-20 14:45:45)',prcsList:[{prcsNo:'1',prcsName:'请假申请',prcsTo:'2,'},{prcsNo:'1',prcsName:'请假申请',prcsTo:'2,'}]}"
     * @throws
     */
    public String getNewMsg(T9FlowType flowType, T9Person loginUser, List<T9FlowProcess> list, Connection conn)
            throws Exception {
        StringBuffer sb = new StringBuffer("{");
        // 取得文号
        String runName = this.getRunName(flowType, loginUser, conn, false);
        sb.append("formId:" + flowType.getFormSeqId() + ",flowName:\""
                + T9Utility.encodeSpecial(flowType.getFlowName()) + "\",runName:'" + runName + "',flowType:"
                + flowType.getFlowType());
        if (flowType.getFlowDesc() != null) {
            sb.append(",introduction:\"" + T9Utility.encodeSpecial(flowType.getFlowDesc()) + "\"");
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            T9FlowProcess tmp = list.get(i);
            sb.append("{");
            sb.append("prcsNo:\"" + tmp.getPrcsId() + "\",");
            sb.append("prcsName:\"" + T9Utility.encodeSpecial(tmp.getPrcsName()) + "\",");
     ...
     * 
     * @return {title:
     *         '分类:ddd,类型:jjj',flowId:8,flowName:'ddd',runId:1,fileNo:'文件1',beginTime:'2003-1-12'},{title:'ddd',flowId:8,flowName:'ddd',runId:1,fileNo:'文件1',beginTime:'2003-1-12
     *         ' }
     * @throws Exception
     */
    public String getRecentlyFlowRun(T9Person user, Connection conn, String sortId) throws Exception {
        // ---- 逐一查询最近的工作 ----
        long date1 = System.currentTimeMillis();
        String query = "SELECT FLOW_ID,RUN_ID,RUN_NAME,BEGIN_TIME from FLOW_RUN where " + " BEGIN_USER="
                + user.getSeqId() + " and DEL_FLAG=0 order by RUN_ID desc";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // 没在里面
                if (!T9WorkFlowUtility.findId(flowIdStr, String.valueOf(flowId))) {
        ...
        for (Map map : flowRunList) {
            ...
            T9FlowProcess flowFirstProcess = null;
            T9FlowType ftTmp = (T9FlowType) orm.loadObjSingle(conn, T9FlowType.class, flowId);
            if (ftTmp == null) {
                continue;
            }
            boolean flag = false;
            if ("1".equals(ftTmp.getFlowType())) {
                ...
                flag = flowFirstProcess != null && tru.prcsRole(ftTmp, flowFirstProcess, 0, user, conn);
            ...
            if (flag
                    && (T9Utility.isNullorEmpty(sortId) || T9WorkFlowUtility.findId(sortId,
                            String.valueOf(sId)))) {
                String querySortName = "SELECT SORT_NAME from FLOW_SORT WHERE SEQ_ID=" + sId;
                String sortName = "";
                Statement stm2 = null;
                ResultSet rs2 = null;
                try {
                ...
                sb.append("{");
                sb.append("title:'分类:" + sortName + ",类型:" + map.get("flowTypeT") + "'");
     ...
     * @throws Exception
     */
    public T9FlowRun getFlowRunByRunId(int runId, Connection conn) throws Exception {
        // TODO Auto-generated method stub
        HashMap map = new HashMap();
        map.put("RUN_ID", runId);
        T9ORM orm = new T9ORM();
        T9FlowRun flowRun = (T9FlowRun) orm.loadObjSingle(conn, T9FlowRun.class, map);
        return flowRun;
    }
     ...
     * @throws Exception
     */
    public Map getPrintForm(T9Person user, T9FlowRun flowRun, T9FlowType flowType, boolean isWord,
            Connection conn, String imgPath, String isAutoPrint) throws Exception {
        ...
        T9ORM orm = new T9ORM();
        List<T9FlowFormItem> itemList = orm.loadListSingle(conn, T9FlowFormItem.class, map);
        String form = "";
        String script = "";
        String css = "";
        if (itemList.size() > 0) {
            ...
            List<T9FlowRunData> frdList = orm.loadListSingle(conn, T9FlowRunData.class, map);
            T9FlowFormType ff = (T9FlowFormType) orm.loadObjSingle(conn, T9FlowFormType.class, formId);
            ...
            T9PraseData2PrintForm pdf = new T9PraseData2PrintForm();
            form = pdf.parsePrintForm(user, modelShort, flowRun.getRunId(), flowType.getSeqId(), frdList,
                    itemList, conn, isWord, isAutoPrint);
        } else {
            form = T9WorkFlowUtility.Message("表单内容为空！", 0);
        }
    ...
    public void updatePrcs(Connection conn, String prcsFlag, Date date, int seqId) throws Exception {
        String query = "update FLOW_RUN_PRCS set PRCS_FLAG='" + prcsFlag + "',PRCS_TIME=? WHERE SEQ_ID="
                + seqId;
     ...
     * @throws Exception
     */
    public String getHandlerMsg(T9Person user, int runId, int prcsId, String flowPrcs, String ip,
            Connection conn, String imgPath, String isWriteLog) throws Exception {
        // TODO Auto-generated method stub
        T9FlowTypeLogic flowTypeLogic = new T9FlowTypeLogic();
        T9FlowRun flowRun = this.getFlowRunByRunId(runId, conn);
        ...
        int parentFlowId = 0;
        if (flowRun != null && flowRun.getRunId() != 0) {
            String query = "SELECT FLOW_ID from FLOW_RUN WHERE RUN_ID='" + flowRun.getRunId() + "'";
            Statement stm = null;
            ResultSet rs = null;
            try {
                stm = conn.createStatement();
                ...
                rs = stm.executeQuery(query);
                if (rs.next()) {
                    parentFlowId = rs.getInt("FLOW_ID");
                }
            } catch (Exception ex) {
                throw ex;
            } finally {
        ...
        if (!T9Utility.isNullorEmpty(flowRun.getAipFiles())) {
        ...
        if (runProcess != null && "1".equals(runProcess.getPrcsFlag())) {
            runProcess.setPrcsFlag("2");
            runProcess.setPrcsTime(date);
            // this.updatePrcs(conn, "2", date, runProcess.getSeqId());
            ...
            if (runProcess.getPrcsId() == 1) {
                ...
                if (flowRun.getParentRun() != 0) {
                    String query = "update FLOW_RUN_PRCS set PRCS_TIME=?,PRCS_FLAG=2 where RUN_ID='"
                            + flowRun.getParentRun() + "' and USER_ID='" + user.getSeqId()
                            + "' and CHILD_RUN='" + flowRun.getRunId() + "'";
                    PreparedStatement stm4 = null;
                    try {
                        stm4 = conn.prepareStatement(query);
        ...
        if (!"0".equals(runProcess.getParent()) && !T9Utility.isNullorEmpty(runProcess.getParent())) {
            ...
            ps = T9WorkFlowUtility.getOutOfTail(ps);
        ...
        if (list.size() > 0) {
            ...
            T9PraseData2Form pdf = new T9PraseData2Form();
            formMsg = pdf.parseForm(user, modelShort, flowProcess, runProcess, flowType, frdList, list, ip,
                    conn);
        ...
        if ("1".equals(allowBack)) {
            String query2 = "SELECT 1 FROM FLOW_RUN_PRCS WHERE " + " RUN_ID='" + runId + "' AND PRCS_ID='"
                    + prcsId + "' AND OP_FLAG='1' AND FLOW_PRCS<>'" + flowPrcs + "' AND PARENT='"
                    + runProcess.getParent() + "'";
            Statement stm1 = null;
            ResultSet rs1 = null;
            try {
                stm1 = conn.createStatement();
        ...
        sb.append(",dispFile:'" + T9Utility.null2Empty((String) aipMap.get(dispAip)) + "'");
        sb.append("}");
        sb.append("}");
     ...
     * @throws Exception
     */
    public int createNewWork(T9Person user, T9FlowType flowType, String runName, Connection conn)
            throws Exception {
        // 查出最的runId
        // conn.setTransactionIsolation(Connection.TRANSACTION_READ_COMMITTED);
        int runId = 0;
        ...
        // 新建 一个工作实例
        query = "insert into FLOW_RUN (RUN_ID " + " ,RUN_NAME " + " ,FLOW_ID " + " ,BEGIN_USER "
                + " ,BEGIN_TIME " + ", FORM_VERSION " + ") values (" + runId + ",?," + flowType.getSeqId()
                + "," + user.getSeqId() + ",?" + " , " + l.getMaxVersion(conn, flowType.getFormSeqId()) + ")";
        Timestamp time = new Timestamp(new Date().getTime());
        ...
        // 新建工作的第一个步骤
        query = "insert into FLOW_RUN_PRCS (RUN_ID  " + " , PRCS_ID  " + " ,USER_ID  " + " ,PRCS_FLAG  "
                + " ,FLOW_PRCS  " + " ,CREATE_TIME) VALUES (" + runId + " , 1, " + user.getSeqId()
                + ",'1','1',?)";
        ...
        if (T9WorkFlowUtility.isSave2DataTable()) {
            String tableName = T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + flowType.getSeqId() + "_"
                    + flowType.getFormSeqId();
            String del = "DELETE FROM " + tableName + " WHERE RUN_ID = " + runId;
            T9WorkFlowUtility.updateTableBySql(del, conn);
            PreparedStatement stm4 = null;
            String query1 = "insert into " + tableName + " (RUN_ID , RUN_NAME , BEGIN_USER, BEGIN_TIME "
                    + fieldName + ") " + "VALUES (" + runId + ", ? ," + user.getSeqId() + ", ? " + fieldVal
                    + ")";
            try {
    ...
    public String getItemData(T9FlowFormItem tmp) {
        String clazz = tmp.getClazz();
     ...
     * @throws Exception
     */
    public String saveFormData(T9Person user, int flowId, int runId, int prcsId, int flowPrcs,
            HttpServletRequest request, String hiddenStr, String readOnlyStr, Connection conn)
            throws Exception {
        T9ORM orm = new T9ORM();
        ...
        T9FlowRunPrcs flowRunPrcs = (T9FlowRunPrcs) orm
                .loadObjSingle(conn, T9FlowRunPrcs.class, queryRunPrcs);
        ...
        if ("1".equals(flowRunPrcs.getOpFlag())) {
            ...
            for (T9FlowFormItem tmp : list) {
                ...
                // 保密字段、只读字段不保存
                if (T9WorkFlowUtility.findId(hiddenStr, String.valueOf(itemId))
                        || T9WorkFlowUtility.findId(readOnlyStr, String.valueOf(itemId))) {
                    continue;
                }
                String clazz = tmp.getClazz();
                if ("DATE".equals(clazz) || "USER".equals(clazz) || "MOBILE_SEAL".equals(clazz)) {
                    continue;
                }
                String aItem = request.getParameter("DATA_" + itemId);
                String itemData = "";
                if (!T9Utility.isNullorEmpty(aItem)) {
                    itemData = aItem;
                }
                if (!T9WorkFlowUtility.isSave2DataTable()) {
                    ...
                    T9FlowRunData flowRunData = (T9FlowRunData) orm.loadObjSingle(conn, T9FlowRunData.class,
                            queryMap);
                    if (flowRunData != null) {
            ...
            if (T9WorkFlowUtility.isSave2DataTable()) {
                ...
                if (!T9Utility.isNullorEmpty(dataField)) {
                    String tableName = T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + flowId + "_"
                            + formSeqId;
                    String update = "update " + tableName + " set " + dataField + " where RUN_ID=" + runId;
                    PreparedStatement stm4 = null;
                    try {
     ...
     * @throws Exception
     */
    public String getTurnData(T9Person loginUser, int runId, int prcsId, String flowPrcsStr, Connection conn,
        ...
        if (!isManage) {
            ...
            if (!"setOk".equals(notPass)) {
                ...
                // notPass = notPass.replaceAll("\'", "\\\\'");
                String s = "{notOutPass:\"" + T9Utility.encodeSpecial(notPass) + "\"}";
                return s;
            }
        }
        StringBuffer sb = new StringBuffer();
        ...
        if (flowProcess.getTurnPriv() != null && "1".equals(flowProcess.getTurnPriv())) {
            isAllowTurn = true;
        }
        sb.append(",isAllowTurn:" + isAllowTurn);// 是否允许强制转交

        if (flowType.getViewPriv() == 1 && flowProcess.getViewPriv() == 1) {
            sb.append(",viewPriv:true");// 是否允许传阅
            sb.append(",viewUser:'" + flowType.getViewUser() + "'");// 是否允许传阅
            T9PersonLogic logic = new T9PersonLogic();
            String viewUserName = T9Utility.encodeSpecial(logic.getNameBySeqIdStr(flowType.getViewUser(),
                    conn));
            sb.append(",viewUserName:\"" + viewUserName + "\"");
        } else {
            sb.append(",viewPriv:false");
        }
        ...
        String str = this.getHandlerState(flowRunPrcList, loginUser, Integer.parseInt(flowPrcsStr), conn,
                isManage);
        if (str == null) {
            return null;
        } else {
        ...
        for (T9FlowProcess tmp : nextPrcs) {
            ...
            if (!"setOk".equals(notInPass)) {
                sb.append(",notInPass:\"" + T9Utility.encodeSpecial(notInPass) + "\"");
            }
            // 取得下一步中的默认人员
            if (tmp.getPrcsId() == 0 && flowRun.getParentRun() != 0) {
                sb.append(","
                        + turnUtility.userSelect2(conn, loginUser, prcsBack, parentFlowId, backUserOp,
                                backUser, null));
            } else {
                sb.append("," + turnUtility.userSelect(loginUser, tmp, flowRun, conn, null));
            }
        ...
        if (T9WorkFlowUtility.findId(typePriv, "40")
                && T9WorkFlowUtility.findId(sms2RemindPriv, String.valueOf(loginUser.getSeqId()))) {
            sms2PrivStart = true;
        }
        if (T9WorkFlowUtility.findId(typePriv, "41")
                && T9WorkFlowUtility.findId(sms2RemindPriv, String.valueOf(loginUser.getSeqId()))) {
            sms2PrivAll = true;
        }
        sb.append(",sms2PrivNext:" + sms2PrivNext + ", sms2PrivStart:" + sms2PrivStart + ", sms2PrivAll:"
                + sms2PrivAll);

        T9MoreOperateLogic opt = new T9MoreOperateLogic();

        sb.append(",opts:" + opt.getFlowAction(conn, loginUser.getUserPriv(), loginUser.getUserPrivOther()));
        sb.append("}");
        return sb.toString();
    }
     ...
     * @throws Exception
     */
    public String getHandlerState(List<T9FlowRunPrcs> flowRunPrcList, T9Person user, int flowPrcs,
            ...
            if (tmp.getFlowPrcs() == flowPrcs) {
                ...
                if (person != null) {
                    if (prcsFlag.equals("1")) {
                        userNameStr += "<font color=red title=未接收办理>" + person.getUserName()
                                + "(未接收)</font>,";
                    } else if (prcsFlag.equals("2")) {
                        userNameStr += "<font color=red title=正在办理>" + person.getUserName() + "(办理中)</font>,";
                    } else if (prcsFlag.equals("4")) {
                        userNameStr += "<font color=green title=已办理完毕>" + person.getUserName()
                                + "(已办结)</font>,";
                    } else {
     ...
     * @throws Exception
     */
    public void turnNext(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs,
            String prcsChoose, Map opUserMap, String ip, Connection conn) throws Exception {
     ...
     * @throws Exception
     */
    public void backTo(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs, String prcsPre,
            String ip, String contextPath, String sortId, String skin, Connection conn) throws Exception {
        T9ORM orm = new T9ORM();
        Map map = new HashMap();
        map.put("RUN_ID", runId);
        map.put("PRCS_ID", Integer.parseInt(prcsPre));// 回退的实际步骤
        List<T9FlowRunPrcs> runPrcs = orm.loadListSingle(conn, T9FlowRunPrcs.class, map);
        if (runPrcs.size() > 0) {
            String prcsUser = "";// 经办人
            String prcsOpUser = "";// 主办人
            String topFlag = "0";// 主办方式,
            T9FlowRunPrcs flowPrcsTmp = runPrcs.get(0);// 主要是取出flowPrcs和topFlag
            topFlag = flowPrcsTmp.getTopFlag();
            String flowPrcsTo = String.valueOf(flowPrcsTmp.getFlowPrcs());
            for (T9FlowRunPrcs tmp : runPrcs) {
                // 1说明是主办
            ...
            sb.setRemindUrl("/core/funcs/workflow/flowrun/list/inputform/index.jsp?skin=" + skin + "&sortId="
                    + sortId + "&runId=" + runId + "&flowId=" + flowId + "&prcsId=" + (prcsId + 1)
                    + "&flowPrcs=" + prcsPre);
     ...
     *            所有主办人
     * @param topFlag
     *            经办方式0-指定主办人,1-无主办人会签 ,2-先接收为主办
     * @return
     * @throws Exception
     */
    public void turnNext(int runId, int flowId, int prcsId, int flowPrcs, String prcsChoose, Map opUserMap,
        ...
        for (String tmp : aPrcsChoose) {
            ...
            String prcsUser = T9Utility.null2Empty((String) opUserMap.get("prcsUser_" + tmp));
            String prcsOpUser = T9Utility.null2Empty((String) opUserMap.get("prcsOpUser_" + tmp));
            String topFlag = (String) opUserMap.get("topFlag_" + tmp);
            // 强制合并节点与子流程
            String query1 = "select " + "  GATHER_NODE," + "   CHILD_FLOW, " + " ALLOW_BACK, " + " RELATION "
                    + "  from " + " FLOW_PROCESS " + "where " + " FLOW_SEQ_ID='" + flowId + "' and "
                    + " PRCS_ID='" + tmp + "'";
            Statement stm9 = null;
            ResultSet rs9 = null;
            String childFlow = "";
            ...
            try {
                ...
                T9DBUtility.close(stm9, rs9, null);
            }
            ...
            if ("0".equals(childFlow)) {
                ...
                String queryTopFlag = "SELECT TOP_FLAG from FLOW_RUN_PRCS where  " + " RUN_ID='" + runId
                        + "'  " + " and PRCS_ID='" + prcsIdNew + "'  " + " and FLOW_PRCS='" + tmp + "'  "
                        + " and TOP_FLAG IN ('1','2')";
                Statement stm = null;
                ResultSet rs = null;
                try {
                ...
                String queryIsHasHander = "SELECT 1 from FLOW_RUN_PRCS where  " + " RUN_ID='" + runId + "'  "
                        + " and PRCS_ID='" + prcsIdNew + "'  " + " and FLOW_PRCS='" + tmp + "'  "
                        + " and OP_FLAG='1'  " + " and TOP_FLAG='0'  " + " and PRCS_FLAG in('1','2')";
                int tFlag = 0;
                Statement stm2 = null;
                ResultSet rs2 = null;
                try {
                ...
                for (String userTmp : aPrcsUser) {
                    if (!userTmp.equals("")) {
                        // 如果是主办人（$PRCS_OP_USER），主办人标识置成1；如果是谁先接收谁主办，则所有经办人的主办标识全置成1.
                        // 091016
                        String opFlag = "0";
                        ...
                        // --什么时候会出现这种情况？？？091016
                        // $query =
                        // "SELECT 1 from FLOW_RUN_PRCS where RUN_ID='$RUN_ID' and PRCS_ID='$PRCS_ID_NEW' and FLOW_PRCS='$FLOW_PRCS_NEXT' and USER_ID='$TOK' and PRCS_FLAG in('1','2')";
                        String query = "SELECT 1 from FLOW_RUN_PRCS where " + " RUN_ID='" + runId + "'  "
                                + " and PRCS_ID='" + prcsIdNew + "'  " + " and FLOW_PRCS='" + tmp + "'  "
                                + " and USER_ID=" + userTmp + " and PRCS_FLAG in('1','2')";
                        Statement stm3 = null;
                        ResultSet rs3 = null;
                        PreparedStatement stm4 = null;
                        try {
                            ...
                            if (!rs3.next()) {
                                ...
                                String query2 = "SELECT 1 from FLOW_RUN_PRCS where " + " RUN_ID='" + runId
                                        + "'  " + " and FLOW_PRCS='" + tmp + "'  " + " and USER_ID="
                                        + userTmp + " and PRCS_FLAG in('1','2')";

                                queryTmp = "insert into FLOW_RUN_PRCS (RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT,CREATE_TIME,OTHER_USER) "
                                        + "values ("
                                        + runId
                                        + ","
                                        + prcsIdNew
                                ...
                                // 和parent相关
                                String queryFlowRunPrcs = "select PARENT from FLOW_RUN_PRCS where  "
                                        + "  RUN_ID='" + runId + "' and  " + "   PRCS_ID='" + prcsIdNew
                                        + "' and  " + "   FLOW_PRCS='" + tmp + "' and  " + "   USER_ID='"
                                        + userTmp + "' and  " + "   PRCS_FLAG in ('1','2')";
                                Statement stm5 = null;
                                ResultSet rs5 = null;
                                ...
                                try {
                                    ...
                                    if (rs5.next()) {
                                        parent = T9Utility.null2Empty(rs5.getString("PARENT"));
                                    }
                                } catch (Exception ex) {
                                    throw ex;
                                } finally {
                                ...
                                String updateFlowRunPrcs = "update " + "  FLOW_RUN_PRCS " + "  set  "
                                        + "   PARENT='" + parent + "'" + "   where  " + "  RUN_ID='" + runId
                                        + "' and  " + "   PRCS_ID='" + prcsIdNew + "' and  "
                                        + "   FLOW_PRCS='" + tmp + "' and  " + "   USER_ID='" + userTmp
                                        + "' and  " + "   PRCS_FLAG in ('1','2')";
                                T9WorkFlowUtility.updateTableBySql(updateFlowRunPrcs, conn);
                            }
                        } catch (Exception ex) {
                            throw ex;
                        } finally {
                    ...
                    String query = "";
                    if (tFlag == 1) {
                        query = "update FLOW_RUN_PRCS set DELIVER_TIME=?,PRCS_FLAG='4' WHERE RUN_ID=" + runId
                                + " and PRCS_ID=" + prcsId + " and FLOW_PRCS='" + flowPrcs
                                + "' and PRCS_FLAG in ('1','2')";
                    } else {
                        query = "update FLOW_RUN_PRCS set DELIVER_TIME=?,PRCS_FLAG='3' WHERE RUN_ID=" + runId
                                + " and PRCS_ID=" + prcsId + " and FLOW_PRCS='" + flowPrcs
                                + "' and PRCS_FLAG in ('1','2')";
                    }
                ...
                int runIdNew = fru.createNewWork(conn, Integer.parseInt(childFlow), prcsOpUserOld, prcsUser,
                        runId, allowBack, relation, flowId);
                String queryTopFlag = "SELECT FLOW_TYPE from FLOW_TYPE where  " + " SEQ_ID ='" + childFlow
                        + "'";
                Statement stm = null;
                ResultSet rs = null;
                String flowType = "1";
                 ...
                 */
                String insertFlowRunPrcs = "insert into FLOW_RUN_PRCS (RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT,CHILD_RUN,CREATE_TIME) "
                        + " values ("
                        + runId
                        + ","
                        + prcsIdNew
                ...
                // 直接更新状态为办结
                String updateState = "update " + "  FLOW_RUN_PRCS  " + "set  " + " DELIVER_TIME=?, "
                        + " PRCS_FLAG='4' " + " where  " + " RUN_ID = " + runId + " and  " + "  PRCS_ID = "
                        + prcsId + " and  " + " FLOW_PRCS = '" + flowPrcs + "' and "
                        + " PRCS_FLAG in ('1','2')";
                PreparedStatement stm3 = null;
                try {
    ...
    public String turnEnd(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs,
            String prcsUser, String prcsOpUser, String topFlag, String ip, Connection conn, String prcsBack,
            String viewUser) throws Exception {
        Timestamp time = new Timestamp(new Date().getTime());
        T9FlowHookUtility hookUtility = new T9FlowHookUtility();
        hookUtility.runHookPlugin(conn, runId);
        // 取出流程实例的几个字段：$PARENT_RUN、$FOCUS_USER、$RUN_NAME
        String query1 = "select " + "  RUN_NAME," + "  FOCUS_USER," + "   PARENT_RUN " + "  from "
                + "    FLOW_RUN " + "  where " + "    RUN_ID='" + runId + "'";
        Statement stm2 = null;
        ResultSet rs2 = null;
        int parentRun = 0;
        String focusUser = "";
        ...
        String updateFlag = "update FLOW_RUN_PRCS set  " + " PRCS_FLAG='4'" + " WHERE " + " RUN_ID='" + runId
                + "'  " + " and PRCS_ID='" + prcsId + "'  " + " and FLOW_PRCS='" + flowPrcs + "'";
        Statement stm = null;
        try {
            stm = conn.createStatement();
        ...
        String updateTime = "update FLOW_RUN_PRCS set  " + " DELIVER_TIME=?  " + " WHERE  " + " RUN_ID='"
                + runId + "'" + " and PRCS_ID='" + prcsId + "'" + " and FLOW_PRCS='" + flowPrcs + "'"
        ...
        /**
         * 091015 判断当前步骤是否为当前流程中唯一一个状态为“执行中”的步骤（即当前步骤状态改为4之后，系统中再无当前流程的“执行中”步骤）
         * 若是则更新当前流程的结束时间为当前系统时间
         */
        String query = "select 1 FROM FLOW_RUN_PRCS WHERE  " + " RUN_ID='" + runId + "'  "
                + " AND PRCS_FLAG IN (1,2)  " + " AND ((TOP_FLAG IN (0,1) AND OP_FLAG=1) OR TOP_FLAG=2)";
        Statement stm3 = null;
        ResultSet rs3 = null;
        PreparedStatement stm5 = null;
        try {
            ...
            rs3 = stm3.executeQuery(query);
            if (!rs3.next()) {
                String updateFlowRun = "update FLOW_RUN set  " + " END_TIME=?,VIEW_USER=? "
                        + " WHERE RUN_ID='" + runId + "'";
                stm5 = conn.prepareStatement(updateFlowRun);
                stm5.setString(2, viewUser);
                stm5.setTimestamp(1, time);
                stm5.executeUpdate();
            }
        } catch (Exception ex) {
        ...
        if (!T9Utility.isNullorEmpty(viewUser)) {
            String SMS_CONTENT = "请查阅：" + runName;
            T9SmsBack sb = new T9SmsBack();
            sb.setSmsType("7");
            sb.setContent(SMS_CONTENT);
            sb.setFromId(loginUser.getSeqId());
            sb.setToId(viewUser);
            sb.setRemindUrl("/core/funcs/workflow/flowrun/list/print/index.jsp?runId=" + runId + "&flowId="
                    + flowId);
            T9SmsUtil.smsBack(conn, sb);
        }
        // 工作流日志
        ...
        if (parentRun != 0) {
             ...
             * 
             * $PARENT_RUN：父流程的RUN_ID，见上面 $RUN_ID：流程实例ID
             */
            int parentPrcsId = 0;
            int parentFlowPrcs = 0;
            query = "select " + "  PRCS_ID," + "  FLOW_PRCS " + "  from " + "     FLOW_RUN_PRCS "
                    + "   where " + "     RUN_ID='" + parentRun + "' and " + "     CHILD_RUN='" + runId + "'";
            Statement stm9 = null;
            ResultSet rs9 = null;
            try {
                stm9 = conn.createStatement();
             ...
             */
            query = "update " + "     FLOW_RUN_PRCS " + "    set " + "     DELIVER_TIME=?,"
                    + "      PRCS_FLAG='4' " + "     where " + "      RUN_ID=" + parentRun + " and "
                    + "     PRCS_ID=" + parentPrcsId + " and " + "     FLOW_PRCS='" + parentFlowPrcs
                    + "' and " + "      PRCS_FLAG in (1,2)";
            ...
            try {
                ...
                T9DBUtility.close(stm10, null, null);
            }
            ...
            if (!"".equals(prcsBack) && !"0".equals(prcsBack)) {
                 ...
                 * 父流程返回步骤的顺序号
                 * 
                 * 说明：1、向父流程插入顺序号为$PARENT_PRCS_ID_NEW的主办/经办人记录时，
                 * 可能父流程中已经插入了比$PARENT_PRCS_ID_NEW更大的步骤经办人记录
                 * 2、如果有则实际的流程图上会出现分叉现象，还是会引起混乱？（不会太混乱，多少会有一点乱）
                 * 
                 * 这里插入的主办人和之前的主办人有无冲突？各个经办人的状态之间有无矛盾？（待测试）
                 * 
                 * 3、PARENT字段赋值为$PARENT_FLOW_RPCS，衔接步骤的定义编号，
                 * 不是只有衔接节点的PARENT字段才有值么？
                 * 
                 * 怎么父流程返回步骤的也有值？（好像没用，待确认）
                 */
                int parentPrcsIdNew = parentPrcsId + 1;
                query = "insert into FLOW_RUN_PRCS(RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT) "
                        + "values ("
                        + parentRun
                        + ","
                        + parentPrcsIdNew
                ...
                for (String ss : backUserArray) {
                    ...
                    query = "insert into FLOW_RUN_PRCS(RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT) "
                            ...
                            + ss
                            + "','1',"
                            + prcsBack + ",0,0,'" + parentFlowPrcs + "')";
                    T9WorkFlowUtility.updateTableBySql(query, conn);
                }
                // remindNext(conn
                // , parentRun , int flowId , parentPrcsIdNew ,
                // Integer.parseInt(prcsBack)
                // , String content , String contextPath , String prcsUser , int
                // userId , String sortId, String skin);
            }
            /**
             * 父流程的衔接节点不设置返回步骤，则检查父流程若所有节点是否均已办结，如果是则更新流程结束时间字段；如果还有尚未办结的节点，
             * 则空操作哦 说明：1、衔接节点在上面已经更新为4，故读这段程序时，可以看作是“除衔接节点外的各个步骤节点都已经是4”
             * 
             * 2、如果这个“衔接节点”没办理完成，整个流程是否可以结束？（好像只有在并发分叉的情况下可以）
             * 
             * 3、一个猜想（待验证）：流程里有多个节点可以“结束流程”，每一个节点结束后都更新FLOW_RUN的END_TIME字段？？？（答：
             * 当然要这样。）
             */
            else {
                query = "select 1 from FLOW_RUN_PRCS where RUN_ID='" + parentRun + "' and PRCS_FLAG!='4'";
                Statement stm7 = null;
                ResultSet rs7 = null;
                ...
                try {
                    ...
                    if (!rs7.next()) {
                        query = "update FLOW_RUN set END_TIME=? where RUN_ID='" + parentRun + "'";
                    ...
                    T9DBUtility.close(stm8, null, null);
                }
            }
        }
        if (!T9Utility.isNullorEmpty(focusUser)) {
            T9SmsBack sb = new T9SmsBack();
            sb.setSmsType("7");
            ...
            sb.setRemindUrl("/core/funcs/workflow/flowrun/list/print/index.jsp?runId=" + runId + "&flowId="
                    + flowId + "&openFlag=1&width=800&height=600");
            T9SmsUtil.smsBack(conn, sb);
        }
        T9FlowRunLogLogic logLogic = new T9FlowRunLogLogic();
     ...
     * @throws Exception
     */
    public String handlerFinish(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs,
            String ip, String sortId, String skin, Connection conn, String contextPath) throws Exception {
        StringBuffer sb = new StringBuffer();
        String result = "";
        ...
        T9FlowRunPrcs flowRunPrcs = (T9FlowRunPrcs) orm.loadObjSingle(conn, T9FlowRunPrcs.class, queryPrcs);
        // 无主办人会签，先检查是否最后一个经办人
        if ("2".equals(flowRunPrcs.getTopFlag())) {
            String query = "select 1 FROM FLOW_RUN_PRCS WHERE " + " RUN_ID='" + runId + "' "
                    + " AND PRCS_ID='" + prcsId + "' " + " AND FLOW_PRCS='" + flowPrcs + "' "
                    + " AND USER_ID<>" + loginUser.getSeqId() + " AND PRCS_FLAG IN(1,2)";
            Statement stm = null;
            ResultSet rs = null;
            try {
                stm = conn.createStatement();
        ...
        Timestamp time = new Timestamp(new Date().getTime());
        if (flowPrcs == 0) {// 自由流程
            updateQuery = "update FLOW_RUN_PRCS set " + "PRCS_FLAG='4'" + ",DELIVER_TIME=?" + " WHERE "
                    + " RUN_ID='" + runId + "' " + " and PRCS_ID='" + prcsId + "' " + " and USER_ID="
                    + loginUser.getSeqId();
            result = "'0'";
        } else {// 固定流程
            updateQuery = "update FLOW_RUN_PRCS set " + " PRCS_FLAG='4'" + " ,DELIVER_TIME=? " + " WHERE "
                    + " RUN_ID='" + runId + "' " + " and PRCS_ID='" + prcsId + "' " + " and FLOW_PRCS='"
                    + flowPrcs + "' " + " and USER_ID=" + loginUser.getSeqId();
            result = "'1'";
        }
        ...
        boolean flag = false;
        String query2 = "SELECT 1 from FLOW_RUN_PRCS WHERE " + "  RUN_ID='" + runId + "' " + "  AND PRCS_ID="
                + prcsId + "  " + "  AND OP_FLAG='0' AND PRCS_FLAG<>'4'";
        Statement stm6 = null;
        ResultSet rs6 = null;
        try {
            stm6 = conn.createStatement();
        ...
        if (flag) {
            String query = "SELECT RUN_NAME " + " ,FLOW_ID  " + " from FLOW_RUN WHERE  " + " RUN_ID=" + runId;
            ...
            query = "SELECT " + "  USER_ID  " + "  from FLOW_RUN_PRCS WHERE  " + " RUN_ID=" + runId
                    + " AND PRCS_ID=" + prcsId + " AND OP_FLAG='1' AND PRCS_FLAG<>'4'";
            ...
            try {
                ...
                T9DBUtility.close(stm8, rs8, null);
            }
            if (userId != 0) {
                String remindUrl = "";
                String smsContent = "工作流已会签完毕：" + runName + "\n您作为主办人，可以转交下一步骤";
                T9SmsBack sb1 = new T9SmsBack();
                sb1.setSmsType("7");
                sb1.setContent(smsContent);
                sb1.setFromId(loginUser.getSeqId());
                sb1.setToId(String.valueOf(userId));
                sb1.setRemindUrl("/core/funcs/workflow/flowrun/list/inputform/index.jsp?skin=" + skin
                        + "&sortId=" + sortId + "&runId=" + runId + "&flowId=" + flowId1 + "&prcsId="
                        + prcsId + "&flowPrcs=" + flowPrcs);
                T9SmsUtil.smsBack(conn, sb1);
            }
        }
        return result;
    }
    ...
    public void cancelRun(int flowId, int runId, int prcsId, int flowPrcs, Connection conn) throws Exception {
        T9ORM orm = new T9ORM();
        String query = "SELECT 1 " + "FROM FLOW_RUN_PRCS  WHERE " + "RUN_ID='" + runId + "' AND PRCS_ID>1";
        Statement stm = null;
        ResultSet rs = null;
        try {
            stm = conn.createStatement();
        ...
        if (flowType.getAutoName() != null && flowType.getAutoName().indexOf("{N}") != -1) {
            ...
            String query1 = "select 1 from FLOW_RUN WHERE " + " RUN_ID>'" + runId + "'  " + "AND FLOW_ID='"
                    + flowId + "'";
            Statement stm2 = null;
            ResultSet rs2 = null;
            try {
        ...
        Statement stm6 = null;
        try {
            String deleteFeedback = "delete from FLOW_RUN_FEEDBACK where  " + "RUN_ID=" + runId;
            stm3 = conn.createStatement();
            stm3.executeUpdate(deleteFeedback);
            String deleteRunData = "delete from FLOW_RUN_DATA where  " + "RUN_ID=" + runId;
            if (T9WorkFlowUtility.isSave2DataTable()) {
                ...
                String tableName = T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + flowId + "_"
                        + formSeqId;
                deleteRunData = "delete from " + tableName + " where " + "RUN_ID=" + runId;
            }
            stm4 = conn.createStatement();
            stm4.executeUpdate(deleteRunData);

            String deleteFlowRunPrcs = "delete from FLOW_RUN_PRCS where  " + "RUN_ID=" + runId;
            stm5 = conn.createStatement();
            stm5.executeUpdate(deleteFlowRunPrcs);
            String deleteFlowRun = "delete from FLOW_RUN where " + " RUN_ID=" + runId;
            stm6 = conn.createStatement();
            ...
            T9DBUtility.close(stm4, null, null);
            T9DBUtility.close(stm5, null, null);
            T9DBUtility.close(stm6, null, null);
        }
    }
    ...
    public String getPreRunPrcs(int runId, int prcsId, int flowId, Connection conn) throws Exception {
        ...
        String query = "select " + "COUNT(distinct PRCS_ID)" + ",PRCS_ID" + ",FLOW_PRCS "
                ...
                + " and PRCS_ID!=" + prcsId + " GROUP BY prcs_id , flow_prcs";
        Statement stm1 = null;
        ResultSet rs1 = null;
        String prcsIdStr = "";
        try {
     ...
     * @throws Exception
     */
    public void remindAllAndSend(Connection conn, int remindFlag, String content, String contextPath,
            int runId, int userId, int flowId) throws Exception {
        Map map2 = new HashMap();
        map2.put("RUN_ID", runId);
        T9ORM orm = new T9ORM();
        T9FlowRun flowRun = (T9FlowRun) orm.loadObjSingle(conn, T9FlowRun.class, map2);
        ...
        // 短信提醒所有经办人
        String ids = "";
        String query = "SELECT USER_ID from FLOW_RUN_PRCS where RUN_ID=" + runId;
        Statement stm = null;
        ResultSet rs = null;
        try {
            stm = conn.createStatement();
            ...
            while (rs.next()) {
                ...
                if (!T9WorkFlowUtility.findId(ids, String.valueOf(userId)) && userId1 != userId) {
                    // 没有，或者userId
        ...
        if ((remindFlag & 4) > 0) {
            if (!"".equals(ids)) {
                ...
                sb.setFromId(userId);
                sb.setToId(ids);
                sb.setRemindUrl("/core/funcs/workflow/flowrun/list/print/index.jsp?runId=" + runId
                        + "&flowId=" + flowId + "&openFlag=1&width=800&height=600");
                T9SmsUtil.smsBack(conn, sb);
            }
            // 短信提醒发起人
        }
     ...
     * @throws Exception
     */
    public void remindNext(Connection conn, int runId, int flowId, int prcsId, int flowPrcs, String content,
            String contextPath, String prcsUser, int userId, String sortId, String skin) throws Exception {
        T9SmsBack sb = new T9SmsBack();
        sb.setSmsType("7");
        sb.setContent(content);
        sb.setFromId(userId);
        sb.setToId(prcsUser);
        sb.setRemindUrl("/core/funcs/workflow/flowrun/list/inputform/index.jsp?skin=" + skin + "&sortId="
                + sortId + "&runId=" + runId + "&flowId=" + flowId + "&prcsId=" + prcsId + "&flowPrcs="
                + flowPrcs);
        T9SmsUtil.smsBack(conn, sb);

        T9MsgPusher.mobilePushNotification(prcsUser, content, "workflow");
    }
     ...
     * @throws Exception
     */
    public void remindNextEmail(Connection conn, String content, String contextPath, String prcsUser,
            int userId) throws Exception {
        // T9EmailUtil.emailNotifier(conn, userId, prcsUser, "工作流转交下一步提醒",
        // content, new Date(), important)
    }
    ...
    public boolean hasDelete(int runId, Connection conn) throws Exception {
        String query = "SELECT DEL_FLAG from FLOW_RUN WHERE RUN_ID=" + runId;
        Statement stm = null;
        ResultSet rs = null;
        try {
            stm = conn.createStatement();
     ...
     * @throws Exception
     */
    public String analysisAutoFlag(T9FlowRun flowRun, T9FlowType ft, T9FlowFormType form, Connection conn,
     ...
     */
    public String analysisAutoFlag(T9FlowRun flowRun, T9FlowType ft, T9FlowFormType form, Connection conn,
            String imgPath, String modelShort) throws Exception {
        Map map = new HashMap();
        ...
        if (!"0".equals(flowDoc) && modelShort.indexOf("#[附件") != -1) {
    ...
    public void remaindEmail(int flowId, int prcsId, int runId, Connection conn, T9Person user,
            String imgPath, String contextPath) throws Exception {
        String query = "SELECT MAIL_TO from FLOW_PROCESS where FLOW_SEQ_ID=" + flowId + " and PRCS_ID="
                + prcsId;
        Statement stm = null;
        ResultSet rs = null;
        String mailTo = "";
        ...
        if (mailTo != null && !"".equals(mailTo)) {
            ...
            // 处理附件
            T9EmailUtil.emailNotifier(conn, user.getSeqId(), mailTo, flowRun.getRunName(), form,
                    new java.util.Date(), "1", contextPath, true);
        }
    }

    public boolean canHandlerWrok(int runId, int prcsId, int flowPrcs, int userId, Connection conn)
            throws Exception {
        boolean flag = false;
    ...
    public void manageTurnNext(Connection conn, int runId, int prcsId, int flowPrcs) throws Exception {
        String query = "SELECT  " + " PARENT " + " from  " + "FLOW_RUN_PRCS  " + " WHERE  " + " RUN_ID='"
                + runId + "' AND  " + " PRCS_ID='" + prcsId + "' AND  " + " FLOW_PRCS='" + flowPrcs + "'";
        Statement stm = null;
        ResultSet rs = null;
        String parent = "";
        ...
        String query1 = "update   " + "   FLOW_RUN_PRCS   " + "  set   " + "  PRCS_FLAG='4'   "
                + "    WHERE   " + "   RUN_ID='" + runId + "' AND   " + "   PRCS_ID='" + prcsIdPre + "'";
        if (!"0".equals(parent) && !T9Utility.isNullorEmpty(parent)) {
