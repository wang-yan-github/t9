...
public class T9FreeFlowLogic {
     ...
     * @param runId
     * @param prcsId
     * @return String {flowName:'自由流程',isPreview:true ,
     *         runName:'自由流程测试(2010-03-09 10:56:34)',
     *         formItem:'',prcsList:[{prcsId:1,handlerState:'<font
     *         color=blue>系统管理员[主办人](已办结)</font>'},{prcsId:2,handlerState:'<font
     *         color=red>系统管理员[主办人](办理中)</font>,<font
     *         color=red>dddd(未接收)</font>' , isNowPrcs:true}]};
     * @throws Exception
     */
    public String getTurnData(T9Person loginUser, int flowId, int runId, int prcsId, Connection conn,
        ...
        String query2 = "SELECT  " + " RUN_NAME  " + " from FLOW_RUN WHERE  " + " RUN_ID=" + runId;
        String runName = "";
        Statement stm1 = null;
        ResultSet rs1 = null;
        try {
        ...
        for (int i = 1; i <= prcsId; i++) {
            String query3 = "SELECT * from FLOW_RUN_PRCS where  " + " RUN_ID=" + runId + "  "
                    + " and PRCS_ID=" + i + " order by OP_FLAG desc";
            Statement stm3 = null;
            ResultSet rs3 = null;
            String userNameStr = "";
            try {
                ...
                while (rs3.next()) {
                    ...
                    if (i == prcsId && (prcsFlag == 3 || prcsFlag == 4) && userId == loginUser.getSeqId()
                            && !isManage) {
                        turnforbindden = 1;
                    } else {
                        turnforbindden = 0;
    ...
    public String turnNext(T9Person user, int flowId, int runId, int prcsId, Connection conn, boolean preSet,
            List<Map> preList, String itemOld) throws Exception {
        T9ORM orm = new T9ORM();
        ...
        if (!preSet) {
            for (int i = 0; i < preList.size(); i++) {
                ...
                if ("2".equals(freeOther)) {
                    ...
                    prcsUser1 = tu.turnOther(prcsUser1, flowId, runId, prcsId, "0", prcsOpUserOld, otherMap,
                            conn);
                }
                int topFlag = 0;
                ...
                for (String tmpStr : str) {
                    ...
                    String insertRunPrcs = "insert into FLOW_RUN_PRCS (" + " RUN_ID" + " ,PRCS_ID"
                            + " ,USER_ID" + " ,PRCS_FLAG" + " ,OP_FLAG" + " ,TOP_FLAG" + " ,FREE_ITEM"
                            + " , CREATE_TIME" + " ) values (" + runId + " ," + prcsIdTmp + " ," + tmpStr
                            + " ,'" + prcsFlag + "'" + " ,'" + opFlag + "'" + " ,'" + topFlag + "'" + " ,'"
                            + freeItem + "'" + " , ? " + " )";
                    PreparedStatement stm = null;
                    try {
                        stm = conn.prepareStatement(insertRunPrcs);
            ...
            String query = "select USER_ID FROM FLOW_RUN_PRCS WHERE " + "  PRCS_FLAG='5'  "
                    + " AND  RUN_ID='" + runId + "'  " + " and PRCS_ID='" + prcsIdNext + "'";
            Statement stm = null;
            ResultSet rs = null;
            try {
                stm = conn.createStatement();
            ...
            String updateFlowRun = "update FLOW_RUN_PRCS set " + "  PRCS_FLAG='1'  " + " WHERE "
                    + "  RUN_ID='" + runId + "'  " + " and PRCS_ID='" + prcsIdNext + "'";
            Statement stm2 = null;
            try {
        ...
        // -- 设置当前步骤所有人为已转交 --
        String query = "update FLOW_RUN_PRCS set  " + " PRCS_FLAG='3' " + " WHERE RUN_ID=" + runId
                + " and PRCS_ID=" + prcsId;
        Statement stm = null;
        try {
            stm = conn.createStatement();
        ...
        // -- 设置当前步骤自己的结束时间 --
        Timestamp time = new Timestamp(new Date().getTime());
        query = "update FLOW_RUN_PRCS set  " + " DELIVER_TIME=?" + " WHERE  " + " RUN_ID=" + runId
                + " and PRCS_ID=" + prcsId + " and USER_ID=" + user.getSeqId();
     ...
     * @throws Exception
     */
    public String stop(int runId, int flowId, int prcsId, T9Person user, Connection conn) throws Exception {
        T9ORM orm = new T9ORM();
        ...
        if (!isManage) {
            ...
            map.put("PRCS_ID", prcsId);
            T9FlowRunPrcs flowRun = (T9FlowRunPrcs) orm.loadObjSingle(conn, T9FlowRunPrcs.class, map);
            flowRun.setDeliverTime(new Date());
            orm.updateSingle(conn, flowRun);
        } else {// 监控人结束补全步骤开始和结束时间为当前时间
            String sql = "update FLOW_RUN_PRCS  set " + " DELIVER_TIME=? " + " WHERE  " + " RUN_ID=" + runId
            ...
            query = "update FLOW_RUN_PRCS set  " + " PRCS_TIME=?" + " WHERE  " + " RUN_ID=" + runId
        ...
        // --- 结束本流程 ---
        T9WorkFlowUtility.updateTable("FLOW_RUN_PRCS", "PRCS_FLAG='4'", "RUN_ID=" + runId, conn);
        // --- 写入结束时间 ---
        String writeEndTime = "update FLOW_RUN set   " + " END_TIME=?   " + " where   " + " RUN_ID=" + runId;
        Timestamp time = new Timestamp(new Date().getTime());
