...
public class T9AttachmentLogic {
  public static String filePath = T9SysProps.getAttachPath() + File.separator  + "workflow";
  ...
  public String createAttachment(int runId, String newType, String newName , Connection conn , String webrootPath) throws Exception {
    ...
    if (success) {
      ...
      T9ORM orm = new T9ORM();
      orm.updateSingle(conn, flowRun);
      return "{attachmentName:\""+ T9Utility.encodeSpecial(fileName) +"\", attachmentId:'"+ hard + "_" + attachmentId +"'}";
    } else {
      return "''";
    }
  }
  ...
  public void delFeedAttachment(int feedId , String attachmentId , String attachmentName , Connection conn) throws Exception{
    //处理数据库
    T9ORM orm = new T9ORM();
    T9FlowRunFeedback fb = (T9FlowRunFeedback) orm.loadObjSingle(conn, T9FlowRunFeedback.class, feedId);
  ...
  public String getDownPrintPriv(int runId , int flowId , int userId , Connection conn ) throws Exception {
    ...
    String query = "select ATTACH_PRIV from FLOW_RUN_PRCS,FLOW_PROCESS WHERE FLOW_RUN_PRCS.RUN_ID="+ runId +" AND FLOW_PROCESS.FLOW_SEQ_ID="+ flowId +" AND FLOW_RUN_PRCS.FLOW_PRCS=FLOW_PROCESS.PRCS_ID AND FLOW_RUN_PRCS.USER_ID="+ userId ;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
