...
public class T9FormVersionLogic {
  public void updateFormVersion(Connection conn , int seqId , int formId , int versionNo ) throws Exception {
    String sql = "UPDATE FLOW_FORM_TYPE SET VERSION_TIME = ? ,VERSION_NO = "+versionNo +", FORM_ID = "+formId+" WHERE SEQ_ID =  " + seqId;
  ...
  public boolean hasFormVersion(Connection conn , int formId ) throws Exception {
    String sql = "select  1  from flow_form_type WHERE form_id =  " + formId;
    PreparedStatement stmt = null;
    ResultSet rs = null;
  ...
  public int getVersionNo(Connection conn  , int runId) throws Exception {
    int formId = 1 ;
    String sql2 = "SELECT FORM_VERSION FROM FLOW_RUN WHERE RUN_ID=" + runId;
  ...
  public String getVersionNoByForm(Connection conn  , int formId) throws Exception {
    String sql2 = "SELECT VERSION_NO FROM FLOW_FORM_TYPE WHERE SEQ_ID=" + formId + " OR FORM_ID=" + formId + " ORDER BY VERSION_NO DESC";
    Statement stmt2= null;
    ResultSet rs2 = null;
    String ss = "";
    try {
    ...
    ss = T9WorkFlowUtility.getOutOfTail(ss);
    return ss;
  }
  public int getMaxFormId(Connection conn ) throws Exception {
    int formId = 0 ;
    String sql2 = "SELECT max(SEQ_ID) FROM FLOW_FORM_TYPE";
    Statement stmt2= null;
    ResultSet rs2 = null;
    try {
      stmt2 = conn.createStatement();
  ...
  public int getMaxVersion(Connection conn , int seqId) throws Exception {
    int formId = 1 ;
    String sql2 = "SELECT VERSION_NO FROM FLOW_FORM_TYPE WHERE SEQ_ID =" + seqId;
    Statement stmt2= null;
    ResultSet rs2 = null;
    try {
      stmt2 = conn.createStatement();
  ...
  public void updateFormId(Connection conn  , int seqId , int formId) throws Exception {
    String sql2 = "update  FLOW_FORM_TYPE set FORM_ID = " + formId + " where SEQ_ID=" + seqId;
    T9WorkFlowUtility.updateTableBySql(sql2, conn);
  }
      ...
      if (!T9Utility.isNullorEmpty(flowTypes)) {
        ...
        for (String ss : flowType) {
          if (!T9Utility.isNullorEmpty(ss)) {
            String tableName = "" +T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + ss + "_" + oldId;
            String newTableName = "" + T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + ss + "_" + newId;
  ...
  public boolean isExistRunFlowRun444(int formId , Connection conn) throws Exception{
    String query  = "SELECT 1 from FLOW_RUN where FORM_VERSIOIN="+ formId + " AND END_TIME is null ";
  ...
  public void updateFormItem(Connection dbConn, int id, int newId) throws Exception {
    ...
    String update = "update FLOW_FORM_ITEM SET FORM_ID = " + newId + " where FORM_ID=" + id;
    T9WorkFlowUtility.updateTableBySql(update, dbConn);
  }
  public static String getTableName(int flowId ,int formSeqId) {
    return  T9WorkflowSave2DataTableLogic.FORM_DATA_TABLE_PRE + flowId + "_" + formSeqId;
  }
  public String getTableName(Connection conn , T9FlowRun flowRun) throws Exception {
    ...
    return T9FormVersionLogic.getTableName(flowRun.getFlowId(), formSeqId);
  }
  public String getTableName(Connection conn , int runId , int flowId) throws Exception {
    ...
    int formSeqId = lo.getFormSeqId(conn, versionNo, formId);
    return T9FormVersionLogic.getTableName(flowId, formSeqId);
  }
  
  public int getFormSeqId(Connection conn , int versionNo , int formId) throws Exception {
    int formId1 = 0 ;
    String sql2 = "SELECT SEQ_ID FROM FLOW_FORM_TYPE WHERE (FORM_ID =" + formId + " or  SEQ_ID = " + formId +") AND VERSION_NO = " + versionNo;
    Statement stmt2= null;
    ResultSet rs2 = null;
    try {
      stmt2 = conn.createStatement();
