...
import t9.core.funcs.workflow.util.sort.T9FormVersionComparator;
...
public class T9FlowFormLogic {
  ...
  public int getFlowFormMax(Connection dbConn, int seqId) throws Exception {
    ...
    ResultSet rs = null;
    int max = 0 ;
    String sql = "SELECT ITEM_MAX FROM FLOW_FORM_TYPE WHERE SEQ_ID = " 
                + seqId;
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
  ...
  public List<T9FlowFormItem> formToMap(Connection dbConn, int seqId) throws Exception{
    List<T9FlowFormItem> list = new ArrayList();
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "select TITLE , ITEM_ID ,TAG, CLAZZ, TYPE , NAME , CONTENT , VALUE , RADIO_CHECK , RADIO_FIELD from FLOW_FORM_ITEM where FORM_ID =" + seqId + " order by ITEM_ID";
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
        T9FlowFormItem item = new T9FlowFormItem();
  ...
  public String getTitle(Connection dbConn, int seqId ) throws Exception{
    List<T9FlowFormItem> hs = formToMap(dbConn, seqId);
    String result = "";
    for (T9FlowFormItem item : hs) {
  ...
  public String getTitle(Connection dbConn, int seqId , StringBuffer sb) throws Exception{
    List<T9FlowFormItem> hs = formToMap(dbConn, seqId);
    String result = "";
    int count = 0 ;
    for (T9FlowFormItem item : hs) {
      ...
      if ("LIST_VIEW".equals(clazz)) {
        count++;
        sb.append("{name:\"").append(item.getName()).append("\",title:\"").append(T9Utility.encodeSpecial(val)).append("\"}").append(",");
      }
      if(!"".equals(result)){
        result += ",";
      }
  ...
  public String getTitle2(Connection dbConn, int seqId) throws Exception{
    ...
    for (T9FlowFormItem item : hs) {
      ...
      sb.append("{");
      sb.append("title:\"" + T9WorkFlowUtility.encodeSpecial(item.getTitle()) + "\"");
      sb.append(",value:\"" + T9WorkFlowUtility.encodeSpecial(value) + "\"");
      sb.append(",content:\"" +  T9WorkFlowUtility.encodeSpecial(content) + "\"");
      sb.append(",type:\"" + type + "\"");
  ...
  public  String getFlowFormTypeOption(Connection conn ,T9Person u) throws Exception{
    String query = "select " 
      ...
      + " , SEQ_ID"
      + " , DEPT_ID "
      + " from FLOW_FORM_TYPE where FORM_ID = '0'";
    StringBuffer sb = new StringBuffer("[");
    int count = 0 ;
    Statement stm = null;
    ResultSet rs = null;
  ...
  public String getMetaDataItem(int formId , Connection conn) throws Exception {
    StringBuffer sb = new StringBuffer();
    String query = "select title , metadata_name from flow_form_item where form_Id =" + formId + " and metadata_name is not null";
    Statement stm = null;
    ResultSet rs = null;
    int count = 0 ;
    try{
  ...
  public String getIdBySort(Connection dbConn,int sortId) throws Exception{
    String sql = "";
    if(sortId == 0){
      sql =  "SELECT SEQ_ID FROM FLOW_FORM_TYPE WHERE SEQ_ID NOT IN ( SELECT FORM_SEQ_ID FROM FLOW_TYPE ) AND FORM_ID = 0 ";
    }else{
      sql = "SELECT SEQ_ID FROM FLOW_FORM_TYPE WHERE SEQ_ID IN ( SELECT FORM_SEQ_ID FROM FLOW_TYPE WHERE FLOW_SORT = " + sortId + ")  AND FORM_ID = 0 ";
    }
    String result = "";
  ...
  public String getIdByForm(Connection dbConn,int seqId) throws Exception{
    String sql = "";
    sql =  "SELECT SEQ_ID FROM FLOW_FORM_TYPE WHERE FORM_ID = " +  seqId;
    String result = seqId + ",";
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try{
  ...
  public StringBuffer flowFormType2JsonByType(Connection dbConn ,int seqId , T9Person u) throws Exception{
    StringBuffer sb = new StringBuffer("[");
    StringBuffer field = new StringBuffer();
    ...
    for (T9FlowFormType fft : list){
      ...
      field.append("{\"seqId\":'").append(fft.getSeqId()).append("'")
           .append(",").append("\"versionTime\":'").append(T9Utility.getDateTimeStr(fft.getVersionTime()))
           .append("',")
  ...
  public boolean isExistFlowRun(int formId , Connection conn) throws Exception{
    String query  = "SELECT 1 from FLOW_TYPE where FORM_SEQ_ID="+ formId;
  ...
  public int getSortId(Connection conn,int formId) throws Exception {
    String query  = "SELECT FLOW_SORT from FLOW_TYPE where FORM_SEQ_ID="+ formId;
    Statement pstmt = null;
    ResultSet rs = null;
    try{
      pstmt = conn.createStatement();
  ...
  public void deleteFormItems(String seqId , Connection conn) throws Exception {
    ...
    if (seqId.endsWith(",")) {
      seqId = T9WorkFlowUtility.getOutOfTail(seqId);
    }
    String query = "delete from FLOW_FORM_ITEM where FORM_ID in (" + seqId + ")";
    Statement stm = null;
    try{
      stm = conn.createStatement();
  ...
  public void deleteFormType(String seqId , Connection conn) throws Exception {
    ...
    String query = "delete from FLOW_FORM_TYPE where SEQ_ID in (" + seqId + ")";
    Statement stm = null;
    try{
      stm = conn.createStatement();
      stm.executeUpdate(query);
  ...
  public void deleteForm (int seqId , Connection conn) throws Exception {
    String query = "select FORM_ID from FLOW_FORM_TYPE where SEQ_ID = " + seqId;
    Statement stm3 = null;
    ResultSet rs3 = null;
    int result = 0;
    try{
      ...
      T9DBUtility.close(stm3, rs3 , log);
    }
    if (result == 0) {
      String query2 = "select SEQ_ID from FLOW_FORM_TYPE where FORM_ID = " + seqId;
      Statement stm4 = null;
      ResultSet rs4 = null;
      String ids = "";
      try{
        ...
        T9DBUtility.close(stm4, rs4 , log);
      }
      ids += seqId + ",";
