...
import t9.core.funcs.workflow.util.sort.T9FeedbackComparator;
...
public class T9FeedbackLogic {
   ...
   * @throws Exception
   */
  public void saveFeedback(T9FlowRunFeedback feedback , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
  ...
  public String getFeedbacksHtml(T9Person loginUser , int flowId ,int runId , Connection conn) throws Exception{
    ...
    String query = "SELECT * from FLOW_RUN_FEEDBACK where RUN_ID="+ runId +" order by PRCS_ID,EDIT_TIME";
...
//          $CONTENT_VIEW=nl2br($CONTENT_VIEW);//将字符串中的换行符转成HTML的换行符号
  ...
  public List<T9FlowRunFeedback> getSignLookFeedback(Map signLookMap , int flowType, int runId  , int userId , Connection conn) throws Exception {
    List<T9FlowRunFeedback> list = new ArrayList();
    String query = "select * from FLOW_RUN_FEEDBACK where RUN_ID =" + runId + " order by EDIT_TIME DESC";
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      ...
      while(rs2.next()){
        ...
        if (seeSign) {
          T9FlowRunFeedback feedback = new T9FlowRunFeedback();
  ...
  public boolean isOpUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    ...
    String query = "select 1 from FLOW_RUN_PRCS where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public boolean isHoUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    boolean isPrcsUser = false;
    String query = "select 1 from FLOW_RUN_PRCS where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId + " and OP_FLAG='1'" ;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public void setPrcsMap(Map prcsNameMap , Map signLookMap , int runId , int flowId, int type ,   Connection conn) throws Exception {
    ...
    String queryPrcs = "select PRCS_ID , FLOW_PRCS from FLOW_RUN_PRCS where RUN_ID=" + runId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
...
//      }
   ...
   * @throws Exception
   */
  public String getFeedbacks(T9Person loginUser  , int runId , int prcsId , int flowId , Connection conn) throws Exception{
    Map prcsNameMap = new HashMap();
    Map signLookMap = new HashMap();
    int type = this.getFlowType(conn, flowId);
    this.setPrcsMap(prcsNameMap, signLookMap, runId, flowId , type, conn);
    List<T9FlowRunFeedback> list = this.getSignLookFeedback(signLookMap ,type , runId, loginUser.getSeqId(), conn);
    StringBuffer sb = new StringBuffer();
  ...
  public void delFeedback(int seqId ,Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9FlowRunFeedback tmp = (T9FlowRunFeedback) orm.loadObjSingle(conn, T9FlowRunFeedback.class, seqId);
    String attachmentId = tmp.getAttachmentId() == null ? "" : tmp.getAttachmentId();
    String attachmentName = tmp.getAttachmentName() == null ? "" : tmp.getAttachmentName();
  ...
  public String getSignData(int feedId , Connection conn) throws Exception {
    String signData = "";
    String query = "select SIGN_DATA from FLOW_RUN_FEEDBACK where SEQ_ID = "+ feedId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
