package t9.core.funcs.workstat.logic;
...
public class T9WorkStatLogic {
   ...
   * @throws Exception
   */
  public String getDeptTreeJson(int deptId, Connection conn, T9Person user)
      throws Exception {
    Statement stmt = null;
    ResultSet rs = null;
    ...
    sql = "select * from module_priv where module_id='8' and user_seq_id="
        + userId;
  ...
  public void getSelfDeptTree(int deptId, StringBuffer sb, Connection conn)
    ...
    String sql = "select SEQ_ID , DEPT_NAME from DEPARTMENT where SEQ_ID="
        + deptId + " order by DEPT_NO, DEPT_NAME asc";
    rs = stmt.executeQuery(sql);
    if (rs.next()) {
      String deptName = rs.getString("DEPT_NAME");
      ...
      sb.append("{");
      sb
          .append("text:'" + "&nbsp;├" + T9Utility.encodeSpecial(deptName)
              + "',");
      sb.append("value:" + seqId);
      sb.append("},");
  ...
  public String getIsStaticLogic(Connection conn, int userId) throws Exception {
    ...
    String sql = "select * from module_priv where module_id='8' and user_seq_id="
        + userId;
    rs = stmt.executeQuery(sql);
    if (rs.next()) {
      deptPriv = rs.getString("dept_priv");
   ...
   * 
   * */
  public String getDataLogic(Connection conn, T9Person user, String deptId,
  ...
  public StringBuffer getDataList(Connection conn, String userIdStr,
    ...
    for (int i = 0; i < user.length; i++) {
      ...
      while (rs.next()) { 
        ...
        sql = "select dept_name from department where seq_id=" + deptId;
        rs1 = stmt1.executeQuery(sql);
        String deptName="";
        if (rs1.next()) {
      ...
      // 内部邮件（收）
      sql = "select EMAIL.TO_ID,count(EMAIL.TO_ID) from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and "
          + T9DBUtility.findNoInSet("", "EMAIL.TO_ID")
          + " and DELETE_FLAG!='1' and "
          + " EMAIL.TO_ID in ("+userIdStrdd+")"
          + " and "
          + T9DBUtility.getDateFilter("send_time", startDate, ">=")
          + " and "
          + T9DBUtility.getDateFilter("send_time", endDate, "<=")
          + " group by EMAIL.TO_ID";
      ...
      // 内部邮件（发）
      sql = "select FROM_ID,count(FROM_ID) from EMAIL_BODY where "
          + " FROM_ID in("+userIdStrdd+") and "
          + T9DBUtility.getDateFilter("send_time", startDate, ">=") + " and "
          + T9DBUtility.getDateFilter("send_time", endDate, "<=")
      ...
      // 日程安排（完成）
      sql = "select USER_ID,count(USER_ID) from CALENDAR where CAL_TYPE!='2' and "
          + T9DBUtility.findNoInSet("", "USER_ID")
          + " and "
          + "USER_ID in ("+userIdStrdd+")"
          + " and "
          + T9DBUtility.getDateFilter("cal_time", startDate, ">=")
          + " and "
          + T9DBUtility.getDateFilter("end_time", endDate, "<=")
      ...
      // 工作日志
      sql = "select USER_ID,count(USER_ID) from DIARY where DIA_TYPE!='2' and "
          + T9DBUtility.findNoInSet("", "USER_ID") + " and "
          + " USER_ID in("+userIdStrdd+") and "
          + T9DBUtility.getDateFilter("DIA_DATE", startDate, ">=") + " and "
          + T9DBUtility.getDateFilter("DIA_DATE", endDate, "<=")
          + " group by USER_ID";
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
        String userId=rs.getString(1);
      ...
      // 工作流主办（完成）
      sql = "select USER_ID,count(distinct(FLOW_RUN.RUN_ID)) from FLOW_RUN,FLOW_RUN_PRCS where FLOW_RUN.DEL_FLAG=0 AND FLOW_RUN.RUN_ID=FLOW_RUN_PRCS.RUN_ID and "
          + "USER_ID in ("+userIdStrdd+")"
          + " and "
          + T9DBUtility.findNoInSet("", "USER_ID")
          + " and PRCS_FLAG=4 and OP_FLAG=1 and "
          + T9DBUtility.getDateFilter("FLOW_RUN.BEGIN_TIME", startDate, ">=")
          + " and "
          + T9DBUtility.getDateFilter("FLOW_RUN.BEGIN_TIME", endDate, "<=")
          + " group by USER_ID";
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
        String userId=rs.getString(1);
      ...
      // 工作流会签（完成）
      sql = "select USER_ID,count(distinct(FLOW_RUN.RUN_ID)) from FLOW_RUN,FLOW_RUN_PRCS where FLOW_RUN.DEL_FLAG=0 AND "
          + T9DBUtility.findNoInSet("", "USER_ID")
      ...
      // 公告通知（发）
      sql = "select FROM_ID,count(FROM_ID) from NOTIFY where "
          + " FROM_ID in("+userIdStrdd+") and "
          + T9DBUtility.findNoInSet("", "FROM_ID") + " and "
          + T9DBUtility.getDateFilter("SEND_TIME", startDate, ">=") + " and "
          + T9DBUtility.getDateFilter("SEND_TIME", endDate, "<=")
          + " group by FROM_ID";
      ...
      // 新闻（发）
      sql = "select PROVIDER,count(PROVIDER) from NEWS where "
          + " PROVIDER in ("+userIdStrdd+") and "
          + T9DBUtility.findNoInSet("", "PROVIDER") + " and "
          + T9DBUtility.getDateFilter("NEWS_TIME", startDate, ">=") + " and "
          + T9DBUtility.getDateFilter("NEWS_TIME", endDate, "<=")
          + " group by PROVIDER";
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
        String userId=rs.getString(1);
   ...
   * 
   * */
  public boolean isDeptPriv(Connection conn, String deptMore, T9Person user)
      throws Exception {
    Statement stmt = null;
    ResultSet rs = null;
    stmt = conn.createStatement();
  ...
  public List<Map<String, String>> getDataToExeclLogic(Connection conn,
      T9Person user, String deptId, String startDate, String endDate,
  ...
  public List<Map<String, String>> getDataToExcelList(Connection conn,
    ...
    for (int i = 0; i < user.length; i++) {
      ...
      sql = "select USER_ID,count(USER_ID) from DIARY where DIA_TYPE!='2' and "
          + T9DBUtility.findNoInSet("", "USER_ID") + " and "
          + " USER_ID in("+userIdStrdd+") and "
          + T9DBUtility.getDateFilter("DIA_TIME", startDate, ">=") + " and "
          + T9DBUtility.getDateFilter("DIA_TIME", endDate, "<=")
          + " group by USER_ID";
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
        String userId=rs.getString(1);
  ...
  public String getCalFinishLogic(Connection dbConn, Map<String, String> map)
    ...
    try {
      ...
      sql += " and " + T9DBUtility.findInSet(userId, "USER_ID");
      sql += " and " + T9DBUtility.getDateFilter("CAL_TIME", startDate, ">=");
      sql += " and " + T9DBUtility.getDateFilter("END_TIME", endDate, "<=");
      if (status.equals("1")) {
      ...
      while (rs.next()) {
        ...
        data.append("content:'" + T9Utility.encodeSpecial(rs.getString("content")) + "'");
        data.append("},");
  ...
  public String toSearchData(Connection conn, Map request, int userId)
      throws Exception {
    String sql = "select SEQ_ID,DIA_DATE,SUBJECT,ATTACHMENT_NAME,ATTACHMENT_ID from DIARY where 1=1 ";
    ...
    filters = " and USER_ID='" + userId + "' and DIA_TYPE!='2' and "
        + T9DBUtility.getDateFilter("DIA_DATE", startDateStr, ">=") + " and "
        + T9DBUtility.getDateFilter("DIA_DATE", endDateStr, "<=");
  ...
  public String getFlowNameLogic(Connection dbConn, String flowId)
    ...
    try {
      String flowName = "";
      String sql = "select FLOW_NAME  from FLOW_TYPE where SEQ_ID=" + flowId;
      rs = stmt.executeQuery(sql);
      if (rs.next()) {
