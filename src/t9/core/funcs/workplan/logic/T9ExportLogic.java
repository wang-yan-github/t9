...
public class T9ExportLogic {
  ...
  public ArrayList<T9DbRecord> getDbRecord(List<T9WorkPlan> worklist,Connection dbConn) throws Exception{
    ...
    for (int i = 0; i < worklist.size(); i++) {
      ...
      if (!T9Utility.isNullorEmpty(plan.getType())) {
      ...
      if (!plan.getDeptParentDesc().equals("0") && !plan.getDeptParentDesc().equals("ALL_DEPT") && !T9Utility.isNullorEmpty(plan.getDeptParentDesc())) {
  ...
  public String getType(Connection dbConn,String typeId) {
    ...
    String nameType = "";
    String sql = "select TYPE_NAME from plan_type where SEQ_ID=" + Integer.parseInt(typeId);
    try {
  ...
  public String getDept(Connection dbConn,String deptId) {
    ...
    try {
      for (int i = 0; i < deptId.split(",").length;i++) {
        String sql = "select DEPT_NAME from DEPARTMENT where SEQ_ID=" + deptId.split(",")[i];
        ps = dbConn.prepareStatement(sql);
        rs = ps.executeQuery();
        if (rs.next()) {
