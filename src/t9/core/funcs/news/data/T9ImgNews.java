package t9.core.funcs.news.data;
...
public class T9ImgNews{
  ...
  public static final String BASE_PATH = T9SysProps.getAttachPath()+ profix +"news" + profix;
  ...
  public List<String> getImagPaths() throws Exception{   
    ...
    if(!T9Utility.isNullorEmpty(attachmentName) && !T9Utility.isNullorEmpty(attachmentId)){
      ...
      for(int i=0; i<attrId.length; i++){
        if(!T9Utility.isNullorEmpty(attrName[i]) && !T9Utility.isNullorEmpty(attrId[i]) && isPicture(attrName[i])){
          String absolutePate = "attachmentId="+attrId[i]+"&module=" + T9NewsCont.MODULE + "&attachmentName="+T9Utility.encodeURL(attrName[i]);
    ...
    if(!T9Utility.isNullorEmpty(attachmentName) && !T9Utility.isNullorEmpty(attachmentId)){
      ...
      for(int i=0; i<attrId.length; i++){
        if(!T9Utility.isNullorEmpty(attrName[i]) && !T9Utility.isNullorEmpty(attrId[i]) &&  isPicture(attrName[i])){
          ...
          String absolutePate = BASE_PATH + attId +"_" +  T9Utility.encodeURL(T9Utility.encodeURL((attrName[i])));
          paths.add(absolutePate);
        }
      }      
    }
  ...
  public String toJson() throws Exception{
    ...
    sb.append("{");
       ...
       sb.append("subject:").append("\"").append(T9Utility.encodeSpecial(subject)).append("\"").append(",");
    ...
    sb.append("{");
       ...
       sb.append("imgPaths:").append("[");
         ...
         if(list != null && list.size() >0){
           for(int i=0; i<list.size(); i++){
             sb.append("{");
               sb.append("path:").append("\"").append(T9Utility.encodeSpecial(list.get(i))).append("\"");
             if(i < list.size()-1){
               sb.append("}").append(",");
             }else{
               sb.append("}");
