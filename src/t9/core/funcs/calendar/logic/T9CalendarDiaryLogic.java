...
public class T9CalendarDiaryLogic {
  private static Logger log = Logger.getLogger("t9.core.act.action.T9SysMenuLog");
  public int addCalDiary(Connection dbConn,T9CalendarDiary cd) throws Exception {
    T9ORM orm = new T9ORM();
    if(cd!=null){
  ...
  public String selectDiaryId(Connection dbConn,String str) throws Exception {
    ...
    ResultSet rs = null;
    String diaryIdStr = "";
    String sql = "select * from CALENDAR_DIARY ";
  ...
  public String selectDiaryIdByDate(Connection dbConn,String date) throws Exception {
    ...
    if(date!=null&&!date.equals("")){
      String beginDate = T9DBUtility.getDateFilter("CAL_DIA_DATE", date, ">=");
      String endDate = T9DBUtility.getDateFilter("CAL_DIA_DATE", date + " 23:59:59", "<=");
      String sql = "select * from CALENDAR_DIARY where " + beginDate + " and " + endDate ;
      try {
        stmt = dbConn.createStatement();
        rs = stmt.executeQuery(sql);
        while(rs.next()){
  ...
  public String selectCalendarId(Connection dbConn,String str) throws Exception {
    ...
    ResultSet rs = null;
    String calendarStr = "";
    String sql = "select * from CALENDAR_DIARY where DIARY_ID =(" + str + ")";
    if(str!=null&&!str.equals("")){
      try {
        stmt = dbConn.createStatement();
        rs = stmt.executeQuery(sql);
