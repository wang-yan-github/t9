...
public class T9BindUsersLogic {
  private static Logger log = Logger.getLogger("t9.core.funcs.person.logic.T9BindUsersLogic");
  ...
  public void bindUser(Connection conn, int userId, String userIdOther, String userDescOther,String sysId)throws Exception {
    PreparedStatement ps = null;
    try {
      String sql = "insert into BIND_USERS" +
      		" (USER_SEQ_ID" +
      		", SYS_ID" +
      		", USER_ID_OTHER" +
      		", USER_DESC_OTHER)" +
  ...
  public void removeBind(Connection conn, int userId)throws Exception {
    PreparedStatement ps = null;
    try {
      String sql = "delete from BIND_USERS" +
      		" where USER_SEQ_ID = ?";
