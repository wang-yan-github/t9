...
public class T9PersonLogic {
  private static Logger log = Logger.getLogger("t9.core.funcs.person.logic.T9PersonLogic");
  ...
  public T9Person getPerson(Connection conn, String seqId)throws Exception{
    T9Person  person = null;
    try {
      T9ORM orm = new T9ORM();
      person = (T9Person)orm.loadObjSingle(conn , T9Person.class , Integer.parseInt(seqId));
    }catch(Exception ex) {
      throw ex;
    }finally {
      T9DBUtility.close(null , null , log);
    }
    return person;
  }
  ...
  public String getDeptNameByDeptId(Connection conn, int deptId)throws Exception{
    ...
    String deptName = "";
    try {
      String queryStr = "select DEPT_NAME from DEPARTMENT where SEQ_ID=" + deptId; 
      stmt = conn.createStatement();
      rs = stmt.executeQuery(queryStr);
      if(rs.next()) {
  ...
  public T9Person getPersonById(String userId , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9Person person = new T9Person();
    Map query = new HashMap();
    query.put("USER_ID", userId);
    person = (T9Person) orm.loadObjSingle(conn, T9Person.class, query);;
    return person;
  }
  
  public T9Person getPersonById(int seqId, Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9Person person = (T9Person) orm.loadObjSingle(conn, T9Person.class, seqId);;
    return person;
  }
  /**
   * 根据userid串返回一个名字串
  ...
  public String getNameByIdStr(String ids, Connection conn) throws Exception, Exception{
    ...
    if(ids != null && !"".equals(ids)){
      ...
      for(String tmp : aId){
        if(!"".equals(tmp)){
          T9Person person = this.getPersonById(tmp , conn);
          if(person != null){
  ...
  public String getDeptNameBySeqIdStr(String ids , Connection conn) throws Exception, Exception{
    String names = ""; 
    if(ids != null && !"".equals(ids.trim())){ 
    if (ids.endsWith(",")){ 
    ids = ids.substring(0, ids.length() - 1); 
    } 
    String query = "select DEPT_NAME from DEPARTMENT where SEQ_ID in (" + ids + ")"; 
    Statement stm = null; 
    ResultSet rs = null; 
    try { 
    stm = conn.createStatement(); 
  ...
  public List<T9Person> getPersonByDept(int deptId  , Connection conn) throws Exception{
    Map query = new HashMap();
    query.put("DEPT_ID", deptId);
    T9ORM orm = new T9ORM();
    List<T9Person> list = orm.loadListSingle(conn, T9Person.class, query);
    return list;
  }
  public List<T9Person> getPersonByRole(int roleId  , Connection conn) throws Exception{
    Map query = new HashMap();
   ...
   * @return
   */
  public String getPersonSimpleJson(List<T9Person> list){
  ...
  public String getPersonSimpleJson(String userIds ,Connection conn) throws Exception{
    ...
    for(String tmp : aId){
      if(!"".equals(tmp)){
        T9Person person = this.getPersonById(Integer.parseInt(tmp) , conn);
        sb.append(person.toJsonSimple());
        count++;
      }
    }
  ...
  public List<T9Person> getDeptIdOther(Connection dbConn) throws Exception {
    T9ORM orm = new T9ORM();
    String[] filters = null;
  ...
  public String getManagePersonList(Connection conn,Map request,String deptId, String deptIdOther, String loginUserPriv, boolean isOaAdmin, String deptIdLogin, String postDept, String postPriv) throws Exception{
    //List<T9Person> personList = getDeptIdOther(conn);
    ...
    T9PageDataList pageDataList1 = null;
...
//          pageDataList = T9PageLoader.loadPageList(conn, queryParam, sql);
//        }else{
//          String sql = "select SEQ_ID" 
//              + ",USER_ID" 
//              + ",USER_NAME" 
...
//    }
    if(isOaAdmin){
      ...
      ",PERSON.NOT_LOGIN" +
      ",PERSON.PASSWORD" +
      ",PERSON.POST_PRIV from PERSON, USER_PRIV where (DEPT_ID ="+Integer.parseInt(deptId)+" or " + T9DBUtility.findInSet(deptId, "DEPT_ID_OTHER")+ ") and PERSON.USER_PRIV = USER_PRIV.SEQ_ID";
    }else{
      sql =  "select PERSON.SEQ_ID" +
      ",PERSON.USER_ID" +
      ",PERSON.USER_NAME" +
      ...
      ",PERSON.NOT_LOGIN" +
      ",PERSON.PASSWORD" +
      ",PERSON.POST_PRIV from PERSON, USER_PRIV where (DEPT_ID =("+ Integer.parseInt(deptId) +") or " + T9DBUtility.findInSet(deptIdLogin, "DEPT_ID_OTHER")+ ") and PERSON.USER_PRIV = USER_PRIV.SEQ_ID and USER_PRIV.PRIV_NO > " + privNo + " and not USER_PRIV.SEQ_ID = 1";
   ...
   // String query = " order by DIA_DATE desc,DIA_TIME DESC ";
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      if (T9PassEncrypt.isValidPas("", pass)) {
  ...
  public String getManagePersonList(Connection conn,Map request,String deptId, T9Person loginPerson) throws Exception{
    String whereStr = "";
    int userPriv = 1; //OA管理员 角色权限表 seqId=1
    
    T9UserPriv up = T9UserPrivCache.getUserPrivCache(conn, loginPerson.getUserPriv());
    String userPrivNo = "";
    if (up != null) {
       userPrivNo = String.valueOf(up.getPrivNo());
    }
    T9UserPriv up2 = T9UserPrivCache.getUserPrivCache(conn, userPriv);
    String adminPrivNo = "";
    if (up2 != null) {
      adminPrivNo = String.valueOf(up2.getPrivNo());
    }
    ...
    T9PageDataList pageDataList = null;
      sql =  "select PERSON.SEQ_ID" +
                   ...
                   ",PERSON.PASSWORD" +
                 //",PERSON.POST_PRIV from PERSON, USER_PRIV where (DEPT_ID =("+ Integer.parseInt(deptId) +") or " + T9DBUtility.findInSet(deptIdLogin, "DEPT_ID_OTHER")+ ") and PERSON.USER_PRIV = USER_PRIV.SEQ_ID and USER_PRIV.PRIV_NO > " + privNo + " and not USER_PRIV.SEQ_ID = 1";
                   ",PERSON.POST_PRIV from PERSON,USER_PRIV where PERSON.SEQ_ID IN ("+ userIds +") AND USER_PRIV.SEQ_ID = PERSON.USER_PRIV " 
   ...
   * @throws Exception 
   */
  public String getFalwUserId(Connection conn,T9Person loginPerson,String moduleId ,int privNoFlag,String deptId) throws Exception{
    String result = "";
    ...
    try {
      ...
      String sql = "";
      if(loginPerson.isAdminRole()){
     //   sql = "select PERSON.SEQ_ID FROM PERSON, USER_PRIV WHERE  USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND DEPT_ID=" + deptId + " OR " + T9DBUtility.findInSet(deptId + "", "DEPT_ID_OTHER") + " order by USER_PRIV.PRIV_NO , PERSON.USER_NO DESC ,PERSON.SEQ_ID";
        sql = "select PERSON.SEQ_ID,DEPT_ID,USER_PRIV FROM PERSON, USER_PRIV WHERE  USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND (DEPT_ID=" + deptId + " OR " + T9DBUtility.findInSet(deptId + "", "DEPT_ID_OTHER") + ") ";
        
      
      }else{
   //     sql = "select PERSON.SEQ_ID FROM PERSON, USER_PRIV WHERE USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND NOT PERSON.SEQ_ID = " + loginPerson.getSeqId() + " and (DEPT_ID=" + deptId + " OR " + T9DBUtility.findInSet(deptId + "", "DEPT_ID_OTHER") + ") order by USER_PRIV.PRIV_NO , PERSON.USER_NO DESC ,PERSON.SEQ_ID";
        sql = "select PERSON.SEQ_ID,DEPT_ID,USER_PRIV FROM PERSON, USER_PRIV WHERE USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND NOT PERSON.SEQ_ID = " + loginPerson.getSeqId() + " and (DEPT_ID=" + deptId + " OR " + T9DBUtility.findInSet(deptId + "", "DEPT_ID_OTHER") + ") ";
      ...
      while(rs.next()){
        ...
        if(T9PrivUtil.isUserPriv(conn, p, mp, loginPerson.getPostPriv(), loginPerson.getPostDept(), loginPerson.getSeqId(), loginPerson.getDeptId())){
          if(!"".equals(result)){
            result += ",";
          }
          result += userId;
  ...
  public String getDutyTypeLogic(Connection conn , int dutyId) throws Exception{
    String result = "";
    String sql = " select DUTY_NAME from ATTEND_CONFIG where SEQ_ID = " + dutyId ;
    PreparedStatement ps = null;
    ResultSet rs = null ;
    try {
      ps = conn.prepareStatement(sql);
  ...
  public void deleteAll(Connection conn, String seqIds, String loginUserId) throws Exception {
    String str = " NOT USER_ID = 'admin' and NOT USER_ID = '" + loginUserId + "' and SEQ_ID IN (" + seqIds + ")";
    String sql = "DELETE FROM PERSON WHERE" + str;
    T9ORM orm = new T9ORM();
    ...
    for (String seqId : seqIdStr) {
      ...
      String sql1 = "DELETE FROM hr_staff_info WHERE USER_ID ='"+person.getUserId()+"'";
      ...
      try {
        ...
        T9DBUtility.close(pstmt1, null, null);
      }
    }
    ...
    if (T9ReportSyncLogic.hasSync) {
      str = " NOT USER_ID = 'admin' and NOT USER_ID = '" + loginUserId + "' and UID IN (" + seqIds + ")";
      Connection reportConn = T9ReportSyncLogic.getReportConn();
      T9PersonSyncLogic logic2 = new T9PersonSyncLogic();
      logic2.delPerson(str, reportConn);
      if (reportConn != null) {
        reportConn.close();
      }
    }
    if (T9OaSyncLogic.hasSync) {
      ...
      str = " NOT USER_ID = 'admin' and NOT USER_ID = '" + loginUserId + "' and UID IN (" + seqIds + ")";
      t9.core.module.oa.logic.T9PersonSyncLogic logic2 = new t9.core.module.oa.logic.T9PersonSyncLogic();
      logic2.delPerson(str, reportConn);
      if (reportConn != null) {
        reportConn.close();
      }
  ...
  public void clearPassword(Connection conn, String seqIdStrs) throws Exception {
    String strs = T9PassEncrypt.encryptPass("");
    String sql = "UPDATE PERSON SET PASSWORD ='"  +  strs + "' WHERE SEQ_ID IN (" + seqIdStrs + ")";
    ...
    if (T9ReportSyncLogic.hasSync) {
      ...
      logic.updatePersonPwd(seqIdStrs, reportConn, strs);
      if (reportConn != null) {
        reportConn.close();
      }
    }
  ...
  public T9Person getMenuPara(Connection conn, int seqId) throws Exception {
    Statement stmt = null;
    ResultSet rs = null;
    T9Person org = null;
    try {
  ...
  public ArrayList<T9Person> getSearchPersonList(Connection conn,Map request, String userId, String userName
    ...
    if(!T9Utility.isNullorEmpty(userId)){ 
      sql = sql + " and USER_ID like '%" + userId + "%'" + T9DBUtility.escapeLike(); 
    } 
    if(!T9Utility.isNullorEmpty(userName)){ 
      sql = sql + " and USER_NAME like '%" + userName + "%'" + T9DBUtility.escapeLike(); 
    } 
    if(!T9Utility.isNullorEmpty(byname)){ 
      sql = sql + " and BYNAME like '%" + byname + "%'" + T9DBUtility.escapeLike(); 
    } 
  ...
  public String getPostPrivOtherLogic(Connection conn , int loginDeptId) throws Exception{
    String result = "";
    String sql = " select DEPT_NAME from DEPARTMENT where SEQ_ID = " + loginDeptId ;
    PreparedStatement ps = null;
    ResultSet rs = null ;
    try {
      ps = conn.prepareStatement(sql);
  ...
  public String getSetLogList(Connection conn,Map request, int loginUserId) throws Exception{
    ...
    if (dbms.equals("sqlserver")) {
      ...
      ",IP" +
      ",TYPE" +
      ",REMARK from SYS_LOG where USER_ID=" + loginUserId + " and TYPE='19' order by TIME desc";
      ...
      ",IP" +
      ",TYPE" +
      ",REMARK from SYS_LOG where USER_ID=" + loginUserId + " and TYPE='19' order by TIME desc limit 10";
      ...
      ",IP" +
      ",TYPE" +
      ",REMARK from SYS_LOG where USER_ID=" + loginUserId + " and TYPE='19' and rownum <= 10 order by TIME desc";
    } else {
      sql = "select SEQ_ID" +
      ",USER_ID" +
      ",TIME" +
  ...
  public void getSearchSet(Connection dbConn , String dept, String role, String user ,String setStr, String privId1) throws Exception{
    ...
    if(!dept.equals("0")){
      ...
      if(!user.equals("")){
        String ss = " PERSON.SEQ_ID IN (" + user + ") or " ;
        if ("oracle".equals(dbms)) {
          ss = this.getIn("PERSON.SEQ_ID", user) + " or ";
        }
        whereStr += " " + ss;
      }
    }
    ...
    if (!T9Utility.isNullorEmpty(userIdStrs)) {
    String userIds = userIdStrs.substring(0, userIdStrs.length() - 1);
      if (!"".equals(setStr)) {
        serStrs = setStr.substring(0, setStr.length() - 1);
        try {
          String ss = " SEQ_ID IN (" + userIds + ")" ;
          if ("oracle".equals(dbms)) {
            ss = this.getIn("SEQ_ID", userIds);
          }
          String queryStr = "update PERSON set " + serStrs + " where not USER_ID = 'admin' and " + ss ;
          stmt = dbConn.createStatement();
          ...
          if (T9ReportSyncLogic.hasSync) {
            Connection reportConn = T9ReportSyncLogic.getReportConn();
            T9PersonSyncLogic logic = new T9PersonSyncLogic();
            logic.updatePerson(serStrs, reportConn, userIds);
            if (reportConn != null) {
              reportConn.close();
            }
          }
  ...
  public boolean existsTableNo(Connection dbConn, String userIdStr)
    ...
    try {
      stmt = dbConn.createStatement();
      String sql = "SELECT count(*) FROM MODULE_PRIV WHERE MODULE_ID='0' and USER_ID like '"+userIdStr+"'";
      rs = stmt.executeQuery(sql);
      //System.out.println(sql);
      long count = 0;
      if (rs.next()) {
  ...
  public void setUpdateModulePriv(Connection dbConn, String privId1, String funcs) throws Exception {
    ...
    String sql = "update MODULE_PRIV set PRIV_ID = '" + privId1 + "' WHERE USER_ID like '" + funcs + "' and MODULE_ID='0'";
    //System.out.println(sql);
    Statement stmt = null;
    ResultSet rs = null;
    try {
  ...
  public void setInsertModulePriv(Connection dbConn, String privId1, String funcs) throws Exception {
    ...
    String sql = "insert into MODULE_PRIV (USER_ID,MODULE_ID,DEPT_PRIV,ROLE_PRIV,PRIV_ID) values('"+funcs+"','0','1','2','"+privId1+"')";
    //System.out.println(sql);
    Statement stmt = null;
    ResultSet rs = null;
    try {
  ...
  public String getNoPassUserId(Connection dbConn) throws Exception{
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if(!T9PassEncrypt.isValidPas("",password.trim())){
          continue;
        }
  ...
  public String getUserInformation(Connection dbConn ,int seqId) throws Exception{
    ...
    if(T9Utility.isNullorEmpty(passwod)){
      passwod = "";
    }
    result = userId + "," + T9UsbKey.md5Hex(passwod);
    return result;
  }
  ...
  public List<T9Person> getPersonByPriv(Connection dbConn, String[] str)
      throws Exception {
  ...
  public List getRoleList(Connection conn, int privNo, T9Person person) throws Exception{
    T9ORM orm = new T9ORM();
    ...
    if(person.isAdminRole()){
      ...
      list  = orm.loadListSingle(conn ,T9UserPriv.class , filterStr);
    }
    return list;
  }
  
  public ArrayList<T9DbRecord> toExportPersonData(Connection conn,Map request, String userId, String userName
      , String byname, String sex, String deptId, String userPriv, String postPriv, String notLogin
      , String notViewUser, String notViewTable, String dutyType, String lastVisitTime
      , String loginUserPriv, boolean isAdminRole) throws Exception{
    String whereStr = "";
  ...
  public ArrayList<T9DbRecord> toExportPersonData1(Connection conn) throws Exception{
    ArrayList<T9DbRecord> result = new ArrayList<T9DbRecord>();
    String sql = "SELECT SEQ_ID"
      + ",USER_ID "
      + ",DEPT_ID "
  ...
  throws Exception {
    long count = 0;
    String sql = "SELECT count(*) FROM DEPARTMENT WHERE DEPT_NAME = '" + deptName + "'";
    PreparedStatement ps = null;
    ResultSet rs = null ;
    try {
  ...
  public int getDeptIdLogic(Connection conn , String deptName) throws Exception{
    int result = 0;
    String sql = " select SEQ_ID from DEPARTMENT where DEPT_NAME = '" + deptName +"'";
    PreparedStatement ps = null;
    ResultSet rs = null ;
    try {
      ps = conn.prepareStatement(sql);
