...
public class T9SysLogSearchLogic {
  /**
   * 统计总天数
   * 保存系统日志
   * @param conn
  ...
  public static String getMySysLog(Connection conn,int userId) throws Exception{
    ...
    try{
        ...
        DateFormat sdf2 = new SimpleDateFormat("M/d/yyyy"); 
        Date Timedesc = null;
        if (time.indexOf("-") >= 0 ) {
          Timedesc = sdf.parse(time);
        } else {
          Timedesc = sdf2.parse(time);
        }
        Date Timeasc = null;
        if (time1.indexOf("-") >= 0 ) {
          Timeasc = sdf.parse(time1);
        } else {
          Timeasc = sdf2.parse(time1);
        }
  ...
  public static String getMySysYearLog(Connection conn,int userId) throws Exception{
    ...
    try{
       ...
       // String SysLogYear = "select count(*) from SYS_LOG where TYPE='1' and TIME >= to_date('2010-01-01 00:00:00','yyyy-mm-dd HH24:MI:SS') and TIME <= to_date('2010-12-31 23:23:59','yyyy-mm-dd HH24:MI:SS')";
        String SysLogYear = "SELECT count(*) from SYS_LOG where TYPE=1 and " + T9DBUtility.curYearFilter("TIME");
  ...
  public static String getMySysMonthLog(Connection conn,int userId) throws Exception{
    ...
    try{
        ...
        String month = "SELECT count(*) from SYS_LOG where TYPE=1 and " + T9DBUtility.curMonthFilter("TIME");
  ...
  public static String getMySysDayLog(Connection conn,int userId) throws Exception{
    ...
    try{
        Date date = new Date();
        Date dates = new Date();
        String date1= T9Utility.getCurDateTimeStr();
       ...
       //String day = "SELECT count(*) from SYS_LOG where TYPE='1' and TIME>= to_timestamp('" + date1 + "', 'yyyy-mm-dd HH24:MI:SS') and TIME<=to_timestamp('" + datetime + "', 'yyyy-mm-dd HH24:MI:SS')";
        String day = "SELECT count(*) from SYS_LOG where TYPE=1 and " + T9DBUtility.curDayFilter("TIME");
  ...
  public static String getMySysTenLog(Connection conn,int userId) throws Exception{
    ...
    try{
        ...
        stmt = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
            ResultSet.CONCUR_READ_ONLY);
        String dbms = T9SysProps.getProp("db.jdbc.dbms");// 通过配置文件配置（sysconfig.properties）
                                                         // 区分是用的那种数据库
        // String SysTenLog =
        // " select * from (SELECT SEQ_ID,USER_ID,TIME,IP,TYPE,REMARK  from SYS_LOG order by TIME desc) where rownum <= 10";
        String SysTenLog = " SELECT SEQ_ID,USER_ID,TIME,IP,TYPE,REMARK from SYS_LOG order by TIME desc";
        ...
        while (rs.next()) {
            ...
            String time = T9Utility.getDateTimeStr(tt);
