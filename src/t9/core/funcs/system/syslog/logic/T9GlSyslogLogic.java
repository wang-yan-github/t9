...
public class T9GlSyslogLogic {
  public String getglsyslog(String type, String users, String statrtime, String endtime,String ip, String remark, String copytime,Connection conn, T9Person user)
    throws Exception {
    ...
    try{
         ...
         if(copytime.equals("on")){
             if( T9Utility.isNullorEmpty(statrtime) && !T9Utility.isNullorEmpty(endtime) ) {
                 ...
                 queryGllogSql = "select seq_id, user_id, time, ip, type, remark from sys_log where "+enddate+"";
             }else if( T9Utility.isNullorEmpty(endtime) && !T9Utility.isNullorEmpty(statrtime)){
                 queryGllogSql = "select seq_id, user_id, time, ip, type, remark from sys_log where "+startdate+" ";
             }else{
                 queryGllogSql = "select seq_id, user_id, time, ip, type, remark from sys_log where "+startdate+" " +
               	"and "+enddate+" ";
             }
        ...
        for(int i=0; i< list.size(); i++){
           ...
           sb.append(",remarks:\"" + T9Utility.encodeSpecial(String.valueOf(tmpMap.get("remarks"))) + "\"");
           sb.append("},");
        }
        if(list.size() > 0)
   ...
   * @return
   */
  public String deletRadio(String type, String users, String statrtime, String endtime,String ip, String remark,String copytime,Connection conn, T9Person user)
    throws Exception {
    StringBuffer sb = new StringBuffer();
    Statement stmt = null;
    ResultSet rs = null;
    ...
    try{
         ...
         //删除之前的 总数
         if(copytime.equals("on")){
            querybeforCount = "select count(*) from sys_log";
         }else{
         ...
         if(copytime.equals("on")){  //等于on 是单选按钮  选重了当前日志             if(T9Utility.isNullorEmpty(statrtime) && !T9Utility.isNullorEmpty(endtime)){
               queryGllogSql = "delete from sys_log where " + enddate + " ";
             }
             else if(T9Utility.isNullorEmpty(endtime) && !T9Utility.isNullorEmpty(statrtime)){
               queryGllogSql = "delete from sys_log where " + startdate + " ";
             }else{
               queryGllogSql = "delete from sys_log where " + startdate + "" +
                  " and " + enddate + " ";
             }
             ...
             if(T9Utility.isNullorEmpty(statrtime) && !T9Utility.isNullorEmpty(endtime)){
                 queryGllogSql = "delete from sys_log_"+ copytime +" where "+ enddate +"";
             }
               else if(T9Utility.isNullorEmpty(endtime) && !T9Utility.isNullorEmpty(statrtime)){
                 queryGllogSql = "delete from sys_log_"+ copytime +" where "+ startdate +"";
             }else{
                 queryGllogSql = "delete from sys_log_" + copytime + " where " + startdate + "" +
                  " and "+enddate+"";
        ...
        // 删除之后的总数
        if(copytime.equals("on")){
           queryafterCount = "select count(*) from sys_log";
        }else{
  ...
  public String getEndWorkList(int pageIndex, String flowIdStr, int showLen,
    ...
    String run_id, String run_name, Connection conn, T9Person user)
    throws Exception {
    ...
    ResultSet rs = null;// 结果集    try {
        ...
        stmt = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
        ResultSet.CONCUR_READ_ONLY);
        String startdate= T9DBUtility.getDateFilter("BEGIN_TIME", starttime, ">=");
        String enddate = T9DBUtility.getDateFilter("BEGIN_TIME", endtime, "<="); 
        String queryFlowRunPrcsSql = "select r.RUN_ID" + " ,RUN_NAME"
            + " ,BEGIN_TIME" + " ,END_TIME" + " ,f.SEQ_ID" + " ,ATTACHMENT_NAME"
            + " ,COMMENT_PRIV" + " ,QUERY_USER"
            + " from FLOW_TYPE f,FLOW_RUN r where " + " f.SEQ_ID=r.FLOW_ID"
            + " and r.DEL_FLAG=0";
        /*
        ...
        // --- “工作名称/文号”条件 ---sql=sql+" and PSN_NAME like '%" + psnName + "%'" + T9DBUtility.escapeLike();
        if (!"".equals(run_name) && run_name != null && !"0".equals(run_name)) {
            queryFlowRunPrcsSql = queryFlowRunPrcsSql + " and r.RUN_NAME like "
                + "'%" + T9DBUtility.escapeLike(run_name) + "%'" + T9DBUtility.escapeLike();
        }
        ...
        while (rs.next()) {
            ...
            if (!T9WorkFlowUtility.findId(seqIdStr, String.valueOf(runId))) {
                ...
                if ("3".equals(comment_priv)
                    && T9WorkFlowUtility.findId(comment_priv, String.valueOf(userId))
                    || T9WorkFlowUtility.findId(query_user, String.valueOf(userId))
   ...
   * @return
   */
  public boolean deleteChecknews(Connection dbConn,T9Person loginperson,String deleteStr) throws IOException, Exception {
    ...
    try {
        stmt = dbConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
            ResultSet.CONCUR_READ_ONLY);
        deletenewsCommentSql = "delete  from sys_log  where seq_id in  ("+ deleteStr + ")";
   ...
   * @return
   */
  public boolean deleteAll(String copytime, Connection dbConn,T9Person loginperson) throws IOException, Exception {
    String deletenewsCommentSql = "";
    String deletenewsSql = ""; 
    Statement stmt = null;
    ResultSet rs = null;
    ...
    try {
       ...
       //qingkong xitong rizhi xingongneng    if(copytime.equals("on")){
        deletenewsCommentSql = "delete from sys_log ";
      //}else{
