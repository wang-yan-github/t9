...
public class T9GlSyslogAct {
  private static Logger log = Logger
      .getLogger(" t9.core.funcs.system.syslog.act.T9SysLogAct");
  /**
   * Request
   */
  ...
  public String SysExport(HttpServletRequest request,
    ...
    try {
        ...
        T9ExportlogLogic exportlog = new T9ExportlogLogic();
        ArrayList<T9DbRecord> dbL = exportlog.sysexportlog(type, users,
   ...
   * T9Person loginUser = null; //System.out.println("sss"); T9FileUploadForm
   * fileForm = new T9FileUploadForm();
   ...
   * dbConn = requestDbConn.getSysDbConn(); T9Person user =
   * (T9Person)request.getSession().getAttribute("LOGIN_USER"); int count = 0;
   * list.remove(0); for (int i = 1;i < list.size(); i++){ String[] s =
   * list.get(i); } for(String[] s : list){ T9Bilingual bi = new T9Bilingual();
   * bi.setCnName(s[0]); bi.setEnName(s[1]); bi.setEntryDate(new
   * Date(System.currentTimeMillis()));
   * 
   * // this.logic.addBilingual(dbConn, bi); T9ExportlogLogic importlog = new
   * T9ExportlogLogic(); DateFormat format = new
  ...
  public void parseUploadRequest(HttpServletRequest request) throws Exception {
    ...
    parseUploadRequest(request, T9SysProps
        .getInt(T9SysPropKeys.MAX_UPLOAD_FILE_SIZE), 10 * T9Const.K, T9SysProps
        .getString("fileUploadTempDir"), T9Const.DEFAULT_CODE);
  }
  /**
   * 取得附件上传缓存路径
   * @return
  ...
  public String deleteAllChecklog(HttpServletRequest request,
    ...
    try {
          ...
          request.setAttribute(T9ActionKeys.RET_STATE, T9Const.RETURN_OK);
          request.setAttribute(T9ActionKeys.RET_MSRG, "成功删除数据");
          request.setAttribute(T9ActionKeys.RET_DATA, st);
    } catch (Exception ex) {
        request.setAttribute(T9ActionKeys.RET_STATE, T9Const.RETURN_ERROR);
        request.setAttribute(T9ActionKeys.RET_MSRG, ex.getMessage());
        throw ex;
