...
public class T9ResManageLogic {
  ...
  public String getModuleData(Connection conn) throws Exception {
    ...
    String[] str = {"EMAIL","EMAIL_BODY","SMS","SMS_BODY","FLOW_RUN","FLOW_RUN_DATA","FILE_SORT","FILE_CONTENT","NOTIFY","DIARY"};
    int count = 0 ;
    for (String tmp : str) {
  ...
  public Map getTableInfoByName(Connection conn ,String tableName) throws Exception {
    ...
    try {
      ...
      if (rs.next()){
        ...
        String tableDesc = "";
        if ("EMAIL".equals(tableName)) {
          tableDesc = "邮件";
       } else if ("EMAIL_BODY".equals(tableName)) {
         tableDesc = "邮件内容";
       } else if ("SMS".equals(tableName)) {
         tableDesc = "短信";
       } else if ("SMS_BODY".equals(tableName)) {
         tableDesc = "短信内容";
       } else if ("FLOW_RUN".equals(tableName)) {
         tableDesc = "工作流";
       } else if ("FLOW_RUN_DATA".equals(tableName)) {
         tableDesc = "工作流数据";
       } else if ("FILE_SORT".equals(tableName)) {
         tableDesc = "文件柜文件夹";
       } else if ("FILE_CONTENT".equals(tableName)) {
         tableDesc = "文件柜文件";
       } else if ("NOTIFY".equals(tableName)) {
         tableDesc = "公告通知";
       } else if ("DIARY".equals(tableName)) {
         tableDesc = "工作日志";
       } 
  ...
  public void deleteDelBox(Connection conn) throws Exception {
    // TODO Auto-generated method stub
    String query  = "select * from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and BOX_ID='0' and SEND_FLAG='1' and DELETE_FLAG='4'";
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
      ...
      if (!"".equals(bodyStr)) {
        ...
        String query5 ="delete from EMAIL_BODY where SEQ_ID in ("+bodyStr+")";
        this.executeSql(query5, conn);
      }
      String query6 = "delete from EMAIL where DELETE_FLAG='4' and BOX_ID='0'";
      this.executeSql(query6, conn);
      String query7 = "update EMAIL set DELETE_FLAG='1' where DELETE_FLAG='3' and BOX_ID='0'";
  ...
  public static void deleteAttachement(String aId , String aName ,String  module) {
    //处理文件
      ...
      String path = T9SysProps.getAttachPath() + File.separator  +  module + File.separator  +  hard  + File.separator  + str + "_" + aName;
      File file = new File(path);
      if(file.exists()){
        file.delete();
      } else {
        //兼容老的数据
        String path2 = T9SysProps.getAttachPath() + File.separator  + module  + File.separator  +  hard  + File.separator  + str + "." + aName;
  ...
  public void deleteDeletedDelbox(Connection conn) throws Exception {
    // TODO Auto-generated method stub
    String query  = "select  BODY_ID from EMAIL where DELETE_FLAG='1'";
    Statement stm3 = null;
    ResultSet rs3 = null;
    String sqlStr = "";
    try {
      ...
      while (rs3.next()) {
        ...
        sqlStr += bodyId + ",";
        String query2 = "select BODY_ID from EMAIL where BODY_ID='"+bodyId+"' and DELETE_FLAG<>'1'";
        Statement stm2 = null;
        ResultSet rs2 = null;
        try {
          stm2 = conn.createStatement();
          rs2 = stm2.executeQuery(query2);
          if (!rs2.next()){
            String query3 = "delete from EMAIL where BODY_ID = '"+bodyId+"'";
            this.executeSql(query3, conn);
            String query4  = "select ATTACHMENT_ID, ATTACHMENT_NAME from EMAIL_BODY where SEQ_ID = '"+bodyId+"'";
            Statement stm5 = null;
            ResultSet rs5 = null;
            try {
              stm5 = conn.createStatement();
            ...
            String query10 = "delete from EMAIL_BODY where SEQ_ID = '"+bodyId+"'";
  ...
  public void woriteLog(Connection conn , String remark , int userId , String ip) throws Exception {
    T9SysLogLogic logic = new T9SysLogLogic();
  ...
  public void deleteSms(Connection conn) throws Exception {
    ...
    String query = "select SMS.SEQ_ID,SMS.BODY_SEQ_ID from SMS,SMS_BODY where SMS.BODY_SEQ_ID=SMS_BODY.SEQ_ID and  REMIND_FLAG='0' and DELETE_FLAG='2' and " + T9DBUtility.getDateFilter("SEND_TIME", curDate, " <= ");
    Statement stm5 = null;
    ResultSet rs5 = null;
    String bodyIdDel = "";
    String smsIdDel = "";
    ...
    if (!"".equals(bodyIdDel)) {
      ...
      String query1 = "delete from SMS_BODY where SEQ_ID in ("+bodyIdDel+")";
      this.executeSql(query1, conn);
    }
    String sql2 = "update SMS a set DELETE_FLAG = '1' where  exists (select 1 from SMS_BODY b where b.SEQ_ID = a.BODY_SEQ_ID and "  + T9DBUtility.getDateFilter("b.SEND_TIME", curDate, " <= ") + " ) and REMIND_FLAG='0' and DELETE_FLAG='0'";
   // String sql ="update SMS a,SMS_BODY set DELETE_FLAG='1' where SMS.BODY_SEQ_ID=SMS_BODY.SEQ_ID  and REMIND_FLAG='0' and DELETE_FLAG='0' and " + T9DBUtility.getDateFilter("SEND_TIME", curDate, " <= ");
      ...
      while (rs5.next()) {
        ...
        try {
          ...
          if (!rs3.next()){
            ...
            sql = "delete from SMS_BODY where SEQ_ID = '"+bodyId+"'";
            this.executeSql(sql, conn);
          }
        } catch(Exception ex) {
          throw ex;
  ...
  public String getDeptName(int deptId , Connection conn) throws Exception {
    String query1 = "select dept_name from department where seq_id=" + deptId;
    Statement stm3 = null;
    ResultSet rs3 = null;
    String deptName = "";
    try {
  ...
  public long getEmailSize( String query , Connection conn) throws Exception {
    ...
    try {
      ...
      while (rs3.next()){
        ...
        if (!T9Utility.isNullorEmpty(ATTACEMENT_ID)) {
  ...
  public long getEmailSize(int seqId , Connection conn) throws Exception {
    String query = "SELECT ENSIZE,ATTACHMENT_ID from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and EMAIL.TO_ID='"+seqId+"' and SEND_FLAG='1' and DELETE_FLAG!='1'";
    long EMAIL_SIZE1=getEmailSize(query, conn);
    query = "SELECT ENSIZE,ATTACHMENT_ID from EMAIL_BODY where FROM_ID='"+seqId+"' and SEND_FLAG='0'";
    long EMAIL_SIZE2= this.getEmailSize(query, conn);
    String sql = "SELECT  EMAIL_BODY.SEQ_ID, ENSIZE , ATTACHMENT_ID from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and FROM_ID='"+seqId+"' and SEND_FLAG='1' and DELETE_FLAG!='2' and DELETE_FLAG!='4'";
    Statement stm3 = null;
    ResultSet rs3 = null;
    long EMAIL_SIZE3 = 0;
    String sqlStr = "";
  ...
  public String getUserRes(Connection conn, int userId) throws Exception {
    ...
    try {
      ...
      if (rs3.next()){
         ...
         int emailCapacity = rs3.getInt("EMAIL_CAPACITY");
         int folderCapacity = rs3.getInt("FOLDER_CAPACITY");
         query = "SELECT ENSIZE, ATTACHMENT_ID from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and EMAIL.TO_ID='"+seqId+"' and SEND_FLAG='1' and DELETE_FLAG!='1'";
         long EMAIL_SIZE1=this.getEmailSize(query, conn);
         query  = "SELECT count(*) from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and EMAIL.TO_ID='"+seqId+"' and SEND_FLAG='1' and (DELETE_FLAG='' or  DELETE_FLAG='0' or DELETE_FLAG='2')";
         int incount = this.getEmailSizeBySql(query, conn);
         query = "SELECT ENSIZE, ATTACHMENT_ID from EMAIL_BODY where FROM_ID='"+seqId+"' and SEND_FLAG='0'";
         long EMAIL_SIZE2= this.getEmailSize(query, conn);
         query = "SELECT count(*) from EMAIL_BODY where FROM_ID='"+seqId+"' and SEND_FLAG='0'";
         int outboxCount = this.getEmailSizeBySql(query, conn);
         query = "SELECT count(*) from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and FROM_ID='"+seqId+"' and SEND_FLAG='1' and DELETE_FLAG!='2' and DELETE_FLAG!='4'";
  ...
  public String delRs(Connection conn, String endDate, String startDate , String sms , String email , String emailInbox) throws Exception {
    ...
    if (!T9Utility.isNullorEmpty(startDate) && !T9Utility.isNullorEmpty(endDate)) {
      remark = "删除" + startDate + "-" + endDate;
    }
    String str = "";
    if (!T9Utility.isNullorEmpty(startDate)) {
      str = T9DBUtility.getDateFilter("SEND_TIME", startDate, " >= ") + " AND ";
    }
    if (!T9Utility.isNullorEmpty(endDate)) {
      str += T9DBUtility.getDateFilter("SEND_TIME", endDate, " <= ")+ " AND ";
    }
    if (str.endsWith(" AND ")) {
    ...
    if ("on".equals(sms)) {
      ...
      if (!"".equals(bodyIdStr)) {
        ...
        query2 = "delete from SMS_BODY where " + str;
        this.executeSql(query2, conn);
      }
      remark += "的所有内部短信";
    }
    if ("on".equals(email) || "on".equals(emailInbox)) {
      String query = "select EMAIL.SEQ_ID AS EMAIL_ID ,BODY_ID ,ATTACHMENT_ID,ATTACHMENT_NAME   from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and  " + str;
      ...
      if (!"".equals(emailIdStr)) {
        String query1 = "delete from EMAIL where SEQ_ID in (" +  emailIdStr + ")";
        this.executeSql(query1, conn);
      }
      if (!"".equals(bodyIdStr)) {
        String query1 = "delete from EMAIL_BODY where SEQ_ID in (" +  bodyIdStr + ")";
        this.executeSql(query1, conn);
      }
      if ("on".equals(sms)) {
        remark = "和所有内部邮件";
  ...
  public String batRes(Connection conn, String endDate, String startDate,
    ...
    // TODO Auto-generated method stub
    String exportStr = "";
    if (!T9Utility.isNullorEmpty(path) && !path.endsWith( File.separator  )) {
      path +=  File.separator  ;
    }
    path += "email";
    String str = "";
    ...
    if ("on".equals(email) || "on".equals(emailInbox)) {
      String query = "select EMAIL.SEQ_ID AS EMAIL_ID , ATTACHMENT_ID , ATTACHMENT_NAME from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID  and " + str ;
      if ("on".equals(emailInbox)) {
        query += "  and BOX_ID='0' and SEND_FLAG='1' and (DELETE_FLAG='' or DELETE_FLAG='0' or DELETE_FLAG='2')";
      }
  ...
  public String getRealPath2(String aName , String aId , String moduleName ) {
    String filePath = T9SysProps.getAttachPath()  + File.separator  +  moduleName;
    int index = aId.indexOf("_");
    String hard = "";
    String str = "";
    if (index > 0) {
  ...
  public String delUserRs(Connection conn 
    ...
    if ("on".equals(address)) {
      String query = "delete from ADDRESS where USER_ID!='' and USER_ID is not null and " + T9DBUtility.findInSet(toId, "USER_ID") ;;
      rowCount += this.executeSql(query, conn);
      query = "delete from ADDRESS_GROUP where USER_ID!='' and USER_ID is not null and " + T9DBUtility.findInSet(toId, "USER_ID") ;;
      rowCount += this.executeSql(query, conn);
    }
  //日程安排
    if ("on".equals(calendar)) {
      String query = "delete from AFFAIR where   " + T9DBUtility.findInSet(toId, "USER_ID") ;;
      rowCount += this.executeSql(query, conn);
      query = "delete from CALENDAR where  " + T9DBUtility.findInSet(toId, "USER_ID") ;;
      rowCount += this.executeSql(query, conn);
    }
  //工作日志
    if ("on".equals(diary)) {
      String query = "delete from DIARY where   " + T9DBUtility.findInSet(toId, "USER_ID") ;;
      rowCount += this.executeSql(query, conn);
    }
  //个人网址
    if ("on".equals(url)) {
      String query = "delete from URL where  USER!='' and USER is not null AND  " + T9DBUtility.findInSet(toId, "USER") ;
      rowCount += this.executeSql(query, conn);
    }
  //邮件
    if ("on".equals(email)) {
    ...
    if ("on".equals(folder)) {
      String query  = "select * from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and " +  T9DBUtility.findInSet(toId, "USER_ID") ;
      Statement stm3 = null;
      ResultSet rs3 = null;
      try {
        stm3 = conn.createStatement();
      ...
      query = "delete from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and "  +  T9DBUtility.findInSet(toId, "USER_ID") ;
      rowCount += this.executeSql(query, conn);

      query = "select * from FILE_CONTENT where SORT_ID=0 and " +  T9DBUtility.findInSet(toId, "USER_ID") ;
      Statement stm4 = null;
      ResultSet rs4 = null;
      try {
        stm4 = conn.createStatement();
      ...
      query = "delete from FILE_CONTENT where SORT_ID=0 and " +  T9DBUtility.findInSet(toId, "USER_ID") ;
      rowCount += this.executeSql(query, conn);
    }
    return null;
  }
  ...
  public int delUserEmail(Connection conn , String toId) throws Exception{
    int rowCount = 0;
    String query ="delete from EMAIL_BOX where "  + T9DBUtility.findInSet(toId, "USER_ID") ;
    rowCount += this.executeSql(query, conn);
    //收到的邮件
    String bodyIdStr = "";
    String query1 ="select * from EMAIL where (DELETE_FLAG='2' or DELETE_FLAG='4') and " + T9DBUtility.findInSet(toId, "TO_ID") ;
    Statement stm3 = null;
    ResultSet rs3 = null;
    try {
      stm3 = conn.createStatement();
      ...
      while (rs3.next()){
        int bodyId = rs3.getInt("BODY_ID");
        String query2 = "select count(*) from EMAIL where BODY_ID='" + bodyId + "'";
        int groupCount =  getEmailSizeBySql(query2 , conn);
        if (groupCount <= 1) {
          query2 = "select * from EMAIL_BODY where SEQ_ID='" + bodyId + "'";
          Statement stm5 = null;
          ResultSet rs5 = null;
          try {
            stm5 = conn.createStatement();
    ...
    bodyIdStr = this.getOutOfTail(bodyIdStr);
    if (!"".equals(bodyIdStr)) {
       query = "delete from EMAIL_BODY where SEQ_ID in(" + bodyIdStr + ")";
       rowCount += this.executeSql(query, conn);
    }
    query = "delete from EMAIL where (DELETE_FLAG='2' or DELETE_FLAG='4') and " + T9DBUtility.findInSet(toId, "TO_ID") ;;
    rowCount += this.executeSql(query, conn);
    
    query = "update EMAIL set DELETE_FLAG='1' where (DELETE_FLAG='3' or DELETE_FLAG='' or DELETE_FLAG='0') and " + T9DBUtility.findInSet(toId, "TO_ID") ;
    this.executeSql(query, conn);
    //发送的邮件
    query = "select EMAIL.SEQ_ID AS EMAIL_ID , BODY_ID , SUBJECT , FROM_ID  ,EMAIL.TO_ID , READ_FLAG,DELETE_FLAG , ATTACHMENT_ID ,ATTACHMENT_NAME from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and (DELETE_FLAG='1' or READ_FLAG!='1') and " + T9DBUtility.findInSet(toId, "FROM_ID") ;;
    Statement stm5 = null;
    ResultSet rs5 = null;
    String emailIdStr = "";
    try {
      ...
      if (rs5.next()){
        ...
        String attId = rs5.getString("ATTACHMENT_ID");
        String attName = rs5.getString("ATTACHMENT_NAME");
        query = "select count(*) from EMAIL where BODY_ID='"+bodyId+"'";
    ...
    if(!"".equals(emailIdStr))
    {
       query = "delete from EMAIL where EMAIL_ID in ("+ emailIdStr +")";
       rowCount += this.executeSql(query, conn);
    }
    ...
    query = "update EMAIL a set DELETE_FLAG='2' where exists (select 1 from EMAIL_BODY b where a.BODY_ID=b.SEQ_ID and ("+T9WorkFlowUtility.createFindSql("b.FROM_ID", toId)+"))" 
      + " and  (DELETE_FLAG='' or DELETE_FLAG='0') and READ_FLAG='1'" ;
    this.executeSql(query, conn);
    
    query = "update EMAIL a set DELETE_FLAG='4' where  exists (select 1 from EMAIL_BODY b where a.BODY_ID=b.SEQ_ID and ("+T9WorkFlowUtility.createFindSql("b.FROM_ID", toId)+"))"
  ...
  public void deleteChildren(int sortId , Connection conn , Map map) throws Exception {
    ...
    String query = "select * from FILE_CONTENT where SORT_ID='" + sortId + "'";
    Statement stm4 = null;
    ResultSet rs4 = null;
    try {
      stm4 = conn.createStatement();
     ...
     query = "delete from FILE_CONTENT where SORT_ID='" + sortId + "'";
     this.executeSql(query, conn);
     query = "select * from FILE_SORT where SORT_PARENT='" + sortId + "'";
     Statement stm5 = null;
     ResultSet rs5 = null;
     try {
       stm5 = conn.createStatement();
     ...
     query = "delete from FILE_SORT where SEQ_ID='"+sortId+"'";
     this.executeSql(query, conn);
  }
  public String batUserRes(Connection conn, String diary, String email,
    ...
    String query = "";
    if ("on".equals(email)) {
      query = "select ATTACHMENT_ID , ATTACHMENT_NAME from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID  and " 
        + "("+  T9WorkFlowUtility.createFindSql("FROM_ID", toId)  +" or "+   T9WorkFlowUtility.createFindSql("EMAIL.TO_ID", toId) +") ";
      Statement stm3 = null;
      ResultSet rs3 = null;
      try {
        stm3 = conn.createStatement();
    ...
    if ("on".equals(folder)) {
      ...
      for (String id : toIds) {
        ...
        query = "select * from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and " + T9DBUtility.findInSet(id, "USER_ID");
        Statement stm5 = null;
        ResultSet rs5 = null;
        try {
          stm5 = conn.createStatement();
        ...
        query = "select * from FILE_CONTENT where SORT_ID=0  and " + T9DBUtility.findInSet(id, "USER_ID") ;
        Statement stm4 = null;
        ResultSet rs4 = null;
        try {
          stm4 = conn.createStatement();
    ...
    if ("on".equals(diary)) {
      query = "select * from DIARY where " + T9DBUtility.findInSet(toId, "USER_ID") ;
      Statement stm6 = null;
      ResultSet rs6 = null;
      try {
  ...
  public String delGarbageCon(Connection conn) throws Exception {
    ...
    //个人通讯簿
    query = "select USER_ID from ADDRESS where USER_ID!='' and USER_ID is not null and not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    query = "select USER_ID from ADDRESS where USER_ID!='' and USER_ID is not null and not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //日程安排
    query  ="select USER_ID from AFFAIR where not " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    query = "select USER_ID from CALENDAR where not " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //工作日志
    query = "select USER_ID from DIARY where not " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //内部邮件
    query = "select USER_ID from EMAIL_BOX where not " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //收到的邮件
    query = "select TO_ID from EMAIL where (DELETE_FLAG='2' or DELETE_FLAG='4') and not " + T9DBUtility.findInSet(userIdStr, "TO_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //发送的邮件
    query = "select FROM_ID from EMAIL,EMAIL_BODY where EMAIL.BODY_ID=EMAIL_BODY.SEQ_ID and (DELETE_FLAG='1' or READ_FLAG!='1') and not " + T9DBUtility.findInSet(userIdStr, "FROM_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //个人文件柜
    query = "select USER_ID from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and not  " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    query ="select USER_ID from FILE_CONTENT where SORT_ID=0 and not " + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    //人事档案
    String userIdStr2 =  this.getUserIdById(conn, userIdStr) ;
    query = "select USER_ID from hr_staff_info where not " + T9DBUtility.findInSet(userIdStr2, "USER_ID") ;
    userGarbageStr = this.getUserIds2(conn, userGarbageStr, query);
    //个人网址
    query ="select USER from URL where USER!='' and not  " + T9DBUtility.findInSet(userIdStr, "USER") ;
    userGarbageStr = this.getUserIds(conn, userGarbageStr, query);
    return userIdStr;
  }
  ...
  public String delGarbage(Connection conn) throws Exception {
    ...
    long spaceCount = 0;
  //个人通讯簿
    query = "delete from ADDRESS where USER_ID!='' and not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);

    query ="delete from ADDRESS_GROUP where USER_ID!='' and not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);
    

    //日程安排
    query="delete from AFFAIR where not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);

    query="delete from CALENDAR where not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);

    //工作日志
    query="delete from DIARY where not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);
    //个人网址
    query="delete from URL where USER!='' and not "  + T9DBUtility.findInSet(userIdStr, "USER") ;
    rowCount += this.executeSql(query, conn);
    //邮件
    rowCount +=  this.delUserEmail(conn, userIdStr);
    
  //个人文件柜
    query ="select * from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and not "  + T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    Statement stm3 = null;
    ResultSet rs3 = null;
    try {
      stm3 = conn.createStatement();
    ...
    query = "delete from FILE_SORT where SORT_TYPE='4' and SORT_PARENT=0 and "  +  T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    rowCount += this.executeSql(query, conn);

    query = "select * from FILE_CONTENT where SORT_ID=0 and " +  T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    Statement stm4 = null;
    ResultSet rs4 = null;
    try {
      stm4 = conn.createStatement();
    ...
    query = "delete from FILE_CONTENT where SORT_ID=0 and " +  T9DBUtility.findInSet(userIdStr, "USER_ID") ;
    this.executeSql(query, conn);
    //人事档案
    String userIdStr2 =  this.getUserIdById(conn, userIdStr) ;
    query ="select * from hr_staff_info where not " +  T9DBUtility.findInSet(userIdStr2, "USER_ID") ;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
      ...
      while (rs2.next()){
        ...
        photo =T9SysProps.getAttachPath()  +  File.separator + "hrms_pic" +  File.separator  + photo;
    ...
    query = "delete from hr_staff_info where not  " + T9DBUtility.findInSet(userIdStr2, "USER_ID") ;
    this.executeSql(query, conn);

    //删除空附件目录
    int folderCount = 0 ;
    String path = T9SysProps.getAttachPath();
