...
import t9.core.funcs.system.resManage.logic.T9ResManageLogic;
...
public class T9ResManageAct {
  private static Logger log = Logger
    .getLogger("t9.core.funcs.system.resManage.act.T9ResManageAct");
  public String getRes(HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    try{
      String root = "";
      if (T9SysProps.isLinux()) {
        root =  "/";
      } else {
  ...
  public String getModuleData(HttpServletRequest request,
    ...
    try {
      ...
      T9ResManageLogic logic = new T9ResManageLogic();
  ...
  public String deleteSms(HttpServletRequest request,
    ...
    try {
      ...
      logic.woriteLog(dbConn, "删除所有已读提醒", person.getSeqId() , this.getIpAddr(request));
      request.setAttribute(T9ActionKeys.RET_STATE,T9Const.RETURN_OK);
    } catch (Exception ex){
       request.setAttribute(T9ActionKeys.RET_STATE, T9Const.RETURN_ERROR);
      request.setAttribute(T9ActionKeys.RET_MSRG, ex.getMessage());
  ...
  public String deleteDeletedSms(HttpServletRequest request,
    ...
    try {
      ...
      logic.woriteLog(dbConn, "清空收信人已删提醒", person.getSeqId() , this.getIpAddr(request));
      request.setAttribute(T9ActionKeys.RET_STATE,T9Const.RETURN_OK);
    } catch (Exception ex){
       request.setAttribute(T9ActionKeys.RET_STATE, T9Const.RETURN_ERROR);
      request.setAttribute(T9ActionKeys.RET_MSRG, ex.getMessage());
  ...
  public String getDeptRes(HttpServletRequest request,
    ...
    try {
      ...
      dbConn = requestDbConn.getSysDbConn();
      int deptId = 0;
      if (T9Utility.isInteger(deptIds)) {
  ...
  public String getUserRes(HttpServletRequest request,
    ...
    try {
      ...
      dbConn = requestDbConn.getSysDbConn();
      int userId = 0;
      if (T9Utility.isInteger(userIds)) {
