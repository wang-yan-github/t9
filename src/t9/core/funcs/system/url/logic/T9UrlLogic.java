package t9.core.funcs.system.url.logic;
...
public class T9UrlLogic {
  private static Logger log = Logger.getLogger("t9.core.funcs.dept.act");
  private T9ORM orm = new T9ORM();
  private Connection conn ;
  public T9UrlLogic(){
  }
  public T9UrlLogic(Connection conn){
    this.conn = conn;
  }
  public List deleteDeptMul(Connection dbConn, int seqId) {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT SEQ_ID FROM DEPARTMENT WHERE DEPT_PARENT = '" + seqId + "'";
    try {
...
//        }
  ...
  public List deleteSelf(Connection dbConn, int seqId) {
    ...
    try {
      ...
      for(Iterator it = list1.iterator(); it.hasNext();){
        T9Department dd = (T9Department)(it.next());
        //System.out.println(dd.getSeqId()+"rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr");
        //T9ORM orm = new T9ORM();
       // T9Department dt = (T9Department)orm.loadObjComplex(dbConn, T9Department.class, dd.getSeqId());
        //dt.setSeqId(seqId);
  ...
  public List getDeptList() throws Exception{
    ...
    list  = orm.loadListSingle(this.conn ,T9Department.class , filters);
    return list;
  }
  
  public T9Department getDepartmentById(int deptId) throws Exception{
    T9Department department = new T9Department();
    department = (T9Department) this.orm.loadObjSingle(this.conn, T9Department.class, deptId);
    return department;
  }
  //根据id串取得名字串
  public String getNameByIdStr(String ids) throws Exception, Exception{
    ...
    for(String tmp : aId){
      if(!"".equals(tmp)){
        T9Department department = this.getDepartmentById(Integer.parseInt(tmp));
  ...
  public void getDeptTree(int deptId , StringBuffer sb , int level) throws Exception{
    //首选分级，然后记录级数，是否为最后一个。。。
    List<T9Department> list = this.getDeptByParentId(deptId);
  ...
  public List<T9Department> getDeptByParentId(int deptId) throws Exception {
