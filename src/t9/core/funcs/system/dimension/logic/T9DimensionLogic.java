package t9.core.funcs.system.dimension.logic;
...
public class T9DimensionLogic {
  private static Logger log = Logger
      .getLogger("t9.core.funcs.system.filefolder.logic");

  public void saveFileSortInfo(Connection dbConn, T9Dimension dimension) {
    T9ORM orm = new T9ORM();
    try {
   ...
   * @return
   */
  public T9Dimension getMaxSeqId(Connection dbConn) {
    // String sql="select MAX(SEQ_ID) from file_sort";
    String sql = "select SEQ_ID,SORT_NAME from DIMENSION where SEQ_ID=(select MAX(SEQ_ID) from DIMENSION ) ";
    T9Dimension fileSort = null;
    int seqId = 0;
    String sortName = "";
  ...
  public List<T9Dimension> getDimensionInfo(Connection dbConn, Map map)
      throws Exception {
    T9ORM orm = new T9ORM();
    return orm.loadListSingle(dbConn, T9Dimension.class, map);
  }
  public List<T9Dimension> getDimensionInfo(Connection dbConn, String[] map) throws Exception {
    T9ORM orm = new T9ORM();
    return orm.loadListSingle(dbConn, T9Dimension.class, map);
  }

  public T9Dimension getFileSortInfoById(Connection dbConn, String seqId)
      throws NumberFormatException, Exception {
    T9ORM orm = new T9ORM();

    return (T9Dimension) orm.loadObjSingle(dbConn, T9Dimension.class, Integer
        .parseInt(seqId));
  }
  ...
  public void getSortNamePath(Connection dbConn, int seqId, StringBuffer buffer)
      throws Exception {
    T9Dimension fileSort = getSortNameById(dbConn, seqId);
   ...
   * @throws Exception
   */
  public T9Dimension getSortNameById(Connection dbConn, int seqId)
      throws Exception {
    T9ORM orm = new T9ORM();
    return (T9Dimension) orm.loadObjSingle(dbConn, T9Dimension.class, seqId);
  }
  ...
  public boolean isHaveSortParent(Connection dbConn, int sortParent)
    ...
    List<T9Dimension> list = orm.loadListSingle(dbConn, T9Dimension.class, map);
    if (list.size() > 0) {
      flag = true;
    }
    return flag;
  ...
  public List selectDimension(Connection dbConn, int seqId,int parentId,List dimensionList,int maxSeqId) throws Exception {
    ...
    list = orm.loadListSingle(dbConn, T9Dimension.class, map);
    if(seqId>maxSeqId){
      return list;
    }
    ...
    for (int i = 0; i < list.size(); i++) {
      T9Dimension dimension = list.get(i);
  ...
  public int getChildDimension(Connection dbConn,int seqId,int parentId,List dimensionList) throws Exception {
    ...
    T9Dimension dimension =  (T9Dimension) orm.loadObjSingle(dbConn,T9Dimension.class, parentId);
    T9Dimension dimension2 =  (T9Dimension) orm.loadObjSingle(dbConn,T9Dimension.class, seqId);
    ...
    orm.saveSingle(dbConn, dimension);
    T9Dimension maxDimension = getMaxSeqId(dbConn);
   ...
   * 级联添加
   */
  public void addDimensionList(Connection dbConn,List<Map<String,String>> list,T9Dimension dimension)throws Exception {
    T9ORM orm = new T9ORM();
    T9Dimension dimensionCopy = new T9Dimension();
  ...
  public String getSortName(Connection dbConn, int seqId) throws Exception {
    ...
    sb.append("[");
    T9ORM orm = new T9ORM();
    T9Dimension fileSort = (T9Dimension) orm.loadObjSingle(dbConn,T9Dimension.class, seqId);
  ...
  public void getChild(Connection dbConn, T9Dimension fileSort, StringBuffer sb,
    ...
    Map map = new HashMap();
    map.put("SORT_PARENT", seqId);
    List<T9Dimension> list = getFileSorts(dbConn, map);
    flag++;
  ...
  public List<T9Dimension> getSortParentId(Connection dbConn, Map map)
      throws Exception {
    T9ORM orm = new T9ORM();
    return orm.loadListSingle(dbConn, T9Dimension.class, map);
  }
 ...
 */
  public void updateVisitOverride(Connection dbConn,int seqId,String setIdStr,String action) throws Exception{
    T9ORM orm = new T9ORM();
    T9Dimension fileSort = (T9Dimension) orm.loadObjSingle(dbConn,
        T9Dimension.class, seqId);   
    getChildFolder(dbConn, fileSort,setIdStr,action);
    
  }
  
  public void getChildFolder(Connection dbConn,T9Dimension fileSort,String setIdStr,String action) throws Exception{
    T9ORM orm=new T9ORM();
    int seqId = fileSort.getSeqId();
  ...
  public void updateVisitById(Connection dbConn, Map map) throws Exception {
    ...
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET USER_ID=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void updateVisitById(Connection dbConn, String parentId,int seqId) throws Exception {
    ...
    // orm.updateSingle(dbConn, "FILE_CONTENT", map);
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET SORT_PARENT=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void updateManageUserById(Connection dbConn, Map map)  {
    ...
    // orm.updateSingle(dbConn, "FILE_CONTENT", map);
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET MANAGE_USER=? where SEQ_ID=? ";
    try {
      ps = dbConn.prepareStatement(sql);
  ...
  public void updateNewUserById(Connection dbConn, Map map) throws Exception {
    ...
    // orm.updateSingle(dbConn, "FILE_CONTENT", map);
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET NEW_USER=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void updateDownLoadById(Connection dbConn, Map map) throws Exception {
    ...
    // orm.updateSingle(dbConn, "FILE_CONTENT", map);
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET DOWN_USER=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void updateOwnerById(Connection dbConn, Map map) throws Exception {
    ...
    // orm.updateSingle(dbConn, "FILE_CONTENT", map);
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET OWNER=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void updateEditFileSort(Connection dbConn, Map map) throws Exception {
    T9ORM orm = new T9ORM();
    PreparedStatement ps = null;
    String sql = "UPDATE DIMENSION SET SORT_NO=?,SORT_NAME=? where SEQ_ID=? ";
    ps = dbConn.prepareStatement(sql);
  ...
  public void delDimensionInfoById(Connection dbConn, T9Dimension dimension)
      throws Exception {
    T9ORM orm = new T9ORM();
    Map map = new HashMap();
    map.put("SORT_PARENT", dimension.getSeqId());
    List<T9Dimension> dimensionList = orm.loadListComplex(dbConn, T9Dimension.class,map );
  ...
  public List<T9Dimension> getFileSorts(Connection dbConn, Map map)
      throws Exception {
    T9ORM orm = new T9ORM();
    return (List<T9Dimension>) orm.loadListSingle(dbConn, T9Dimension.class, map);
  }
  ...
  public String selectManagerIds(Connection dbConn, Map map, String action)
    ...
    T9ORM orm = new T9ORM();
    String ids = "";
    ArrayList<T9Dimension> managerList = (ArrayList<T9Dimension>) orm
        .loadListSingle(dbConn, T9Dimension.class, map);
    if (managerList.size() > 0) {
      T9Dimension manager = managerList.get(0);
      if ("USER_ID".equals(action)) {
  ...
  public String getRoleNamesByIds(Connection dbConn, Map map, String action)
      throws Exception {
    String names = "";
    T9DimensionLogic tpl = new T9DimensionLogic();
    ...
    if (ids != null && !"".equals(ids)) {
      ...
      for (String tmp : aId) {
        if (!"".equals(tmp)) {
          ...
          T9UserPriv privName = this.getPersonById(dbConn, Integer
              .parseInt(tmp));
          if (privName != null) {
   ...
   * @throws Exception
   */
  public T9UserPriv getPersonById(Connection dbConn, int seqId)
      throws Exception {
    T9ORM orm = new T9ORM();
    T9UserPriv userPriv = (T9UserPriv) orm.loadObjSingle(dbConn,
        T9UserPriv.class, seqId);
    ;
    return userPriv;
  }
  ...
  public String getDeptNameBySeqIdStr(Connection dbConn, String ids)
    ...
    if (ids != null && !"".equals(ids)) {
      ...
      for (String tmp : aId) {
        if (!"".equals(tmp)) {
          T9Department deptName = getDeptById(dbConn, Integer.parseInt(tmp));
          if (deptName != null) {
   ...
   * @throws Exception
   */
  public T9Department getDeptById(Connection dbConn, int seqId)
      throws Exception {
    T9ORM orm = new T9ORM();
    T9Department department = (T9Department) orm.loadObjSingle(dbConn,
        T9Department.class, seqId);
    ;
    return department;
  }
  ...
  public String getVisiPriv(Connection dbConn, Map map, int loginUserSeqId,
    ...
    T9ORM orm=new T9ORM();
    T9Dimension fileSort= (T9Dimension) orm.loadObjSingle(dbConn, T9Dimension.class, map);
    int sortParent=fileSort.getSortParent();
  ...
  public void getSortName(Connection dbConn,int seqId,int parentId){
    String sql="select SEQ_ID,SORT_PARENT,SORT_NO,SORT_NAME,SORT_TYPE from DIMENSION where SORT_PARENT =146 ";
    PreparedStatement ps=null;
    ResultSet rs=null;
    try {
      ps=dbConn.prepareStatement(sql);
