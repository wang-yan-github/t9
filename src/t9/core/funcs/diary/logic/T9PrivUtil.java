 ...
 *
 */
public class T9PrivUtil {
  private static Logger log = Logger
      .getLogger("t9.core.funcs.diary.logic.T9PrivUtil");
   ...
   * @throws Exception
   */
  public static T9ModulePriv getMyPrivByModel(Connection conn, int userId,
      String moduleId) throws Exception {
    T9ModulePriv mp = null;
    ...
    String sql = "select " + " DEPT_PRIV " + ",ROLE_PRIV" + ",SEQ_ID"
        ...
        + " from " + "MODULE_PRIV" + " where " + "USER_SEQ_ID=?" + " and "
        + "MODULE_ID=?";
  ...
  public static boolean isParentDept(Connection conn, int parentDeptId,
    ...
    T9Department d  = T9DepartmentCache.getDepartmentCache(conn, parentDeptId);
    if (d != null ) {
      if (d.getDeptParent()== childDeptId) {
         result = true;
      } else {
         result = isParentDept(conn, d.getDeptParent(), childDeptId);
      }
    }
    return result;
  }
   ...
   * @throws Exception 
   */
  public static boolean isDeptPriv(Connection conn,int deptId,String postPriv,String postDept,int LoginUserId,int loginUserDept,T9MyPriv mp) throws Exception{
    boolean result = false;
   ...
   * @throws Exception 
   */
  public static  ArrayList<Integer> getLawfulUserId(Connection conn,T9Person person,String moduleId,int privNoFlag) throws Exception{
    ArrayList<Integer> result = new ArrayList<Integer>();
    int loginUserId = person.getSeqId();
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      T9MyPriv mp = getMyPriv(conn, person, moduleId, privNoFlag);
      String sql = "SELECT SEQ_ID FROM PERSON WHERE SEQ_ID != " + loginUserId;
      ps = conn.prepareStatement(sql);
      rs = ps.executeQuery();
      while(rs.next()){
        int userId = rs.getInt(1);
        if(T9PrivUtil.isUserPriv(conn, userId, mp, person.getPostPriv(), person.getPostDept(), loginUserId, person.getDeptId())){
          result.add(userId);
        }
      }
    } catch (Exception e) {
  ...
  public static String getDeptManager(Connection conn,  T9ModulePriv priv ,T9Person person,String moduleId,int privNoFlag ) throws Exception {
    String manager = "";
  ...
  public static String getDeptManagerByDept(Connection conn ,  int dept ) throws Exception {
    String manager = "";
    int deptParent = 0 ;
    T9Department d = T9DepartmentCache.getDepartmentCache(conn, dept);
    if (d != null) {
      manager = T9Utility.null2Empty(d.getManager());
 ...
 * @throws Exception
 */
  public static T9MyPriv getMyPriv(Connection conn,T9Person person,String moduleId,int privNoFlag) throws Exception{
    int userId = person.getSeqId();
    String postPriv =  person.getPostPriv();
    String postDept = person.getPostDept();
    T9MyPriv mpriv = new T9MyPriv();
    String deptPriv = "0";
    String rolePriv = "0";
    String privId = "";
    String deptId = "";
    ...
    String userIsStr = "";
    try {
      T9ModulePriv mp = getMyPrivByModel(conn, userId, moduleId);
   ...
   // privNoFlag = 3; //自己的管理范围、低角色，OA管理员除外的用户
      if(moduleId == null || "".equals(moduleId)){//模块名称为全部人员模块&不按 模块选择权限
        ...
        if(person.isAdminRole()|| person.isAdmin() || privNoFlag == 0){
            deptPriv = "1";//人员范围：全体            rolePriv = "2";//人员角色：所有角色的用户
          } else{//不是管理员则按非模块权限处理
...
//  rolePriv =  3-指定角色的用户
   ...
   * @throws Exception
   */
    public static T9MyPriv getMyPriv(Connection conn,T9Person person,String moduleId,int privNoFlag , T9ModulePriv mp) throws Exception{
      int userId = person.getSeqId();
      String postPriv =  person.getPostPriv();
      String postDept = person.getPostDept();
      T9MyPriv mpriv = new T9MyPriv();
...
//    rolePriv =  3-指定角色的用户
   ...
   * @throws Exception
   */
  public static boolean isUserPriv(Connection conn,int userId,T9MyPriv mp
 ...
 /*   if("1".equals(mp.getDeptPriv()) || "2".equals(mp.getDeptPriv()) ){
   ...
   * @throws Exception
   */
  public static boolean isUserPriv(Connection conn,T9Person p,T9MyPriv mp
      ,String postPriv,String postDept,int LoginUserId,int loginUserDept) throws Exception{
     //本部门,指定部门
 /*   if("1".equals(mp.getDeptPriv()) || "2".equals(mp.getDeptPriv()) ){
      return true;
   ...
   //取出待判断用户（$USER_ID）的部门ID、角色ID、角色排序号
   int deptId = p.getDeptId();
   int userPriv = Integer.parseInt(p.getUserPriv() == null ? "0" :p.getUserPriv());
   T9UserPriv up = T9UserPrivCache.getUserPrivCache(conn, userPriv);
   int privNo = up.getPrivNo();
   //判断哪些情况不是自己的管理范围
   ...
   if(("1".equals(mp.getDeptPriv()) && isRole)
     ...
     ||
     ("3".equals(mp.getDeptPriv())//人员范围：3-指定人员
          && findId(mp.getUserId(), p.getSeqId()) && isRole) )//USER_ID不在指定的人员范围之内
       return true;
     return false;
  }
  ...
  public static boolean isRoleRule(T9MyPriv mp, int privNo,int userPriv){
   ...
   * @throws Exception
   */
  public static boolean isUserPriv(Connection conn,T9Person userId,T9MyPriv mp
      ,T9Person person) throws Exception{
     String postPriv = person.getPostPriv();
     String postDept = person.getPostDept();
     int LoginUserId = person.getSeqId();
   ...
   * @throws Exception
   */
  public static boolean isDeptPriv(Connection conn,int deptId,T9MyPriv mp
      ,String postPriv,String postDept,int LoginUserId,int loginUserDept) throws Exception{
   ...
   * @throws Exception
   */
  public static T9Department getGrandParentDept(Connection conn,int dept) throws Exception{
    T9Department result = null ;
    String sql = "select SEQ_ID,DEPT_PARENT,DEPT_NAME from DEPARTMENT where SEQ_ID =" + dept;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
   ...
   * @throws Exception
   */
  public static boolean isHrManageDept(Connection conn, int dept, T9Person person) throws Exception{
    String sql = "select 1 from hr_manager where DEPT_ID =" + dept + " and " + T9DBUtility.findInSet(String.valueOf(person.getSeqId()), "DEPT_HR_MANAGER");
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
