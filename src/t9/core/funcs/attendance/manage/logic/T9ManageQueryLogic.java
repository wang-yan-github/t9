...
public class T9ManageQueryLogic {
  ...
  public String getDutyType(Connection conn)throws Exception{
      ...
      try{
       ...
       String sql = "SELECT * from ATTEND_CONFIG order by seq_id";
  ...
  public String getDeptDuty(Connection conn,Map request) throws Exception{
    ...
    try {
      ...
      T9DBUtility.close(stmt2, rs2, null);
    } 
    ...
    Date end = null;
    if (startDate != null) {
      start = T9Utility.parseDate(startDate);
    }
    if (endDate != null) {
      end = T9Utility.parseDate(endDate);
    }
    //---- 取规定上下班时间 -----
     String query2  = "SELECT * from ATTEND_CONFIG";
     ...
     try {
       ...
       while (rs3.next()) {
         ...
         for (int i = 1 ;i <= 6 ;i++) {
           ...
           if (T9Utility.isNullorEmpty(dutyTimeI)) {
             continue;
           }
           c++;
       ...
       T9DBUtility.close(stmt3, rs3, null);
     } 
     
     String query3 = "SELECT * from PERSON,USER_PRIV,DEPARTMENT where PERSON.NOT_LOGIN='0' and "+ T9DBUtility.findNoInSet(paraValue, "PERSON.SEQ_ID") +" and DEPARTMENT.SEQ_ID=PERSON.DEPT_ID ";
     String deptChildId= "";
     if(!"ALL_DEPT".equals(dept) 
         && !T9Utility.isNullorEmpty(dept)){
     ...
     try {
       ...
       while (rs5.next() ) {
         ...
         for (Date j = start ;  end.compareTo(j) >= 0 ;  j = new Date(j.getTime() + 24 * 3600 * 1000)) {
           String jj = T9Utility.getDateTimeStr(j);
           int week = j.getDay();
           int holiday = 0 ;
           int holiday1 = 0 ;
           
           if (T9WorkFlowUtility.findId(general, week + "")) {
             holiday = 1;
           }
           if (holiday == 0) {
             String query5 = "select count(*)  from ATTEND_HOLIDAY where  "+T9DBUtility.getDateFilter("BEGIN_DATE", jj,"<=") +" and " + T9DBUtility.getDateFilter("END_DATE", jj,">=");
           ...
           if (holiday == 0) {
             String query5 = "select count(*) from ATTEND_EVECTION where   USER_ID='"
               +seqId+"' and ALLOW='1' and "
               +  T9DBUtility.getDateFilter("EVECTION_DATE1", jj,"<=") +" and " + T9DBUtility.getDateFilter("EVECTION_DATE2", jj,">=");
             
             query5 = "select count(*) from ATTEND_EVECTION where USER_ID='"
               +seqId+"' and ALLOW='1' and "+T9DBUtility.getDateFilter("EVECTION_DATE1", jj,"<=")+" and "+T9DBUtility.getDateFilter("EVECTION_DATE2", jj,">=");
             holiday = getCount( conn , query5);
             if (holiday > 0 ) {
               holiday1 = 1;
             }
           ...
           if (holiday == 0) {
             String query5 = "select count(*) from ATTEND_LEAVE where   USER_ID='"
               +seqId+"' and ALLOW='1' and "
               +  T9DBUtility.getDateFilter("LEAVE_DATE1", jj,"<=") +" and " + T9DBUtility.getDateFilter("LEAVE_DATE2", jj,">=");
             query5 = "select count(*) from ATTEND_LEAVE where USER_ID='"
               +seqId+"' and ALLOW='1' and "+T9DBUtility.getSqlSubstr("LEAVE_DATE1", jj,0, 10,"<=")+" and "+T9DBUtility.getSqlSubstr("LEAVE_DATE2", jj,0, 10,">=");
             holiday = getCount( conn , query5);
             if (holiday > 0 ) {
               holiday1 = 1;
             }
           ...
           int perfectFlag = 0 ;
           String query6 = "SELECT * from ATTEND_DUTY where USER_ID='"+seqId+"' and "+T9DBUtility.getDateFilter("REGISTER_TIME", jj,"=")+" order by REGISTER_TIME";
           //query6 = "SELECT * from ATTEND_DUTY where USER_ID='"+seqId+"' and to_days(REGISTER_TIME)=to_days('"+jj+"') order by REGISTER_TIME";
           ...
           try {
             ...
             T9DBUtility.close(stmt9, rs9, null);
           } 
         ...
         sb.append(",deptName:\"" + T9Utility.encodeSpecial(userDeptName) + "\"");
       ...
       T9DBUtility.close(stmt5, rs5, null);
     } 
     if (count > 0 ) {
  ...
  try{
    //获取子部门
     String deptChildId="";
      ...
      if(T9Utility.isNullorEmpty(deptChildId)){
        dept+=",";
        dept+=deptChildId;
      }    
      String userIds="";
      ...
      if(!"".equals(userIds)){
        String  sql="select a.seq_id ,d.dept_name,p.user_name,a.EVECTION_DEST,a.REGISTER_IP,a.evection_date1,a.evection_date2,a.LEADER_ID,a.status from person p,attend_evection a,department d where "
          +" p.seq_id=a.user_id and p.dept_id=d.seq_id and p.seq_id in ("+userIds+") and a.allow='1' "
          +" and ( ("+T9DBUtility.getDateFilter("evection_date1", startDate, ">=")+" and "+T9DBUtility.getDateFilter("evection_date1", endDate+" 23:59:59", "<=")+") "
          +" or  ("+T9DBUtility.getDateFilter("evection_date2", startDate, ">=")+" and "+T9DBUtility.getDateFilter("evection_date2", endDate+" 23:59:59", "<=")+") "
          +" or  ("+T9DBUtility.getDateFilter("evection_date1", startDate, ">=")+" and "+T9DBUtility.getDateFilter("evection_date2", endDate+" 23:59:59", "<=")+") "
  ...
  try{
    //获取子部门
     String deptChildId="";
      ...
      userIds=this.getUserIdsByDept(conn,dept);
      if(!"".equals(userIds)){
        String  sql="select a.seq_id ,d.dept_name,p.user_name,a.CREATE_DATE,a.out_type,a.REGISTER_IP,a.SUBMIT_TIME,a.OUT_TIME1,a.OUT_TIME2,a.LEADER_ID,a.status ,a.allow from person p,attend_out a,department d where "
          +" p.seq_id=a.user_id and p.dept_id=d.seq_id and p.seq_id in ("+userIds+") "
          +" and "+T9DBUtility.getDateFilter("submit_time", startDate, " >= ")+" and "+T9DBUtility.getDateFilter("submit_time", endDate+" 23:59:59", "<=")
          +" order by d.seq_id,p.SEQ_ID,p.USER_NAME ";
        stmt=conn.createStatement();
        rs=stmt.executeQuery(sql);
  ...
  try{
    //获取子部门
     String deptChildId="";
      ...
      userIds=this.getUserIdsByDept(conn,dept);
      if(!"".equals(userIds)){
        String  sql="select a.seq_id ,d.dept_name,p.user_name,a.leave_type,a.ANNUAL_LEAVE,a.REGISTER_IP,a.LEAVE_DATE1,a.LEAVE_DATE2,a.LEADER_ID,a.STATUS from person p,attend_leave a,department d where "
          +" p.seq_id=a.user_id and p.dept_id=d.seq_id and p.seq_id in ("+userIds+") and a.allow='1' "
          +" and ( ("+T9DBUtility.getDateFilter("leave_date1", startDate, ">=")+" and "+T9DBUtility.getDateFilter("leave_date1", endDate+" 23:59:59", "<=")+") "
          +" or  ("+T9DBUtility.getDateFilter("leave_date2", startDate, ">=")+" and "+T9DBUtility.getDateFilter("leave_date2", endDate+" 23:59:59", "<=")+") "
          +" or  ("+T9DBUtility.getDateFilter("leave_date1", startDate, ">=")+" and "+T9DBUtility.getDateFilter("leave_date2", endDate+" 23:59:59", "<=")+") "
          +" ) order by d.seq_id,p.SEQ_ID,p.USER_NAME ";
        stmt=conn.createStatement();
        rs=stmt.executeQuery(sql);
  ...
  try{
    //获取子部门
     String deptChildId="";
      ...
      userIds=this.getUserIdsByDept(conn,dept);
      if(!"".equals(userIds)){
        String  sql="select a.seq_id ,d.dept_name,p.user_name,a.OVERTIME_TIME,a.OVERTIME_DESC,a.BEGIN_TIME,a.END_DATE,a.HOUR,a.LEADER_ID,a.STATUS from person p,overtime_record a,department d where "
          +" p.seq_id=a.user_id and p.dept_id=d.seq_id and p.seq_id in ("+userIds+") and a.status in ('1','3') "
          +" and "+T9DBUtility.getDateFilter("begin_time", startDate, ">=")+" and "+T9DBUtility.getDateFilter("begin_time", endDate+" 23:59:59", "<=") 
          +" order by d.seq_id,p.SEQ_ID,p.USER_NAME ";
        stmt=conn.createStatement();
        rs=stmt.executeQuery(sql);
...
public void delEvection(Connection conn,String seqId)throws Exception {
  Statement stmt=null;
  try{
      String sql="delete from  attend_evection where seq_id='"+seqId+"'";
      stmt=conn.createStatement();
      stmt.execute(sql);
    }catch(Exception ex){
      throw ex;
...
public void delOvertime(Connection conn,String seqId)throws Exception {
  Statement stmt=null;
  try{
      String sql="delete from  overtime_record where seq_id='"+seqId+"'";
      stmt=conn.createStatement();
      stmt.execute(sql);
    }catch(Exception ex){
      throw ex;
...
public void delLeave(Connection conn,String seqId)throws Exception {
  Statement stmt=null;
  try{
      String sql="delete from  attend_Leave where seq_id='"+seqId+"'";
      stmt=conn.createStatement();
      stmt.execute(sql);
    }catch(Exception ex){
      throw ex;
...
public void delOut(Connection conn,String seqId)throws Exception {
  Statement stmt=null;
  try{
      String sql="delete  from attend_out where seq_id='"+seqId+"'";
      stmt=conn.createStatement();
      stmt.execute(sql);
    }catch(Exception ex){
      ex.printStackTrace();
...
public String getLateCount(Connection conn,String userId,String Type,String startDate,String endDate)throws Exception{
  ...
  try{
    String sql="select * from attend_config where seq_id='"+Type+"'";
    stmt=conn.createStatement();
    rs=stmt.executeQuery(sql);
    while(rs.next()){
    ...
    sql="select * from attend_duty where user_id='"+userId+"' and "+T9DBUtility.getDateFilter("register_time", startDate, ">=")+" and "+T9DBUtility.getDateFilter("register_time", endDate, "<=");
    rs=stmt.executeQuery(sql);
    while(rs.next()){
...
public String getNoRegister(Connection conn,String userId,String Type,String startDate,String endDate)throws Exception{
  ...
  try{
    ...
    for(int i=1;i<=6;i++){
      ...
      if(T9Utility.isNullorEmpty(map.get(dutytime))){
        num=i;
        break;
      }
    }
    ...
    sql="select * from attend_duty where  and user_id='"+userId+"' and "+T9DBUtility.getDateFilter("register_time", startDate, ">=")+" and "+T9DBUtility.getDateFilter("register_time", endDate, "<=");
    rs=stmt.executeQuery(sql);
    while(rs.next()){
      String type=rs.getString("register_type");
...
public String getDeptChildId(Connection conn, String deptId) throws Exception {
  ...
  String sql = "";
  String deptIdStr = "";
  sql = "select SEQ_ID from DEPARTMENT where dept_parent=" + deptId;
  rs = stmt.executeQuery(sql);
  while (rs.next()) {
...
public ArrayList<T9DbRecord> convertOutList(List<Map<String, String>> list){
...
public ArrayList<T9DbRecord> convertEvectionList(List<Map<String, String>> list){
  ArrayList<T9DbRecord > dbL = new ArrayList<T9DbRecord>();
  if(list != null && list.size() >0){
    for (int i = 0; i < list.size(); i++) {
      T9DbRecord dbrec = new T9DbRecord();
...
public ArrayList<T9DbRecord> convertLeaveList(List<Map<String, String>> list){
  ArrayList<T9DbRecord > dbL = new ArrayList<T9DbRecord>();
  if(list != null && list.size() >0){
    for (int i = 0; i < list.size(); i++) {
      T9DbRecord dbrec = new T9DbRecord();
...
public ArrayList<T9DbRecord> convertOvertimeList(List<Map<String, String>> list){
  ArrayList<T9DbRecord > dbL = new ArrayList<T9DbRecord>();
  if(list != null && list.size() >0){
    for (int i = 0; i < list.size(); i++) {
      T9DbRecord dbrec = new T9DbRecord();
...
public ArrayList<T9DbRecord> convertExelDutyList( List<LinkedList<String>> list){
  ArrayList<T9DbRecord > dbL = new ArrayList<T9DbRecord>();
  if(list != null && list.size() >0){
...
public int getCount(Connection conn , String sql) throws Exception {
  ...
  try {
    ...
    T9DBUtility.close(stmt7, rs7, null);
  }
  return result;
}
...
public String getUserDutyInfoLogic(Connection dbConn,HttpServletRequest request,
  ...
  T9Person person = personLogic.getPerson(dbConn, userId);
  ...
  for(int i = 0; i < dayArray.length; i++){
    ...
    for(int j = 1;j<=6;j++){
      ...
      if(dutyTime!=null&&!dutyTime.trim().equals("")){       
        ...
        //查出当天有没有登记 记录
        List<T9AttendDuty> dutyList =getAttendDuty(request,response,dayArray[i],String.valueOf(j),userId,config);
        String td = "" ;
        if(dutyList.size()>0){
          T9AttendDuty duty = dutyList.get(0);
...
public  LinkedList<String> getExeclTitleByUserId(Connection conn,String userId)throws Exception{
  ...
  list.add("日期");
  try{
    String sql="select a.* from attend_config a,person p where p.duty_type=a.seq_id and p.seq_id='"+userId+"'";
    stmt=conn.createStatement();
    rs=stmt.executeQuery(sql);
    if(rs.next()){
      for(int i=1;i<=6;i++){
         ...
         if(!T9Utility.isNullorEmpty(time)){
            String duty="";
            if("1".equals(dutyType)){
              duty="上班时间("+time+")";
...
public String isLeaveTemp(Connection dbConn,HttpServletRequest request,
  ...
  try {
    ...
    String sql="select * from ATTEND_LEAVE where USER_ID='"+userId+"' and ALLOW in ('1','3') and "+T9DBUtility.getDateFilter("LEAVE_DATE1",date, "<=")+" and "+T9DBUtility.getDateFilter("LEAVE_DATE2",date, ">=");
...
public String isOutTemp(Connection dbConn,HttpServletRequest request,
  ...
  try {
    ...
    //是否为外出    
    String  sql="select * from ATTEND_OUT where USER_ID='"+userId+"' and ALLOW in ('1') and "+T9DBUtility.getDateFilter("SUBMIT_TIME",date.substring(0,10)+" 00:00:00", ">=") + " and  "+T9DBUtility.getDateFilter("SUBMIT_TIME",date.substring(0,10)+" 23:59:59", "<=") + " and OUT_TIME1<='"+date.substring(11,16)+"' and OUT_TIME2>='"+date.substring(11,16)+"'";
    rs=stmt.executeQuery(sql);   
 ...
 */
  public String isEvectionTemp(Connection dbConn,HttpServletRequest request,
    ...
    try {
     ...
     String sql="select * from ATTEND_EVECTION where USER_ID='"+userId+"' and ALLOW='1' and "+T9DBUtility.getDateFilter("EVECTION_DATE1",date, "<=")+" and "+T9DBUtility.getDateFilter("EVECTION_DATE2",date, ">=");
      rs=stmt.executeQuery(sql);
      if(rs.next()){
        isEvection="1";
      }
...
public List<T9AttendDuty> getAttendDuty(HttpServletRequest request,
    HttpServletResponse response,String date,String registerType,String userId,T9AttendConfig config) throws Exception {
  ...
  try {
    ...
    date1 = t9dbu.getDateFilter("REGISTER_TIME", date1, ">=");
    date2 = t9dbu.getDateFilter("REGISTER_TIME", date2, "<=");
    //System.out.println(userId);
    dutyList = t9adl.selectDuty(dbConn, String.valueOf(userId), date1,date2,registerType);
    T9ManageAttendLogic attendLogic = new T9ManageAttendLogic();
    String dutyName = config.getDutyName();
    String dutyTime1 = config.getDutyTime1();
    String dutyTime2 = config.getDutyTime2();
    String dutyTime3 = config.getDutyTime3();
