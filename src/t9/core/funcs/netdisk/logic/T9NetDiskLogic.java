 ...
 * 
 */
package t9.core.funcs.netdisk.logic;
...
public class T9NetDiskLogic {
	...
	public void saveFileFolder(Connection dbConn, T9Netdisk netDisk) {
		T9ORM orm = new T9ORM();
		try {
	...
	public List<T9Netdisk> getNetDisksInfo(Connection dbConn) throws Exception {
		T9ORM orm = new T9ORM();
		return orm.loadListSingle(dbConn, T9Netdisk.class, new HashMap());
	}

	public T9Netdisk getFileSortInfoById(Connection dbConn, String seqId) throws NumberFormatException, Exception {
		T9ORM orm = new T9ORM();
		if (seqId == null) {
			seqId = "0";
		}
		return (T9Netdisk) orm.loadObjSingle(dbConn, T9Netdisk.class, Integer.parseInt(seqId));
	}

	public void updateFileSortInfoById(Connection dbConn, T9Netdisk fileSort) throws Exception {
		T9ORM orm = new T9ORM();
		orm.updateSingle(dbConn, fileSort);
	}

	public void delFileSortInfoById(Connection dbConn, T9Netdisk fileSort) throws Exception {
		T9ORM orm = new T9ORM();
		orm.deleteSingle(dbConn, fileSort);
	}

	public List<T9Netdisk> getNetDisks(Connection dbConn, Map map) throws Exception {
		T9ORM orm = new T9ORM();
		return (List<T9Netdisk>) orm.loadListSingle(dbConn, T9Netdisk.class, map);
	}
	...
	public String getUserIds(Connection dbConn, T9Netdisk netdisk, String action) throws Exception {
    T9ORM orm = new T9ORM();
    String ids = "";
    if (netdisk != null) {
      if ("USER_ID".equals(action)) {
        if (!"".equals(netdisk.getUserId()) && netdisk.getUserId() != null) {
          String idString = T9Utility.null2Empty(netdisk.getUserId());
        ...
        if (!"".equals(netdisk.getManageUser()) && netdisk.getManageUser() != null) {
          String idString = T9Utility.null2Empty(netdisk.getManageUser());
          String[] idsStrings = idString.split("\\|");
          if (idsStrings.length == 2) {
            ids = "";
          } else if (idsStrings.length == 1) {
        ...
        if (!"".equals(netdisk.getNewUser()) && netdisk.getNewUser() != null) {
          String idString = T9Utility.null2Empty(netdisk.getNewUser());
          String[] idsStrings = idString.split("\\|");
          if (idsStrings.length == 2) {
            ids = "";
          } else if (idsStrings.length == 1) {
        ...
        if (!"".equals(netdisk.getDownUser()) && netdisk.getDownUser() != null) {
          String idString = T9Utility.null2Empty(netdisk.getDownUser());
          String[] idsStrings = idString.split("\\|");
          if (idsStrings.length == 2) {
            ids = "";
          } else if (idsStrings.length == 1) {
	...
	public String getRoleIds(Connection dbConn, T9Netdisk netdisk , String action) throws Exception {
    T9ORM orm = new T9ORM();
    String ids = "";
    if (netdisk != null) {
      if ("USER_ID".equals(action)) {
	...
	public String getDeptIds(Connection dbConn, T9Netdisk netdisk, String action) throws Exception {
    T9ORM orm = new T9ORM();
    String ids = "";
    if (netdisk != null) {
      if ("USER_ID".equals(action)) {
	...
	public String getVisiPriv(Connection dbConn, Map map, int loginUserSeqId, int loginUserDeptId, String loginUserRoleId , String otherUserPriv , String otherDept, String pathId)
		...
		T9ORM orm = new T9ORM();
		T9Netdisk netdisk = (T9Netdisk) orm.loadObjSingle(dbConn, T9Netdisk.class, map);
		String diskPath = T9Utility.null2Empty(netdisk.getDiskPath());
		...
		if (netdisk != null) {
			...
			for (int i = 0; i < actions.length; i++) {
				if ("USER_ID".equals(actions[i])) {
					...
					boolean otherPrivFlag = !"".equals(T9WorkFlowUtility.checkId(otherUserPriv, rolePrivs, true))  ;
					boolean otherDeptFlag = !"".equals(T9WorkFlowUtility.checkId(otherDept, deptPrivs, true))  ;
	...
	public void copyEexistsFile(String srcPath, String desPath, String fileName, String nameTitle, String fileType) throws Exception {
		try {
			...
			T9FileUtility.copyFile(srcPath + "/" + fileName, newFile.getAbsolutePath());
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public void cutEexistsFile(String srcPath, String desPath, String fileName, String nameTitle, String fileType) throws Exception {
		...
		try {
			...
			if (newFile!=null) {
				if (!newFile.exists()) {
					T9FileUtility.xcopyFile(srcPath + "/" + fileName, newFile.getAbsolutePath());
				} else {
	...
	public static void deleteAllToRecycle(File file, String recyclePath) throws Exception {
		...
		if (file.isFile()) {
			...
			T9FileUtility.xcopyFile(file.getAbsolutePath(), recyclePath + File.separator + fileName);
			return;
		}
	 ...
	 * @throws Exception
	 */
	public boolean getUserIdAccessPriv(Connection dbConn, T9Person loginUser, T9Netdisk netdisk) throws Exception {
		...
		String userDeptOther = T9Utility.null2Empty(loginUser.getDeptIdOther());
		String userPrivOther = T9Utility.null2Empty(loginUser.getUserPrivOther());
	 ...
	 * @throws Exception
	 */
	public boolean getManageAccessPriv(Connection dbConn, T9Person loginUser, T9Netdisk netdisk) throws Exception {
		boolean returnFlag = false;
		T9FileSortLogic sortLogic = new T9FileSortLogic();
	 ...
	 * @throws Exception
	 */
	public boolean getDownAccessPriv(Connection dbConn, T9Person loginUser, T9Netdisk netdisk) throws Exception {
		boolean returnFlag = false;
		T9FileSortLogic sortLogic = new T9FileSortLogic();
	 ...
	 * @throws Exception
	 */
	public boolean getNewAccessPriv(Connection dbConn, T9Person loginUser, T9Netdisk netdisk) throws Exception {
		boolean returnFlag = false;
		T9FileSortLogic sortLogic = new T9FileSortLogic();
	 ...
	 * @return
	 */
	public String getUserIdString(Connection dbConn, T9Netdisk netdisk, String action) {
		String ids = "";
		if (netdisk != null) {
			if ("USER_ID".equals(action)) {
				if (!"".equals(netdisk.getUserId()) && netdisk.getUserId() != null) {
	 ...
	 * @throws Exception
	 */
	public String getRoleIdString(Connection dbConn, T9Netdisk netdisk, String action) throws Exception {
		String ids = "";
		if (netdisk != null) {
			if ("USER_ID".equals(action)) {
				if (!"".equals(netdisk.getUserId()) && netdisk.getUserId() != null) {
	 ...
	 * @throws Exception
	 */
	public String getDeptIdString(Connection dbConn, T9Netdisk netdisk, String action) throws Exception {
		String ids = "";
		if (netdisk != null) {
			if ("USER_ID".equals(action)) {
				if (!"".equals(netdisk.getUserId()) && netdisk.getUserId() != null) {
	...
	public List<Map<Object, Object>> getGlobalFileContentsByList(Connection dbConn, Map map, T9Person loginUser) throws Exception {
		...
		try {
			...
			if (userIdPriv) {
				...
				//
				// String fileType =
				// T9FileUtility.getFileExtName(file.getAbsolutePath()); // txt
				//
				...
				// StringBuffer buffer =
				// T9FileUtility.loadLine2Buff(file.getAbsolutePath(), "GBK");
				...
				// urlString = T9Utility.null2Empty(netdisk.getDiskName()) + "/" +
				// urlString;
				// }
				//
				...
				// returnMap.put("fileModifyTime", T9Utility.getDateTimeStr(new
	...
	public List queryFileInfo(String dirPath, List list, Map toMap, T9Netdisk netdisk) throws Exception {
		...
		try {
			...
			if (pathFile.exists()) {
				...
				for (File file : files) {
					...
					String fileType = T9FileUtility.getFileExtName(file.getAbsolutePath()); // txt
					...
					if (!"".equals(keyStr.trim())) {
						StringBuffer buffer = T9FileUtility.loadLine2Buff(file.getAbsolutePath(), "GBK");
						contentValue = buffer.indexOf(keyStr.trim());
					...
					returnMap.put("fileName", file.getName());
					returnMap.put("fileSize", this.transformSize(fileSizeLong));
					returnMap.put("fileModifyTime", T9Utility.getDateTimeStr(new Date(file.lastModified())));
					// returnMap.put("img", "");
					returnMap.put("typeName", typeName);
					returnMap.put("userPriv", userPriv);
					returnMap.put("managePriv", managePriv);
	...
	public List<Map<String, String>> getfileInforByList(Connection dbConn, T9Person loginUser, T9Netdisk netdisk, String fromDirPath) throws Exception {
		List list = new ArrayList();
				...
				if (files != null && files.length != 0) {
					for (int i = 0; i < files.length; i++) {
						...
						if (!subfile.isDirectory()) {
							...
							String fileType = T9FileUtility.getFileExtName(subfile.getAbsolutePath());
							...
							m.put("fileModifyTime", T9Utility.getDateTimeStr(new Date(subfile.lastModified())));
	 ...
	 * @throws Exception
	 */
	public T9PortalProducer getNetdiskInfoToDeskTop(Connection dbConn, T9Person person, String filePath, int startCount, int endCount) throws Exception {
		int k = 0;
		...
		try {
			...
			if (files != null && files.length != 0) {
				for (int i = startCount; i < startCount + endCount && k <= endCount && i < files.length; i++) {
					...
					map.put("fileModifyTime", T9Utility.getDateTimeStr(new Date(subfile.lastModified())) + "=======");
					list.add(map);
					k++;
				}
				rule.setData(list);
...
//			System.out.println(rule.toJson());
	...
	public String getInfoFromTextLogic(StringBuffer buffer,String filePath) throws Exception{
		...
		try {
			...
			File file = new File(filePath);
			if (file!=null) {
				 buffer = T9FileUtility.loadLine2Buff(filePath, "gbk");
