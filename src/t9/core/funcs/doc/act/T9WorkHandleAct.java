...
import t9.core.funcs.doc.logic.T9FlowRunAssistLogic;
...
public class T9WorkHandleAct {
  ...
  .getLogger("t9.core.funcs.doc.act.T9WorkHandleAct");
  public boolean getAip(HttpServletRequest request,
    ...
    try {
      ...
      T9DocFlowType flowType = flowTypeLogic.getFlowTypeById(Integer.parseInt(flowIdStr), dbConn);
      String dispAip = "";
      if ("1".equals(flowType.getFlowType())) {
        T9FlowProcessLogic flowPrcsLogic = new  T9FlowProcessLogic();
        //查出相关步骤
        T9DocFlowProcess flowProcess = flowPrcsLogic.getFlowProcessById(Integer.parseInt(flowIdStr), flowPrcsStr , dbConn);
  ...
  public boolean getWebsign(HttpServletRequest request,
    ...
    try {
      T9RequestDbConn requestDbConn = (T9RequestDbConn) request
          .getAttribute(T9BeanKeys.REQUEST_DB_CONN_MGR);
      dbConn = requestDbConn.getSysDbConn();
      String sql = "select 1 FROM DOC_FLOW_TYPE , DOC_FLOW_FORM_ITEM WHERE DOC_FLOW_TYPE.SEQ_ID=" + flowIdStr + " and DOC_FLOW_TYPE.FORM_SEQ_ID = DOC_FLOW_FORM_ITEM.FORM_ID AND CLAZZ='SIGN'";
      Statement stm1 = null;
      ResultSet rs1 = null;
      try {
