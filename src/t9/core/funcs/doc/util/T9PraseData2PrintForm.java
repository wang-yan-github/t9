...
public class T9PraseData2PrintForm {
  ...
  public String parsePrintForm(T9Person user,String modelShort, 
    int  runId, int flowId, List<T9DocFlowRunData> frdList,
    List<T9DocFlowFormItem> itemList ,Connection conn , boolean isWord) throws Exception {
      //-----------判断字段对于当前用户是否为隐藏----------------
      String hidden = "";
      String query = "select HIDDEN_ITEM from "+ T9WorkFlowConst.FLOW_PROCESS +" FLOW_PROCESS,"+ T9WorkFlowConst.FLOW_RUN_PRCS +" FLOW_RUN_PRCS " 
      ...
      for (T9DocFlowFormItem item : itemList) {
        ...
        T9DocFlowRunData flowRunData = this.getFlowRunData(frdList, itemId);
        if(flowRunData != null 
        ...
        if("DATE".equals(clazz) || "USER".equals(clazz)  || "WORDSELECT".equals(clazz)){
          ...
          if(child != null && !"".equals(child)){
            for (T9DocFlowFormItem tmp : itemList) {
              ...
              if (T9WorkFlowUtility.findId(child , title2) 
                  && "SELECT".equals(tag2)) {
                T9DocFlowRunData rd = this.getFlowRunData(frdList, itemId2);
  ...
  public T9DocFlowRunData getFlowRunData(List<T9DocFlowRunData> frdList, int itemId) {
