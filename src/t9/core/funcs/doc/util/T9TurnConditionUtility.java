...
import t9.core.funcs.doc.util.sort.T9UserSortComparatorUtility;
...
public class T9TurnConditionUtility {
   ...
   * @throws Exception
   */
  public String userSelect2(Connection conn,  T9Person user , String prcsBack , int flowId ,String autoUserOp,String ids) throws Exception{
    String prcsOpUser = "";
    ...
    String query2  = "SELECT * from "+ T9WorkFlowConst.FLOW_PROCESS +" where FLOW_SEQ_ID='"+ flowId +"' and PRCS_ID='"+prcsBack+"'";
    Statement stm2 = null;
    ResultSet rs2 = null;
  ...
  //默认经办人
    if (!T9Utility.isNullorEmpty(ids) && !T9Utility.isNullorEmpty(autoUserOp) ) {
      ...
      T9ORM orm = new T9ORM();
      T9Person person = (T9Person) orm.loadObjSingle(conn, T9Person.class, Integer.parseInt(autoUserOp));
      if(person != null){
   ...
   * @throws Exception
   */
  public String userSelect( T9Person user , T9DocFlowProcess fp , T9DocRun flowRun , Connection conn ) throws Exception{
    T9ORM orm = new T9ORM();
    StringBuffer sb = new StringBuffer();
    ...
    if ( fp.getChildFlow() != 0 ) {
      String query2  = "SELECT * from "+ T9WorkFlowConst.FLOW_PROCESS +" where FLOW_SEQ_ID='"+ fp.getChildFlow() +"' and PRCS_ID='1'";
      Statement stm2 = null;
      ResultSet rs2 = null;
    ...
    if ( fp.getAutoType() != null && fp.getAutoType().equals("1") ) {
      ...
      if(autoUser != null && !"".equals(autoUser)){
        ...
        //默认经办人
        //String findSql = T9WorkFlowUtility.createFindSql("SEQ_ID", fp.getAutoUser());
    ...
    //自动选择部门
      ...
      if(T9WorkFlowUtility.isNumeric(fp.getAutoUser())){
        ...
        T9DocFlowRunData frd = (T9DocFlowRunData) orm.loadObjSingle(conn, T9DocFlowRunData.class, map);
        String itemData = "";
        if(frd != null){
    ...
    //按照表单选择
      ...
      if(aPrcsUser.length == 1){
        ...
        T9Person tmp = (T9Person) orm.loadObjSingle(conn, T9Person.class,  Integer.parseInt(aPrcsUser[0]));
        if(tmp != null){
          prcsUserName = tmp.getUserName() + ",";
    ...
    //解析条件，把条件中的左侧项目、条件运算符、右侧项目分别放到三个变量里
      ...
      if (value != null && itemCon.indexOf("include") != - 1) {
        ...
        if ("数值".equals(itemValue) 
            && flag == T9Utility.isInteger(value)) {
          checkPass = true;//2010-12-08
        } else if ("日期".equals(itemValue)
            && flag == T9Utility.isDay(value)) {
          checkPass = true;
        } else if ("日期+时间".equals(itemValue)
            && flag == T9Utility.isDayTime(value)) {
          checkPass = true;
        } 
      } else if (value != null 
          && itemCon.indexOf(">=") != -1) {
  ...
  public String checkCondition(T9Person user ,Map formData , T9DocFlowProcess fp  , boolean isIn , int runId , int prcsId , int flowPrcs, Connection conn) throws Exception {
    //是否验证转入条件
    String condition = "";
    String conditionSetValue = "";
    String conditionSetDesc = "";
      ...
      if (itemTitle.indexOf("[主办人会签意见]") != -1 
        ...
        try {
          ...
          while (rs.next()) {
            ...
            String query1 = "select CONTENT from "+ T9WorkFlowConst.FLOW_RUN_FEEDBACK +" where "
              + " RUN_ID = " + runId
              + " and PRCS_ID=" + prcsId
  ...
  public Map getForm(int formId , int runId , Connection conn) throws Exception {
    ...
    T9ORM orm = new T9ORM();
    List<T9DocFlowFormItem> items = orm.loadListSingle(conn, T9DocFlowFormItem.class, map);
    //----- 取表单数据 --------
    Map formData = new HashMap();
    String  query1 = "select ITEM_ID" 
      + " ,ITEM_DATA " 
      + " from "+ T9WorkFlowConst.FLOW_RUN_DATA +" where "
    ...
    String  query = "select PRINT_MODEL_SHORT" 
      + " from "+ T9WorkFlowConst.FLOW_FORM_TYPE +" where "
      + " SEQ_ID = " + formId;
    String printModel = "";
    Statement stm = null;
    ResultSet rs = null;
    ...
    for (T9DocFlowFormItem tmp : items) {
  ...
  public String turnOther(String prcsUser , int flowId , int runId , int prcsId , String flowPrcs , String opUser , Map otherMap , Connection conn) throws Exception {
    ...
    for (int i = 0 ; i< uArray.length ; i ++) {
      ...
      if (tmp != null && !"".equals(tmp)) {
        String query = "select * from "+ T9WorkFlowConst.FLOW_RULE +" where " 
          + " FLOW_ID=" + flowId 
          + " and USER_ID="+ tmp 
          +" and STATUS = 1";
        Statement stmt = null;
  ...
  public String getUserByDeptRole(int deptId , String roleId , Connection conn) throws Exception {
    String query = "select PERSON.SEQ_ID from PERSON , USER_PRIV where USER_PRIV.SEQ_ID = PERSON.USER_PRIV AND (DEPT_ID ='" + deptId + "' OR " + T9DBUtility.findInSet(String.valueOf(deptId), "DEPT_ID_OTHER") + ")"
    + " and (USER_PRIV = '" + roleId + "' OR " + T9DBUtility.findInSet(roleId, "USER_PRIV_OTHER") + ")  order by USER_PRIV.PRIV_NO , PERSON.USER_NO DESC  ,PERSON.SEQ_ID";
    String manager = "";
    Statement stmt = null;
    ResultSet rs = null;
    try {
