...
public class T9FlowFormLogic {
  ...
  public String deleteDeptd(Connection dbConn, Object object) {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT MANAGER FROM DEPARTMENT WHERE SEQ_ID = '" + object + "'";
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
  ...
  public int deptFirstLogic(Connection dbConn, int seqId) {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT DEPT_PARENT, SEQ_ID FROM DEPARTMENT WHERE SEQ_ID = '" + seqId + "'";
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
  ...
  public List getDeptNameLong(Connection dbConn, int seqId) {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT DEPT_NAME, DEPT_PARENT, SEQ_ID FROM DEPARTMENT WHERE SEQ_ID = '" + seqId + "'";
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
  ...
  public void updateJs(Connection conn, int seqId, String script)throws Exception {
    ...
    try{   
      String sql = "UPDATE "+ T9WorkFlowConst.FLOW_FORM_TYPE +" SET SCRIPT = ? where SEQ_ID =" + seqId;
  ...
  public void updateCss(Connection conn, int seqId, String css)throws Exception {
    PreparedStatement pstmt = null;
    try{   
      String sql = "UPDATE "+ T9WorkFlowConst.FLOW_FORM_TYPE +" SET CSS = ? where SEQ_ID =" + seqId;
      pstmt = conn.prepareStatement(sql);
  ...
  public String getPrcsList(Connection dbConn, int seqId) throws Exception {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT PRCS_USER, SEQ_ID FROM "+ T9WorkFlowConst.FLOW_PROCESS +" WHERE SEQ_ID = " + seqId;
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
