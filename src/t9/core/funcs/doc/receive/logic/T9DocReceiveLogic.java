...
import t9.core.funcs.doc.receive.data.T9DocNext;
 ...
 *
 */
public class T9DocReceiveLogic{
  /**
   * 我的未(已)发收文
   * @param conn
   * @param doc
   ...
   * @throws Exception 
   */
  public List<T9DocReceive> faDocReceive(Connection conn, T9Person user, int sendStatus, String column, String asc) throws Exception{
    String sql = " select dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
    		                 " dr.RECE_USER_ID, dr.DOC_TYPE,dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,dr.ATTACHNAME,dr.ATTACHID,dr.RUN_ID , dr.REC_DOC_ID , dr.REC_DOC_NAME from DOC_RECEIVE dr   " +
    		                 " where dr.RECE_USER_ID = "+ user.getSeqId() +" and dr.SEND_STATUS = " + sendStatus;
    if(T9Utility.isNullorEmpty(column)){
    ...
    ResultSet rs = null;
    List<T9DocReceive> dosc = new ArrayList<T9DocReceive>();
    try{
   ...
   * @throws Exception 
   */
  public List<T9DocReceive> faDocReceive1(Connection conn, T9Person user, int sendStatus, String column, String asc) throws Exception{
    String tmp = " dr.SPONSOR = '"+ user.getDeptId() +"' ";
    //支持辅助部门
    if (!T9Utility.isNullorEmpty(user.getDeptIdOther())) {
      tmp += " OR dr.SPONSOR IN ( " + T9WorkFlowUtility.getInStr(user.getDeptIdOther()) + " ))";
      tmp = "( " + tmp;
    }
    String sql = " select dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE,dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,dr.ATTACHNAME,dr.ATTACHID,dr.RUN_ID,dr.REC_DOC_ID, dr.REC_DOC_NAME from DOC_RECEIVE dr   " +
  ...
  public String getDeptName(Connection dbConn, int seqId) throws Exception{
    String sql = "select DEPT_NAME from department dr where dr.SEQ_ID=" + seqId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = dbConn.prepareStatement(sql);
   ...
   * @throws Exception 
   */
  public T9DocReceive faDoc(Connection conn,  int seqId) throws Exception{
    String sql = " select dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE,dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,dr.ATTACHNAME,dr.ATTACHID,dr.RUN_ID from DOC_RECEIVE dr  " +
   ...
   * @throws Exception
   */
  public void insertBeanChT9DocReceive(Connection conn, T9DocReceive doc) throws Exception{
    String sql = " insert into doc_receive(DOC_NO, RES_DATE, FROMUNITS, OPPDOC_NO, TITLE, COPIES, CONF_LEVEL, INSTRUCT, SPONSOR, RECE_USER_ID, DOC_TYPE, STATUS, SEND_STATUS, CREATE_USER_ID,ATTACHNAME,ATTACHID) ";
           sql += " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    PreparedStatement ps = null;
    try {
  ...
  public int updateDocReceive(Connection dbConn, int seqId, int userId) throws SQLException{
    String sql = "update doc_receive set SEND_STATUS = 1,  RECE_USER_ID="+ userId +" where SEQ_ID=" + seqId;
    PreparedStatement ps = null;
    int k =0;
    try{
      ps = dbConn.prepareStatement(sql);
  ...
  public int updateDocReceive(Connection dbConn, int seqId, String runId, int status) throws SQLException{
    String sql = "update doc_receive set RUN_ID = "+ runId +",STATUS="+ status +"  where SEQ_ID=" + seqId;
    PreparedStatement ps = null;
    int k =0;
    try{
      ps = dbConn.prepareStatement(sql);
   ...
   * @throws Exception 
   */
  public List<T9DocReceive> myReadDocReceive1(Connection conn, T9Person user, String status) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
    ...
    if ("1".equals(status)) {
      ...
      sql += " , DOC_RUN  where dr.CREATE_USER_ID = P.SEQ_ID  and dr.RECE_USER_ID = "+ user.getSeqId() + " AND DOC_RUN.RUN_ID = dr.RUN_ID" ;
      sql += " and (dr.RUN_ID is not null and dr.RUN_ID != '')  AND DOC_RUN.END_TIME  is null";
    } else {
      sql += " , DOC_RUN  where dr.CREATE_USER_ID = P.SEQ_ID  and dr.RECE_USER_ID = "+ user.getSeqId() + " AND DOC_RUN.RUN_ID = dr.RUN_ID" ;
      sql += " and (dr.RUN_ID is not null and dr.RUN_ID != '')  AND DOC_RUN.END_TIME  is not null";
    }
   ...
   * @throws Exception 
   */
  public List<T9DocReceive> myReadDocReceive(Connection conn, T9Person user, int status ) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
  ...
  public int updateReadDocReceive(Connection dbConn, int seqId) throws SQLException{
    String sql = "update doc_receive set STATUS = 1 where SEQ_ID=" + seqId;
    PreparedStatement ps = null;
    int k =0;
    try{
      ps = dbConn.prepareStatement(sql);
 ...
 /*   String sql = "update doc_receive set SEND_STATUS = 1, RECE_USER_ID=? where SEQ_ID=?";
   ...
   * @throws Exception
   */
  public T9DocReceive beanChAlarm(Connection dbConn, int i) throws Exception{
    String sql = "select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.RECE_USER_ID from DOC_RECEIVE dr where dr.SEQ_ID=" + i;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      if(rs.next()){
         T9DocReceive dos = new T9DocReceive();
   ...
   * @throws Exception
   */
  public List<T9DocReceive> beanChAlarm(Connection dbConn, String[] seqId) throws Exception, Exception{
    List<T9DocReceive> docs = new ArrayList<T9DocReceive>();
    if(seqId.length > 0){
       for(int i=0; i<seqId.length; i++){
         T9DocReceive doc =  beanChAlarm(dbConn, Integer.parseInt(seqId[i]));
         docs.add(doc);
       }
       return docs;
    }
  ...
  public Map<String, String> fileUploadLogic(T9FileUploadForm fileForm,
    ...
    try {
      ...
      while (iKeys.hasNext()) {
        ...
        fileForm.saveFile(fieldName, filePath + File.separator+ T9WorkFlowConst.MODULE  + File.separator+ hard  + File.separator+ fileName);
      }
    } catch (Exception e) {
      throw e;
    }
...
//多文件上传时用到的----
  ...
  public boolean delFloatFile(Connection dbConn, String attId, String attName, int seqId) throws Exception {
    ...
    if (seqId != 0) {
      T9ORM orm = new T9ORM();
      T9DocReceive news = faDoc(dbConn,  seqId);
      String[] attIdArray = {};
      String[] attNameArray = {};
  ...
  //处理文件
    ...
    String path = T9DocConst.filePath  + File.separator+ tmp[0]  + File.separator+ tmp[1] + "_" + attName;
    File file = new File(path);
    if(file.exists()){
      file.delete();
    } else {
      //兼容老的数据
      String path2 = T9DocConst.filePath  + File.separator+ tmp[0]  + File.separator+tmp[1] + "." + attName;
  ...
  public void updateSingle(Connection dbConn, T9DocReceive news) throws SQLException{
    String sql = "update doc_receive set ATTACHID ='"+ news.getAttachIds() +"',ATTACHNAME='"+ news.getAttachNames() +"' where SEQ_ID="+ news.getSeq_id();
    PreparedStatement ps = null;
    try{
        ps = dbConn.prepareStatement(sql);
        ps.executeUpdate();
   ...
   * @throws Exception 
   */
  public String myDocReceiveJson(Connection conn, T9Person user, int seqId   , String webroot) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE, dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,P.USER_NAME ,dr.ATTACHNAME,dr.ATTACHID, dr.RUN_ID " +
                         " from DOC_RECEIVE dr, PERSON P  " +
  ...
  public String getFlowId(Connection conn, String runId) throws Exception{
    String sql = "select FLOW_ID, RUN_ID, END_TIME from doc_run where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getMaxProcsId(Connection conn, String runId)throws Exception{
    String sql = "select max(PRCS_ID) as prcdId from doc_flow_run_prcs where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getFlowPrcs(Connection conn, String prcsId) throws Exception{
    String sql = "select FLOW_PRCS from doc_flow_run_prcs where PRCS_ID="+ prcsId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getPrcsName(Connection conn, String flowPrcs, String flowId)throws Exception{
    String sql = "select PRCS_NAME from DOC_FLOW_PROCESS where PRCS_ID = "+ flowPrcs +" and FLOW_SEQ_ID ="+ flowId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public T9DocNext returnNext(Connection conn, String runId)throws Exception{
    try{
      T9DocNext next = new T9DocNext();
  ...
  public List<Integer> getMaxOrderNo(Connection dbConn,  int typeId) throws Exception{
    String sql = "select  dr.DOC_NO from DOC_RECEIVE dr where dr.DOC_TYPE=" + typeId;
    PreparedStatement ps = null;
    ResultSet rs = null;
   ...
   * @throws Exception
   */
  public T9DocReceive getAdocById(Connection dbConn,  int seqId) throws Exception{
   ...
   * @throws Exception 
   */
  public void update(Connection conn,  T9DocReceive doc) throws Exception{
    String sql = "update doc_receive " 
    		             +" set DOC_NO = ?,"
                     +" FROMUNITS = ?,"
                     +" OPPDOC_NO = ?,"
                     +" TITLE = ?,"
   ...
   * @throws Exception 
   */
  public T9DocReceive siglePrint(Connection conn,  int seqId) throws Exception{
   ...
   * @throws Exception
   */
  public List<T9DocReceive> printDocs(Connection conn,  String ids) throws Exception{
    String sql = " select dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE,dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,dr.ATTACHNAME,dr.ATTACHID,dr.RUN_ID from DOC_RECEIVE dr   " +
  ...
  public List<String> cuiBanUser(Connection conn,  int seqId, int prcsId, int runId) throws Exception{
    try{
      String sql = "select  USER_ID from flow_run_prcs where PRCS_ID = "+ prcsId +" and RUN_ID = "+ runId +" and (PRCS_FLAG = 1 or PRCS_FLAG = 2)";
      PreparedStatement ps = null;
      ResultSet rs = null;
