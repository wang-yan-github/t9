...
public class T9FlowProcessLogic {
  public void saveFlowProcess(T9DocFlowProcess flowProcess , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    orm.saveSingle(conn, flowProcess);
  }
  public String getDisAip(int flowId , Connection conn) throws Exception {
    StringBuffer sb = new StringBuffer();
    String query = "select SEQ_ID , T_NAME from "+ T9WorkFlowConst.FLOW_PRINT_TPL +" where FLOW_ID = '"+flowId+"' AND T_TYPE='2'";
    Statement stm = null;
    ResultSet rs = null;
    sb.append("[");
    int count = 0 ;
  ...
  public List<T9DocFlowProcess> getFlowProcessByFlowId(int flowId , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    List<T9DocFlowProcess> list = new ArrayList();
    Map filters = new HashMap();
    filters.put("FLOW_SEQ_ID", flowId);
    list  = orm.loadListSingle(conn ,T9DocFlowProcess.class , filters);
    return  list;
  }
  public List<Map> getFlowPrcsByFlowId(int flowId , Connection conn) throws Exception{
    List<Map> list = new ArrayList<Map>();
    String query = "select SEQ_ID , PRCS_ID , PRCS_NAME from "+ T9WorkFlowConst.FLOW_PROCESS +" WHERE FLOW_SEQ_ID =" + flowId + " order by prcs_id";
    Statement stm = null;
    ResultSet rs = null;
    try{
      stm = conn.createStatement();
  ...
  public List<T9DocFlowProcess> getFlowPrrocessByFlowId1(int flowId , Connection conn) throws Exception{
    String query = "select " 
      + " PRCS_ID"
      + " , PRCS_USER"
      + " , PRCS_DEPT"
      + " , PRCS_PRIV"
      + " from "+ T9WorkFlowConst.FLOW_PROCESS +" where FLOW_SEQ_ID=" + flowId + " and PRCS_ID=1";
  ...
  public T9DocFlowProcess getFlowProcessById(int flowId , String prcsId , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9DocFlowProcess flowProcess = null;
    Map filters = new HashMap();
    filters.put("FLOW_SEQ_ID", flowId);
    filters.put("PRCS_ID", prcsId);
    flowProcess  = (T9DocFlowProcess) orm.loadObjSingle(conn, T9DocFlowProcess.class, filters);
    return flowProcess;
  }
 
  public T9DocFlowProcess getFlowProcessById(int processId , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9DocFlowProcess flowProcess = new T9DocFlowProcess();
    flowProcess = (T9DocFlowProcess) orm.loadObjSingle(conn, T9DocFlowProcess.class, processId);
    return flowProcess;
  }
  public void delFlowProcess(int processId , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9DocFlowProcess fp = this.getFlowProcessById(processId , conn);
    int prcsId = fp.getPrcsId();
    int flowId = fp.getFlowSeqId();
    List<T9DocFlowProcess> list = this.getFlowProcessByFlowId(flowId , conn);
    boolean isChange = false;
    for(T9DocFlowProcess tmp : list){
    ...
    orm.deleteSingle(conn,T9DocFlowProcess.class, processId);
  }
  ...
  public void updateFlowProcess(T9DocFlowProcess flowProcess , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
  ...
  public StringBuffer getDeptJson(List depts , int procId , int deptId , Connection conn) throws Exception{
    T9DocFlowProcess proc = this.getFlowProcessById(procId , conn);
  ...
  public List<T9DocFlowProcess> getNextProcess(T9DocFlowProcess prcs , Connection conn) throws Exception{
    List<T9DocFlowProcess> list = this.getFlowProcessByFlowId(prcs.getFlowSeqId() , conn);
    Collections.sort(list,new T9FlowProcessComparator());
    List<T9DocFlowProcess> result = new ArrayList();
    String prcsTo = prcs.getPrcsTo();
    if(prcsTo == null || "".equals(prcsTo)){
      for(int i = 0 ; i< list.size() ; i++){
        T9DocFlowProcess prcsTmp = list.get(i);
        if(prcsTmp.getSeqId() == prcs.getSeqId()){
          if(i + 1 == list.size()){
            prcsTmp = new T9DocFlowProcess();
            prcsTmp.setPrcsId(0);
            prcsTmp.setSeqId(0);
      ...
      for(String tmp : aPrcsTo){
        if(tmp.equals("0")){
          T9DocFlowProcess prcsTmp = new T9DocFlowProcess();
          prcsTmp.setPrcsId(0);
          prcsTmp.setSeqId(0);
          prcsTmp.setPrcsName("结束");
          result.add(prcsTmp);
        ...
        for(T9DocFlowProcess prcsTmp : list){
  ...
  public void savePosition(String strSql , Connection conn) throws Exception{
    ...
    String query = "UPDATE "+ T9WorkFlowConst.FLOW_PROCESS +" SET ";
  ...
  public String getPluginStr(int prcsId , int flowId , Connection conn) throws Exception{
    String query = "select  PLUGIN from "+ T9WorkFlowConst.FLOW_PROCESS +"  where " 
      + "  PRCS_ID= " + prcsId 
  ...
  public void setMatadataItem(String items , int seqId , Connection conn) throws Exception{
    String query = "update "+ T9WorkFlowConst.FLOW_PROCESS +" set metadata_item='" + items + "' where seq_id=" + seqId;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
