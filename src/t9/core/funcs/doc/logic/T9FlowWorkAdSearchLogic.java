...
import t9.core.funcs.doc.util.sort.T9FlowComparator;
 ...
 *
 */
public class T9FlowWorkAdSearchLogic extends T9FlowWorkSearchLogic{
  public String checkFormula(String condFormula , String flowId , String sortId  , String skin , String contextPath) {
    String result  = null;
    if (!T9Utility.isNullorEmpty(condFormula)) {
      ...
      if (tmpA1.length != tmpA2.length) {
        String msg = T9WorkFlowUtility.Message("条件公式的左右括号 ( 和 ) 个数不一致！", 0);
        msg += "<br><div align=\"center\"><input type=\"button\" value=\"返回\" class=\"BigButton\" onClick=\"location='"+contextPath+T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/query/query.jsp?flowId="+flowId+"&sortId="+sortId+"&skin="+skin+"'\"><div>";
        return msg;
      }
      ...
      if (tmpA1.length != t) {
        String msg = T9WorkFlowUtility.Message("条件公式的左右中括号 [ 和 ] 个数不一致！", 0);
        msg += "<br><div align=\"center\"><input type=\"button\" value=\"返回\" class=\"BigButton\" onClick=\"location='"+contextPath+T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/query/query.jsp?flowId="+flowId+"&sortId="+sortId+"&skin="+skin+"'\"><div>";
        return msg;
      }
      tmpA1 = condFormula.split("\\{");
      ...
      if (tmpA1.length != tmpA2.length) {
        String msg = T9WorkFlowUtility.Message("条件公式的左右大括号 { 和 } 个数不一致！", 0);
        msg += "<br><div align=\"center\"><input type=\"button\" value=\"返回\" class=\"BigButton\" onClick=\"location='"+contextPath+T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/query/query.jsp?flowId="+flowId+"&sortId="+sortId+"&skin="+skin+"'\"><div>";
        return msg;
      }
    }
  ...
  public Map flowReport(int flowId, HttpServletRequest request,
      Connection conn, T9Person user, String listFldsStr) throws Exception {
    // TODO Auto-generated method stub
    List<Map> mapList = new ArrayList<Map>();
    Map titles = new HashMap();
    String condFormula = request.getParameter("condFormula");
    T9FlowRunUtility fu = new T9FlowRunUtility();
    ...
    try {
      ...
      while (rs.next()) {
        ...
        List<T9DocFlowRunData> frdList = orm.loadListSingle(conn, T9DocFlowRunData.class, map2);
        ...
        for (T9DocFlowFormItem item : list) {
          ...
          if ((!T9Utility.isNullorEmpty(value) 
            ...
            String query1 = "select ITEM_DATA from "+ T9WorkFlowConst.FLOW_RUN_DATA +" where RUN_ID=" + runId + " and item_id=" + id;
            Statement stm1 = null;
            ResultSet rs1 = null;
            try {
              ...
              if (rs1.next()) {
                ...
                itemData = T9WorkFlowUtility.clob2String(itemDataTmp);
              }
       ...
       //流程状态显示        String status = "";
        ...
        for (T9DocFlowFormItem item : list) {
          ...
          //不显示的列
          if (T9WorkFlowUtility.findId(listFldsStr, String.valueOf(itemId))) {
            continue;
          }
  ...
  public List<Map> doQuery(int flowId, HttpServletRequest request,
      Connection conn, T9Person user) throws Exception {
    // TODO Auto-generated method stub
    List<Map> mapList = new ArrayList<Map>();
    String condFormula = request.getParameter("condFormula");
    T9FlowRunUtility fu = new T9FlowRunUtility();
  ...
  public void setCommentPriv(Map map , String commentPriv , String manageUser , String queryUser , String queryUserDept , String manageUserDept , Connection conn , T9Person user) throws Exception {
    //显示“点评”超链接 begin
    if (manageUser == null) {
      manageUser = "";
    }
  ...
  public void setAttach(Map map
      ...
      , int flowId
      , Connection conn 
      , T9Person user) throws Exception{
  ...
  public void setRunPrcs(Map map , int runId , String freeOther , Connection conn , int userId) throws Exception {
    ...
    String query = "select  PRCS_ID , PRCS_FLAG , FLOW_PRCS , OP_FLAG from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID=" + runId  + " and USER_ID=" + userId + " and PRCS_FLAG <>'4'  order by PRCS_FLAG " ;//“limit 1”的作用？针对并发或者子流程？
  ...
  public void setEditPriv(Map map ,Timestamp endTime , String privStr , Connection conn , T9Person user) throws Exception{
    if (endTime != null) {
  ...
  public void setFocusPriv(Map map ,Timestamp endTime , String focusUser , T9Person user) {
  ...
  private boolean checkCondition(String relation ,String itemData , String value) {
    int iItemData = 0;
    int iValue = 0 ;
    if (T9Utility.isInteger(itemData)) {
      iItemData = Integer.parseInt(itemData);
    }
    if (T9Utility.isInteger(value)) {
  ...
  private String getQuerySql(int flowId, HttpServletRequest request ,Connection conn , T9Person user) throws Exception {
    // TODO Auto-generated method stub
    String query = "SELECT * from "+ T9WorkFlowConst.FLOW_TYPE +" FLOW_TYPE,"+ T9WorkFlowConst.FLOW_RUN +" FLOW_RUN,PERSON"
      + " WHERE  "
    ...
    if(!T9Utility.isNullorEmpty(runName)) {
    ...
    if(!T9Utility.isNullorEmpty(attachmentName)) {
      query += " and FLOW_RUN.ATTACHMENT_NAME like '%" + T9Utility.encodeLike(attachmentName) + "%' "  + T9DBUtility.escapeLike();
    }
    //--- 日期范围 ---
    String prcsDate1 = request.getParameter("prcsDate1");
    if(!T9Utility.isNullorEmpty(prcsDate1)){
      prcsDate1 +=  " 00:00:00";
      String dbDateF = T9DBUtility.getDateFilter("BEGIN_TIME", prcsDate1, " >= ");
      query += " and " + dbDateF;
    }
       
    String prcsDate2 = request.getParameter("prcsDate2");
    if(!T9Utility.isNullorEmpty(prcsDate2)){
      prcsDate2 +=  " 23:59:59";
      String dbDateF = T9DBUtility.getDateFilter("BEGIN_TIME", prcsDate2, " <= ");
      query += " and " + dbDateF;
    }
    ...
    if(!T9Utility.isNullorEmpty(endDate1)){
      endDate1 +=  " 00:00:00";
      String dbDateF = T9DBUtility.getDateFilter("FLOW_RUN.END_TIME", endDate1, " >= ");
      query += " and " + dbDateF;
    }
       
    String endDate2 = request.getParameter("endDate2");
    if(!T9Utility.isNullorEmpty(endDate2)){
      endDate2 +=  " 23:59:59";
      String dbDateF = T9DBUtility.getDateFilter("FLOW_RUN.END_TIME", endDate2, " <= ");
      query += " and " + dbDateF;
    }
    //查询范围
    ...
    if (T9Utility.isInteger(beginUser)) {
  ...
  public String getRunNameCondition(String runName , String rr) throws Exception{
    ...
    if ("1".equals(rr)) {
      ...
      whereRun += " like '" + T9Utility.encodeLike(runName) + "%' " + T9DBUtility.escapeLike();
    } else if ("8".equals(rr)) {
      whereRun += " like '%" + T9Utility.encodeLike(runName) + "%' " + T9DBUtility.escapeLike();
    } else if ("9".equals(rr)) {
      whereRun += " like '%" + T9Utility.encodeLike(runName) + "' " + T9DBUtility.escapeLike();
  ...
  public String getRealValue(List<T9DocFlowRunData> frdList, T9DocFlowFormItem item){
    String realValue = "";
    for (T9DocFlowRunData flowRunData : frdList) {
  ...
  public String getHiddenItem(Connection conn , T9Person loginUser , int flowId , int runId) throws Exception {
    String sql = "select HIDDEN_ITEM from "+ T9WorkFlowConst.FLOW_PROCESS +" FLOW_PROCESS,"+ T9WorkFlowConst.FLOW_RUN_PRCS +" FLOW_RUN_PRCS where FLOW_PROCESS.FLOW_SEQ_ID="+flowId+" and FLOW_RUN_PRCS.RUN_ID="+ runId +" and FLOW_RUN_PRCS.USER_ID="+loginUser.getSeqId()+" and FLOW_PROCESS.PRCS_ID=FLOW_RUN_PRCS.FLOW_PRCS";
    Statement stm2 = null;
    ResultSet rs2 = null;
    String hidden = "";
    try {
  ...
  public ArrayList<T9DbRecord> getExcelData(int flowId, String runIds,
      Connection conn, String contextPath, T9Person loginUser) throws Exception {
    // TODO Auto-generated method stub
    T9FlowRunUtility fu = new T9FlowRunUtility();
    int formId = fu.getFormId(conn, flowId);
    T9ORM orm = new T9ORM();
    Map formItemQuery = new HashMap();
    formItemQuery.put("FORM_ID", formId);
    List<T9DocFlowFormItem> itemList = orm.loadListSingle(conn, T9DocFlowFormItem.class , formItemQuery);
    ArrayList<T9DbRecord> list = new ArrayList<T9DbRecord>(); 
    runIds = T9WorkFlowUtility.getOutOfTail(runIds);
    String query = "select run_id , run_NAME , END_TIME , BEGIN_TIME from "+ T9WorkFlowConst.FLOW_RUN +" where run_id in (" + runIds + ") order by run_id";
    Statement stm = null;
    ResultSet rs = null;
    try{
      stm = conn.createStatement();
      ...
      while (rs.next()) {
        ...
        String sBeginTime = T9Utility.getDateTimeStr(beginTime);
        ...
        for (T9DocFlowFormItem item : itemList) {
          String realValue = "";
          String tag = item.getTag();
  ...
  public void sortList(HttpServletRequest request,List<Map> list) {
    ...
    Collections.sort(list, new T9FlowComparator(groupFld , groupSort));
  }
}
