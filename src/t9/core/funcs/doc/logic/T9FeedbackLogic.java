...
import t9.core.funcs.doc.util.sort.T9FeedbackComparator;
import t9.core.util.T9Utility;
import t9.core.util.db.T9DBUtility;
import t9.core.util.db.T9ORM;
import t9.core.util.file.T9FileUtility;

public class T9FeedbackLogic {
  /**
   * 添加会签意见
   * @param feedback
   * @throws Exception
   */
  public void saveFeedback(T9DocFlowRunFeedback feedback , Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
   ...
   * @throws Exception
   */
  public String getFeedbacksHtml(T9Person loginUser , int flowId ,int runId , Connection conn) throws Exception{
    ...
    String query = "SELECT * from "+ T9WorkFlowConst.FLOW_RUN_FEEDBACK +" where RUN_ID="+ runId +" order by PRCS_ID,EDIT_TIME";
      ...
      while (rs1.next()) {
        ...
        if (seeSign) {//可见
         ...
         // $ATTACHMENT_NAME=$ROW["ATTACHMENT_NAME"];//附件名称串
          ...
          String editTime = T9Utility.getDateTimeStr(date);
...
//          $CONTENT_VIEW=nl2br($CONTENT_VIEW);//将字符串中的换行符转成HTML的换行符号
   ...
   * @throws Exception
   */
  public String getFeedbacks(T9Person loginUser , int flowId ,int runId , Connection conn) throws Exception{
    Map prcsNameMap = new HashMap();
    Map signLookMap = new HashMap();
    int type = this.getFlowType(conn, flowId);
    this.setPrcsMap(prcsNameMap, signLookMap, runId, flowId , type, conn);
    ...
    try {
      ...
      while (rs1.next()) {
        ...
        if (seeSign) {//可见
          ...
          for ( int i = 0 ;i < attachsId.length ;i ++ ) {
            ...
            if (!"".equals(id)) {
              ...
              sb.append(",attachmentId:'" + id + "'" +
                  ",ext:'" +  T9FileUtility.getFileExtName(name) + "'},");
              count1++;
            }
          }
          if ( count1 > 0 ) {
          ...
          int feedFlag = rs1.getInt("FEED_FLAG");
          String sign = T9WorkFlowUtility.clob2String(rs1.getClob("SIGN_DATA"));
          if( !"".equals(sign.trim()) ){
  ...
  public List<T9DocFlowRunFeedback> getSignLookFeedback(Map signLookMap , int flowType, int runId  , int userId , Connection conn) throws Exception {
    List<T9DocFlowRunFeedback> list = new ArrayList();
    String query = "select * from "+ T9WorkFlowConst.FLOW_RUN_FEEDBACK +" where RUN_ID =" + runId + " order by EDIT_TIME DESC";
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      ...
      while(rs2.next()){
        ...
        if (seeSign) {
          T9DocFlowRunFeedback feedback = new T9DocFlowRunFeedback();
          feedback.setSeqId(rs2.getInt("SEQ_ID"));
          feedback.setAttachmentId(T9WorkFlowUtility.clob2String(rs2.getClob("ATTACHMENT_ID")));
          feedback.setAttachmentName(T9WorkFlowUtility.clob2String(rs2.getClob("ATTACHMENT_NAME")));
          feedback.setContent(T9WorkFlowUtility.clob2String(rs2.getClob("CONTENT")));
          ...
          feedback.setSignData(T9WorkFlowUtility.clob2String(rs2.getClob("SIGN_DATA")));
  ...
  public boolean isOpUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    ...
    String query = "select 1 from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public boolean isHoUser(int prcsId , int runId , int userId , Connection conn) throws Exception{ 
    boolean isPrcsUser = false;
    String query = "select 1 from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID =" + runId + " and PRCS_ID=" + prcsId + " and USER_ID=" + userId + " and OP_FLAG='1'" ;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
  ...
  public void setPrcsMap(Map prcsNameMap , Map signLookMap , int runId , int flowId, int type ,   Connection conn) throws Exception {
    ...
    String queryPrcs = "select PRCS_ID , FLOW_PRCS from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID=" + runId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
...
//      } else if (!T9WorkFlowUtility.findId(prcsStr, prcsName)) {
//        str += "," + prcsName;
...
//      }
  ...
  public Map getNames(int userId , Connection conn) throws Exception {
    ...
    String queryName = "SELECT USER_NAME "
      + " ,DEPT_NAME  "
      + " FROM PERSON p , DEPARTMENT d WHERE  "
      + " p.SEQ_ID = " + userId
   ...
   * @throws Exception
   */
  public String getFeedbacks(T9Person loginUser  , int runId , int prcsId , int flowId , int flowPrcs , Connection conn) throws Exception{
    Map prcsNameMap = new HashMap();
    Map signLookMap = new HashMap();
    int type = this.getFlowType(conn, flowId);
    this.setPrcsMap(prcsNameMap, signLookMap, runId, flowId , type, conn);
    List<T9DocFlowRunFeedback> list = this.getSignLookFeedback(signLookMap ,type , runId, loginUser.getSeqId(), conn);
    StringBuffer sb = new StringBuffer();
    for(T9DocFlowRunFeedback tmp : list){
  ...
  public void delFeedback(int seqId ,Connection conn) throws Exception{
    T9ORM orm = new T9ORM();
    T9DocFlowRunFeedback tmp = (T9DocFlowRunFeedback) orm.loadObjSingle(conn, T9DocFlowRunFeedback.class, seqId);
    String attachmentId = tmp.getAttachmentId() == null ? "" : tmp.getAttachmentId();
    String attachmentName = tmp.getAttachmentName() == null ? "" : tmp.getAttachmentName();
  ...
  public String getSignData(int feedId , Connection conn) throws Exception {
    String signData = "";
    String query = "select SIGN_DATA from "+ T9WorkFlowConst.FLOW_RUN_FEEDBACK +" where SEQ_ID = "+ feedId;
    Statement stm2 = null;
    ResultSet rs2 = null;
    try {
      stm2 = conn.createStatement();
