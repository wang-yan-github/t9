...
public class T9WorkDestroyLogic {
  private static Logger log = Logger.getLogger("t9.core.funcs.doc.logic.T9WorkDestroyLogic");
 /**
  * 得到所有流程
  * @param conn
  * @return
  ...
  */
  public String getFlow(Connection conn , String sortId) throws Exception{
    ...
    String sql = "SELECT FLOW_TYPE.SEQ_ID,FLOW_NAME,FREE_OTHER from "+ T9WorkFlowConst.FLOW_TYPE +" FLOW_TYPE,"+ T9WorkFlowConst.FLOW_SORT +" FLOW_SORT where FLOW_TYPE.FLOW_SORT = FLOW_SORT.SEQ_ID "+ tmp +" order by SORT_NO,FLOW_NO";
  ...
  public StringBuffer getWorkListLogic(Connection conn,T9Person user,Map request) throws Exception{
    ...
    try {
      String sql =" SELECT " 
          ...
          + " ,FLOW_ID "
      		+ " FROM " 
      		+ " "+ T9WorkFlowConst.FLOW_RUN +" FLOW_RUN "
      		+ " ,"+ T9WorkFlowConst.FLOW_TYPE +" FLOW_TYPE "
      	  + " ,PERSON"
      		+ " WHERE DEL_FLAG = 1 " 
...
public StringBuffer getAlldeleteRunId(Connection conn,T9Person user,Map request) throws Exception{
  ...
  public void destroyFlowWork(Connection conn , String module,int runId,int userId,String ip) throws Exception{
    ...
    //logic.destroyNum(conn, runId);
    
    String content = "销毁此工作";
    T9FlowRunLogLogic frll = new T9FlowRunLogLogic();
    frll.runLog(runId, 0, 0, userId, 4, content, ip, conn);
    //删除附件
    deleteAttachByWork(conn, T9WorkFlowConst.FLOW_RUN, runId, module);
    //删除会签附件
    deleteAttachByWork(conn, T9WorkFlowConst.FLOW_RUN_FEEDBACK, runId, module);
    //删除记录
    deleteFlowWorkUtil(conn, T9WorkFlowConst.FLOW_RUN_FEEDBACK, runId);
    deleteFlowWorkUtil(conn, T9WorkFlowConst.FLOW_RUN_DATA, runId);
    deleteFlowWorkUtil(conn, T9WorkFlowConst.FLOW_RUN_PRCS, runId);
    deleteFlowWorkUtil(conn, T9WorkFlowConst.FLOW_RUN, runId);
  }
  ...
  public int recoverWork(Connection conn  , String runIds,String module,int userId) throws Exception{
    ...
    String sql = "update "+ T9WorkFlowConst.FLOW_RUN +" set DEL_FLAG=0 where RUN_ID IN (" + runIds + ")";
    PreparedStatement ps = null;
    ResultSet rs = null; 
    try {
      ps = conn.prepareStatement(sql);
