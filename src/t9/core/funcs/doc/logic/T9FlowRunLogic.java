...
import t9.core.funcs.doc.util.T9FlowHookUtility;
import t9.core.funcs.doc.util.T9FlowRunUtility;
import t9.core.funcs.doc.util.T9PraseData2Form;
import t9.core.funcs.doc.util.T9PraseData2PrintForm;
...
public class T9FlowRunLogic {
   ...
   * @throws Exception 文号
   */
  public String getRunName(T9DocFlowType flowType , T9Person loginUser , Connection conn , boolean isUpdateAutoNum) throws Exception{
    ...
    T9DocFlowSort flowSort = fsLogic.getFlowSortById(flowType.getFlowSort() , conn);
    ...
    if(flowType.getAutoName() == null || "".equals(flowType.getAutoName())){
      ...
      //更新字流程文号计数器
        if (runName.indexOf("{N}") != -1) {
          String query ="update "+ T9WorkFlowConst.FLOW_TYPE +" set AUTO_NUM="+ autoNum +" where SEQ_ID=" + flowType.getSeqId();
   ...
   * @throws 
   */
  public String getNewMsg(T9DocFlowType flowType , T9Person loginUser ,List<T9DocFlowProcess> list , Connection conn) throws Exception{
    StringBuffer sb = new StringBuffer("{");
    //取得文号
    ...
    for(int i = 0 ;i < list.size() ;i ++){
      T9DocFlowProcess tmp = list.get(i);
      sb.append("{");
  ...
  public String getRecentlyFlowRun(T9Person user , Connection conn , String sortId) throws Exception{
    ...
    String query = "SELECT FLOW_ID,RUN_ID,RUN_NAME,BEGIN_TIME from "+ T9WorkFlowConst.FLOW_RUN +" where " 
    ...
    for(Map map : flowRunList){
      ...
      T9DocFlowProcess flowFirstProcess = null;
      T9DocFlowType ftTmp = (T9DocFlowType) orm.loadObjSingle(conn, T9DocFlowType.class, flowId);
      if (ftTmp == null) {
        continue;
      }
      boolean flag = false;
      ...
      if (flag && (T9Utility.isNullorEmpty(sortId) || T9WorkFlowUtility.findId(sortId, String.valueOf(sId)))) {
        String querySortName = "SELECT SORT_NAME from "+ T9WorkFlowConst.FLOW_SORT +" WHERE SEQ_ID=" + sId;
        String sortName = "";
        Statement stm2 = null;
        ResultSet rs2 = null;
        try {
   ...
   * @throws Exception 
   */
  public Map getPrintForm(T9Person user ,T9DocRun flowRun , T9DocFlowType flowType , boolean isWord , Connection conn, String imgPath) throws Exception{
    Map result = new HashMap();
    T9FlowFormUtility ffu = new T9FlowFormUtility();
    Map map = new HashMap();
    map.put("FORM_ID", flowType.getFormSeqId());
    T9ORM orm = new T9ORM();
    List<T9DocFlowFormItem> itemList = orm.loadListSingle(conn, T9DocFlowFormItem.class, map);
    String form = "";
    String script = "";
    String css = "";
    if (itemList.size() > 0) {
      ...
      List<T9DocFlowRunData> frdList = orm.loadListSingle(conn, T9DocFlowRunData.class, map);
      T9DocFlowFormType ff = (T9DocFlowFormType) orm.loadObjSingle(conn, T9DocFlowFormType.class, flowType.getFormSeqId());
   ...
   * @throws Exception 
   */
  public String getHandlerMsg(T9Person user , int runId, int prcsId, String flowPrcs , String ip , Connection conn  , String imgPath , String isWriteLog) throws Exception {
    // TODO Auto-generated method stub
    ...
    T9DocFlowProcess flowProcess = null;
    String attachPriv = "";
    ...
    T9ORM orm = new T9ORM();
    String dispAip = "";
    T9DocFlowRunPrcs runProcess = (T9DocFlowRunPrcs) orm.loadObjSingle(conn, T9DocFlowRunPrcs.class, runPrcsQuery);
    ...
    if ("2".equals(flowTypeStr) && "1".equals(runProcess.getOpFlag())) {
       query = "SELECT * from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID="+runId+" and PRCS_ID>'"+prcsId+"' and PRCS_FLAG='5'";
       Statement stm1 = null;
       ResultSet rs1 = null;
       try {
      ...
      query = "select 1 FROM "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE RUN_ID='"+runId+"' AND PRCS_ID='"+prcsId+"' AND FLOW_PRCS='"+flowPrcs+"' AND USER_ID<>'"+user.getSeqId()+"' AND PRCS_FLAG IN('1','2')";
      Statement stm1 = null;
      ResultSet rs1 = null;
      try {
        stm1 = conn.createStatement();
    ...
    if ("1".equals(allowBack)) {
      String query2 = "SELECT 1 FROM "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE " 
    ...
    if ("1".equals(docSendFlag)) {
      T9DocLogic logic2 = new T9DocLogic();
   ...
   * @throws Exception
   */
  public int createNewWork(T9Person user ,T9DocFlowType flowType , String runName , Connection conn ) throws Exception{
    //查出最的runId
    T9ORM orm = new T9ORM();
    String query = "select MAX(RUN_ID) max FROM "+ T9WorkFlowConst.FLOW_RUN;
    int runId = 0;
    Statement stm = null;
    ResultSet rs = null;
    try {
   ...
   // runName = runName.replaceAll("'", "&rsquo;");
    ...
    //新建 一个工作实例 
    query = "insert into "+ T9WorkFlowConst.FLOW_RUN +" (RUN_ID "
      + " ,RUN_NAME "
      + " ,FLOW_ID "
      + " ,BEGIN_USER "
    ...
    query = "insert into "+ T9WorkFlowConst.FLOW_RUN_PRCS +"(RUN_ID  "
      + " , PRCS_ID  "
      + " ,USER_ID  "
      + " ,PRCS_FLAG  "
      + " ,FLOW_PRCS  "
   ...
   // ffu.cacheForm(flowType.getFormSeqId() , conn);
    ...
    for(T9DocFlowFormItem tmp : list){
      ...
      T9DocFlowRunData runData = new T9DocFlowRunData();
  ...
  public String saveFormData(T9Person user , int flowId 
    ...
    T9DocFlowRunPrcs flowRunPrcs = (T9DocFlowRunPrcs) orm.loadObjSingle(conn, T9DocFlowRunPrcs.class, queryRunPrcs);
   ...
   * @throws Exception
   */
  public String getTurnData(T9Person loginUser, int runId, int prcsId, String flowPrcsStr , Connection conn , boolean isManage) throws Exception{
    T9ORM orm = new T9ORM();
    Map queryMap = new HashMap();
    queryMap.put("RUN_ID", runId);
    T9DocRun flowRun = (T9DocRun) orm.loadObjSingle(conn, T9DocRun.class, queryMap);
    ...
    List<T9DocFlowRunPrcs> flowRunPrcList = orm.loadListSingle(conn, T9DocFlowRunPrcs.class, queryPrcs);
    String parentStr = "";
    for (T9DocFlowRunPrcs p : flowRunPrcList) {
    ...
    T9DocFlowProcess flowProcess = flowProcessLogic.getFlowProcessById(flowRun.getFlowId(), flowPrcsStr , conn);
    T9DocFlowType flowType = (T9DocFlowType) orm.loadObjSingle(conn, T9DocFlowType.class , flowRun.getFlowId());
    //------------------------------------------- 转出条件检查 ----------------------------------
...
//          if (!T9WorkFlowUtility.findId(parentStr, ss)) {
...
//    }
      ...
      String query1 = "select "
        + " PRCS_ID  "
        + "  from  "
        + "  "+ T9WorkFlowConst.FLOW_PROCESS  
        + " where  "
        + " FLOW_SEQ_ID='"+flowRun.getFlowId()+"' "
        + "  and  (" + T9DBUtility.findInSet(flowPrcsStr, "PRCS_TO") + " ||  (PRCS_ID = "+ (iflowPrc - 1) +" AND PRCS_TO=''))";
        Statement stm3 = null;
        ResultSet rs3 = null;
        try {
          stm3 = conn.createStatement();
          ...
          while (rs3.next()){
            ...
            String  query2 = "select PRCS_FLAG from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE RUN_ID='"+runId+"' AND FLOW_PRCS='"+prcsId1+"' and OP_FLAG='1'";
            Statement stm4 = null;
            ResultSet rs4 = null;
            String prcsFlag1 = "1";
    ...
    sb.append ( "}" ) ;
    //取得下一步的信息
    List<T9DocFlowProcess> nextPrcs = flowProcessLogic.getNextProcess ( flowProcess , conn );
    sb.append( ",nextPrcs:[" );
    for ( T9DocFlowProcess tmp : nextPrcs ) {
      
      sb.append("{");
      sb.append ( "seqId:" + tmp.getSeqId() );
      sb.append ( ",prcsId:'" + tmp.getPrcsId() + "'" );
      String docSmsStyle = T9Utility.encodeSpecial(T9Utility.null2Empty(tmp.getDocSmsStyle()));
      ...
      if (tmp.getPrcsId() == 0 && flowRun.getParentRun() != 0) {
        ...
        String query3 = "select FLOW_PRCS FROM "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE RUN_ID='"+flowRun.getParentRun()+"' AND CHILD_RUN='"+runId+"'";
        Statement stm = null;
        ResultSet rs = null;
        int flowPrcs = 0;
        try {
        ...
        String query5 = "select PRCS_TO,AUTO_USER_OP,AUTO_USER FROM "+ T9WorkFlowConst.FLOW_PROCESS +" WHERE FLOW_SEQ_ID='"+ parentFlowId +"' AND PRCS_ID='"+ flowPrcs +"'";
        Statement stm6 = null;
        ResultSet rs6 = null;
        try {
   ...
   * @throws Exception 
   */
  public String getHandlerState(List<T9DocFlowRunPrcs> flowRunPrcList  , T9Person user , int flowPrcs ,Connection conn , boolean isManage) throws Exception{
    String userNameStr = "";
    //?
    String parentStr = "";
    String notAllFinish = "";
    ...
    for(T9DocFlowRunPrcs tmp : flowRunPrcList){
   ...
   * @throws Exception
   */
  public void turnNext(T9Person loginUser
      , int runId
      , int flowId
      , int prcsId
      , int flowPrcs
  ...
  public void backTo(T9Person loginUser
    ...
    if (runPrcs.size() > 0 ){
      ...
      sb.setRemindUrl(T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/list/inputform/index.jsp?skin="+skin+"&sortId="+ sortId +"&runId=" + runId + "&flowId=" + flowId + "&prcsId=" + (prcsId + 1)+ "&flowPrcs=" + prcsPre);
  ...
  public void turnNext(int runId
    ...
    for(String tmp : aPrcsChoose){
      ...
      //如果检测到曾经按先到先得转交或者无主办人会签，则此次转交也按先到先得规则 ？？？091016
        String queryTopFlag = "SELECT TOP_FLAG from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where  "
          + " RUN_ID='"+ runId +"'  "
        ...
        String queryIsHasHander = "SELECT 1 from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where  "
          + " RUN_ID='"+ runId +"'  "
          + " and PRCS_ID='"+ prcsIdNew +"'  "
          + " and FLOW_PRCS='"+ tmp +"'  "
          + " and OP_FLAG='1'  "
        ...
        for(String userTmp : aPrcsUser){
          if(!userTmp.equals("")){
            ...
            try {
              ...
              if(!rs3.next()){
                ...
                queryTmp = "insert into "+ T9WorkFlowConst.FLOW_RUN_PRCS +"(RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT,CREATE_TIME,OTHER_USER) "
                  +"values ("
                  + runId 
                  +","+prcsIdNew
                  +","+userTmp
                ...
                //和parent相关
                String queryFlowRunPrcs = "select PARENT from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where  "
                + "  RUN_ID='"+runId+"' and  "
                ...
                if (!T9Utility.isNullorEmpty(parent)) {
                  parent += "," + flowPrcs;
                } else {
                  parent = flowPrcs + "";
                }
          ...
          String query = "";
          if(tFlag == 1){
            query = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set DELIVER_TIME=?,PRCS_FLAG='4' WHERE RUN_ID="+runId+" and PRCS_ID="+prcsId+" and FLOW_PRCS='"+flowPrcs+"' and PRCS_FLAG in ('1','2')";
          }else{
            query = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set DELIVER_TIME=?,PRCS_FLAG='3' WHERE RUN_ID="+runId+" and PRCS_ID="+prcsId+" and FLOW_PRCS='"+flowPrcs+"' and PRCS_FLAG in ('1','2')";
          }
        ...
        String queryTopFlag = "SELECT FLOW_TYPE from "+ T9WorkFlowConst.FLOW_TYPE +" where  "
          + " SEQ_ID ='"+ childFlow +"'";
        Statement stm = null;
        ResultSet rs = null;
        String flowType  = "1";
         ...
         */
        String insertFlowRunPrcs ="insert into "+ T9WorkFlowConst.FLOW_RUN_PRCS +" (RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT,CHILD_RUN,CREATE_TIME) " 
  ...
  public String turnEnd(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs, String prcsUser, String prcsOpUser, String topFlag , String ip  , Connection conn , String prcsBack) throws Exception{
    Timestamp time =  new  Timestamp(new Date().getTime());
  ...
  //取出流程实例的几个字段：$PARENT_RUN、$FOCUS_USER、$RUN_NAME
    ...
    try {
      ...
      rs3 = stm3.executeQuery(query);
      if(!rs3.next()){
        String updateFlowRun = "update "+ T9WorkFlowConst.FLOW_RUN +" set  "
          + " END_TIME=? "
          + " WHERE RUN_ID='"+runId+"'";
    ...
    if(parentRun !=0){
      ...
      if (!"".equals(prcsBack) && !"0".equals(prcsBack)) {
        ...
        query = "insert into "+ T9WorkFlowConst.FLOW_RUN_PRCS +"(RUN_ID,PRCS_ID,USER_ID,PRCS_FLAG,FLOW_PRCS,OP_FLAG,TOP_FLAG,PARENT) " 
...
//            , String content , String contextPath , String prcsUser , int userId , String sortId, String skin);
       ...
       */
        ...
        query  = "select 1 from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where RUN_ID='"
        ...
        try {
          ...
          if (!rs7.next()) {
            query = "update "+ T9WorkFlowConst.FLOW_RUN +" set END_TIME=? where RUN_ID='"+ parentRun +"'";
   ...
   * @throws Exception
   */
  public String handlerFinish(T9Person loginUser, int runId, int flowId, int prcsId, int flowPrcs , String ip ,String sortId, String skin, Connection conn , String contextPath) throws Exception{
    StringBuffer sb = new StringBuffer();
    String result = "";
    ...
    T9DocFlowRunPrcs flowRunPrcs = (T9DocFlowRunPrcs) orm.loadObjSingle(conn, T9DocFlowRunPrcs.class, queryPrcs);
    //无主办人会签，先检查是否最后一个经办人
    ...
    Timestamp time =  new  Timestamp(new Date().getTime());
    if(flowPrcs == 0){//自由流程
      updateQuery = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set "
        +"PRCS_FLAG='4'"
        +",DELIVER_TIME=?"
        +" WHERE "
        +" RUN_ID='" + runId + "' "
    ...
    if (flag) {
      ...
      if (userId != 0 ) {
        ...
        sb1.setRemindUrl(T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/list/inputform/index.jsp?skin="+ skin +"&sortId="+ sortId +"&runId=" + runId + "&flowId=" + flowId1 + "&prcsId=" + prcsId + "&flowPrcs=" + flowPrcs);
...
//       $CONTENT=$LOGIN_USER_NAME."强制结束流程";
    //T9FlowRunLogLogic logLogic = new T9FlowRunLogLogic(this.conn);
  ...
  public String getPreRunPrcs(int runId , int prcsId , int flowId , Connection conn) throws Exception {
    ...
    try {
      ...
      while(rs1.next()){
        ...
        if (!T9WorkFlowUtility.findId(prcsIdStr, String.valueOf(flowPrcs))) {
          T9DocFlowRunPrcs rp = new T9DocFlowRunPrcs();
    ...
    for (T9DocFlowRunPrcs runPrcs : list) {
 ...
 //短信提醒所有经办人 短信(remindFlag&4)>0 邮件 (remindFlag&1)>0
      ...
      try {
        ...
        if(rs.next()){
          ...
          if(toId != null){
            ...
            sb.setFromId(userId);
            sb.setToId(tmp);
            sb.setRemindUrl(T9WorkFlowConst.MODULE_CONTEXT_PATH + "/flowrun/list/inputform/index.jsp?skin="+skin+"&sortId="+sortId +"&runId=" + runId + "&flowId=" + flowId + "&prcsId=" + prcsId + "&flowPrcs=" + flowPrcs);
            T9SmsUtil.smsBack(conn, sb);
          }
        }
      } catch (Exception e) {
  ...
  public void remindNextEmail(Connection conn  , String content , String contextPath , String prcsUser , int userId) throws Exception {
    //T9EmailUtil.emailNotifier(conn, userId, prcsUser, "工作流转交下一步提醒", content, new Date(), important)
  }
  /**
   * 工作流是否删除
  ...
  public boolean hasDelete(int runId , Connection conn) throws Exception{
    String query  = "SELECT DEL_FLAG from "+ T9WorkFlowConst.FLOW_RUN +" WHERE RUN_ID=" + runId;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
   ...
   * @throws Exception
   */
  public String analysisAutoFlag (T9DocRun flowRun , T9DocFlowType ft , T9DocFlowFormType form , Connection conn , String imgPath) throws Exception {
    String modelShort = "";
    if (form != null) {
   ...
   * @throws Exception
   */
  public String analysisAutoFlag (T9DocRun flowRun , T9DocFlowType ft , T9DocFlowFormType form , Connection conn , String imgPath , String modelShort) throws Exception {
    Map map = new HashMap();
  ...
  public int getFreeOther(int flowId , Connection conn) throws Exception { 
    int freeOther = 2;
    String query = "select FREE_OTHER from "+ T9WorkFlowConst.FLOW_TYPE +" where SEQ_ID=" + flowId;
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
  ...
  public boolean canHandlerWrok(int runId , int prcsId , int flowPrcs , int userId , Connection conn) throws Exception {
    boolean flag = false;
    String query  = "SELECT 1 from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE RUN_ID="+runId+" AND PRCS_ID="+ prcsId +" AND FLOW_PRCS="+ flowPrcs +" and USER_ID="+ userId +" and PRCS_FLAG in(1,2)";
    Statement stm = null;
    ResultSet rs = null;
    try {
      stm = conn.createStatement();
