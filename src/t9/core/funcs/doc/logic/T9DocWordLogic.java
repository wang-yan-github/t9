...
import t9.core.funcs.doc.data.T9DocWord;
...
public class T9DocWordLogic {
 public void addDocWordLogic(Connection conn,String dwName,String indexStyle,String deptPriv,String rolePriv,String userPriv , int wordNo)throws Exception{
   T9ORM orm=new T9ORM();
 ...
 public String getDocWordListLogic(Connection dbConn, Map request, T9Person person) throws Exception {
   try {
     String sql = " select c1.SEQ_ID , c1.WORD_NO ,c1.DW_NAME, c1.index_style, c1.depart_priv, c1.role_priv, c1.user_priv"
                + " from DOC_WORD c1 "
                + " ORDER BY c1.WORD_NO desc ";
 ...
 public boolean isHaveDocWord(Connection conn, String dwName)throws Exception{
   ...
   ResultSet rs=null;
   try{
       String sql=" select 1 from DOC_WORD where DW_NAME=? ";
 ...
 public String getDocWordLogic(Connection conn, String seqId)throws Exception{
   ...
   try{
     ...
     data="{" ;
     data+="seqId:'"+dw.getSeqId()+"',dwName:\""+T9Utility.encodeSpecial(dw.getDwName())+"\",indexStyle:\""+T9Utility.encodeSpecial(dw.getIndexStyle())+"\"";
 ...
 public void updateDocWordLogic(Connection conn,String dwName,String indexStyle,String deptPriv,String rolePriv,String userPriv,String seqId, int wordNo)throws Exception{
   T9ORM orm=new T9ORM();
   T9DocWord dw=new T9DocWord();
   dw=(T9DocWord)orm.loadObjSingle(conn, T9DocWord.class, Integer.parseInt(seqId));
   dw.setDwName(dwName);
