...
public class T9FlowOverTimeLogic {
  public String getSql(int flowId, String flowStatus, String starttime, String endtime ,Map request, T9Person user, String sortId) throws Exception {
    String query="select A.RUN_ID,B.RUN_NAME,C.PRCS_NAME,A.PRCS_FLAG,E.USER_NAME,C.TIME_OUT,C.TIME_EXCEPT,A.OP_FLAG,A.PRCS_TIME,A.PRCS_FLAG,"
      ...
      +" from  "+ T9WorkFlowConst.FLOW_RUN_PRCS +" A "
      +" inner join "+ T9WorkFlowConst.FLOW_RUN +" B on A.RUN_ID=B.RUN_ID"
      +" inner join "+ T9WorkFlowConst.FLOW_PROCESS +" C on B.FLOW_ID=C.FLOW_SEQ_ID and C.PRCS_ID = A.FLOW_PRCS"
      +" inner join "+ T9WorkFlowConst.FLOW_TYPE +" D on B.FLOW_ID=D.seq_id"
  ...
  public String WorkFlow(String flowquery,Connection conn, T9Person user)throws Exception {
   StringBuffer sb = new StringBuffer();
    ...
    try {
      ...
      String flowquerys = "select d.flow_name from "+ T9WorkFlowConst.FLOW_TYPE +" d, flow_run b where d.seq_id=b.flow_id and  flow_id = "+flowquery;
 ...
 public String viewDetail(Connection conn , Map request  , int flowId , int userId , String prcsDate1Query , String prcsDate2Query) throws Exception {
   ...
   String sql = "select "
           ...
           + "  from  "
           + "   "+ T9WorkFlowConst.FLOW_RUN_PRCS +" A  "
           + "    inner join "+ T9WorkFlowConst.FLOW_RUN +" B on A.RUN_ID=B.RUN_ID "
           + "    inner join "+ T9WorkFlowConst.FLOW_PROCESS +" C on B.FLOW_ID=C.FLOW_SEQ_ID and A.FLOW_PRCS=C.PRCS_ID "
           + "    inner join "+ T9WorkFlowConst.FLOW_TYPE +" D on B.FLOW_ID=D.SEQ_ID "
 ...
 public int getCount (Connection conn , String flowId , int userId , boolean isTimeOut) throws Exception {
   int result = 0 ;
   String query = "select count(distinct(flow_run.run_ID)) as count  from   "+ T9WorkFlowConst.FLOW_RUN +" flow_run, "+ T9WorkFlowConst.FLOW_RUN_PRCS +" flow_run_prcs where flow_run.RUN_ID = flow_run_prcs.RUN_ID "  
 ...
 //属于那个部门   获得各个部门的名称
  ...
  // A.RUN_ID=B.RUN_ID and  与OA 相差一个条件   完成后在加上这个条件， 现在加上没有数据，所以先去了这个条件
   String sqlquery =  "select D.flow_name, A.prcs_time, A.del_flag,C.prcs_id ,B.flow_id, D.seq_id, E.SEQ_ID as USER_ID,E.user_name,"
      ...
      + " from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" A , "+ T9WorkFlowConst.FLOW_PROCESS +" C ,"+ T9WorkFlowConst.FLOW_RUN +" B,"+ T9WorkFlowConst.FLOW_TYPE +" D,PERSON E"
