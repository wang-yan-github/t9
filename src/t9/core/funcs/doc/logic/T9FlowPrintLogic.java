...
import t9.core.funcs.doc.data.T9DocFlowPrintTpl;
...
public class T9FlowPrintLogic {
  ...
  public String savePrintTpl(Connection conn,String tName,String content,String tType,String flowId) throws Exception{
     String seq_id="";
    try{    
      T9DocFlowPrintTpl tpl=new  T9DocFlowPrintTpl();
      ...
      int attendEvectionId = ut.getMax(conn, " select max(SEQ_ID) FROM "+ T9WorkFlowConst.FLOW_PRINT_TPL );
  ...
  public void updatePrintTpl(Connection conn,String seqId,String prcsStr,String tName,String content,String tType,String flowId) throws Exception{
    ...
    try{    
      ...
      T9DocFlowPrintTpl tpl=new  T9DocFlowPrintTpl();
      tpl=(T9DocFlowPrintTpl)orm.loadObjSingle(conn,T9DocFlowPrintTpl.class , Integer.parseInt(seqId));
    ...
    public void delTplLogic(Connection conn,String seq_id) throws Exception{
       ...
       try{
         T9ORM orm =new T9ORM();
        orm.deleteSingle(conn, T9DocFlowPrintTpl.class, Integer.parseInt(seq_id));
       }catch(Exception ex){
         throw ex;
       }finally{
    ...
    public String loadAip(Connection conn,String seq_id) throws Exception{
     ...
     try{
       ...
       tpl=(T9DocFlowPrintTpl)orm.loadObjSingle(conn, T9DocFlowPrintTpl.class, Integer.parseInt(seq_id));
     }catch(Exception ex){
       throw ex;
     }finally{
     //  T9DBUtility.close(stmt, rs, null);
...
public void updateAip(Connection conn,String seqId , String runId , String attachmentId) throws Exception{
  Statement stmt=null;
  ResultSet rs=null;
  String sql = "select AIP_FILES from "+ T9WorkFlowConst.FLOW_RUN +" WHERE RUN_ID='"+runId+"'";
  String aipFiles = "";
  try{
  ...
  String sql2 = "update "+ T9WorkFlowConst.FLOW_RUN +" set AIP_FILES = '"+ aipFiles +"' WHERE RUN_ID='"+runId+"'";
  try{
    ...
    public String getTplList(Connection conn,String flow_id) throws Exception{
         ...
         String data="";
       try{
          String sql=" select * from "+ T9WorkFlowConst.FLOW_PRINT_TPL +" where flow_id='"+flow_id+"'";
          stmt=conn.createStatement();
          rs=stmt.executeQuery(sql);
          while(rs.next()){
    ...
    try{
      for(int i=0;i<prcs.length;i++){
         ...
         String sql=" select * from "+ T9WorkFlowConst.FLOW_PROCESS +" where flow_seq_id='"+flow_id+"' and prcs_id='"+flowPcs+"' order by prcs_id asc";
         stmt=conn.createStatement();
         rs=stmt.executeQuery(sql);
         if(rs.next()){         
   ...
   return data;
    ...
    public String getDisSelectByFlowIdLogic(Connection conn,String flow_id) throws Exception{
      ...
      String data="";
    try{
       String sql=" select * from "+ T9WorkFlowConst.FLOW_PROCESS +" where flow_seq_id='"+flow_id+"' order by prcs_id asc";
       stmt=conn.createStatement();
       rs=stmt.executeQuery(sql);
       while(rs.next()){
   ...
   return data;
    ...
    public String getSelectByFlowIdLogic(Connection conn,String seq_id) throws Exception{
      ...
      String data="";
    try{
       String sql=" select * from "+ T9WorkFlowConst.FLOW_PRINT_TPL +" where seq_id='"+seq_id+"' ";
       stmt=conn.createStatement();
       rs=stmt.executeQuery(sql);
       if(rs.next()){
   ...
   return data;
    ...
    public String getTempOptionLogic(Connection conn,T9Person person,String flowId,String runId) throws Exception{
      ...
      String data="";
    try{
      String sql = "select SEQ_ID,T_NAME,FLOW_PRCS FROM "+ T9WorkFlowConst.FLOW_PRINT_TPL +" WHERE FLOW_ID='"+flowId+"' and T_TYPE = '1'";
     stmt=conn.createStatement();
     rs=stmt.executeQuery(sql);
      while(rs.next())
      {
         ...
         sql = "select * from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE RUN_ID='"+runId+"' and USER_ID='"+person.getSeqId()+"' and FLOW_PRCS IN ("+flowPrcs+")";
          stmt1=conn.createStatement();
   ...
   return data;
    ...
    public String getFlowItemData(Connection conn,String flowId,String runId) throws Exception{
      ...
      String data="";
    try{
     String sql = "select f.title,f.name  FROM "+ T9WorkFlowConst.FLOW_TYPE +" t,"+ T9WorkFlowConst.FLOW_FORM_ITEM +" f WHERE t.form_seq_id=f.form_id and t.seq_id='"+flowId+"'";
     stmt=conn.createStatement();
     rs=stmt.executeQuery(sql);
      while(rs.next())
      {
   ...
   return data;
    ...
    public String getDataByFidAndItem(Connection conn,String rId,String Item) throws Exception{
      ...
      try{
        String sql="select * from "+ T9WorkFlowConst.FLOW_RUN_DATA +" where run_id="+rId+" and item_id="+Integer.parseInt(Item.substring(Item.indexOf("_")+1,Item.length()));
        stmt=conn.createStatement();
        rs=stmt.executeQuery(sql);
        if(rs.next()){
