...
import t9.core.funcs.doc.data.T9DocumentsType;
...
public class T9DocumentsTypeLogic {
  ...
  public void addDocumentsTypeInfo(Connection dbConn, Map map, T9Person person) throws Exception{
    ...
    String typeNo = (String)map.get("typeNo");
    if (!T9Utility.isInteger(typeNo)) {
      typeNo = "0";
    }
    documentsType.setTypeNo(Integer.parseInt(typeNo));
    T9ORM orm = new T9ORM();
  ...
  public String getDocumentsTypeList(Connection dbConn, Map request, T9Person person) throws Exception {
    try {
      String sql = " SELECT SEQ_ID , TYPE_NO, documents_name, flow_type, documents_font, documents_word_model FROM documents_type ORDER BY TYPE_NO";
  ...
  public String queryDocumentsTypeList(Connection dbConn, Map request, Map map, T9Person person) throws Exception {
    ...
    try {
      ...
      if (!T9Utility.isNullorEmpty(flowType)) {
        conditionStr = " and documents_type.flow_type ='" + flowType + "'";
      }
      ...
      sql = " SELECT documents_type.SEQ_ID , TYPE_NO, documents_name, flow_name, documents_font, documents_word_model FROM documents_type LEFT OUTER JOIN  "+ T9WorkFlowConst.FLOW_TYPE +" flow_type " 
          + " ON documents_type.flow_type = flow_type.seq_id WHERE 1=1 " + conditionStr + " ORDER BY documents_type.TYPE_NO desc";
      T9PageQueryParam queryParam = (T9PageQueryParam) T9FOM.build(request);
      T9PageDataList pageDataList = T9PageLoader.loadPageList(dbConn, queryParam, sql);
      return pageDataList.toJson();
    } catch (Exception e) {
