...
public class T9FreeFlowLogic {
   ...
   * @throws Exception
   */
  public String getTurnData(T9Person loginUser,int flowId, int runId, int prcsId , Connection conn ,boolean isManage) throws Exception{
    StringBuffer sb = new StringBuffer();
    
    T9ORM orm = new T9ORM();
    String formItem = "";
    T9FlowTypeLogic flowTypelogic = new T9FlowTypeLogic();
    T9DocFlowType ft = flowTypelogic.getFlowTypeById(flowId , conn);
    if (prcsId == 1) {
      ...
      map.put("OP_FLAG", 1);
      T9DocFlowRunPrcs runPrcs = (T9DocFlowRunPrcs) orm.loadObjSingle(conn, T9DocFlowRunPrcs.class, map);
      if (runPrcs != null) {
    ...
    for (int i = 1 ;i <= prcsId ;i ++ ) {
      String query3 = "SELECT * from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where  " 
        + " RUN_ID="+ runId +"  " 
        + " and PRCS_ID=" + i 
    ...
    String preSet = "";
    String queryPre = "SELECT * from "+ T9WorkFlowConst.FLOW_RUN_PRCS +" where " 
        + "  RUN_ID= "+ runId +"  " 
  ...
  public String  turnNext(T9Person user ,int flowId , int runId , int prcsId , Connection conn , boolean preSet , List<Map> preList , String itemOld) throws Exception {
    T9ORM orm = new T9ORM();
    ...
    if (!preSet) {
      for (int i = 0 ; i < preList.size() ; i++) {
        ...
        for (String tmpStr : str) {
          ...
          String insertRunPrcs = "insert into "+ T9WorkFlowConst.FLOW_RUN_PRCS +"("
            + " RUN_ID"
            + " ,PRCS_ID"
            + " ,USER_ID"
            + " ,PRCS_FLAG"
      ...
      String query =  "select USER_ID FROM "+ T9WorkFlowConst.FLOW_RUN_PRCS +" WHERE " 
        + "  PRCS_FLAG='5'  " 
        + " AND  RUN_ID='"+ runId +"'  " 
      ...
      String updateFlowRun =  "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set " 
        + "  PRCS_FLAG='1'  " 
        + " WHERE " 
        + "  RUN_ID='"+ runId +"'  " 
        + " and PRCS_ID='"+ prcsIdNext +"'";
  ...
  //-- 设置当前步骤所有人为已转交 --
   String query = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set  " 
     + " PRCS_FLAG='3' " 
     + " WHERE RUN_ID="+ runId
     + " and PRCS_ID=" + prcsId;
   Statement stm = null;
   ...
   Timestamp time =  new  Timestamp(new Date().getTime());
    query = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +" set  " 
     + " DELIVER_TIME=?" 
     + " WHERE  " 
     + " RUN_ID=" + runId 
     + " and PRCS_ID=" + prcsId 
  ...
  public String stop(int runId , int flowId ,int prcsId , T9Person user ,Connection conn) throws Exception {
    ...
    try {
      ...
      if(rs.next()){
        if (isManage) {
          //是管理者,删除后面预设步骤
          T9WorkFlowUtility.deleteTable("FLOW_RUN_PRCS", " RUN_ID="+ runId +" and PRCS_ID >= " + prcsIdNext, conn);
        } else {
          //不是管理者
    ...
    if(!isManage) {
      ...
      map.put("PRCS_ID", prcsId);
      T9DocFlowRunPrcs flowRun = (T9DocFlowRunPrcs) orm.loadObjSingle(conn, T9DocFlowRunPrcs.class, map);
      flowRun.setDeliverTime(new Date());
      orm.updateSingle(conn, flowRun);
    } else {//监控人结束补全步骤开始和结束时间为当前时间
      String sql = "update "+ T9WorkFlowConst.FLOW_RUN_PRCS +"  set " 
        + " DELIVER_TIME=? " 
        + " WHERE  " 
        + " RUN_ID=" + runId 
    ...
    //--- 写入结束时间 ---
    String writeEndTime = "update  "+ T9WorkFlowConst.FLOW_RUN +" set   " 
        + " END_TIME=?   " 
        + " where   " 
        + " RUN_ID="+ runId;
    Timestamp time =  new  Timestamp(new Date().getTime());
