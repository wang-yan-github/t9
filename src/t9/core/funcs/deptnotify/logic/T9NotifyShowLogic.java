...
public class T9NotifyShowLogic {
    ...
    // 未读公告
    public String getPrivNoReadList(Connection conn, T9Person loginUser,
            int showLen, String type, String ascDesc, String field,
        ...
        try {// 是发布状态，而且不是待发布，且不是已终止状态，而且属于发布范围之内，而且未读过
            ...
            if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
                queryNotifySql = "SELECT SEQ_ID,FROM_ID,TO_ID,SUBJECT,[TOP],PRIV_ID,USER_ID,READERS,"
                        + "TYPE_ID,SEND_TIME,FORMAT from dept_notify  where  BEGIN_DATE<="
                        + T9DBUtility.currDateTime()
                        + " and (END_DATE>="
                        + T9DBUtility.currDateTime()
                        + " or END_DATE is null) and PUBLISH='1' and ("
                        + T9DBUtility.findInSet(Integer.toString(loginDeptId),
                                "TO_ID")
                        + " or "
                        + T9DBUtility.findInSet(userPriv, "PRIV_ID")
                        + " or "
                        + T9DBUtility.findInSet(Integer.toString(seqUserId),
                                "USER_ID")
                        + " or "
                        + T9DBUtility.findInSet("0", "TO_ID")
                        + ") "
                        + " and ("
                        + T9DBUtility.findNoInSet(Integer.toString(seqUserId),
                                "READERS") + " or READERS is NULL) ";
            } else {
                queryNotifySql = "SELECT SEQ_ID,FROM_ID,TO_ID,SUBJECT,TOP,PRIV_ID,USER_ID,READERS,"
                        + "TYPE_ID,SEND_TIME,FORMAT from dept_notify  where  BEGIN_DATE<="
                        + T9DBUtility.currDateTime()
            ...
            if ("".equals(type)) {// 选择“无类型“
                ...
                queryNotifySql = queryNotifySql
                        + " and (TYPE_ID='' or TYPE_ID=' ' or TYPE_ID is null)";
            ...
            if ("".equals(field)) {
                ...
                if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
                    queryNotifySql = queryNotifySql + " order by [TOP] desc,"
                            + field;
                } else {
                    queryNotifySql = queryNotifySql + " order by TOP desc,"
                            + field;
            ...
            for (int i = 0; i < showLen && !rs.isAfterLast() && recordCount > 0; i++) {
                ...
                String fromName = "";
                String deptName = "";
                String toId = T9WorkFlowUtility
                        .clob2String(rs.getClob("TO_ID"));
                String subject = rs.getString("SUBJECT");
                String top = rs.getString("TOP");
                String privId = T9WorkFlowUtility.clob2String(rs
                        .getClob("PRIV_ID"));
                String userId = T9WorkFlowUtility.clob2String(rs
                        .getClob("USER_ID"));
                ...
                String deptId = "";
                try {
                    String queryuserNameStr = "select USER_NAME,DEPT_ID from PERSON where SEQ_ID='"
                            + fromId + "'";
                ...
                String querydeptNameStr = "select DEPT_NAME from DEPARTMENT where SEQ_ID='"
                        + deptId + "'";
                stmtt = conn.createStatement();
    ...
    // 未读公告
    public String getnotifyNoReadList(Connection conn, T9Person loginUser,
            int showLen, String type, String ascDesc, String field,
            int pageIndex,String portName) throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        boolean temp = false;// 判断是否阅读过
        ...
        String deptSeq = getDeptId(conn, portName);
        sb.append("{");
        sb.append("listData:[");
        try {// 是发布状态，而且不是待发布，且不是已终止状态，而且属于发布范围之内，而且未读过
            String dbms = T9SysProps.getString(T9SysPropKeys.DBCONN_DBMS);
    ...
    private String getDeptId(Connection conn, String portName) throws Exception {
        String sql = "SELECT SEQ_ID FROM `department` WHERE DEPT_NAME='" + portName + "'";
        Map map = T9QuickQuery.quickQuery(conn, sql);
        String deptSeq = (String) map.get("SEQ_ID");
        return deptSeq;
    }
    ...
    // 我的办公桌的全部公告
    public String getnotifyShowList(Connection conn, T9Person loginUser,
            int showLen, String type, String ascDesc, String field,
            int pageIndex, String sendTimetemp,String portName) throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        boolean temp = false;
        int count = 0;
        ...
        try {
            ...
            if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
                queryNotifySql = "SELECT SEQ_ID,FROM_ID,TO_ID,SUBJECT,[TOP],TOP_DAYS,PRIV_ID,USER_ID,READERS,"
                        + "TYPE_ID,SEND_TIME,BEGIN_DATE,FORMAT from dept_notify  where  BEGIN_DATE<="
                        + T9DBUtility.currDateTime()
                        + " and (END_DATE>="
                        + T9DBUtility.currDateTime()
                        + " or END_DATE is null) and PUBLISH='1' and ("
            ...
            if (!"".equals(sendTimetemp) && sendTimetemp != null
                    && !"null".equals(sendTimetemp)) {
                queryNotifySql = queryNotifySql
                        + " and "
                        + T9DBUtility.getDayFilter("SEND_TIME",
                                T9Utility.parseDate(sendTimetemp));// to_char(SEND_TIME,'yyyy-MM-dd')='"+
                                                                   // sendTimetemp
                                                                   // + "'";
            }
            if ("".equals(field)) {
            ...
            for (int i = 0; i < showLen && !rs.isAfterLast() && recordCount > 0; i++) {
                ...
                sb.append(",sendTime:\"" + sendTime + "\"");
                sb.append(",iread:\""
                        + haveRead(conn, loginUser.getSeqId(), seqId) + "\"");
                sb.append("},");
                rs.next();
            }
    ...
    public String showObject(Connection dbConn, T9Person person, int seqId,
            String isManage, T9DeptNotify notify) throws Exception {
        Statement st = null;
        ResultSet rs = null;
        ...
        try {
            ...
            if (!contains) {// 如果没有读过
                readers = readers
                        ...
                        + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(
                                new Date()).toString() + ",";
                updateSql = "update NOTIFY set READERS='" + readers
                        + "' where SEQ_ID='" + seqId + "'";
            ...
            // "'";
            // }
            String queryUserSql = "SELECT USER_NAME,DEPT_ID from PERSON where SEQ_ID ="
                    + notify.getFromId();
            stmtt = dbConn.createStatement();
            ...
            if (notify.getAuditer() != null) {
                if (!"".equals(notify.getAuditer().trim())
                        && !"null".equals(notify.getAuditer().trim())) {
                    String queryAuditerSql = "SELECT USER_NAME,DEPT_ID from PERSON where SEQ_ID ="
                            + notify.getAuditer();
                    stmtt = dbConn.createStatement();
            ...
            String queryDeptSql = "SELECT DEPT_NAME FROM DEPARTMENT Where SEQ_ID="
                    + deptId;
            stmtt = dbConn.createStatement();
            ...
            if (!"".equals(auditerDeptId) && auditerDeptId != 0) {
                String queryAuditerDeptSql = "SELECT DEPT_NAME FROM DEPARTMENT Where SEQ_ID="
                        + auditerDeptId;
                stmtt = dbConn.createStatement();
            ...
            sb.append(",fromName:\"" + T9Utility.encodeSpecial(fromName) + "\"");
            sb.append(",deptName:\"" + T9Utility.encodeSpecial(deptName) + "\"");
            sb.append(",notifyAuditingSingle:\""
                    + T9Utility.encodeSpecial(notifyAuditingSingle) + "\"");
            sb.append(",auditer:\""
                    + T9Utility.encodeSpecial(notify.getAuditer()) + "\"");
            sb.append(",auditerName:\"" + T9Utility.encodeSpecial(auditerName)
                    + "\"");
            sb.append(",auditerDeptName:\""
                    + T9Utility.encodeSpecial(auditerDeptName) + "\"");
            sb.append(",download:\""
                    + T9Utility.encodeSpecial(notify.getDownload()) + "\"");
            sb.append(",print:\"" + T9Utility.encodeSpecial(notify.getPrint())
                    + "\"");
            sb.append(",subject:\""
                    + T9Utility.encodeSpecial(notify.getSubject()) + "\"");
            sb.append(",content:\""
                    + T9Utility.encodeSpecial(notify.getContent()) + "\"");
            sb.append(",format:\"" + notify.getFormat() + "\"");
            sb.append(",isHasNotifyFunc:\""
                    + T9Utility.encodeSpecial(isHasNotifyFunc) + "\"");
            sb.append(",beginDate:\"" + notify.getSendTime() + "\"");
            sb.append(",ym:\"" + hard + "\"");
            if (!"".equals(T9Utility.null2Empty(notify.getAttachmentId())
                    .trim())
                    && T9Utility.null2Empty(notify.getAttachmentId()).trim() != null
                    && !",".equals(T9Utility.null2Empty(
                            notify.getAttachmentId()).trim())) {
                sb.append(",attachFile:\""
                        + notify.getAttachmentId().replaceAll("[\\w]+_", "")
                        + "_"
                        + notify.getAttachmentName().substring(0,
                                notify.getAttachmentName().length() - 1) + "\"");
            }
            sb.append(",attachmentId:\"" + notify.getAttachmentId() + "\"");
            sb.append(",attachmentName:\""
                    + T9Utility.encodeSpecial(notify.getAttachmentName())
                    + "\"");
            sb.append(",subjectFont:\""
                    + T9Utility.encodeSpecial(notify.getSubjectFont()) + "\"");
            sb.append("}");
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
    ...
    // 判断是不是可以转发的权限
    public boolean isHasFuns(Connection dbConn, String userPriv)
            throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        ...
        String sql = "select FUNC_ID_STR from USER_PRIV where SEQ_ID="
                + userPriv;
        // T9Out.println(sql+"**********");
    ...
    // 我的办公桌里面，查询公告
    public String queryNotify(Connection dbConn, T9DeptNotify notify,
            T9Person loginUser, String beginDatetemp, String endDatetemp,
            String ascDesc, String field) throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        int temp = 0;
        int pageCount = 0;// 页码数
        ...
        if (!"".equals(beginDatetemp))
            querynewsParam = querynewsParam
                    ...
                    + T9DBUtility.getDateFilter("n.SEND_TIME", beginDatetemp,
                            ">=");// to_char(n.SEND_TIME,'yyyy-mm-dd')>='" +
                                  // beginDatetemp + "'";
            ...
            querynewsParam = querynewsParam
                    ...
                    + T9DBUtility.getDateFilter("n.SEND_TIME", T9Utility
                            .getDateTimeStr(T9Utility.getDayAfter(endDatetemp,
                                    1)), "<");// to_char(n.SEND_TIME,'yyyy-mm-dd')<='"
                                              // + endDatetemp + "'";
        String content = notify.getContent();
        if (!"".equals(content))
            querynewsParam = querynewsParam + " and n.CONTENT like '%"
                    + T9DBUtility.escapeLike(content) + "%' "
                    + T9DBUtility.escapeLike();
        String queryNotifySql = "SELECT p.USER_NAME,d.DEPT_NAME,n.* from dept_notify n,PERSON p,DEPARTMENT d where PUBLISH='1' and n.FROM_ID=p.SEQ_ID and p.DEPT_ID=d.SEQ_ID"
                + " and BEGIN_DATE<="
                + T9DBUtility.currDateTime()
                + " and (END_DATE>="
                + T9DBUtility.currDateTime()
                ...
                + T9DBUtility.findInSet(Integer.toString(loginDeptId),
                        "n.TO_ID")
                + " or "
                + T9DBUtility.findInSet(userPriv, "n.PRIV_ID")
                + " or "
                + T9DBUtility.findInSet(Integer.toString(seqUserId),
                        "n.USER_ID")
                + " or "
    ...
    // 我的办公桌里面，查询公告
    public String queryNotify2(Connection dbConn, T9DeptNotify notify,
            T9Person loginUser, String beginDatetemp, String endDatetemp,
            String ascDesc, String field, String portName) throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        ...
        String deptSeq = getDeptId(dbConn,portName);
        sb.append("{");
        sb.append("listData:[");
        String querynewsParam = "";
        String formattemp = notify.getFormat();
        ...
        if ("".equals(field)) {
            if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
                querySql = querySql
                        + " order by [TOP] desc, BEGIN_DATE desc,SEND_TIME desc";
            } else {
                querySql = querySql
                        + " order by TOP desc, BEGIN_DATE desc,SEND_TIME desc";
            }
    ...
    // --
    public String showReader(Connection conn, String seqId, String displayAll)
            throws Exception {
        T9ORM orm = new T9ORM();
        ResultSet rs = null;
        Statement st = null;
        ...
        int unReaderCount = 0;
        try {
            T9DeptNotify notify = (T9DeptNotify) orm.loadObjSingle(conn,
                    T9DeptNotify.class, seqID);
            String queryUserSql = "SELECT p.USER_NAME,p.DEPT_ID,d.DEPT_NAME from PERSON p,DEPARTMENT d where p.DEPT_ID = d.SEQ_ID and p.SEQ_ID='"
                    + notify.getFromId() + "'";
            ...
            sb.append(",providerName:\""
                    + T9Utility.encodeSpecial(providerName) + "\"");
            ...
            if (!"0".equals(toId) || !"ALL_DEPT".equals(toId)) {
                ...
                if (privId != null && !"".equals(privId.trim())) {
                    ...
                    try {
                        String queryPrivSql = "SELECT DEPT_ID from PERSON where USER_PRIV in ("
                                + privId + ")";
            ...
            if (!"0".equals(toId) || !"ALL_DEPT".equals(toId)) {
                ...
                if (!T9Utility.isNullorEmpty(userId)) {
                    ...
                    try {
                        queryUserSql = "SELECT DEPT_ID from PERSON where SEQ_ID in ("
                                + userId + ")";
    ...
    public String getDeptTreeJson(T9DeptNotify notify, int deptId, Connection conn,
    ...
    public void getDeptTree(T9DeptNotify notify, int deptId, StringBuffer sb,
            int level, Connection conn, String toId) throws Exception {
        // 首选分级，然后记录级数，是否为最后一个。。。
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            sb.append("{");
            sb.append("deptName:\"" + flag
                    + T9Utility.encodeSpecial(dp.getDeptName()) + "\"");
    ...
    public List<T9Department> getDeptByParentId(int deptId, Connection conn,
            String toId) throws Exception {
                ...
                if (toId != null && !"".equals(toId)) {
                    ...
                    for (int j = 0; j < toIds.length; j++) {
                        ...
                        if (toIdTemp.equals(Integer.toString(dept.getSeqId()))
                                || childToId(conn, toId,
                                        Integer.toString(dept.getSeqId())) == true) {
                            temp = true;
                            break;
                        }
                    }
    ...
    public boolean childToId(Connection conn, String toId, String deptId)
            throws Exception {
        T9ORM orm = new T9ORM();
        List<T9Department> list = new ArrayList();
        Map filters = new HashMap();
    ...
    // 没有从toId中取
    public StringBuffer getUserById(T9DeptNotify notify, Connection conn,
            int deptIdtemp, StringBuffer sb) throws Exception {
        String unUser = "";
        ...
        try {
            ...
            String queryUserSql = "select SEQ_ID,USER_PRIV,USER_NAME from PERSON where DEPT_ID='"
                    + deptIdtemp + "'";
            userSt = conn.createStatement();
            userRs = userSt.executeQuery(queryUserSql);
            while (userRs.next()) {
                ...
                if (notify.getReaders() != null
                        && getReaderNames(conn, notify.getReaders(), userName)) {
                    userNameStr = userNameStr + userName + ",";
                    String[] readers = notify.getReaders().split(",");
                    for (int j = 0; j < readers.length; j++) {
                        readers[j] = readers[j].substring(
                                readers[j].indexOf("_") + 1,
                                readers[j].length() - 1);
                        readerDate += readers[j] + ",";
                    }
                    readerDate = readerDate.substring(0,
                            readerDate.length() - 1);
                    // System.err.println(readerDate);

                    // readCount ++;
                } else {
                    if ("0".equals(notify.getToId())
                            || findToId(notify.getToId(),
                                    Integer.toString(deptIdtemp)) == true
                            || findToId(notify.getPrivId(), userPriv) == true
                            || findToId(notify.getUserId(),
                                    Integer.toString(userId)) == true) {
    ...
    public boolean getReaderNames(Connection conn, String reader,
        ...
        try {
            ...
            if (reader != null && !"".equals(reader)) {
                ...
                reader = "";
                for (int j = 0; j < readers.length; j++) {
                    readers[j] = readers[j].substring(0,
                            readers[j].indexOf("_"));
                    reader += readers[j] + ",";
                }
    ...
    public String readStatus(Connection conn, int id) throws Exception {
        ...
        ResultSet rs = null;
        String sql = "Select readers from dept_notify where seq_id =" + id;
        try {
     ...
     * @throws Exception
     */
    public String haveRead(Connection conn, int userId, int noId)
            throws Exception {
