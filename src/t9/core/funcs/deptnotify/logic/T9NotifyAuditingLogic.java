 ...
 * 
 */
public class T9NotifyAuditingLogic {
    ...
    // 待批公告
    public String getUnAuditedList(Connection conn, T9Person person,
        ...
        if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
            querySql = "SELECT n.SEQ_ID,n.FROM_ID,n.TO_ID,n.SUBJECT,n.FORMAT,n.[TOP],n.PRIV_ID,n.USER_ID,n.TYPE_ID,n.PUBLISH,"
                    + "n.SEND_TIME,n.BEGIN_DATE,n.END_DATE,n.TOP_DAYS,p.USER_NAME,d.DEPT_NAME from DEPTNOTIFY n,PERSON p,DEPARTMENT d where "
    ...
    // 已批公告
    public String getAuditedList(Connection conn, T9Person person, String type,
            String ascDesc, String field, int showLen, int pageIndex)
            throws Exception {
        Date currentDate = new Date();
        Statement stmt = null;
        ResultSet rs = null;
        ...
        if (dbms.equals(T9Const.DBMS_SQLSERVER)) {
            querySql = "SELECT n.SEQ_ID,n.FROM_ID,n.TO_ID,n.SUBJECT,n.FORMAT,n.[TOP],n.PRIV_ID,n.USER_ID,n.TYPE_ID,n.PUBLISH,"
                    + "n.SEND_TIME,n.BEGIN_DATE,n.END_DATE,p.USER_NAME,d.DEPT_NAME from DEPTNOTIFY n,PERSON p,DEPARTMENT d where "
                    + "n.FROM_ID=p.SEQ_ID and p.DEPT_ID=d.SEQ_ID and n.PUBLISH!='2' and n.AUDITER='"
                    + userSeqId + "'";
        } else {
            querySql = "SELECT n.SEQ_ID,n.FROM_ID,n.TO_ID,n.SUBJECT,n.FORMAT,n.TOP,n.PRIV_ID,n.USER_ID,n.TYPE_ID,n.PUBLISH,"
        ...
        try {
            ...
            for (int i = 0; i < showLen && !rs.isAfterLast() && recordCount > 0; i++) {
                ...
                sb.append(",format:\"" + format + "\"");
                sb.append(",top:\"" + top + "\"");
                sb.append(",result:\"" + T9Utility.encodeSpecial(result) + "\"");
                sb.append(",sendTime:\"" + sendTime + "\"");
                sb.append(",beginDate:\"" + beginDate + "\"");
                sb.append(",endDate:\"" + endDateStr + "\"");
                sb.append("},");
    ...
    public boolean operation(Connection dbConn, T9Person loginUser,
            T9DeptNotify notify, String operation, String mailRemind)
            throws Exception {
        ResultSet rs = null;
        Statement st = null;
        T9ORM orm = new T9ORM();
        ...
        if ("0".equals(operation)) {// 0--不批准1--批准
            changeStateSql = "update NOTIFY set PUBLISH='3',AUDITER='"
                    + loginUser.getSeqId() + "',AUDIT_DATE="
                    + T9DBUtility.currDateTime() + ",REASON='"
                    + T9Utility.encodeLike(notify.getReason())
            ...
            String content = "您提交的公告通知，标题：" + notify.getSubject()
                    + "审批未通过，原因是：" + notify.getReason();
            String remindUrl = "/core/funcs/deptnotify/manage/index.jsp?seqId="
            ...
            if ("0".equals(notify.getToId())
                    || "ALL_DEPT".equals(notify.getToId())) {// 如果是全体部门
            ...
            // ------------提醒所有在发布范围内的人-------------
            if ("on".equalsIgnoreCase(mailRemind)) {
                T9SmsBack smsBack2 = new T9SmsBack();
                ...
                if (!"".equals(toIdFW.trim()) && toIdFW != null
                        && toIdFW.contains(",") == true) {
                    T9SmsUtil.smsBack(dbConn, smsBack2);
                }
            }
        }
        return success;
