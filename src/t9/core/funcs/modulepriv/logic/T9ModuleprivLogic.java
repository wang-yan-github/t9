package t9.core.funcs.modulepriv.logic;
...
public class T9ModuleprivLogic {
...
//  }
   ...
   *   当前正在编辑的用户
   */
  public void queryNeedSetMoudle(Connection conn,T9ModulePriv modulepriv,String apply_to_priv,String apply_to_dept,String apply_to_module)throws Exception {
    Statement stmt = null;
...
//        userIdtemp = userSeqIdtemp;
        for(int i=0;i<applyModules.length;i++) {
          ...
          if(!("".equals(dept_priv)||"".equals(role_priv))) {
            String setmoduleStr = "select * from MODULE_PRIV m where m.USER_SEQ_ID='"+userSeqIdtemp+"' and m.MODULE_ID='"+moduleIdtemp+"'";
            ...
            if(setModulers.next()) {
              ...
              if("4".equals(dept_priv)) {
                ...
                String query="update MODULE_PRIV set DEPT_PRIV='"+dept_priv+"',ROLE_PRIV='"+role_priv+"',DEPT_ID='"+dept_id+"',PRIV_ID='"+priv_id+"',USER_ID='"+user_id+"' where USER_SEQ_ID='"+userSeqIdtemp+"' and MODULE_ID='"+moduleIdtemp+"'";
                stmt = conn.createStatement();
              ...
              if("4".equals(dept_priv)) {
                ...
                String query = "insert into MODULE_PRIV (MODULE_ID,DEPT_PRIV,ROLE_PRIV,DEPT_ID,PRIV_ID,USER_SEQ_ID,USER_ID) values ('"+moduleIdtemp+"','"+dept_priv+"','"+role_priv+"','"+dept_id+"','"+priv_id+"','"+userSeqIdtemp+"','"+user_id+"')";
                stmt = conn.createStatement();
                stmt.execute(query);
            }
          }else {
            String deleteSql = "delete from MODULE_PRIV where USER_SEQ_ID='"+userSeqIdtemp+"' and MODULE_ID='"+moduleIdtemp+"'";
            stmt = conn.createStatement();
  ...
  public static List<T9ModulePriv> selectModulePriv(Connection dbConn,String userId) throws Exception{
    List<T9ModulePriv> moduleList = new ArrayList<T9ModulePriv>();
    String[] str = {"USER_SEQ_ID = " + userId ,"MODULE_ID = 3"};
    T9ORM orm = new T9ORM();
    moduleList  =  orm.loadListSingle(dbConn, T9ModulePriv.class, str);
    return moduleList;
  }
}
