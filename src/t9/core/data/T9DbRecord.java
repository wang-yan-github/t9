...
public class T9DbRecord {
   ...
   * 连接另外一个记录集
   */
  public void join(T9DbRecord other) {
    if (other == null) {
      return;
    }
    ...
    for (int i = 0; i < fieldCnt; i++) {
      ...
      if (fieldValue == null) {
        ...
        if (Integer.class.equals(fieldType)) {        
          ...
          value = T9Utility.getFormatedStr(((Double)fieldValue).doubleValue(), T9Utility.WITHOUTGROUP);
        }else if (Date.class.equals(fieldType)) {
          value = "\"" + T9Utility.getDateTimeStr((Date)fieldValue) + "\"";
        }else {
          if (fieldValue == null) {
            value = "\"\"";
          }else {
            String tmpStr = T9Utility.null2Empty(fieldValue.toString());
  ...
  public Object clone(String fieldNamePrefix) {
    T9DbRecord rtObj = new T9DbRecord();
    
    for (int i = 0; i < fieldCnt; i++) {
      String fieldName = getNameByIndex(i);
      Object fieldValue = getValueByIndex(i);
      if (!T9Utility.isNullorEmpty(fieldNamePrefix)) {
  ...
  public String toString() {
    return "T9DbRecord [fieldCnt=" + fieldCnt + ", fieldIndexMap="
