...
public class ClientPropertiesUtil {
  ...
  private static void getClientProperties(){
    try{
      FileInputStream fis = new FileInputStream(new File(T9SysProps.getWebPath() + "\\WEB-INF\\config\\esbselfconfig.properties"));
  ...
  public static void store() {
    Writer wr =  null;
    try{
      FileOutputStream fos = new FileOutputStream(new File(T9SysProps.getWebPath() + "\\WEB-INF\\config\\esbselfconfig.properties"));
  ...
  public static int getHostPort() {
    ...
    if (T9Utility.isNullorEmpty(s) || !T9Utility.isNumber(s)) {
      return 80;
    }
