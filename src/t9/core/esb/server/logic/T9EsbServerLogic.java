package t9.core.esb.server.logic;
...
import t9.core.esb.server.act.T9EsbServerAct;
...
public class T9EsbServerLogic {
  ...
  private static Logger log = Logger.getLogger("t9.core.esb.server.logic.T9EsbServerLogic.java");
  /**
   * 增加传送状态
   * @param dbConn
   * @param transId
  ...
  private void addStatus(Connection dbConn, String transId, int toId) throws Exception {
    T9EsbTransferStatus status = new T9EsbTransferStatus();
  ...
  public void recvCompleted(Connection dbConn, String guid, int toId, String fromId, File file) throws Exception {
    ...
    addSysmsg(dbConn, guid, T9FOM.toJson(result).toString(), fromId);
    if (isTaskCompleted(dbConn, guid)) {
      ...
      setTransStatus(dbConn, guid, T9EsbServerLogic.TRANSFER_STATUS_ALLCOMPLETE);
      T9EsbServerTasksMgr.removeDownFile(guid);
    }
  }
   ...
   * @throws Exception
   */
  public void addTransferTask(Connection dbConn, String guid, int fromId, String filePath, String content, String type, String toId , String optGuid , String message , long fileSize) throws Exception {
    T9EsbTransfer trans = new T9EsbTransfer();
    trans.setFromId(fromId);
    trans.setGuid(guid);
    trans.setFilePath(filePath);
    trans.setContent(content);
    trans.setStatus("1");//上传中 状态标示    trans.setType(type);
    trans.setCreateTime(T9Utility.parseTimeStamp());
    trans.setToId(toId);
    trans.setOptGuid(optGuid);
    trans.setMessage(message);
    trans.setFileLength((int)fileSize);
  ...
  public void uploadComplete(Connection dbConn, String guid, int fromId, String toId, String filePath, String content, String type) throws Exception {
    ...
    if (toId != null && toId.trim().length() > 0) {
      ...
      if ("ALL_USERS".equalsIgnoreCase(toId)) {
        ...
        String sql = this.getInsertSql(dbConn, toId, guid);
        if (!T9Utility.isNullorEmpty(sql)) {
          this.exeUpdate(dbConn, sql);
        }
      }
    }
  } 
  public String getInsertSql(Connection dbConn, String toId , String guid) {
    String[] ids = toId.split(",");
    StringBuffer sb  = new StringBuffer("insert into ESB_TRANSFER_STATUS  (trans_id, status, to_id) values ");
    int count = 0 ;
    for (String s : ids) {
      try {
        int id = Integer.parseInt(s);
        if (!this.hasStatus(dbConn, guid, id)) {
          sb.append("('").append(guid).append("'").append(",'").append(TRANSFER_STATUS_READY).append("','").append(id).append("'),");
          count++;
        }
  ...
  public String codeStr2IdStr(Connection dbConn, String code) throws Exception {
    String ids = "";
    if (!T9Utility.isNullorEmpty(code)) {
  ...
  public void uploadComplete(Connection dbConn, T9TaskInfo info) throws Exception {
    this.uploadComplete(dbConn, info.getGuid(), info.getFromId(), info.getToId(), info.getFile().getAbsolutePath(), info.getContent(), "0");
  }
  public String getGuids(Connection dbConn , int toId) {
    PreparedStatement ps = null;
    ResultSet rs = null;
    int count = 0 ;
    StringBuffer sb = new StringBuffer();
    try {
      String sql = "select TRANS_ID from ESB_TRANSFER_STATUS where TO_ID = ? and STATUS  = ?" ;
      ps = dbConn.prepareStatement(sql);
      ps.setInt(1, toId);
      ps.setString(2, TRANSFER_STATUS_READY);
      rs = ps.executeQuery();
      while (rs.next() && count <= T9EsbServerAct.DOWNLOADS_LIMIT) {
        sb.append("'").append(rs.getString("TRANS_ID")).append("',");
        count++;
      }
    } catch (Exception e) {
      e.printStackTrace();
   ...
   * @throws Exception
   */
  public List<T9EsbTransfer> queryDownloadTask(Connection dbConn, int toId) throws Exception {
    String guids = this.getGuids(dbConn, toId);
    
    PreparedStatement ps = null;
    ResultSet rs = null;
    List<T9EsbTransfer> list = new ArrayList<T9EsbTransfer>();
    if (T9Utility.isNullorEmpty(guids)) {
      return list;
    }
    try {
      String sql = "select SEQ_ID" +
          ...
          " , OPT_GUID" + 
      		" from ESB_TRANSFER" +
      		" where GUID in ("+ guids + ") order by SEQ_ID desc";
      ps = dbConn.prepareStatement(sql);
      rs = ps.executeQuery();
      while (rs.next()) {
        T9EsbTransfer transfer = new T9EsbTransfer();
        ...
        transfer.setOptGuid(rs.getString("OPT_GUID"));
        list.add(transfer);
      }
    } catch (Exception e) {
      e.printStackTrace();
   ...
   * @throws Exception
   */
  public List<T9EsbSysMsg> querySysMsg(Connection dbConn, int toId) throws Exception {
    T9ORM orm = new T9ORM();
    Map<String, String> filters = new HashMap<String, String>();
    filters.put("TO_ID", String.valueOf(toId));
    filters.put("STATUS", "0");
    
    List<T9EsbSysMsg> list = orm.loadListSingle(dbConn, T9EsbSysMsg.class, filters);
   ...
   * @throws Exception
   */
  public T9EsbTransfer queryDownloadInfo(Connection dbConn, String guid) throws Exception {
    T9ORM orm = new T9ORM();
    Map<String, String> filters = new HashMap<String, String>();
    filters.put("GUID", guid);
    return (T9EsbTransfer) orm.loadObjSingle(dbConn, T9EsbTransfer.class, filters);
  }
  ...
  public void broadcastConfig(Connection dbConn, String configPath, String optGuid ,String message) throws Exception {
    ...
    this.addTransferTask(dbConn, guid, 0, configPath, "", "1", "ALL_USERS" , optGuid , message , 0);
  ...
  public void addSysmsg(Connection dbConn, String guid, String content, String toId) throws Exception {
    if (!T9Utility.isNullorEmpty(toId)) {
      T9EsbSysMsg msg = new T9EsbSysMsg();
...
//      ps.setTimestamp(1, T9Utility.parseTimeStamp());
  ...
  public static void setDownloadFailedMessage(Connection dbConn, String guid, String failedMessage, String toId , int fromId , boolean checked ) throws Exception{
    ...
    if (checked) {
      T9EsbServerLogic lo = new T9EsbServerLogic();
      Map<String, String> result = new HashMap<String, String>();
      result.put("code", "-3");
      result.put("to", lo.seqId2UserCode(dbConn, Integer.parseInt(toId)));
      result.put("guid", guid);
      result.put("msg", failedMessage);
      lo.addSysmsg(dbConn, guid, T9FOM.toJson(result).toString(), String.valueOf(fromId));
      /*
      //给下载方一个消息，不让其后台服务循环了
      ...
      lo.addSysmsg(dbConn, guid, T9FOM.toJson(result).toString(), toId);
      */
    }
  }
  ...
  public int exeUpdate(Connection dbConn, String sql) {
    // TODO Auto-generated method stub
    PreparedStatement ps = null;
    try {
      ps = dbConn.prepareStatement(sql);
  ...
  public void addClientMsg(Connection dbConn, int seqId, String message) {
    // TODO Auto-generated method stub
    PreparedStatement ps = null;
    String sql = " insert into ESB_CLIENT_MESSAGE (USER_ID , TIME , MESSAGE) VALUES (" + seqId + " , ? ,?)" ; 
    try {
      ps = dbConn.prepareStatement(sql);
      ps.setTimestamp(1, T9Utility.parseTimeStamp());
