package t9.core.esb.test.logic;
...
public class T9EsbTesterLogic {

  private static Logger log = Logger.getLogger("t9.core.esb.test.logic.T9EsbTesterLogic.java");
  ...
  public void test(Connection dbConn, File log, File testFolder, String toId, int toCount, long time) throws Exception {
    ...
    while (!T9EsbFrontend.isCompleted() || !isUploadFinished(dbConn)) {
      Thread.sleep(1000);
    }
  ...
  private void writeLog(File log, long start, long uEnd, long dEnd, int u, int us, int d, int ds, double rate) throws IOException {
    try {
      ...
      String startStr = T9Utility.getDateTimeStr(new Date(start));
      String uEndStr = T9Utility.getDateTimeStr(new Date(uEnd));
      String dEndStr = T9Utility.getDateTimeStr(new Date(dEnd));
            ...
            try {
              ...
              for (File f : testFolder.listFiles()) {
                ...
                if (f.isFile()) {
                  ...
                  int code = T9EsbFrontend.send(f, to  ,guid  ,"", "");
                  while (code != 0 && flag) {
                    Thread.sleep(1000);
                    code = T9EsbFrontend.send(f, to , guid  ,"", "");
                  }
                }
              }
