...
public class T9FOM {
  ...
  public static Object build(Map request) throws Exception {
    Object paramObj = request.get(T9ActionKeys.DTO_CLASS);    
    ...
    if (T9Utility.isNullorEmpty(mainClassName)) {
      return null;
    }
    ...
    for (int i = 0; i < fieldArray.length; i++) {
      ...
      if (List.class.isAssignableFrom(fieldType)
        ...
        if(paramObj!=null) {
          ...
          paramObj = request.get(tmpStr + (T9Utility.isNullorEmpty(postFix) ? "" : "_" + postFix) + "Cnt");    
        ...
        if (T9Utility.isNullorEmpty(className) || T9Utility.isNullorEmpty(detlCntStr)) {
          continue;          
        }
      ...
      T9ReflectUtility.setValue(obj, fieldName, field.getType(), paramValue);
    }
    return obj;
  }
  /**
  ...
  public static StringBuffer toJson(Object obj) throws Exception {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (List.class.isAssignableFrom(fieldType)
        ...
        buf.append("[");
        Iterator<Object> iObj = T9ReflectUtility.itObject(obj, tmpStr);
        int cntFlag = 0;
        while (iObj.hasNext()) {
        ...
        if (int.class.equals(fieldType)
            ...
            || Double.class.equals(fieldType)) {
          value = T9ReflectUtility.getValue(obj, fieldName);
        }else if (Date.class.equals(fieldType)) {
          if((Date)T9ReflectUtility.getValue(obj, fieldName)!=null){
            value = "\"" + T9Utility.getDateTimeStr((Date)T9ReflectUtility.getValue(obj, fieldName)) + "\"";
          }else{
            value = "\"\"";
          }   
        }else {
          Object valueObj = T9ReflectUtility.getValue(obj, fieldName);
          if (valueObj == null) {
            value = "\"\"";
          }else {
            String tmpStr = T9Utility.null2Empty(valueObj.toString());
