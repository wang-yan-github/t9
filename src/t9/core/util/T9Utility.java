...
public class T9Utility {
  ...
  public static int getDaySpan(String toDateStr) throws Exception {
    return (int)((parseDate(toDateStr).getTime() - new Date().getTime()) / T9Const.DT_D);
  }
  ...
  public static int getDaySpan(Date toDate) {
    return (int)((toDate.getTime() - new Date().getTime()) / T9Const.DT_D);
  }
  ...
  public static int getDaySpan(Date fromDate, Date toDate) {
    return (int)((toDate.getTime() - fromDate.getTime()) / T9Const.DT_D);
  }
  ...
  public static long get10BaseNum(String srcNum) throws Exception {
    ...
    for (int i = charArray.length - 1; i >= 0; i--) {
      ...
      if (tmpNum >= 0) {
        ...
        throw new Exception(T9I18n.ln(T9Utility.class, "1"));
      }
    }
    return num36;
  }
...
//        throw new Exception("无效的36进制数字");
  ...
  private static String positiveIntegerToChinsesStr(String numStr) {
    ...
    if (len > 15) {
//      return "数值过大!";
      return T9I18n.ln(T9Utility.class, "2");
    }
    for (int i = len - 1; i >= 0; i--) {
      ...
      if (n < 0 || n > 9) {
//        return "输入含非数字字符!";
        return T9I18n.ln(T9Utility.class, "3");
      }
...
//      return "数值位数过大!";
    ...
    if (jiao == 0 && fen == 0) {
//      TailStr = "整";
      TailStr = T9I18n.ln(T9Utility.class, "5");
    } else {
      TailStr = chineseDigiStr[jiao];
      if (jiao != 0) {
//        TailStr += "角";
        TailStr += T9I18n.ln(T9Utility.class, "6");
      }
...
//        TailStr += chineseDigiStr[fen] + "分";
        TailStr += chineseDigiStr[fen] + T9I18n.ln(T9Utility.class, "7");
      }
    }
...
//        + "元" + TailStr;
    + T9I18n.ln(T9Utility.class, "8") + TailStr;
  }
...
//            n = Integer.parseInt(intString.substring(curPoint - 2, 3));
  ...
  public static boolean isBracketMatched(String expr) {
    ...
    int tmpIndex = expr.indexOf(T9TokenSplit.TOKEN_SPLIT_LEFTBRACKET);
    if (tmpIndex < 0) {
      return true;
    }
  ...
  public static java.sql.Timestamp parseTimeStamp(String dateStr) throws ParseException {
    ...
    if (dateStr == null) {
      ...
      utilDate = T9Utility.parseDate(dateStr);
    }
    java.sql.Timestamp sqlDate = new java.sql.Timestamp(utilDate.getTime());
    return sqlDate;
  }
  ...
  public static String getHtmlIdent(int identCnt) {
    return getIdentStr(identCnt, T9TokenConst.HTML_SPACE);
  }
  /**
   * 取得中文空格形式的缩进字符串
   * @return
  ...
  public static String getChinaSpaceIdent(int identCnt) {
    return getIdentStr(identCnt, T9TokenConst.CHINA_SPACE);
  }
  ...
  public static String parseDoubleStr(BigDecimal bg, int format) {
    return T9Utility.getFormatedStr(bg.doubleValue(), format);
  }
  /**
   * 解析双精度型字符串，解决科学计数法问题
   * @param bg
  ...
  public static String getRandomName() {
    String currTimeStr = T9Utility.getCurDateTimeStr(T9Utility.DATE_FORMAT_NOSPLIT);
  ...
  public static String encodeURL(String srcStr) throws Exception {
    ...
    return URLEncoder.encode(srcStr, T9Const.DEFAULT_CODE);
  }
  ...
  public static String decodeURL(String srcStr) throws Exception {
    ...
    return URLDecoder.decode(srcStr, T9Const.DEFAULT_CODE);
  }
  /**
   * 处理\ " '
   * @param srcStr
  ...
  public static void sortDesc(List<Map<String, String>> list, String key, int type) {
    java.util.Collections.sort(list, new T9MapComparator(type, key, "DESC"));
  }
  /**
   * 升序排序
   * @param list
  ...
  public static void sortAsc(List<Map<String, String>> list, String key, int type) {
    java.util.Collections.sort(list, new T9MapComparator(type, key, "ASC"));
  }
  /**
   * 去掉所有html标签
   * @param html
   ...
   * @return
   */
  public static Long cast2Long(Object value) throws T9InvalidParamException {
    ...
    try {
      ...
      try {
        ...
        try {          
          ...
          throw new T9InvalidParamException("不能转换成长整型 >> " + value.toString() + " >> " + value.getClass().getName() );
        }
      }
    }
  }
