 ...
 * 简单邮件（不带附件的邮件）发送器
 */
public class T9SimpleMailSender {
  /**
   * 以文本格式发送邮件
   * @param mailInfo 待发送的邮件的信息
   */
  public void sendTextMail(T9MailSenderInfo mailInfo) throws Exception {
    // 根据session创建一个邮件消息
   ...
   */
  public void sendHtmlAndAttachMail(T9MailSenderInfo mailInfo) throws Exception {
    // 根据session创建一个邮件消息
    Message mailMessage = buildMessage(mailInfo);
  ...
  private BodyPart buildAttachPart(String filePath) throws Exception {
    ...
    String fileName = T9FileUtility.getFileName(filePath);
   ...
   * @return
   */
  private Message buildMessage(T9MailSenderInfo mailInfo) throws Exception {
    // 判断是否需要身份认证
    T9MailAuthenticator authenticator = null;
    Properties pro = mailInfo.getProperties();
    // 如果需要身份认证，则创建一个密码验证器
    if (mailInfo.isValidate()) {
      authenticator = new T9MailAuthenticator(mailInfo.getUserName(), mailInfo.getPassword());
    }
