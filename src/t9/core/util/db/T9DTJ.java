 ...
 */
public class T9DTJ {
  private static Logger log = Logger.getLogger("t9.core.util.db.T9DTJ");
  ...
  private StringBuffer toSqlString(Connection conn, String tableNo,String[] filters) throws Exception{
    ...
    dsTable = (T9DsTable) orm.loadObjComplex(conn, T9DsTable.class, filtersMap);
    //2.数据库表名
  ...
  * new String[]{"SEQ_ID = 1","NAME='DDD'"}
  ...
  public String toJson(Connection dbConn, Map request, String categoryNo) throws Exception{
    try {
      String where = "";
      if(!"null".equals(categoryNo) && !T9Utility.isNullorEmpty(categoryNo)){
        where = " where d.CATEGORY_NO = " + categoryNo;
      }
      String sql = " select d.SEQ_ID, d.TABLE_NO, d.TABLE_NAME, d.TABLE_DESC, d.CLASS_NAME, T1.CLASS_DESC, d.CATEGORY_NO from ds_table d "
                 + " left join CODE_ITEM T1 on T1.CLASS_CODE = d.CATEGORY_NO and T1.CLASS_NO = 'S01' "
                 + where
                 + " order by TABLE_NO asc ";
      T9PageQueryParamNew queryParam = (T9PageQueryParamNew) T9FOM.build(request);
