...
public class T9JObject2SQLHepler {
  ...
  public static Map<String, Object> javaObject2InsertSQL(
    ...
    if ("im_group".equalsIgnoreCase(tableName)) {
      seqIdField = "GROUP_ID";
      fieldInfo.remove(seqIdField);
    }
    if ("im_offline_file".equalsIgnoreCase(tableName)) {
      seqIdField = "ID";
      fieldInfo.remove(seqIdField);
    }
    if ("im_group_maxmsgid".equalsIgnoreCase(tableName)) {
      seqIdField = "MSG_ID";
      fieldInfo.remove(seqIdField);
    }
    if ("im_group_msg".equalsIgnoreCase(tableName)) {
      seqIdField =  "MSG_ID";
      fieldInfo.remove(seqIdField);
    }
    ...
    try {
      ...
      T9SQLParamHepler.javaParam2SQLParam(paramList.toArray(), pstmt, tableName);
      pstmt.executeUpdate();
    ...
    while (iter.hasNext()) {
      ...
      if (value != null && List.class.isAssignableFrom(value.getClass())) {
        ...
        while (clsInfoIter.hasNext()) {
          ...
          if (T9SysProps.getString(T9SysPropKeys.DBCONN_DBMS).equals(T9Const.DBMS_SQLSERVER)) {
            val = "[" + val + "]";
          }
          sql.append(val);
