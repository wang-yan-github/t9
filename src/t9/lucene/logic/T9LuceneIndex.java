...
import t9.lucene.logic.MutiFilter;
import t9.lucene.logic.strFilter;
import t9.lucene.logic.timeFilter;



public class T9LuceneIndex {
  ...
  public Connection getTsDbConn()throws Exception{
     String url=T9SysProps.getProp("ts.db.jdbc.conurl.mysql");
     String pwd=T9SysProps.getProp("ts.db.jdbc.passward.mysql");
     String un =T9SysProps.getProp("ts.db.jdbc.userName.mysql");
     String dbName = T9SysProps.getProp("ts.db.jdbc.dbName");
  ...
  public void setIndex(List<Map<String,String>> dataList){
    try{
      String path=T9SysProps.getProp("lucene_path");
  ...
  public List<Map<String,String>> setNewsContent(Connection conn,List<Map<String,String>> dataList){
    ...
    ResultSet rs=null;
    try{
      String sql=" select seq_id,content_name,content_title,CONTENT_ABSTRACT,CONTENT_DATE,STATION_ID,COLUMN_ID,CONTENT from cms_content where content_status='5' order by CONTENT_DATE desc ";
      ...
      while(rs.next()){
        Map<String,String> map= new HashMap();
          ...
          if(T9Utility.isNullorEmpty(contentDate)){
            contentDate="";
          }
  ...
  public List<Map<String,String>> setNewsIndex(Connection conn,List<Map<String,String>> dataList){
    ...
    ResultSet rs=null;
    try{
      String sql=" select seq_id,subject,content,news_time,module_id,subinfo,type_id from news order by news_time desc ";
      stmt = conn.createStatement();
      rs = stmt.executeQuery(sql);
      while(rs.next()){
        Map<String,String> map= new HashMap();
          ...
          if(T9Utility.isNullorEmpty(module_id)){
            module_id="";
          }
          String subinfo=rs.getString("subinfo");
          if(T9Utility.isNullorEmpty(subinfo)){
            subinfo="";
          }
    ...
    try{
      ...
      while(rs.next()){
        Map<String,String> map= new HashMap();
          ...
          if(T9Utility.isNullorEmpty(cn_name)){
            cn_name="";
          }
          String en_name=rs.getString("en_name");
          if(T9Utility.isNullorEmpty(en_name)){
            en_name="";
          }
          String country=rs.getString("country");
          if(T9Utility.isNullorEmpty(country)){
            country="";
          }
          String profession=rs.getString("profession");
          if(T9Utility.isNullorEmpty(profession)){
            profession="";
          }
          String nation=rs.getString("nation");
          if(T9Utility.isNullorEmpty(nation)){
            nation="";
          }
          String grah_sch=rs.getString("grad_sch");
          if(T9Utility.isNullorEmpty(grah_sch)){
            grah_sch="";
          }
          String c_native=rs.getString("c_native");
          if(T9Utility.isNullorEmpty(c_native)){
            c_native="";
          }
          String birth_place=rs.getString("birth_place");
          if(T9Utility.isNullorEmpty(birth_place)){
            birth_place="";
          }
...
/*  public IKAnalyzer getIKAnalyzer(){
  ...
  public List<Map<String,String>> seacher(String queryString,int pageSize,int pageIndex,String condition){
    ...
    try{
      String indexPath=T9SysProps.getProp("lucene_path");
 ...
 //    }
  ...
  public String setHighlighter(String keyWord, String content,String RGB)throws Exception {
       if(T9Utility.isNullorEmpty(RGB)){
         RGB="RED";
       } 
