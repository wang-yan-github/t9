...
public class T9SystemLogLogic {
  ...
  public String[][] getLogByDate(Connection conn , String beginDate,String endDate , int start , int length) throws Exception {
    ...
    sys.addAll(workflow);
    Collections.sort(sys, new T9SysLogComparaor());
    List<Map> list = new ArrayList();
    if (start < 0) {
      start = 0;
    }
  ...
  public List<Map> getWorkflowLogByDate(Connection conn , String beginDate,String endDate ) throws Exception {
    String query = "select * from flow_run_log where 1=1 ";
    query += this.getQueryDate(beginDate, endDate);
    Statement stm = null;
    ResultSet rs = null;
    List<Map> list = new ArrayList();
  ...
  public String getQueryDate(String beginDate,String endDate) throws Exception {
    ...
    if(beginDate != null && !"".equals(beginDate)){
      String dbDateF = T9DBUtility.getDateFilter("TIME", beginDate, " >= ");
      query +=  " and " + dbDateF;
    }
    if(endDate != null && !"".equals(endDate)){
      String dbDateF = T9DBUtility.getDateFilter("TIME", endDate, " <= ");
      query += " and " + dbDateF;
    }
    return query;
  }
