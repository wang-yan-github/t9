...
public class T9DbconnWrap {
  T9IRequestDbConn requestDbConn = null;
  
  public T9DbconnWrap() {
    try {
      requestDbConn = (T9IRequestDbConn)Class.forName("t9.core.data.T9RequestDbConn").newInstance();
    }catch(Exception ex) {       
    }
  }
  public T9DbconnWrap(HttpServletRequest request) {
    requestDbConn = (T9IRequestDbConn) request.getAttribute("requestDbConnMgr");
  }
  /**
   * 关闭数据库资源
