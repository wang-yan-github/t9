 ...
 *
 */
public class T9ERPSetupUitl {
  private static Logger log = Logger.getLogger("cy.t9.setup.util.T9ERPSetupUitl");
    ...
    if(installedPlatDir.exists()){
      ...
      for (File file : files) {
        if("Installed.properties".equalsIgnoreCase(file.getName())){
          try {
            T9FileUtility.load2Map(file.getPath(), rtMap);
  ...
  public static void executionSQLFile(String path,String dbName,String[] strSrc) throws Exception {
    ...
    T9DBUtility dbUtil = new T9DBUtility("sqlserver");
    try {
      ...
      for (int i = 0; i < strSrc.length; i++) {
       ...
       // T9InstallConfig ic = new T9InstallConfig();
        exeSql(filePath,"sqlserver",dbName);
      }
    } catch (Exception Ex) {
      Ex.printStackTrace();
      T9DBUtility.forceDropDb(dbName);
    } finally {
  ...
  public static void exectBatchSql(
    ...
    String exSql = null;
    try {
      T9DBUtility dbUtil = new T9DBUtility(dbmsName);
      /** 对所指定的库执行Sql语句 **/
  ...
  public static void regEaDbms2TDSYS(String path,String dbName,String[] strSrc) throws Exception{
    for (int i = 0; i < strSrc.length; i++) {
      String filePath = path + strSrc[i];
      T9ERPSetupUitl.exeSql(filePath,"sqlserver",dbName);
    }
  }
  /**
   * 清空T9中ERP的信息
   ...
   * @throws Exception
   */
  public static void deleteErpMenu2T9(String flag,String sysDbName) throws Exception{
    Statement st = null;
    String sql = "delete from sys_menu where menu_id='" + flag + "'";
    String sql2 = "delete from sys_function where menu_id like '" + flag + "%' ";
    Connection conn = null;
    T9DBUtility DBUtil = null;
    try {
      DBUtil = new T9DBUtility();
  ...
  public static boolean testDbConn(Map paramsMap,String dbmsName) throws Exception{
    ...
    T9SysProps.updateProp("db.jdbc.driver." + dbmsName,driver);
    T9SysProps.updateProp("db.jdbc.conurl." + dbmsName,conurl);
    T9SysProps.updateProp("db.jdbc.userName." + dbmsName,userName);
    T9SysProps.updateProp("db.jdbc.passward." + dbmsName,passward);
    T9DBUtility dbutil = new T9DBUtility(dbmsName);
    Connection conn = null;
    try {
      ...
      T9DBUtility.closeDbConn(conn, log);
    }
  }
  /**
   * 修改安装配置项
      ...
      for (File file : files) {
        if("Installed.properties".equalsIgnoreCase(file.getName())){
          try {
            ...
            T9PropUtility.updatePropFile(file.getPath(), rtMap);
          } catch (Exception e) {
            throw e;
          }
        }
  ...
  public static void modifySysConfig(Map configMap,String fileName) {
    T9PropUtility.updatePropFile(fileName, configMap);
  }
  ...
  public  static String[] backupDb(String[] dbNameArray,
    ...
    try {
      T9DBUtility dbUtil = new T9DBUtility(dbmsName);
      dbConn = dbUtil.getConnection(true, "master");
      return T9DBUtility.backupDb(dbConn, dbNameArray, outDir);
    }catch(Exception ex) {
      throw ex;
    }finally {
      T9DBUtility.closeDbConn(dbConn, null);
  ...
  public static void forceDropDb(String dbName,String dbmsName) throws Exception{
    ...
    try {
      T9DBUtility dbUtil = new T9DBUtility(dbmsName);
      dbConn = dbUtil.getConnection(true, "master");
      T9DBUtility.forceDropDb(dbConn, dbName);
    }catch(Exception ex) {
      throw ex;
    }finally {
      T9DBUtility.closeDbConn(dbConn, null);
