package t9.setup.fis.acset.data;
...
import t9.setup.fis.acset.util.TDAcsetBuilder;
import t9.setup.fis.acset.util.TDPeriodSave;
...
public class TDAcsetCreateDatabase {
    ...
    try {
      ...
      if (rs.next()) {
        if(rs.getString(1) == null) {
          ...
          dbNo = T9Utility.getNextNum(rs.getString(1), 1, 3);
        }
      }
      ...
      //T9DataSources.addAcset(acset);
      
      if (T9Utility.isNullorEmpty(periodCntrl)) {
        return;
      }
    ...
    try {
      ...
      if (rs.next()) {
        if(rs.getString(1) == null) {
          ...
          return T9Utility.getNextNum(rs.getString(1), 1, 2);
        }
      }else {
        return "01";
      }
  ...
  public static boolean createDatabase(
    ...
    String acsetNo = null;    
    int deptId = 0;
    if(T9Utility.isNullorEmpty(deptId_String)) {
      deptId_String = "0";
    }
    ...
    try {
      ...
      if (T9ERPSetupUitl.isDbActive(accountDbName,dbmsName)) {
        try {
          T9ERPSetupUitl.backupDb(accountDbName,"installbackup",dbmsName);
        }catch(Exception ex) {          
        }
        try {
          T9ERPSetupUitl.forceDropDb(accountDbName,dbmsName);
        }catch(Exception ex) {          
        }
      }
