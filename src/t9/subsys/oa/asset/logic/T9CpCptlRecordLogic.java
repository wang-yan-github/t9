...
public class T9CpCptlRecordLogic {
  private static Logger log = Logger.getLogger("t9.core.funcs.asset.act.T9CpCptlRecordAct");
   ...
   * @throws Exception
   */
  public String listSelect(Connection dbConn,Map request,T9CpCptlInfo cp,T9CpCptlRecord cptlRecord) throws Exception {
    String sql = "select p.SEQ_ID as seqId,up.CPTL_NO as cptlNo,up.CPTL_NAME as cptlName,cpa.TYPE_NAME as typeName,"
      ...
      + " from cp_cptl_record p "
      + " left outer join cp_cptl_info up on p.CPTL_ID = up.SEQ_ID "
      + " left outer join CP_ASSET_TYPE cpa on cpa.SEQ_ID=up.CPTL_SPEC "   
    ...
    if (!T9Utility.isNullorEmpty(cp.getCptlName())) {
      sql += " and up.CPTL_NAME like '%" + T9DBUtility.escapeLike(cp.getCptlName()) + "%' " + T9DBUtility.escapeLike();
    }
    ...
    if (!T9Utility.isNullorEmpty(cp.getUseUser())) {
      sql += " and p.CPRE_USER like '%" + T9DBUtility.escapeLike(cp.getUseUser()) + "%' " + T9DBUtility.escapeLike();
    }
   ...
   * @throws Exception 
   */
  public List<T9Cpcptl> getCpreFlag1(Connection dbConn,int seqId) throws Exception {
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    List<T9Cpcptl> list = new ArrayList<T9Cpcptl>();
    T9Cpcptl cpcptl = null;
  ...
  public String getName(Connection dbConn,int seqId) throws Exception {
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    String sql = "select CPTL_NAME from cp_cptl_info where SEQ_ID=" + seqId;
    String name = null;
    try {
      stmt = dbConn.prepareStatement(sql);
      //System.out.println(sql);
   ...
   * @throws Exception 
   */
  public List<T9Cpcptl> getCpreFlag2(Connection dbConn,int seqId) throws Exception {
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    List<T9Cpcptl> list = new ArrayList<T9Cpcptl>();
    T9Cpcptl cpcptl = null;
  ...
  public void getDelete(Connection dbConn,int seqId) throws Exception {
    PreparedStatement stmt = null ;
    String sql = "delete from cp_cptl_record where cptl_id=" + seqId;
    try {
      stmt = dbConn.prepareStatement(sql);
      stmt.execute();
    }catch (Exception e) {
   ...
   * @throws Exception 
   */
  public List<T9Cpcptl> importOut(Connection dbConn,T9CpCptlInfo cp,T9CpCptlRecord cptlRecord) throws Exception {
    T9Cpcptl cpcptl = null;
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    List<T9Cpcptl> list = new ArrayList<T9Cpcptl>();    
  ...
  //分类代码,去除重复数据
  public static List<T9CpCptlInfo> typeList(Connection dbConn) {
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    T9CpCptlInfo cpInfo = null;
    List<T9CpCptlInfo> list = new ArrayList<T9CpCptlInfo>();
    String sql = "SELECT TYPE_ID from cp_cptl_info group by TYPE_ID";
    try {
      stmt = dbConn.prepareStatement(sql);
      rs = stmt.executeQuery();
      while (rs.next()) {
...
//使用方向,去除重复数据
  public static List<T9CpCptlInfo> useStateList(Connection dbConn) {
    ResultSet rs = null;
    PreparedStatement stmt = null ;
    T9CpCptlInfo cpInfo = null;
    List<T9CpCptlInfo> list = new ArrayList<T9CpCptlInfo>();
    String sql = "SELECT use_state from cp_cptl_info group by use_state";
    try {
      stmt = dbConn.prepareStatement(sql);
      rs = stmt.executeQuery();
      while (rs.next()) {
   ...
   * @throws Exception 
   */
  public void getUpdate(Connection dbConn,T9CpCptlInfo info) throws Exception {
    PreparedStatement stmt = null ;
    String sql = "update cp_cptl_info set use_user=?,use_state=?,use_dept=? where seq_id=?";
    try {
      stmt = dbConn.prepareStatement(sql);
  ...
  //导出数据
  public ArrayList<T9DbRecord> getDbRecord(List<T9Cpcptl> list){
    //System.out.println(worklist.size());
    ArrayList<T9DbRecord> dbL = new ArrayList<T9DbRecord>();
    T9Cpcptl cp = new T9Cpcptl();
   ...
   * @throws Exception 
   */
  public static void editAsset(Connection dbConn,T9CpCptlRecord record) throws Exception {
    PreparedStatement stmt = null ;
    String sql = "update cp_cptl_record set cpre_memo=?,cpre_place=?,cpre_reason=?,dept_id=? where seq_id=?";
    try {
      stmt = dbConn.prepareStatement(sql);
