package t9.subsys.oa.giftProduct.outstock.logic;
...
public class T9GiftOutstockLogic {
  private static Logger log = Logger.getLogger("t9.core.act.action.T9SysMenuLog");
  public int addGiftOutstock(Connection dbConn,T9GiftOutstock giftOutstock) throws Exception {
    T9ORM orm = new T9ORM();
    orm.saveSingle(dbConn, giftOutstock);
    return T9CalendarLogic.getMaSeqId(dbConn, "GIFT_OUTSTOCK");
  }
  public void updateGiftOutstock(Connection dbConn,T9GiftOutstock giftOutstock) throws Exception {
    T9ORM orm = new T9ORM();
    orm.updateSingle(dbConn, giftOutstock);
  }
  public void delOutstockById(Connection dbConn,int seqId) throws Exception {
    T9ORM orm = new T9ORM();
    orm.deleteSingle(dbConn, T9GiftOutstock.class, seqId);
  }
  public void delOutstock(Connection dbConn,String giftId) throws Exception {
    ...
    String sql = "delete from GIFT_OUTSTOCK where GIFT_ID in (" + newGiftId + ")";
   try{
  ...
  public T9GiftOutstock selectOutstockById(Connection dbConn,int seqId) throws Exception {
    T9ORM orm = new T9ORM();
    T9GiftOutstock outLogic = (T9GiftOutstock) orm.loadObjSingle(dbConn, T9GiftOutstock.class, seqId);
    return outLogic;
  }
  public  Map<String ,String> selectGiftOutstockById(Connection dbConn,int seqId) throws Exception {
    ...
    String sql = "select go.SEQ_ID as SEQ_ID,go.TRANS_USER as TRANS_USER,p.USER_NAME as TRANS_NAME , go.TRANS_QTY as TRANS_QTY,go.TRANS_FLAG " 
        ...
        +"from GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id = gi.seq_id "  
        +"left outer join person p on go.TRANS_USER = p.seq_id left join CODE_ITEM ci on gi.GIFT_TYPE=ci.seq_id where go.SEQ_ID="+seqId;
    try{
      stmt = dbConn.createStatement();
  ...
  public List<Map<String ,String>> getGiftOutstockToday(Connection dbConn,String today,int userId) throws Exception {
    ...
    ResultSet rs = null; 
    T9ORM orm = new T9ORM();
    String begin = T9DBUtility.getDateFilter("go.TRANS_DATE", today, ">=");
    String end = T9DBUtility.getDateFilter("go.TRANS_DATE", today + " 23:59:59", "<=");
    ...
    String queryStr = "select goip.SEQ_ID as SEQ_ID,goip.GIFT_ID as GIFT_ID,goip.gift_name as GIFT_NAME,p2.USER_NAME as TRANS_USER ,goip.GIFT_QTY as GIFT_QTY, "
      ...
      +"GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id=gi.seq_Id left outer join person p on go.operator=p.SEQ_ID where "
      + begin + " and " + end + " and go.OPERATOR='" + userId + "') goip left outer join person p2 on goip.TRANS_USER = p2.SEQ_ID left outer join "
      +"(select sum(trans_qty- trans_flag) as use_gift_qty,gift_id as gift_id  FROM gift_outstock   group by gift_id) go2 on goip.GIFT_ID = go2.gift_id";
  ...
  public List<Map<String ,String>> getGiftOutstockByUserId(Connection dbConn,String userId) throws Exception {
    ...
    String queryStr = "select goip.SEQ_ID as SEQ_ID,goip.gift_name as GIFT_NAME,p2.USER_NAME as TRANS_USER ,goip.GIFT_QTY as GIFT_QTY, "
      ...
      +"GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id=gi.seq_Id left outer join person p on go.operator=p.SEQ_ID where go.TRANS_USER='" + userId + "') goip left outer join person p2 on goip.TRANS_USER = p2.SEQ_ID";
    //System.out.println(queryStr);
    try{
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(queryStr);
  ...
  public Map<String,String> getOutstockByIdBack(Connection dbConn,int seqId) throws Exception {
    ...
    String sql = "select goc.SEQ_ID as SEQ_ID,goc.OPERATOR_ID as OPERATOR_ID ,goc.OPERATOR as OPERATOR , goc.TRANS_QTY as TRANS_QTY,"
        ...
        +"from GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id = gi.seq_id "  
        +"left outer join person p on go.operator = p.seq_id where go.SEQ_ID="+seqId+") goc left outer join CODE_ITEM ci on goc.GIFT_TYPE=ci.SEQ_ID";
    try{
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      if(rs.next()){
  ...
  public void updateGiftBack(Connection dbConn,int seqId,String transFlag) throws Exception {
    Statement stmt = null;
    ResultSet rs = null; 
    String sql = "update GIFT_OUTSTOCK set TRANS_FLAG = TRANS_FLAG + " + transFlag + " where SEQ_ID=" + seqId;
    try{
      stmt = dbConn.createStatement();
      stmt.executeUpdate(sql);
    }catch(Exception ex) {
  ...
  public String toSearchData(Connection conn,Map request,String giftType,String giftId,String giftName,String fromDate,String toDate) throws Exception{
   ...
   String sql = "select goip.SEQ_ID as SEQ_ID,goip.gift_name as GIFT_NAME,p2.USER_NAME as TRANS_USER ,goip.GIFT_QTY as GIFT_QTY, "
    ...
    +"go.TRANS_QTY as TRANS_QTY,go.trans_flag as TRANS_FLAG, go.TRANS_QTY-go.trans_flag as USE_TRANS,"
    +"go.TRANS_USES as TRANS_USES, gi.gift_price as GIFT_PRICE,go.TRANS_DATE as TRANS_DATE,p.USER_NAME as OPERATOR ,go.TRANS_MEMO as TRANS_MEMO,go.RUN_ID as RUN_ID from "
    +"GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id=gi.seq_Id left outer join person p on go.operator=p.SEQ_ID where 1=1 ";
    ...
    if(!fromDate.equals("")){
      sql = sql + " and " + T9DBUtility.getDateFilter("go.TRANS_DATE",fromDate, ">=");
    }
    if(!fromDate.equals("")){
      sql = sql + " and " + T9DBUtility.getDateFilter("go.TRANS_DATE",fromDate, ">=");
    }
    if(!toDate.equals("")){
      sql = sql + " and " + T9DBUtility.getDateFilter("go.TRANS_DATE",toDate + " 23:59:59", "<=");
    }
 ...
 */     String dateStr = dateFormat.format(new Date());
  ...
  public List<Map<String,String>> selectReport(Connection dbConn,String giftType,String giftId,String fromDate,String toDate) throws Exception{
    String sql ="select gi.SEQ_ID as SEQ_ID,gi.gift_name as GIFT_NAME,gi.GIFT_QTY as GIFT_QTY"
     ...
     +"GIFT_INSTOCK gi  left outer join person p on gi.gift_creator=p.SEQ_ID left outer join CODE_ITEM ci on gi.GIFT_TYPE=ci.seq_id where 1=1 ";
     if(!giftId.equals("")){
       sql = sql + " and gi.SEQ_ID = "+giftId;
     }
     if(!giftType.equals("")){
     ...
     if(!fromDate.equals("")){
       sql = sql + " and " + T9DBUtility.getDateFilter("gi.CREATE_DATE",fromDate, ">=");
     }
     if(!toDate.equals("")){
       sql = sql + " and " + T9DBUtility.getDateFilter("gi.CREATE_DATE",toDate, "<=");
     }
  ...
  public List<Map<String,String>> selectProductInfo(Connection dbConn,String giftType,String giftId,String fromDate,String toDate) throws Exception{
    String sql ="select gi.SEQ_ID as SEQ_ID, gi.GIFT_NAME  as GIFT_NAME ,gi.GIFT_QTY as GIFT_QTY,gi.GIFT_UNIT as GIFT_UNIT,go.use_gift_qty as USE_GIFT_QTY from gift_instock gi left outer join "
           +"(select sum(trans_qty- trans_flag) as use_gift_qty,gift_id as gift_id  FROM gift_outstock   group by gift_id) go on gi.seq_id = go.gift_id where 1=1 "; 
     if(!giftId.equals("")){
       sql = sql + " and gi.SEQ_ID = "+giftId;
     }
     if(!giftType.equals("")){
     ...
     if(!toDate.equals("")){
       sql = sql + " and " + T9DBUtility.getDateFilter("gi.CREATE_DATE",toDate + " 23:59:59", "<=");
     }
     sql = sql  +" order by gi.gift_name";
     Statement stmt = null;
     ResultSet rs = null; 
  ...
  public List<Map<String,String>> getGiftOutByDeptId(Connection dbConn,String giftType,String giftId,String fromDate,String toDate,String deptId) throws Exception{
    ...
    +"GIFT_OUTSTOCK go left outer join GIFT_INSTOCK gi on go.gift_id=gi.seq_Id left outer join person p on go.TRANS_USER=p.SEQ_ID where p.dept_id=" + deptId;
     if(!giftId.equals("")){
       sql = sql + " and gi.SEQ_ID = "+giftId;
     }
     if(!giftType.equals("")){
