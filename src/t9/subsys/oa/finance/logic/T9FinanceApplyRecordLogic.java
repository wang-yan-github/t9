...
public class T9FinanceApplyRecordLogic {
  /***
  ...
  public String applySelect(Connection dbConn,Map request,String claimerName,String momey,String item,Date statrTime,Date endTime,String chequeAccount,String financeDirectorName) throws Exception {
    String sql = "select f.SEQ_ID,p.USER_NAME,f.APPLY_MONEY,f.CHEQUE_ACCOUNT,f.APPLY_MEMO,f.APPLY_ITEM,f.APPLY_DATE,"
      ...
      + " from finance_apply_record f "
      + " left outer join PERSON p on f.APPLY_CLAIMER = p.SEQ_ID where 1=1 and f.SIGN_TYPE=1";
    if (!T9Utility.isNullorEmpty(claimerName)) {
      sql += " and f.APPLY_CLAIMER ='" + claimerName + "'";
    }
    if (!T9Utility.isNullorEmpty(momey)) {
      sql += " and f.APPLY_MONEY >=" + momey ;
    }
    if (!T9Utility.isNullorEmpty(item)) {
      sql += " and f.APPLY_ITEM like '%" + T9Utility.encodeLike(item) + "%' " + T9DBUtility.escapeLike();
    }
    if (!T9Utility.isNullorEmpty(chequeAccount)) {
      sql += " and f.CHEQUE_ACCOUNT='" + T9Utility.encodeLike(chequeAccount) + "'";
    }
    if (!T9Utility.isNullorEmpty(financeDirectorName)) {
      sql += " and f.FINANCE_DIRECTOR='" + financeDirectorName + "'";
    }
    if (statrTime != null) {
      String str =  T9DBUtility.getDateFilter("f.APPLY_DATE", T9Utility.getDateTimeStr(statrTime), ">=");
      sql += " and " + str;
    }
    if (endTime != null) {
      String str =  T9DBUtility.getDateFilter("f.APPLY_DATE", T9Utility.getDateTimeStr(endTime), "<=");
      sql += " and " + str;
    }
  ...
  public String applySelectByBudgetId(Connection dbConn,Map request,String budgetId) throws Exception {
    String sql = "select f.SEQ_ID as SEQ_ID,p.USER_NAME as APPLY_NAME"
      ...
      + " from finance_apply_record f left outer join BUDGET_APPLY b on f.BUDGET_ID = b.SEQ_ID "
   ...
   * @throws Exception 
   */
  public List<T9FinanceApplyRecord> printSeqId(Connection dbConn,String seqIdStr) throws Exception {
    String seqId = "";
    ...
    String sql = "select p.SEQ_ID as SEQ_ID,p.APPLY_CLAIMER as APPLY_CLAIMER,p.APPLY_MONEY as APPLY_MONEY,"
      ...
      + " left outer join department b on x.dept_id=b.seq_id where p.SEQ_ID in (" + seqId + ")";
    ResultSet rs = null;
    PreparedStatement stmt = null ; 
    T9FinanceApplyRecord record = null;
    List<T9FinanceApplyRecord> list = new ArrayList<T9FinanceApplyRecord>();                                                                  
    try {
      stmt = dbConn.prepareStatement(sql);
      rs = stmt.executeQuery();
      while (rs.next()) {
        record = new T9FinanceApplyRecord(); 
  ...
  public void updateExpense(Connection dbConn,String seqIdStr) throws Exception {
    ...
    String sql = "update finance_apply_record set IS_PRINT='1' where SEQ_ID in (" + seqId + ")";
    ResultSet rs = null;
    PreparedStatement stmt = null ;                                                                 
    try {
      stmt = dbConn.prepareStatement(sql);
  ...
  public void updateExpense2(Connection dbConn,String seqIdStr) throws Exception {
    ...
    String sql = "update finance_apply_record set expense='1' where SEQ_ID in (" + seqId + ")";
    ResultSet rs = null;
    PreparedStatement stmt = null ;                                                                 
    try {
      stmt = dbConn.prepareStatement(sql);
  ...
  public String chequeTotalByBudgetId (Connection dbConn,String budgetId) throws Exception {
    String sql = "select sum(APPLY_MONEY) as TOTAL from finance_apply_record where BUDGET_ID in ('" + budgetId + "')";
    ResultSet rs = null;
    PreparedStatement stmt = null ;  
   ...
   * @throws Exception 
   */
  public void addFlow(Connection dbConn,T9FinanceApplyRecord record) {
    String sql = "insert into finance_apply_record(APPLY_CLAIMER,APPLY_DATE,APPLY_YEAR,APPLY_ITEM,BUDGET_ID,APPLY_MONEY,"
