package t9.subsys.oa.assessmentScore.logic;
...
public class T9AssessmentScoreLogic {
  private static Logger log = Logger
      .getLogger("t9.subsys.oa.assessmentScore.logic.T9AssessmentScoreLogic.java");
  ...
  public String getDayList(Connection dbConn, String beginTime, String endTime)
    ...
    // 相隔多少天
    SimpleDateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd");
    long daySpace = T9Utility.getDaySpan(dateFormat1.parse(beginTime),
  ...
  public double getAttendScoreOld(Connection conn, String year, String month,
      String userId, T9Person person) throws Exception {
    T9AttendConfig attend = selectConfigById(conn, String.valueOf(person
        .getDutyType()));
...
//        - totalScore;
  ...
  public static List<T9AttendScore> selectData(Connection dbConn,String[] str) throws Exception {
    T9ORM orm = new T9ORM();
    List<T9AttendScore> dataList = new ArrayList<T9AttendScore>();
    dataList = orm.loadListSingle(dbConn, T9AttendScore.class, str);
    return dataList;
  }
  
  public static List<T9AttendFill> selectFill(Connection dbConn,String[] str) throws Exception {
    T9ORM orm = new T9ORM();
    List<T9AttendFill> dataList = new ArrayList<T9AttendFill>();
    dataList = orm.loadListSingle(dbConn, T9AttendFill.class, str);
    return dataList;
  }
  ...
  public double getAttendScore(Connection conn, String year, String month,
    ...
    double type5 = 0;
    double type6 = 0;
    String[] str = {"USER_ID=" + userId + " and "+T9DBUtility.getYearFilter("CREATE_TIME", T9Utility.parseDate(ymd)) + " order by CREATE_TIME, REGISTER_TYPE asc"};
    List<T9AttendScore> scorelist = selectData(conn, str);
    
    String[] strFill = {"PROPOSER = " + userId + " and "+T9DBUtility.getYearFilter("FILL_TIME", T9Utility.parseDate(ymd)) + " and ASSESSING_STATUS = '1' order by FILL_TIME, REGISTER_TYPE asc"};
    List<T9AttendFill> scoreFill = selectFill(conn, strFill);
  ...
  public double getMonthAttendScoreLogic(Connection conn, String year, String month,
    ...
    double type5 = 0;
    double type6 = 0;
    String[] str = {"USER_ID=" + userId + " and "+T9DBUtility.getMonthFilter("CREATE_TIME", T9Utility.parseDate(ymd)) + " order by CREATE_TIME, REGISTER_TYPE asc"};
    List<T9AttendScore> scorelist = selectData(conn, str);
    
    String[] strFill = {"PROPOSER = " + userId + " and "+T9DBUtility.getMonthFilter("FILL_TIME", T9Utility.parseDate(ymd)) + " and ASSESSING_STATUS = '1' order by FILL_TIME, REGISTER_TYPE asc"};
    List<T9AttendFill> scoreFill = selectFill(conn, strFill);
  ...
  public double getMonthTotalScore(Connection conn, String year, String month,
      ...
      throws Exception {
    T9CoefficientLogic cof = new T9CoefficientLogic();
...
//    double chiefCof = cof.getChiefScoreLogic(conn); // 处长主关分系数//    double awardCof = cof.getAwardScoreLogic(conn); // 奖惩分系数//    double yearCof = cof.getYearScoreLogic(conn); // 年终考核分系数    SimpleDateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd");
  ...
  public String getAttendFillStr(Connection conn, String beginDate,
      String endDate, String userId, String registerType) throws Exception {
    String result = "0";
    String sql = " select ATTEND_FLAG from ATTEND_FILL where PROPOSER ='"
        + userId + "' and "
        + T9DBUtility.getDateFilter("FILL_TIME", beginDate, ">=") + " and "
        + T9DBUtility.getDateFilter("FILL_TIME", endDate, "<=")
        + " and REGISTER_TYPE = '" + registerType
  ...
  public String getDirectorScoreStr(Connection conn, String year, String month,
    ...
    String sql = " select SCORE from SCORE_SHOW where CHECK_FLAG = '0' and PARTICIPANT='" + userId + "' and "
        + T9DBUtility.getMonthFilter("RANK_DATE", T9Utility.parseDate(ymd));
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
  ...
  public String getDirectorScoreYearStr(Connection conn, String year,
    ...
    String sql = " select SCORE from SCORE_DATA where CHECK_FLAG = '1' and PARTICIPANT='" + userId + "' and "
        + T9DBUtility.getYearFilter("RANK_DATE", T9Utility.parseDate(ymd));
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
      ps = conn.prepareStatement(sql);
  ...
  public double getNoCheckInfo(Connection dbConn, String date) throws Exception {
    ...
    date = date + "-01" + "-01";
    try {
      String sql = "select count(*) from ATTEND_NO_CHECK where "
          + T9DBUtility.getYearFilter("NO_CHECK_DATE", T9Utility
              .parseDate(date));
  ...
  public double getStaffYearScoreLogic(Connection dbConn, String year,
    ...
    ResultSet rs = null;
    try {
      String sql = "select ADD_SCORE,REDUCE_SCORE,YEAR_SCORE from HR_STAFF_INCENTIVE where "
          + T9DBUtility.findInSet(userIdStr, "STAFF_NAME")
          + " and "
          + T9DBUtility.getYearFilter("INCENTIVE_TIME", T9Utility
              .parseDate(ymd));
      stmt = dbConn.prepareStatement(sql);
      rs = stmt.executeQuery();
      while (rs.next()) {
        String yearScoreStr = rs.getString("YEAR_SCORE");
        if (T9Utility.isNullorEmpty(yearScoreStr) || "0".equals(yearScoreStr)) {
          yearScore = 0;
        } else {
  ...
  public double getMonthScoreLogic(Connection dbConn, String year,
    ...
    try {
      String sql = "select ADD_SCORE,REDUCE_SCORE,YEAR_SCORE from HR_STAFF_INCENTIVE where "
          + T9DBUtility.findInSet(userIdStr, "STAFF_NAME")
          + " and "
          + T9DBUtility.getMonthFilter("INCENTIVE_TIME", T9Utility
              .parseDate(ymd));
      stmt = dbConn.prepareStatement(sql);
      rs = stmt.executeQuery();
      while (rs.next()) {
