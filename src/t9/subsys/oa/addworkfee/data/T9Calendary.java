package t9.subsys.oa.addworkfee.data;
/**
 * 
 */
...
public class T9Calendary {
	 ...
	 */
	public static boolean isFestival(List<T9Festival> fest, String date, String pattern) throws ParseException{
		SimpleDateFormat df = new SimpleDateFormat(pattern);
		if(fest == null || fest.size() == 0){
			return false;
		}
		for(int i=0; i<fest.size(); i++){
			T9Festival fet = fest.get(i);
   ...
   * @throws ParseException 
   */
  public static boolean isChangeRest(List<T9ChangeRest> fest, String date, String pattern) throws ParseException{
    SimpleDateFormat df = new SimpleDateFormat(pattern);
    if(fest == null || fest.size() == 0){
      return false;
    }
    for(int i=0; i<fest.size(); i++){
      T9ChangeRest fet = fest.get(i);
      Date comp = df.parse(date);
      if(comp.getTime() <= fet.getEndDate().getTime() && comp.getTime() >= fet.getBeginDate().getTime()){
        return true;
      }
   ...
   * @throws ParseException 
   */
  public static boolean isChangeRest(List<T9ChangeRest> fest, Date date, String pattern) throws ParseException{
	 ...
	 * @throws ParseException 
	 */
	public static boolean isFestival(List<T9Festival> fest, Date date, String pattern) throws ParseException{
		String time = parseDate(date, pattern);
	 ...
	 * @throws Exception
	 */
	public static void checkDateType(String begin, String end, String pattern, List<T9Festival> fest) throws Exception{
		double diff = getDateDiff(begin, end, T9Calendary.PATTERN);
		if(diff == 0){
			Date d = afterDate(begin, "yyyy-MM-dd", 0);
			if(isWeekend(d, T9Calendary.PATTERN)){
				System.out.println(parseDate(d, null)+"是周末");
			}
			if(isFestival(fest, d,  T9Calendary.PATTERN)){
		...
		for(int i=0; i<diff; i++){
			...
			if(isWeekend(d, T9Calendary.PATTERN) && isFestival(fest, d,  T9Calendary.PATTERN)){
	 ...
	 * @throws ParseException
	 */
	public static List<Date> getFestivalList(T9Festival fest, String begin, String end) throws ParseException{
		if(fest == null){
			return null;
		}
	 ...
	 * @throws ParseException
	 */
	public static Map<String, List<Date>> getFestivalMap(T9Festival fest, String begin, String end) throws ParseException{
		if(fest == null){
			return null;
		}
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, List<Date>>> getAllFestivalMap(List<T9Festival> fest, String begin, String end) throws Exception{
	...
	public static void main(String[] args) throws ParseException{
		...
		SimpleDateFormat df = new SimpleDateFormat(T9Calendary.PATTERN);
