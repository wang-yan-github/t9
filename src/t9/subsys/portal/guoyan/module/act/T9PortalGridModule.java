...
import t9.subsys.portal.guoyan.module.logic.T9PortalGridModuleLogic;
/**
 * 如果你使用过Servlet这个层就相当于action层
 * act层只是对Servlet的ACTION层进行了封装
 * get/post通过请求到这层，地址类似于:/t9/subsys/portal/guoyan/module/T9PortalGridModule/loadGridData.act.
 * 分解为 1.类名称：t9.subsys.portal.guoyan.module.act.T9PortalGridModule
 *       2.方法名称：loadGridData
 *       3.后缀：.act
 * 返回的数据分两种情况
 ...
 *
 */
public class T9PortalGridModule {
  /**
   * 加载列表数据
   * @param request
   * @param response
  ...
  public String loadGridData(HttpServletRequest request,
    ...
    try {
      ...
      T9PortalGridModuleLogic lal = new T9PortalGridModuleLogic();
      String data = lal.loadGridDataLogic(dbConn, request.getParameterMap()).toString();
      /**
       *    页面得到的数据
       *    
       *    js代码 ： 
       *       var url = contextPacth + "/t9/subsys/portal/guoyan/module/T9PortalGridModule/loadGridData.act";
  ...
  public String loadOneData(HttpServletRequest request,
    ...
    try {
       ...
       * T9ActionKeys.RET_STATE 即上面rtState的值，0(T9Const.RETURN_OK)表示正常，1(T9Const.RETURN_ERROR)表示错误
       * T9ActionKeys.RET_MSRG  即上面rtMsrg的值
       * T9ActionKeys.RET_DATA  即上面rtData的值
       * ******/
  ...
  public String downloadImage(HttpServletRequest request,
    ...
    try {
      ...
      T9NtkoStream ns = new T9NtkoStream();
