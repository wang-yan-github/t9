...
public class T9ImageManage{
  ...
  public String getDescription(){
    if(T9Utility.isNullorEmpty(description) || "null".equalsIgnoreCase(description)){
      description = "";
    }
  ...
  public String getSubject(){
    if(T9Utility.isNullorEmpty(subject) || "null".equalsIgnoreCase(subject)){
      subject = "";
    }
    return subject;
  }
  ...
  public String toJson() throws Exception{
    StringBuffer sb = new StringBuffer();
     sb.append("{");
       ...
       sb.append("\"description\":").append("\"").append(T9Utility.encodeSpecial(description)).append("\"").append(",");
    ...
    if(!T9Utility.isNullorEmpty(attachmentName) && !T9Utility.isNullorEmpty(attachmentId)){
        ...
        for(int i=0; i<len; i++){
          if(!T9Utility.isNullorEmpty(attrName[i]) && !T9Utility.isNullorEmpty(attrId[i]) && isPicture(attrName[i])){
            String absolutePate = getBaseContext() + imgBasePath + "attachmentId="+attrId[i]+"&module=" + T9NewsCont.MODULE + "&attachmentName="+T9Utility.encodeURL(attrName[i]);
