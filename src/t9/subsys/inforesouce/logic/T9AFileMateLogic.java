...
public class T9AFileMateLogic{
  ...
  public String findMate(int seqId, String fileString,Connection dbConn) throws Exception{
    ...
    try{
      ...
      if(number>100){ // 从file_attrs02 表查询 
        nums = "" + number;
        fileTable = "file_attrs02";
      }else {         //从file_attrs01 表查询 
        fileTable = "file_attrs01";
        if(number<10){
          nums = "00"+number;
        } else{
          nums = "0"+number;
  ...
  public  String findFileNameAndMoudle(Connection dbConn, String fileId)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    String sql ="select FILE_PATH,MODULE_NO from sign_files where FILE_ID ='"+ fileId +"'";
    ps = dbConn.prepareStatement(sql);
    rs = ps.executeQuery();
    StringBuffer sb = new StringBuffer();
    sb.append("{");
    if(rs.next()){
      sb.append("\"fileName\":").append("\"").append(T9Utility.encodeSpecial(paraFilePath(rs.getString("FILE_PATH"), fileId))).append("\"").append(",");
  ...
  public Map<String, String> findAFileMate(Connection dbConn, String pName, String addrName, String orgName)throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    String sql ="select  NUMBER_ID, CHNAME from mate_type where CHNAME = '"+ pName +"' or CHNAME = '"+ addrName +"' or CHNAME = '"+ orgName +"'";
    ps = dbConn.prepareStatement(sql);
    rs = ps.executeQuery();
  ...
  public String mateJaon(Connection dbConn, Map<String, String> map, String attachmentId) throws Exception{
    PreparedStatement ps = null;
    ResultSet rs = null;
    String sql = "select " + getColum(map) +" from file_attrs01 a join ( select  SEQ_ID from sign_Files where FILE_ID='"+ attachmentId +"') b on a.FILE_SEQ_ID=b.SEQ_ID";
    StringBuffer sb = new StringBuffer();
    ps = dbConn.prepareStatement(sql);
    rs = ps.executeQuery();
    sb.append("{");
    if(rs.next()){
       sb.append("name").append(":").append("'").append(T9Utility.encodeSpecial(subAString(rs.getString(1)))).append("',");
       sb.append("area").append(":").append("'").append(T9Utility.encodeSpecial(subAString(rs.getString(2)))).append("',");
       sb.append("orge").append(":").append("'").append(T9Utility.encodeSpecial(subAString(rs.getString(3)))).append("'");
    }
    sb.append("}");
    return sb.toString();
  }
  ...
  public String mateJson(Connection dbConn, String attrmentId)throws Exception{
    Map<String, String> map = findAFileMate(dbConn, T9FileMateConstUtil.userName, T9FileMateConstUtil.areaName, T9FileMateConstUtil.Org);
  ...
  public static String  getColum(Map<String, String> map){
    String colums = "";
       colums += "a.attr_" + paramNumber(map.get(T9FileMateConstUtil.userName));
       colums += "," + "a.attr_" + paramNumber(map.get(T9FileMateConstUtil.areaName));
       colums += "," + "a.attr_" + paramNumber(map.get(T9FileMateConstUtil.Org));
    return colums;
  }
  ...
  public static void main(String[] args) throws Exception{
   // String filePath= "D:\\t9\\attach\\email\\1005\\31545a4aaf1d6b274826a2a392146fd5_文档 9765.docx";
