...
public class T9ImageMetaLogic {
  ...
  private static final String BASE_PATH = "D:"+ profix +"project"+ profix +"t9"+ profix +"attach"+ profix ;
  public Map<String , String> readImageMeta(Connection conn, String imagePath) throws Exception {
    ...
    List<T9MateType> list = this.getMetaProperty(conn, 2);
    this.defAttaMap();
    for (T9MateType metaType : list) {
  ...
  public List<T9MateType> getMetaProperty(Connection conn , int typeId) throws Exception {
    List<T9MateType> list = new ArrayList<T9MateType>();
    String query = "SELECT * FROM MATE_TYPE WHERE PARENT_ID=0 and " +T9DBUtility.findInSet(""+ typeId, "ELEMENT_TYP");
    Statement stm = null;
    ResultSet rs = null;
    try {
      ...
      while(rs.next()) {
        ...
        T9MateType meta = new T9MateType();
  ...
  public  Map json2Map(String json) throws Exception {
    ...
    List<String> tmpArray = T9RegexpUtility.splitJson(json);
  ...
  public String getImageMeta(Connection conn , String fileId) throws Exception {
    ...
    if (!T9Utility.isNullorEmpty(typeQuery)) {
      String query = "select " + typeQuery + " from FILE_ATTRS02 where FILE_SEQ_ID = (SELECT SEQ_ID FROM SIGN_FILES WHERE FILE_ID = '" + fileId + "')";
      Statement stm = null;
      ResultSet rs = null;
      try {
        stm = conn.createStatement();
  ...
  public String mapToXml(Connection conn , Map<String , String> valueMap , Map<String , String> map) throws Exception {
    ...
    for (String key : keySet) {
      String value = valueMap.get(key);
        ...
        String query = "select value_name from mate_value where type_number='" + numberId + "'";
        Statement stm = null;
        ResultSet rs = null;
        try {
          stm = conn.createStatement();
          ...
          while (rs.next()) {
            ...
            if (!T9Utility.isNullorEmpty(valueName)) {
