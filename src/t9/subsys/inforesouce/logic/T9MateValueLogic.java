...
public class T9MateValueLogic{
  ...
  public int deleteMateVale(Connection dbConn,  int seqIda ) throws Exception{
    ...
    ResultSet rs = null; 
    int flag =0;
    String sql = "delete from mate_value where seq_id="+seqIda;
    //T9Out.println(sql);
    try{
  ...
  public int updateMate(Connection dbConn, int seqIda, int seqIdb)throws Exception{
    ...
    ResultSet rs = null; 
    int flag =0;
    String sql = "update mate_type set value_range='"+updateRange(dbConn,seqIda+"",seqIdb)+"' where seq_id="+seqIdb;
    //T9Out.println(sql);
    try{
      ps = dbConn.prepareStatement(sql);
      flag = ps.executeUpdate();
