...
public class T9MateUtil{
  ...
  public  static String filterSub(String str, String profix,String userId){    
    ...
    for(int i=0; i<sub.length; i++){      
      if(T9StringUtil.isEmpty(sub[i])){
        continue;
      }     
      if(sub[i].indexOf("_")==-1 && T9StringUtil.isNotEmpty(sub[i]) && !"|".equalsIgnoreCase(sub[i])){//“_”过滤子元素下的值域 如1_V102 就是子元素下的值域，添上sub[i].indexOf("_")==-1 就过滤掉了
  ...
  public  static void self(String str, String profix, String userId){
    ...
    if(subs != null && subs !="," && subs.length()>0){
      ...
      for(int i=0; i<child.length; i++){
        if(T9StringUtil.isEmpty(child[i])){
          continue;
        }
  ...
  public static String findParents(String parent){
    ...
    if(T9StringUtil.isNotEmpty(parent)){
      ...
      for(int i=0; i<ps.length; i++){
        ...
        if(T9StringUtil.isNotEmpty(astr)){
          newPIds += astr +",";
        }
      }
  ...
  public static void main(String[] args){    
    ...
    String teststr = "M1----A---------128_,M1-128_V103,M1-128_V104,M1-128_12,M1-128_15,M1-128_178|M1-129_,M1-129_11,M1-129_V51,M1-129_2,M1-129_27,M1-129_1";
      T9MateUtil.self(teststr, "M1-129_","123");
    //System.out.println("子元素为:"+T9MateUtil.getMateMap().get("123_M1-129_sub"));
    //System.out.println("值域是为:"+T9MateUtil.getMateMap().get("123_M1-129_rage"));
  }
}
