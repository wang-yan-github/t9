...
import t9.subsys.inforesouce.docmgr.data.T9DocConst;
import t9.subsys.inforesouce.docmgr.data.T9DocNext;
...
public class T9DocReceiveLogic{
   ...
   * @throws Exception 
   */
  public List<T9DocReceive> myReadDocReceive1(Connection conn, T9Person user, int status ,boolean hasEnd) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
  ...
  public Map<String, String> fileUploadLogic(T9FileUploadForm fileForm,
    ...
    try {
      ...
      while (iKeys.hasNext()) {
        ...
        while (T9DiaryUtil.getExist(filePath + "\\" + hard, fileName)) {
          rand = T9DiaryUtil.getRondom();
          fileName = rand + "_" + fileName;
        }
        result.put(hard + "_" + rand, fileNameV);
        fileForm.saveFile(fieldName, filePath + "\\" + T9DocConst.MODULE + "\\" + hard + "\\" + fileName);
      }
    } catch (Exception e) {
      throw e;
    }
...
//多文件上传时用到的----
  ...
  //处理文件
    ...
    String path = T9DocConst.filePath + "\\"  + tmp[0] + "\\" + tmp[1] + "_" + attName;
    File file = new File(path);
    if(file.exists()){
      file.delete();
    } else {
      //兼容老的数据
      String path2 = T9DocConst.filePath + "\\"  + tmp[0] + "\\" + tmp[1] + "." + attName;
   ...
   * @throws Exception 
   */
  public String myDocReceiveJson(Connection conn, T9Person user, int seqId) throws Exception{
    String sql = " select  dr.SEQ_ID, dr.DOC_NO, dr.RES_DATE, dr.FROMUNITS, dr.OPPDOC_NO, dr.TITLE, dr.COPIES, dr.CONF_LEVEL, dr.INSTRUCT, dr.SPONSOR, " +
                         " dr.RECE_USER_ID, dr.DOC_TYPE, dr.STATUS, dr.SEND_STATUS, dr.CREATE_USER_ID,P.USER_NAME ,dr.ATTACHNAME,dr.ATTACHID, dr.RUN_ID " +
                         " from DOC_RECEIVE dr, PERSON P  " +
  ...
  public String getFlowId(Connection conn, String runId) throws Exception{
    String sql = "select FLOW_ID, RUN_ID, END_TIME from flow_run where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getMaxProcsId(Connection conn, String runId)throws Exception{
    String sql = "select max(PRCS_ID) as prcdId from flow_run_prcs where run_id="+ runId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getFlowPrcs(Connection conn, String prcsId) throws Exception{
    String sql = "select FLOW_PRCS from flow_run_prcs where PRCS_ID="+ prcsId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
  ...
  public String getPrcsName(Connection conn, String flowPrcs, String flowId)throws Exception{
    String sql = "select PRCS_NAME from FLOW_PROCESS where PRCS_ID = "+ flowPrcs +" and FLOW_SEQ_ID ="+ flowId;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try{
      ps = conn.prepareStatement(sql);
