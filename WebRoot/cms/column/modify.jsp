...
function doInit(){
  var url = "<%=contextPath%>/t9/cms/column/act/T9ColumnAct/getColumnDetail.act?seqId=<%=seqId%>";
...
function getTemplate(stationId){
  var url = "<%=contextPath%>/t9/cms/station/act/T9StationAct/getTemplate.act?stationId="+stationId;
	var rtJson = getJsonRs(url);
	if(rtJson.rtState == "0"){
  ...
  var url = "<%=contextPath%>/t9/cms/column/act/T9ColumnAct/getTemplate.act?stationId="+stationId;
	var rtJson = getJsonRs(url);
	if(rtJson.rtState == "0"){
		var prcs = rtJson.rtData;
...
function doSubmit(){
  if(checkForm()){
    var url = "<%=contextPath%>/t9/cms/column/act/T9ColumnAct/updateColumn.act";
...
function checkPath(){
  ...
  var url = "<%=contextPath%>/t9/cms/column/act/T9ColumnAct/checkPath.act?stationId="+$('stationId').value+"&parentId="+$('parentId').value+"&seqId="+$('seqId').value+"&columnPath="+$('columnPath').value;
  var rtJson = getJsonRs(url);
	if(rtJson.rtState == "0"){
	  var prcs = rtJson.rtData;
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td><img src="<%=imgPath %>/notify_new.gif" align="middle"><span class="big3"> 编辑栏目（ID:<%=seqId %>）</span>&nbsp;&nbsp;
    </td>
  </tr>
</table>
<br>
<form action="" method="post" name="form1" id="form1">
  ...
  <input type="hidden" name="columnIndex" id="columnIndex" >
    ...
    <tr align="center" class="TableControl">
      <td colspan=4 nowrap>
        <input type="hidden" name="dtoClass" id="dtoClass" value="t9.cms.column.data.T9CmsColumn">
        <% if("1".equals(disable)){ %>
