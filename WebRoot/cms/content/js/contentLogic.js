...
function opts(cellData, recordIndex, columIndex){
  ...
  if(isIssuedCms == 0){
    switch(contentStatus){
      case 0 : control = release + update + deleteStr;break;
      case 1 : control = release + update + deleteStr;break;
      case 2 : control = update + deleteStr;break;
      case 3 : control = release + receive;break;
      case 4 : control = release + update + deleteStr;break;
    ...
    switch(contentStatus){
      ...
      case 2 : control = update + deleteStr;break;
      case 3 : control = release + receive;break;
      case 4 : control = issued + no + update + deleteStr;break;
      case 5 : control = toSee + receive;break;
    }
  }
  return "<center>"
...
function doEdit(seqId){
	location.href = contextPath + "/cms/content/modify.jsp?seqId=" + seqId+"&stationId="+stationId;
}

function toIssued(seqId){
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toIssued.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
...
function toNo(seqId){
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toNo.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
...
function toRelease(seqId){
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toRelease.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    if(rtJson.rtData == 1){
      alert("发布成功！")
...
function toReleaseColumn(seqId){
  var requestURLStr = contextPath + "/t9/cms/column/act/T9ColumnAct/toRelease.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    if(rtJson.rtData == 1){
      alert("发布成功！")
...
function toReceive(seqId){
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toReceive.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
...
function toSee(seqId){
  var requestURLStr = contextPath + "/t9/cms/column/act/T9ColumnAct/toSee.act?seqId=" + seqId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    if(rtJson.rtData == 1){
      alert("该站点模板配置不正确！");
    ...
    newWindow(URL,window.screen.availWidth-10, window.screen.availHeight-30);
  }else{
    alert(rtJson.rtMsrg); 
  }
}
...
function deleteSingle(seqId){
	...
	var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct";
...
function checkBoxRender(cellData, recordIndex, columIndex){
  ...
  var status = this.getCellData(recordIndex,"contentStatus");
  if(status==5){
	  return "<center><input disabled  type=\"checkbox\" name=\"deleteFlag2\" value=\"" + diaId + "\" onclick=\"checkSelf("+status+")\" ></center>";
  }else{
	  return "<center><input type=\"checkbox\" name=\"deleteFlag\" value=\"" + diaId + "\" onclick=\"checkSelf("+status+")\" ></center>";
  }
}
