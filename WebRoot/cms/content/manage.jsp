...
<%@ page import="t9.cms.content.logic.T9ContentLogic" %>
<%
...
//判断参数
String IS_ISSUED_CMS = T9ContentLogic.isIssued(request);
%>
...
function doInit(){
  var url = "<%=contextPath%>/t9/cms/content/act/T9ContentAct/getContentList.act?stationId="+stationId+"&columnId="+columnId;
  var cfgs = {
    dataAction: url,
...
function toTop(seqId, contentTop){
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toTop.act?seqId="+seqId+"&contentTop="+contentTop;
...
function toSort(seqId, recordIndex, flag){
  ...
  var requestURLStr = contextPath + "/t9/cms/content/act/T9ContentAct/toSort.act?seqId="+seqId+"&toSeqId="+toSeqId+"&flag="+flag+"&columnId="+columnId;
  var rtJson = getJsonRs(requestURLStr);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
