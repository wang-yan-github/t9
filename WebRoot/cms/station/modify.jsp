...
var flag = "<%=flag %>";
function doInit(){
  var url = "<%=contextPath%>/t9/cms/station/act/T9StationAct/getTemplate.act?stationId=<%=seqId %>";
  ...
  var url = "<%=contextPath%>/t9/cms/station/act/T9StationAct/getStationDetail.act?seqId=<%=seqId%>";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function doSubmit(){
  if(checkForm()){
    var url = "<%=contextPath%>/t9/cms/station/act/T9StationAct/updateStation.act";
  	...
  	if(rtJson.rtState == "0"){
  	  ...
  	  if(flag==1){
  	  	...
  	  	location.href = "/t9/cms/station/manage.jsp";
  	  }
  	}
  }
}
...
function checkPath(){
  ...
  var url = "<%=contextPath%>/t9/cms/station/act/T9StationAct/checkPath.act?seqId="+$('seqId').value+"&stationPath="+$('stationPath').value;
  var rtJson = getJsonRs(url);
	if(rtJson.rtState == "0"){
	  var prcs = rtJson.rtData;
...
<form action="" method="post" name="form1" id="form1">
    ...
    <tr align="center" class="TableControl">
      <td colspan=4 nowrap>
        <input type="hidden" name="dtoClass" id="dtoClass" value="t9.cms.station.data.T9CmsStation">
