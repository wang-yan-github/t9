...
function checkForm(){
	...
	if(!$("EDIT_USER_CONTENT").checked && !$("APPROVAL_USER_CONTENT").checked && !$("RELEASE_USER_CONTENT").checked && !$("RECEVIE_USER_CONTENT").checked && !$("ORDER_CONTENT").checked ){
		alert("请指定设置选项!");
		return false
	}
	return true;
...
function sendForm(){
	...
	if(opt=="addPriv"){
		if(checkForm()){
		  var url=requestURL + "/setContentBatchPriv.act?seqId=<%=seqId%>&idStr=" + idStr + "&check=" + check + "&opt=" + opt + "&override=" + override+"&flag=station";
...
<table class="TableBlock" width="100%" align="center">
  ...
  <tr>
    ...
    <td class="TableData">
      <input type="checkbox" name="check" id="EDIT_USER_CONTENT" value="EDIT_USER_CONTENT" checked><label for="EDIT_USER_CONTENT">编辑权限</label><br>
      <input type="checkbox" name="check" id="APPROVAL_USER_CONTENT" value="APPROVAL_USER_CONTENT" checked><label for="APPROVAL_USER_CONTENT">审批权限</label><br>
      <input type="checkbox" name="check" id="RELEASE_USER_CONTENT" checked value="RELEASE_USER_CONTENT"><label for="RELEASE_USER_CONTENT">发布权限</label><br>
      <input type="checkbox" name="check" id="RECEVIE_USER_CONTENT" value="RECEVIE_USER_CONTENT" checked><label for="RECEVIE_USER_CONTENT">回撤权限</label><br>
      <input type="checkbox" name="check" id="ORDER_CONTENT" checked value="ORDER_CONTENT"><label for="ORDER_CONTENT">排序权限</label><br>
    </td>
  </tr>
  <tr>
   ...
   <tr style="display:none">
    ...
    <td class="TableData">
      <input type="checkbox" name="override" id="override" value="override" checked><label for="override">重置所有下级子栏目的权限</label><br>
    </td>
  </tr> 
  <tr>
