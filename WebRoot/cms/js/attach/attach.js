...
function addImagMenu(cntrl,cntrlId,attachPre){
  ...
  var attachUrls = $( attachPre + 'attachUrl').value;
  ...
  if(ids.length > 0){
    var imgMenuItems = getImageMenu(names,ids,moduel,urls);
...
function getImageMenu(names,ids,moduel,urls){
  var imageMenusItem = new Array(); 
  for(var i = 0 ; i < names.length ; i ++){
    var item = getImageMenuItem(names[i],ids[i],moduel,urls[i]);
    imageMenusItem.push(item);
  }
  return imageMenusItem;
}
function getImageMenuItem(name,id,moduel,url){
  var extData = {name:name,id:id,moduel:moduel,url:url};
