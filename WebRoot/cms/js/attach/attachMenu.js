...
T9JsAttachMenu.prototype = {
  ...
  */
  ...
  config : function(cfgs) {
    ...
    this.attachUrls = cfgs.attachUrls;
  ...
  show: function(){
    if(this.container){
      var attachHTML1 = this.attachHtml(this.attachNames,this.attachIds,this.attachUrls);
      if (attachHTML1) {
        $(this.container).innerHTML = attachHTML1;
        this.bindAttachMenu(this.attachNames,this.attachIds,this.attachUrls);
      }
    }
  },
  showTable: function(){
    ...
    if(this.container){
      ...
      if (attachHTML1.length) {
        ...
        if(this.checked){
          ...
          attachHTML1 = '<div onclick="window.location.href=\''+url+'\'">' + attachHTML1 + '</div>';
        }
        td1.innerHTML = attachHTML1;
   ...
   */
  bindAttachMenu: function(attachNames, attachIds,attachUrls) {
    if (!attachNames) {
      return;
    }
    var rtStr = "";
   ...
   * 取得附件的HTML
   */
  attachHtml: function(attachNames, attachIds,attachUrls) {
    if (!attachNames) {
      return "";
    }
    var rtStr = "";
    ...
    for (var i = 0; i < nameArray.length; i++) {
      ...
      rtStr += "<div>" 
        ...
        + "<input type=\"hidden\" value=\"" + url + "\" id=\"attachLink_" + this.code   + prcsName + "_url\">"
...
function InserImgToCentent(dom,cntrlId,extData){
  ...
  var cntrlattId = $(cntrlId + "_id");
  var cntrlMoudel = $(cntrlId + "_moudel");
  var cntrlUrl = $(cntrlId + "_url");
  //cntrl =  document.getElementById(cntrlId);
  var attachId = cntrlattId.value;
  var attachName = cntrlattName.value;
  var moudel = cntrlMoudel.value;
  var attachUrl = cntrlUrl.value;
  //insertImgc(attachName,attachId,moudel);
  insertImgc(attachUrl);
}
/**
 * 转存附件
 * @param dom
...
function deleteAttach(dom,cntrlId,extData,j){
  ...
  var pra = cntrl.up();
  var isDel =   deleteAttachBackHand(attachName,attachId,attachUrl,seqId,j);
  if(isDel){
...
function insertImgc(attachUrl){
	try{
		InsertImage(attachUrl);
	}catch(e){
		alert("");
	}
}
 ...
 * @return
 */
function attachMenuUtil(container,moduleName,deleteAction,attachNames,attachIds,attachUrls,readOnly,prec , code,isCanPrint,canDown,opFlag,seqId){
 ...
 * @return
 */
function attachMenuSelfUtil(container,moduleName,attachNames,attachIds,attachUrls,prec , code,seqId,selfdefMenu){
  var cfgs = {container:container
      ,moduleName:moduleName
      ,attachIds:attachIds
      ,attachNames:attachNames
