...
package {
	...
	public class SWFUpload extends Sprite {
		...
		private var imageSizeLimit:Number;
        private var audioSizeLimit:Number;
        private var videoSizeLimit:Number;
        private var otherSizeLimit:Number;
		...
		private var IMAGE_SIZE_TOO_BIG:Number		 = 2;
		private var AUDIO_SIZE_TOO_BIG:Number		 = 3;
		private var VIDEO_SIZE_TOO_BIG:Number		 = 4;
		private var OTHER_SIZE_TOO_BIG:Number		 = 5;
		...
		private var ERROR_CODE_IMAGE_EXCEEDS_SIZE_LIMIT:Number 		= -140;
		private var ERROR_CODE_AUDIO_EXCEEDS_SIZE_LIMIT:Number		= -150;
		private var ERROR_CODE_VIDEO_EXCEEDS_SIZE_LIMIT:Number 		= -160;
		private var ERROR_CODE_OTHER_EXCEEDS_SIZE_LIMIT:Number 		= -170;
		// Upload Errors
		...
		public function SWFUpload() {
			...
			try{
				this.SetImageSizeLimit(String(root.loaderInfo.parameters.imageSizeLimit));
			} catch(ex:Object){
				this.imageSizeLimit = 0;
			}
			try{
				this.SetAudioSizeLimit(String(root.loaderInfo.parameters.audioSizeLimit));
			} catch(ex:Object){
				this.audioSizeLimit = 0;
			}
			try{
				this.SetVideoSizeLimit(String(root.loaderInfo.parameters.videoSizeLimit));
			} catch(ex:Object){
				this.videoSizeLimit = 0;
			}
			try{
				this.SetOtherSizeLimit(String(root.loaderInfo.parameters.otherSizeLimit));
			} catch(ex:Object){
				this.otherSizeLimit = 0;
			}
		...
		private function SetupExternalInterface():void {
			try {
				...
				ExternalInterface.addCallback("SetImageSizeLimit", this.SetImageSizeLimit);
				ExternalInterface.addCallback("SetAudioSizeLimit", this.SetAudioSizeLimit);
				ExternalInterface.addCallback("SetVideoSizeLimit", this.SetVideoSizeLimit);
				ExternalInterface.addCallback("SetOtherSizeLimit", this.SetOtherSizeLimit);
			...
			if (queue_slots_remaining < file_reference_list.length) {
				...
				for (var i:Number = 0; i < file_reference_list.length; i++) {
					...
					if (is_valid_file_reference) {
						...
						var size_result:Number = this.CheckFileSize2(file_item);
						...
						else if (size_result == this.IMAGE_SIZE_TOO_BIG)
                        {
                            file_item.file_reference = null;
                            this.queue_errors++;
                            this.Debug("Event: fileQueueError : Image exceeds size limit.");
                            ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_IMAGE_EXCEEDS_SIZE_LIMIT, file_item.ToJavaScriptObject(), "Image size exceeds allowed limit.");
                        }
						else if (size_result == this.AUDIO_SIZE_TOO_BIG)
                        {
                            file_item.file_reference = null;
                            this.queue_errors++;
                            this.Debug("Event: fileQueueError : Audio exceeds size limit.");
                            ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_AUDIO_EXCEEDS_SIZE_LIMIT, file_item.ToJavaScriptObject(), "Audio size exceeds allowed limit.");
                        }
						else if (size_result == this.VIDEO_SIZE_TOO_BIG)
                        {
                            file_item.file_reference = null;
                            this.queue_errors++;
                            this.Debug("Event: fileQueueError : Video exceeds size limit.");
                            ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_VIDEO_EXCEEDS_SIZE_LIMIT, file_item.ToJavaScriptObject(), "Video size exceeds allowed limit.");
                        }
						else if (size_result == this.OTHER_SIZE_TOO_BIG)
                        {
                            file_item.file_reference = null;
                            this.queue_errors++;
                            this.Debug("Event: fileQueueError : Other exceeds size limit.");
                            ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_OTHER_EXCEEDS_SIZE_LIMIT, file_item.ToJavaScriptObject(), "Other size exceeds allowed limit.");
                        }
		...
		****************************************************************** */
		...
		private function SetImageSizeLimit(size:String):void {
			var value:Number = 0;
			var unit:String = "kb";
			...
			this.imageSizeLimit = value * multiplier;
		}
		private function SetAudioSizeLimit(size:String):void {
			var value:Number = 0;
			var unit:String = "kb";
			...
			this.audioSizeLimit = value * multiplier;
		}
		private function SetVideoSizeLimit(size:String):void {
			var value:Number = 0;
			var unit:String = "kb";
			...
			this.videoSizeLimit = value * multiplier;
		}
		private function SetOtherSizeLimit(size:String):void {
			var value:Number = 0;
			var unit:String = "kb";
			...
			this.otherSizeLimit = value * multiplier;
		}
		...
		*************************************************************** */
		...
		//重写CheckFileSize,判断文件类型
		private function CheckFileSize2(StopUpload:FileItem) : Number
        {
			var fileType:String ;
			fileType=this.GetFileExt(StopUpload.file_reference.name);
            if (StopUpload.file_reference.size == 0)
            {
                return this.SIZE_ZERO_BYTE;
            }// end if
			if (this.fileSizeLimit != 0 && StopUpload.file_reference.size > this.fileSizeLimit)
			{
				return this.SIZE_TOO_BIG;
			}// end if
			else if (this.imageSizeLimit != 0 && StopUpload.file_reference.size > this.imageSizeLimit && (fileType=="jpg" || fileType=="jpeg" || fileType=="bmp" || fileType=="gif" || fileType=="png"))
			{
				return this.IMAGE_SIZE_TOO_BIG;
			}// end if
			else if (this.audioSizeLimit != 0 && StopUpload.file_reference.size > this.audioSizeLimit && (fileType=="mp3" || fileType=="wav"))
			{
				return this.AUDIO_SIZE_TOO_BIG;
			}// end if
			else if (this.videoSizeLimit != 0 && StopUpload.file_reference.size > this.videoSizeLimit && (fileType=="rmvb" || fileType=="avi" || fileType=="mp4" || fileType=="flv"))
			{
				return this.VIDEO_SIZE_TOO_BIG;
			}// end if
			else if (this.otherSizeLimit != 0 && StopUpload.file_reference.size > this.otherSizeLimit && fileType!="jpg" && fileType!="jpeg" && fileType!="bmp" && fileType!="gif" && fileType!="png" && fileType!="mp3" && fileType!="wav" && fileType!="rmvb" && fileType!="avi" && fileType!="mp4" && fileType!="flv" )
			{
				return this.OTHER_SIZE_TOO_BIG;
			}// end if
            return this.SIZE_OK;
        }
		//获取当前文件的后缀名
		...
		private function PrintDebugInfo():void {
			...
			debug_info += "Image Size limit:       " + this.imageSizeLimit + " bytes\n";
			debug_info += "Audio Size limit:       " + this.audioSizeLimit + " bytes\n";
			debug_info += "Video Size limit:       " + this.videoSizeLimit + " bytes\n";
			debug_info += "Other Size limit:       " + this.otherSizeLimit + " bytes\n";
