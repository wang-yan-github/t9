...
function deleteLeave(seqId){
  ...
  if(window.confirm(msg)){
    var requestURL =  "<%=contextPath%>/t9/custom/attendance/act/T9PersonalLeaveAct/deleteLeaveById.act?seqId=" + seqId;
...
function distroyLeave(seqId,allow){
  ...
  window.location.href = "<%=contextPath%>/t9/custom/attendance/act/T9PersonalLeaveAct/updateAllow.act?seqId=" + seqId + "&allow=" + allow+"&checkLeave="+checkLeave;
}
function doOnload(){
  var moblieRemindFlag = getMoblieSmsRemind();
  var requestURL = "<%=contextPath%>/t9/custom/attendance/act/T9PersonalLeaveAct/selectLeave.act";
  var rtJson = getJsonRs(requestURL);
  if(rtJson.rtState == "1"){
    alert(rtJson.rtMsrg); 
    return ;
