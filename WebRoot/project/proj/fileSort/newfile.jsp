  ...
  String actionFlag = (String) request.getParameter("actionFlag");
	...
	if(newAttachNameStr==null){
	  newAttachNameStr="";
		...
		newAttachNameStr=T9Utility.encodeSpecial(newAttachNameStr);
	}
	 ...
	 //alert(ATTACHMENT_div);
	  if(ATTACHMENT_div){
	  	...
	  	if(checkForm()){
	  		...
	  		var url=requestURL+"/addNewFileInfo.act?seqId="+ seqId + "&projId="+projId+"&fileType=" + fileType +"&contentId=<%=contentId%>&folderPath=<%=folderPath%>&smsPerson="+smsPerson + "&mobileSmsPerson=" + mobileSmsPerson;
	  	  //var URL=url;
	  	  var enUrl=encodeURI(url);
	  		...
	  		if(rtJson.rtState == '0'){
	  	  	...
	  	  	location="<%=contextPath%>/project/proj/fileSort/folder.jsp?seqId="+seqId+"&sortName=<%=folderPath%>&projId="+projId;
	  	  }else{
	  		 	  alert(rtJson.rtMsrg); 
	  		}	
	  	}
...
function newAttach(){
  var oEditor = FCKeditorAPI.GetInstance('fileFolder') ;
	...
	if(checkForm()){
		...
		location.href = "<%=contextPath%>/t9/project/file/act/T9ProjFileAct/createFile.act?" + pars + "&seqId=<%=seqId%>&contentId=<%=contentId%>";
...
function editNewAttach(){
  ...
  //alert("newAttId>>" + '<%=newAttachIdStr%>' +"  newAttName>>>" + '<%=newAttachNameStr%>');
  	office(newAttachNameStr,newAttachIdStr,"proj_file",4,'','');
  }
}
...
function showAttName(conStr){
	...
	var url=requestURL + "/getFileInfoById.act?seqId="+conStr;
	...
	if(prcsJson.length>0){
	  ...
	  $("subject").value=prcs.subject;
	   ...
	   //attachMenuUtil(container,moduleName,deleteAction,attachNames,attachIds,readOnly,prec , code,isCanPrint,canDown,opFlag,seqId)
			...
			//attachMenuUtil("attr","proj_file",null,$("returnAttName").value ,$("returnAttId").value,false,"","","","","",'<%=contentId%>');
	  ...
	  var  selfdefMenu = {
		...
		attachMenuSelfUtil(attr,"proj_file",$('returnAttName').value ,$('returnAttId').value, '','','<%=contentId%>',selfdefMenu);
...
function moblieSmsRemind(remidDiv,remind,type){
	var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=" + type; 
	var rtJson = getJsonRs(requestUrl); 
	if(rtJson.rtState == "1"){ 
		alert(rtJson.rtMsrg); 
		return ; 
...
<input type="hidden" value="t9.project.file.data.T9ProjFile"
	name="dtoClass">
<table class="TableBlock" width="100%" align="center">
	...
	<tr>
		<td  class="TableData" valign="top">文件内容：</td>
		<td class="TableData">
	...
	<tr height="25">
		...
		<td class="TableData"><script>ShowAddFile();</script> <script>ShowAddImage('','img');</script>
			...
			<input type="hidden" id="moduel" name="moduel" value="proj_file">
	...
	<tr id="smsRemindDiv" style="display: none">
		...
		<input type="radio"	name="SMS_SELECT_REMIND" id="SMS_SELECT_REMIND0" value="0" onclick="document.getElementById('SMS_SELECT_REMIND_SPAN').style.display='';"
			checked="checked"><label for="SMS_SELECT_REMIND0">手动选择被提醒人员</label> 
			<!-- 
		...
		<textarea cols="40"	name="userDesc" id="userDesc" rows="2" class="BigStatic" wrap="yes"	readonly="readonly"></textarea>
	...
	<tr id="sms2RemindDiv" style="display: none">
		...
		<input type="radio"	name="moblieSMS_SELECT_REMIND" id="moblieSMS_SELECT_REMIND0" value="0" onclick="document.getElementById('moblieSMS_SELECT_REMIND_SPAN').style.display='';"
			checked="checked"><label for="moblieSMS_SELECT_REMIND0">手动选择被提醒人员</label> 
			<!--  暂不使用
		...
		<input type="hidden"	name="user1" id="user1" value="">
		 <textarea cols="40"	name="userDesc1" id="userDesc1" rows="2" class="BigStatic" wrap="yes"	readonly="readonly"></textarea> 
	...
	<tr align="center" class="TableControl">
		...
		<input type="button" value="返回" class="BigButton"	onClick="location='<%=contextPath%>/project/proj/fileSort/folder.jsp?seqId=<%=seqId%>&sortName=<%=folderPath%>'">
		</td>
	</tr>
</table>
</form>
...
<div>
<form id="formFile"	action="<%=contextPath%>/t9/project/file/act/T9ProjFileAct/newFileSingleUpload.act?seqId=<%=seqId%>&contentId=<%=contentId%>&folderPath=<%=folderPath%>"
