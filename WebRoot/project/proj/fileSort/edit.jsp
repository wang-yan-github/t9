  ...
  if(ATTACHMENT_div){
  	...
  	if(checkForm()){
  		...
  		if(rtJson.rtState == '0'){
  	  	...
  	  	location="<%=contextPath%>/project/proj/fileSort/folder.jsp?seqId="+seqId+"&sortName=<%=sortName%>";
  	  }else{
  		 	  alert(rtJson.rtMsrg); 
  		}	
  	}
...
function newAttach(){
	...
	if(checkForm()){
		...
		location.href = "<%=contextPath%>/t9/project/file/act/T9ProjFileAct/newFile.act?" + pars +"&seqId=<%=seqId%>&contentId=<%=contentId%>";
...
function doInit(){
	...
	var url=requestURL + "/getFileInfoById.act?seqId="+contentId;
...
function renameFile(attachName,attachId,moudel){
	...
	//newWindow("<%=contextPath%>/project/proj/fileSort/rename.jsp?seqId=<%=seqId%>&DISK_ID=" + getPath() + "&FILE_NAME=" + file_list);
	var url=contextPath + "/project/proj/fileSort/renameFile.jsp?contentId=<%=contentId%>&attachName=" + encodeURIComponent(attachName) +"&attachId=" + attachId + "&moudel=" + moudel;
	//alert(url);
...
<input type="hidden" value="t9.core.funcs.filefolder.data.T9FileContent" name="dtoClass">
...
<input type="checkbox" name="SMS_REMIND" id="SMS_REMIND"><label for="SMS_REMIND">发送事务提醒消息</label>&nbsp;&nbsp;      </td>
  ...
  <tr align="center" class="TableControl">
    ...
    %>
     	<input type="button" value="返回" class="BigButton" onClick="location='<%=contextPath%>/project/proj/fileSort/query.jsp?seqId=<%=seqId %>'">
    <%
    	}else{
    %>
...
<div>
 <form id="formFile" action="<%=contextPath%>/t9/project/file/act/T9ProjFileAct/newFileSingleUpload.act?seqId=<%=seqId%>&contentId=<%=contentId %>&actionFlag=edit" method="post" enctype="multipart/form-data">
