...
<%
  String projId = "1";//(String)request.getParameter("seqId")==null?"":(String)request.getParameter("seqId");
%>
...
function  doInit(){
    var  tree  =  new  DTree({bindToContainerId:'content'
        ,requestUrl:'<%=contextPath%>/t9/project/file/act/T9ProjFileSortAct/getAllTree.act?id='
        ,isOnceLoad:false
...
function getUrl(id){  
		if(id){
			...
			var url="<%=contextPath%>/project/proj/fileSort/folder.jsp?seqId=" + id +"&sortName="+$('-a-'+id).innerHTML+"&projId="+projId;
			parent.file_main.location.href=url;
				}
		}
	}
