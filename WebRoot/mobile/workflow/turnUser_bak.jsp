...
<div class="container">
	<div class="tform tformshow">
		<div class="read_detail">
			<em>工作名称/文号：</em><%=runName %></div>
		<div class="read_detail">
			<em>发起人：</em><%=beginUser %></div>
...
<div class="tform tformshow">
	   ...
	   if (T9Utility.isNullorEmpty(preSet) || prcsId ==1) {
		 ...
		 <%}else{  %> 
	      <script type="text/javascript">
	         $("#USER_ZB_<%=prcsIdNext %>").find("em").die("click");
	         $("#USER_CB_<%=prcsIdNext %>").find("em").die("click");
	      </script>
	       <% } %> 
	   </div>
...
<% 
  ...
  //如果是自动选人
    ...
    boolean isAutoSelect = (Boolean)extend.get("isAutoSelect");
    if(isAutoSelect){
    	zb = (List<String[]>)extend.get("prcsOpUser");
    	jb = (List<String[]>)extend.get("prcsUser");
    }
   ...
   <% } else {%>
    <script type="text/javascript">
	         $("#USER_ZB_<%=prcsNext %>").find("em").die("click");
	         $("#USER_CB_<%=prcsNext %>").find("em").die("click");  
	      </script>
   <% } %>
	    </div>      	
