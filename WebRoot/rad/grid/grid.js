 ...
 *          是否同步操作
 */
function T9Grid(hdarray,url,isSynchronous,numOfPage,pageN){
  this.pageN = pageN - 1;
  this.num;
	this.body ;
	this.total ; 
    ...
    this.num = num;
	  ...
	  var dom = document.getElementById("T9_grid_bottomBarId");
	  var gridP = document.getElementById("T9_gridId");
   ...
   */
	function _createBottomBar(){
		...
		gridBottomBar.id = 'T9_grid_bottomBarId';
    ...
    button.page = 0;
    if(grid.pageN){
      button.className = 'T9_grid_bottomBar_firstButton';
    } else {
    ...
    button.onclick = function(){
      ...
      grid.pageN = document.getElementById('T9_grid_bottomBar_inputID').value -1;
    ...
    if(grid.pageN){
      button.className = 'T9_grid_bottomBar_priButton';
    } else {
  ...
  // ----------------------------------------------------------------------------------
	...
	this.reShow = function(url,pageN){
	  ...
	  if(!pageN) {
	    ...
	    pageNum = document.getElementById('T9_grid_bottomBar_priButtonID').page;
	    if(pageNum){
	      pageNum = 0;
	    }
	  }
  ...
  // ----------------------------------------------------------------------------------
	...
	this.addFilter  = function(type,fieldName,op){
	  var filter = new T9Filter(type,fieldName,op);
...
function T9Filter(type,name,op){
  this.type = type;
  this.name = name;
  this.op = op;
}
function T9OprateCon(config){
