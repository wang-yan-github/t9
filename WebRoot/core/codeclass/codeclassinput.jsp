...
function doInit() {
  //传递了seqId，编辑情况  if (sqlId) {
    var url = "<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/getCodeClass.act";
...
function commitItem() {
  ...
  if (sqlId) {
    var url = "<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/updateCodeClass.act";
    ...
    var url = "<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/addCodeClass.act";
    var rtJson = getJsonRs(url, mergeQueryString($("form1")));
    if (rtJson.rtState == "0") {
      alert(rtJson.rtMsrg);
      parent.navigateFrame.location.reload();
