...
<%@ page import="java.util.*,t9.core.codeclass.data.*" %>
...
function delCodeClass(itemId) {
  var url = "<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/deleteCodeClass.act";
...
--%>
    ...
    <%
      ArrayList<T9CodeClass> codeClassList = null;
      codeClassList = (ArrayList<T9CodeClass>)request.getAttribute("codeClassList");
      for(int i = 0; i < codeClassList.size(); i++) {
        T9CodeClass code = codeClassList.get(i);
 ...
 --%>
	  ...
	  <td>	   
        <a href="<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/listCodeItem.act?seqId=<%=code.getSqlId() %>"  target="contentFrame">下一级</a>
	  </td>
	  <td>
	    <%
