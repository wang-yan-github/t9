...
function delCodeItem(itemId) {
  var url = "<%=contextPath %>/t9/core/codeclass/act/T9CodeClassAct/deleteCodeItem.act";
 ...
 <br>
    ...
    <%
      ...
      for(int i = 0; i < codeList.size(); i++) {
        T9CodeItem code = codeList.get(i);
    %>	
	<tr class="<%=((i % 2 == 0) ? "TableLine1" : "TableLine2") %>">
	  ...
	  <td>
		<a href="<%=contextPath %>/core/codeclass/codeiteminput.jsp?sqlId=<%=code.getSeqId() %>&classNo=<%=classNo %>&codeClassId=<%=seqId %>" >编辑</a>
	  </td>
	  <td>
	    <a href="javascript:delCodeItem('<%=code.getSeqId() %>');" onclick="return confirmDel()">删除</a>
	  </td>
	</tr>
    <%
      }
