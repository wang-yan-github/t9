...
<%@ page import="java.util.*,t9.core.funcs.person.data.*" %>
...
{
	var urlRequest = '<%=contextPath%>/t9/core/module/priv_select/act/T9PrivSelectAct/selectPriv.act?PRIV_ID=' 
...
function loadData()
{
	var url = "<%=contextPath%>/t9/core/module/priv_select/act/T9PrivSelectAct/getUserPriv.act?TO_ID=" 
...
function getTree()
{
  var url =  contextPath + "/t9/core/module/dept_select/act/T9DeptSelectAct/getTree.act?DEPT_PAR_ID=";
...
function getDeptInfo(id)
{
  ...
  var url = '<%=contextPath%>/t9/core/module/dept_select/act/T9DeptSelectAct/selectDept.act?DEPT_PAR_ID=' + id 
...
{
   parent.user.location="/t9/core/module/user_select/selected.jsp?TO_ID=<%=TO_ID%>&TO_NAME=<%=TO_NAME%>";
}
...
function CheckSend()
{
  ...
  if(kword.value == "" && $('search_icon').src.indexOf("/t9/core/module/user_select/quicksearch.gif") == -1)
	{
	   $('search_icon').src = "/t9/core/module/user_select/quicksearch.gif";
	}
	if(key != kword.value && kword.value != "")
	{
     key = kword.value;
	   parent.user.location = "/t9/core/module/user_select/query.jsp?" + kword.value;
	   if($('search_icon').src.indexOf("/t9/core/module/user_select/quicksearch.gif") >= 0)
	   {
	   	   $('search_icon').src = "/t9/core/module/user_select/closesearch.gif";
...
<div style="border:1px solid #000000;margin-left:2px;background:#FFFFFF;">
  <input type="text" id="kword" name="kword" value="按用户名或姓名搜索..." onfocus="ctroltime=setTimeout(CheckSend,100);" onblur="clearTimeout(ctroltime);if(this.value=='')this.value='按用户名或姓名搜索...';" class="SmallInput" style="border:0px; color:#A0A0A0;width:145px;"><img id="search_icon" src="/t9/core/module/user_select/quicksearch.gif" align=absmiddle style="cursor:pointer;">
</div>
<div id="menuList">
</div>
