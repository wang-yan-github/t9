...
function doInit(){
  ...
  var requestURL = "<%=contextPath%>/t9/core/esb/server/taskstatus/act/T9TaskStatusAct/getUploadPage.act";
  var cfgs = {
    ...
    colums: [
     ...
     {type:"text", name:"guid", text:"GUID", width:'15%',align:'center'},
      {type:"text", name:"fromId", text:"发送方", width:'5%',align:'center',render:userNameFunc},
       ...
       {type:"text", name:"createTime", text:"发送开始时间", width:'10%',align:'center' ,render:splitDateTime},
       {type:"text", name:"completetime", text:"发送结束时间", width:'10%',align:'center' ,render:splitDateTime},
       {type:"text", name:"message", text:"标题", width:'27%',align:'center'}
       ]
  };
...
function statusRender(cellData, recordIndex, columInde) {
  if (cellData * 1 >= 1 && cellData * 1 < 5) {
    return ["发送中", "<span style=\"color:green;\">发送完毕</span>","<span style=\"color:blue;\">接收完毕</span>", "<a href=\"javascript:void(0)\" onclick=\"failedMessage("+recordIndex+")\" style=\"color:red;\" title=\"点击查看错误详情\">发送失败</span>"][cellData * 1 - 1];
  }
  else {
    return "";
  }
...
&nbsp;
发送状态：
  <select  id="uplodType" name="uplodType" >
  <option value="">所有</option>
  <option value="1">发送中</option>
  <option value="4">发送失败</option>
  <option value="2">发送完毕</option>
  <option value="3">接收完毕</option>
  </select>
