...
function doInit(){
  ...
  var requestURL = "<%=contextPath%>/t9/core/esb/server/taskstatus/act/T9TaskStatusAct/getDownloadPage.act";
  var cfgs = {
    dataAction: requestURL,
    container: "listDiv",
    paramFunc: getParam,
    colums: [
       {type:"text", name:"seqId", text:"流水号", width:'6%',align:'center',render:serialNumber},
       ...
       {type:"text", name:"createTime", text:"接收开始时间", width:'10%',align:'center' ,render:splitDateTime},
       {type:"text", name:"completetime", text:"接收结束时间", width:'10%',align:'center' ,render:splitDateTime},
       {type:"text", name:"message", text:"标题", width:'20%',align:'center'},
       {type:"selfdef", text:"操作", width: '10%',render:opts1}]
  };
...
function opts1(cellData, recordIndex, columIndex){
  ...
  var str = "<center>"
    + "<a href=javascript:failTask('"+ guid +"'," + toId + ")>任务挂起</a>&nbsp;"
    str += "<a href=javascript:redown('"+ guid +"'," + toId + ")>重新下载</a>&nbsp;";
  str +="</center>";
  return  str ;
}
function ipAddress() {
...
function redown(guid , toId) {
  window.open("redown.jsp?guid=" + guid + "&toId=" + toId);
}
function failTask(guid , toId) {
  window.open("failTask.jsp?guid=" + guid + "&toId=" + toId);
}
...
function ext(){
  ...
  location =  "<%=contextPath%>/t9/core/esb/server/taskstatus/act/T9TaskStatusAct/expDown.act?"+pars;
}
</script>
</head>
...
接收状态：
  ...
  标题：<input type="text" value="" id="message" name="message" size=40	>
  </td> 
  </tr>
  <tr class="TableLine1">
  <td align="left">查询类型：
 ...
 发送时间： 从
  ...
  &nbsp;发送方账号：<input type="text" value="" id="userCode" name="userCode"> 
