...
function opts(cellData, recordIndex, columIndex){
  var seqId = this.getCellData(recordIndex,"seqId");
  var userCode = this.getCellData(recordIndex,"userCode");
  var moniter = this.getCellData(recordIndex,"moniter");
  ...
  if (moniter == '1') {
    str+= "<a href=javascript:stopMoniter('" + userCode + "')>停止监控</a>";
  } else {
    str+= "<a href=javascript:moniter('" + userCode + "')>监控</a>";
  }
  str += "&nbsp;<a href=javascript:updatePwd("+seqId+")>修改密码</a>";
  str +="</center>";
  return  str ;
}
function updatePwd(seqId) {
 window.open("updatePwd.jsp?seqId=" + seqId);
}
/**
 * 全选
...
function userNameFunc(cellData, recordIndex, columIndex){
  ...
  var url = contextPath + "/t9/core/esb/server/user/act/TdUserAct/getUserName.act";
