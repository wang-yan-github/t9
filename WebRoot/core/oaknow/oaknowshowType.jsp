...
<%
	T9CategoriesType aType = (T9CategoriesType)request.getAttribute("aType");//分类列表
...
function gotoDiv(typeId, flag,crrNo){
  ...
  var rtJson = getJsonRs("<%=contextPath %>/t9/core/oaknow/act/T9OAKnowTypeAct/findTypeAjax.act",param);
  var jsonParse = rtJson["rtData"].length; 
  var temp = "";  
  if(jsonParse != 0){
     for(i=0; i<jsonParse; i++){
      var url="<%=contextPath%>/t9/core/oaknow/act/T9OAAskReferenceAct/findAskRef.act?askId=";
