...
function  pager(currNo){
	...
	window.location.href = "<%=contextPath%>/t9/core/oaknow/act/T9OAKnowPanelAct/leftPanel.act?" + param;
}
...
function deleteAsk(askId){
  ...
  if(window.confirm(msg)){
    ...
    var url = "<%=contextPath %>/t9/core/oaknow/act/T9OAKnowPanelAct/deleteAsk.act?"+param;
    window.location.href = url;
  }
}
</script>
    <%
    List<T9OAAsk> asks = (List<T9OAAsk>)request.getAttribute("asks");
       ...
       <TR class="header">
         ...
         <TD width="15%">操作</TD>   
           ...
           <%for(int i =0; i< asks.size(); i++){ %>
       	     	<TR>
					       <TD class="tctd tctd1">&nbsp;<a href="<%=contextPath%>/t9/core/oaknow/act/T9OAAskReferenceAct/findAskRef.act?askId=<%=asks.get(i).getSeqId()%>" target="_blank"><%=asks.get(i).getAsk() %></a></TD>
					       <TD class="tctd"><%=asks.get(i).getCreateDateStr()%></TD>
					       		...
					       		<%if(asks.get(i).getStatus()==0){%>					       		 
					       		  ...
					       		  <TD class="tctd">
								       	 <a href="<%=contextPath%>/t9/core/oaknow/act/T9OAKnowPanelAct/goToEditPage.act?askId=<%=asks.get(i).getSeqId()%>">编辑</a>
