...
function parseJson(json){
  ...
  for(var i=0; i<catelogies.length; i++){
		temp += "<li><a href=\"<%=contextPath%>/t9/core/oaknow/act/T9CategoriesAct/goToCategoty.act?seqId="+ catelogies[i].seqId+"\">"+ catelogies[i].name +"</a></li>";
		for(var j=0; j<catelogies[i].list.length; j++){
		  temp += "<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"<%=contextPath%>/t9/core/oaknow/act/T9CategoriesAct/goToCategoty.act?seqId="+catelogies[i].list[j].seqId+"\">"+ catelogies[i].list[j].name +"</a></li>";
		}
  }
  $('tree').innerHTML = temp;
}
...
function ajax(){
  ...
  var url = "<%=contextPath %>/t9/core/oaknow/act/T9CategoriesAct/findCategory.act";
...
function gotofirst(){
  window.location.href="<%=contextPath%>/t9/core/oaknow/act/T9CategoriesAct/goToCategoty.act";
}
...
function deleteType(id){
  var msgstr = "此操作将删除该分类及分类中的数据，是否继续？";
	if(confirm(msgstr))
	{	
	  var url ="<%=contextPath%>/t9/core/oaknow/act/T9CategoriesAct/deleteType.act?seqId=" + id;
	  var rtJson = getJsonRs(url);
	  if (rtJson.rtState == "0") {
	    window.location.href = "<%=contextPath%>/t9/core/oaknow/act/T9CategoriesAct/goToCategoty.act?seqId=";
	  }else {
	    alert("保存失败");
	    return;
	  }
