...
/*var f = new T9Field("dd","cc");
var f1 = new T9Field("dd","cc");
var f2 = new T9Field("dd","cc");
...
//将json数据解析成一组T9Recod对象 返回一个包含T9Recod对象的数组
 ...
 */
function T9Grid(domId,hdarray,url,isSynchronous,stytl){
	this.data ;
	this.herder ;
	this.el ;
	
}
T9Grid.prototype.init = _init ; 
T9Grid.prototype.rendTo = _rendTo ; 
T9Grid.prototype.createGrid = _createGrid ; 
...
function _createGrid(recods,stytl,hdarray){
	...
	with(gridTitleDiv){
		setAttribute('id','T9_grid_title');
		setAttribute('align','left');
	}
	
	with(gridDataDiv){
		setAttribute('id','T9_grid_data');
	}
	
	with(gridBarDiv){
		setAttribute('id','T9_grid_bar');
	}
	
	with(gridTable){
		setAttribute('id','T9_grid_table');
	}
