...
function addTab(id, title, url, selected) {
  $('#tabs_container').tabs('add', {
    ...
    selected: selected,
    style: 'height: 100%;',
    content: '<iframe src="' + url + '?menuModleId=' + id + '" style="height: 100%;width: 100%;" allowTransparency="true" frameborder="0"></iframe>'
  });
...
function updateTheme(themeid){
  ...
  $.ajax({
     ...
     url: contextPath + '/t9/core/frame/act/T9TdoaAct/updateUserParamStyle.act',
     success: function(text) {
       var json = T9.parseJson(text);
       if (json.rtState == "0") {
           flag = true;  
...
function initSearch(){
  $("#keyword").bind('keyup',function(){
    ...
    $.ajax({
      ...
      url: contextPath + "/t9/core/funcs/search/act/T9FrameSerach/searchUser.act",
      success: function(text) {
        var json = T9.parseJson(text);
        if (json.rtState == "0") {
