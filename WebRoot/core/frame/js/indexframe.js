...
function checkShortMsrg() {
  ...
  //处理服务器故障或者断网情况
  try {
    rtJson = getJsonRs(contextPath + "/t9/core/funcs/sms/act/T9SmsAct/remindCheck.act");
  }catch (e) {
    //轮训调用下一次检查
...
function dispParts(src){
  ...
  //$('main-body-parts').contentWindow.location = "/t9/core/funcs/menu/menu.jsp";
}
...
function initOnStatus(){
  Ext.Ajax.request({
    url: contextPath + '/t9/core/funcs/setdescktop/userinfo/act/T9UserinfoAct/getOnStatus.act',
    method:'post',
    failure:function(o){},
    success:function(o){
...
function changeStatus(index){
  Ext.Ajax.request({
    url: contextPath + '/t9/core/funcs/setdescktop/userinfo/act/T9UserinfoAct/modifyOnStatus.act',
    method:'post',
...
function submitUserInfo(){
  ...
  if ($('editInfoText').value != '我的留言'){
    ...
    Ext.Ajax.request({
      url: contextPath + '/t9/core/funcs/setdescktop/userinfo/act/T9UserinfoAct/modifyMyStatus.act',
      method:'post',
...
function queryLogoutText(){
  try {
    var url = contextPath + '/t9/core/funcs/setdescktop/syspara/act/T9SysparaAct/queryLogoutText.act';
    var rtJson = getJsonRs(url);
