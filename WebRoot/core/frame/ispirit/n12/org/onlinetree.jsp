...
function doInit(){
 ...
 // onlineView();
  tree = new DTree({requestUrl:'<%=contextPath%>/t9/core/funcs/system/ispirit/n12/org/act/T9IsPiritOrgAct/getOnLineTree.act?MODULE_ID='+ 1 +'&privNoFlag=0&id='
    , isOnceLoad:true
...
function getChildOrEdit(id){
  if(id.indexOf('r')!= -1){
    id=id.substring(1,id.length);
   //dispParts("/t9/core/funcs/userinfo/index.jsp?userId="+id+"&windows=1",0);
    ...
    if(typeof(window.external.OA_SMS) != 'undefined' &&  window.top.bIMLogin== true){  //在精灵中打开
     //  var name =getUserName(id);
       window.external.OA_SMS(id,"aa","SEND_MSG");
    	...
    	//在浏览器中打开
    	var url="/t9/core/funcs/message/smsback.jsp?fromId="+id;
    	 window.external.OA_SMS(url,"","OPEN_URL");
     //window.open (url, 'newwindow', 'height=340, top='+(screen.height-340)/2+',left='+(screen.width-700)/2+', width=700, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=n o, status=no');
...
function getUserName(id){
	var url=contextPath + "/t9/core/funcs/system/ispirit/n12/org/act/T9IsPiritOrgAct/getUserName.act?uid="+id;
