        ...
        // 无新短信提示
          ...
          $('#smsbox_detail_all').click(
              function() {
                ...
                if(""!=sms_id_str){
                openURL(contextPath + '/t9/core/funcs/system/ispirit/sms/act/T9SmsBox/viewDetails.act?smsIds=' + sms_id_str );
                selectedRecvSmsIdStr = selectedSendSmsIdStr = '';
              }
              });
          ...
          $('#smsbox_toolbar_detail').click(
              function() {//alert(selectedRecvSmsIdStr);
                ...
                if(linkIdStr.indexOf("r")==-1 && linkIdStr!=""){
                    markRead(linkIdStr);
	                openURL(contextPath + '/t9/core/funcs/system/ispirit/sms/act/T9SmsBox/viewDetails.act?smsIds=' + linkIdStr);
	                newSmsArray = array;
	                FormatSms(); 
	                close_window();
                }
          ...
          $('#smsbox_send_msg')
              .click(
                  function() {
                    ...
                    jQuery.ajax( {
                      type : 'POST',
                      url : contextPath + '/t9/core/funcs/message/act/T9MessageAct/addMessageBody.act',
                      data : {
                        'user' : user_id,
                        'content' : msg,
                        'smsType' : '0'
...
function LoadSms(flag) {
  ...
  jQuery.ajax( {
    type : 'GET',
    url : contextPath + '/t9/core/funcs/system/ispirit/sms/act/T9SmsBox/getNewsMessageForBox.act',
    data : {
      'FLAG' : flag
    },
    success : function(data) {
...
function RemoveSms(recvIdStr, sendIdStr, del) {
  ...
  var data = '';
  if(del == 0){
    url = contextPath + '/t9/core/funcs/sms/act/T9SmsAct/resetFlag.act?';
    data = {
        'seqId' : recvIdStr,
        'DEL' : del
      };
  }else{
    url = contextPath + "/t9/core/funcs/sms/act/T9SmsAct/delSms.act";
    data = {
        'bodyId' : recvIdStr,
        'deType':1,
        'DEL' : del
...
if (top.T9 && top.T9.isTouchDevice) {
