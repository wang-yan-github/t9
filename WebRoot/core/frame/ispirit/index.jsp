...
<script type="text/Javascript" src="<%=contextPath%>/core/frame/ispirit/js/param.js" ></script>
...
var checkTimeout = parseNumber("<%=smsRef%>", 30) * 1000;
...
var loadHandler = [
    function(el, index) {
      ...
      bodyCards[0] = new T9.layouts.CardLayout({
        el: body,
        lazyLoad: true,
        reload: true,
        loadHandler: [
    ...
    function(el, index) {
      ...
      bodyCards[1] = new T9.layouts.CardLayout({
        el: body,
        lazyLoad: true,
        reload: true,
        loadHandler: [
    ...
    function(el, index) {
      ...
      bodyCards[2] = new T9.layouts.CardLayout({
        el: body,
        lazyLoad: true,
        reload: true,
        loadHandler: [
    ...
    function(el, index) {
     ...
     bodyCards[3] = new T9.layouts.CardLayout({
        el: body,
        lazyLoad: true,
        reload: true,
        loadHandler: [
...
window.card = new T9.layouts.CardLayout({
  el: $('#tabpanelLv1'),
  lazyLoad: true,
...
function checkShortMsrg() { 
  ...
  var url = contextPath + "/t9/core/funcs/message/act/T9MessageAct/remindCheck.act";
  $.ajax({
    type: "GET",
    dataType: "text",
    url: url,
...
function ispirit_js(op_str)
{
   ...
   */    
   ...
   {
	   window.external.OA_SMS("/t9/core/frame/2/index.jsp","","OPEN_URL");
   }
   else if(op_str=="get_client_type")  //获取用户临时状态
   {
     //0,1,2,3
	   //var status = get_client_type(arguments[1], arguments[2]);
     var status = userStates["u-"+arguments[1]];
	   if (status) {
	     return status;
	   }
	   return "";
...
function sms_mon() {   //检查提醒及邮件数目
 ...
 // sms.checkShortMsrg();
//  return;
  var url = contextPath + "/t9/core/funcs/sms/act/T9SmsAct/remindCheck.act";
...
function checkMessage() {   //检查微讯;
	  var url = contextPath + "/t9/core/funcs/sms/act/T9SmsAct/remindCheck1.act";
	  $.ajax({
	    type: "GET",
	    dataType: "text",
	    url: url,
	    success: function(text){
	    	    if(text!="0"){
	    	      var url = contextPath+"/t9/core/funcs/message/act/T9MessageAct/get_msg.act?IM_FLAG=1";
	    	       $.ajax({
	    	         type: "GET",
	    	         dataType: "text",
	    	         url: url,
 ...
 */
 var smsMonInterval;
 var showSmsInterval;
 function set_sms_ref(smsInterval)
 {
	//  alert(smsInterval);
	   // 周期检查提醒
	  // sms_mon = 
	    if (smsMonInterval)
	    	window.clearInterval(smsMonInterval); 
	   if (showSmsInterval)
	    window.clearInterval(showSmsInterval); 
	   smsMonInterval  = setInterval(sms_mon,smsInterval);
	   //周期检查微讯
	  //show_sms =  
      showSmsInterval = setInterval(show_sms,smsInterval);
 }
 ...
 if(module_id == "email"){            // 邮件
    windowtype = 'MAX';
    URL ="/t9/core/funcs/email/index.jsp";        
 }else if(module_id == "person_info"){  //用户信息
    URL ="/t9/core/funcs/userinfo/person.jsp?userId=<%=person.getSeqId()%>&windows=1";    
 }else if(module_id == "index"){        //首页
    windowtype = 'MAX';
 <%  String sessionid = request.getSession().getId(); %>
    URL = "/t9/core/frame/2/index.jsp";   
    //window.open(URL);
    //return ;
 }else if(module_id == "group_msg"){     //群发信息
    windowtype = '416,300';
    URL ="/t9/core/funcs/message/smsgSend.jsp";   
 }
...
function weixun_share(shareFlag,content){
	var url=contextPath + "/t9/core/funcs/system/ispirit/n12/weixun_share/act/T9WeiXunShareAct/addWeiXun.act?content="+  encodeURIComponent(content);
	  $.ajax({
	    type: "GET",
	    dataType: "text",
	    url: url,
...
function setUserStatus(content){
	  var url="/t9/t9/core/funcs/system/ispirit/n12/weixun_share/act/T9WeiXunShareAct/setUserStatus.act?content="+ escape(content);
...
function get_client_type(uId,deptId){ //取得用户状态
	 var url="/t9/t9/core/funcs/system/ispirit/n12/weixun_share/act/T9WeiXunShareAct/getUserStatus.act?uId="+ escape(uId)+"&deptId="+deptId;
	 var status="";
     $.ajax({
       type: "GET",
       async:false,
...
function moduleIE(){
	   ...
	   var url = "/t9/t9/core/funcs/system/security/act/T9SecurityAct/getSecrityImModule.act";
	   $.ajax({
	       type: "GET",
	       async:false,
	       dataType: "text",
...
function getOrgName(){
     var name="";
     var url = "/t9/t9/core/funcs/system/ispirit/n12/org/act/T9IsPiritOrgAct/getOrgName.act";
     $.ajax({
         type: "GET",
         async:false,
         dataType: "text",
