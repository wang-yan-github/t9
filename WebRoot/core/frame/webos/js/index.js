...
function lockScreen() {
  var unlock = T9.getCmp('unlock');
  if (!unlock) {
    unlock = new T9.Window({
      id: 'unlock',
      modal: true,
      width: 300,
      height: 150,
...
$(document).ready(function() {
  ...
  var tray = new T9.Container({
    ...
    items: [
      ...
      new T9.Button({
        ...
        toggleHandler: [
          function(e, t) {
            var win = T9.getCmp('menu');
            if (win) {
              win.show();
              return;
            }
             
            var menuCtn = new T9.Container({
              renderTo: 'body',
              style: {
                width: 257,
                height: 'auto',
            ...
            win = new T9.Window({
              ...
              items: [
                {
                  ...
                  loader: {
                    url: contextPath + '/t9/core/funcs/system/act/T9SystemAct/listMenu.act',
              ...
              listeners: {
                initComponent: {
                  after: function(e, t) {
                    ...
                    t.el.bind('mouseleave', function() {
                      if (T9.getCmp('menuBtn').status.isPressed) {
                        clearTimeout(timer);
                ...
                hide: {
                  'after': function() {
                    T9.getCmp('menuBtn').setStatus('default');
                    menuCtn.el.hide();
                  }
                },
                show: {
      ...
      T9.packCmp({cls: 'btn-sp'})
    ]
  });

  tray.render('.south .left');
  
  var cmd = new T9.Container({
    style: {
      height: '',
      width: ''
    },
    ...
    handler: classicHome
      ...
      function() {
        ...
        T9.getCmp('themeSelect') && T9.getCmp('themeSelect').hide();
      }],
      title: '主题切换'
    }, T9.packCmp({cls: 'btn-sp'}), {
      xtype: 'button',
  ...
  //门户选择
  var url = contextPath + '/t9/core/funcs/portal/act/T9PortalAct/listAllPortals.act';
  var info = $('#info span');
  $.post(url, function(t) {
    var json = T9.parseJson(t);
    var items = [];
    if (json.rtState == '0') {
      json = json.rtData;
      var tip = new T9.Tip({
        ...
        listeners: {
          show: {
            after: function(e, t) {
              T9.getCmp('portalBtn').setStatus('active');
            }
          },
          hide: {
            after: function(e, t) {
              T9.getCmp('portalBtn').setStatus('default');
            }
          }
        },
        delay: 2,
      ...
      $.each(json.records, function(i, e) {
        var btn = new T9.Button({
          ...
          title: e.name,
          handler: function() {
            $.post(contextPath + '/t9/core/funcs/portal/act/T9PortalAct/setDefaultPortal.act', {id: this.id}, function(t) {
              var json = T9.parseJson(t);
              if (json.rtState == '0') {
                winMgr.removeAll();
  ...
  var menu = new T9.Menu({
    ...
    loader: {
      url : contextPath + '/t9/core/frame/act/T9ClassicInterfaceAct/listShortCut.act',
      dataRender: function(data) {
        $.each(data, function(i, e) {
          data[i].icon = contextPath + '/core/frame/webos/styles/icons/' + e.icon;
        });
          ...
          t.el.find('ul').sortable({
            ...
            stop: function(e, ui) {
              setTimeout(function() {
                ...
                //保存顺序
                var url = contextPath + '/t9/core/funcs/setdescktop/shortcut/act/T9ShortcutAct/modifyShortCut.act';
                var idStr = '';
  ...
  if (T9.browser.isIE6 || T9.browser.isIE7) {
...
function reloadShortcut() {
  T9.getCmp("menuContent").reload();
  setTimeout(function() {
    var th = topMenu.el.find('table').height();
    var uh = topMenu.el.find('ul').height();
    var height = th > uh ? th : uh;
    if (height <= 100) {
      T9.getCmp("shortcutBtn").hide();
    }
    else {
      T9.getCmp("shortcutBtn").show();
    }
  }, 0);
}
