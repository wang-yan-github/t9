...
$(document).ready(function() {
  ...
  function changeLayout(no) {
    ...
    window.ctnInfo.ctn = new T9.Container(windowCfg);

    ctnInfo.panels = [];
  }

	function listAllPorts() {
	  var url = contextPath + "/t9/core/funcs/portal/act/T9PortalAct/listAllPorts.act";
	  $.ajax({
	    type: "GET",
	    dataType: "text",
	    url: url,
	...
	if (designType) {
	  ...
	  creator.units.portal.callback = function(w) {
		  ...
		  $.each(ctnInfo.panels, function(i, e) {
			  T9.Event.add(e, 'bind', 'destroy', 'before', function(e, panel) {
...
function addLayout(file, id, parentCmp) {
  $.post("<%= contextPath%>/t9/core/funcs/portal/act/T9PortalAct/viewLayout.act", {file:file}, function(d) {
...
function addPanel(file, id, parentCmp, opts) {
  $.post("<%= contextPath%>/t9/core/funcs/portal/act/T9PortalAct/viewPort.act", {
    file: file,
    id : id
  }, function(d) {
    var cfg = T9.parseJson(d);
 ...
 */
function savePortal() {
  var url = contextPath + "/t9/core/funcs/portal/act/T9PortalAct/updateUserPortal.act";
  ...
  var portalStr = T9.jsonToString(T9.formatJson(t, ctnInfo.ctn));
...
function savePorts() {
  ...
  var url = contextPath + "/t9/core/funcs/portal/act/T9PortalAct/updatePortalPorts.act";
  $.ajax({
    type: "POST",
    dataType: "text",
    data: params,
