...
function AddSignFromURL(key) {
  ...
  if (document.all("DISK_ID").value == "") {
    ...
    URL = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?" + param;
  }  else {
    ...
    URL = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal.act?" + param;
  }
  if (TANGER_OCX_bDocOpen) {
    if (TANGER_OCX_OBJ.IsNTKOSecSignInstalled())
...
function fileExits(param,type){
  ...
  var url = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/fileExists.act?type=" + type + "&" + param;
 ...
 */
 function saveZhaiYao(){
   ...
   var rtJson = getJsonRs(contextPath + "/t9/subsys/inforesouce/act/T9FileMetaSaveAct/saveAbstract.act", queryParam);
   if (rtJson.rtState == "0") {
     alert("摘要保存成功!");
   }else {
     alert("摘要保存失败!");
