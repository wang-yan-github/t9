...
function resetSms(smsBodyId,content,smsType,remindUrl){
  ...
  var url = contextPath + "/t9/core/funcs/sms/act/T9SmsAct/getSmsBodyContent.act";
...
function bindStauts(smsBodyId){
  var obj;
  var html = "";
  var url = "<%=contextPath%>/t9/core/funcs/sms/act/T9SmsAct/getStauts.act";
  var rtJson = getJsonRs(url, "bodyId=" + smsBodyId);
  if (rtJson.rtState == "0") {
    obj = rtJson.rtData;
  }
...
<table cellscpacing="1" cellpadding="3" width="100%" id="sentSmscon" class="TableList">
  <tr class="TableHeader">
   ...
   <tr class="<%= (i % 2 == 0) ? "TableLine1" : "TableLine2" %>">
     ...
     <td>
     <%if("0".equals(smsType)){ %>
     <img src="<%=imgPath%>/collapsed.gif" align="absMiddle" title="点击展开消息记录" style="cursor:pointer;" id="img4_history_<%=smsBodyId%>" onclick="redHistory('history_<%=smsBodyId%>',history_con_<%=smsBodyId%>,<%=fromId%>,'<%=T9Utility.getDateTimeStr(sendTime)%>',0,'<%=smsBodyId%>');">
     <%} %>
