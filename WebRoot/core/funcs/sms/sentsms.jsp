...
<%
  ...
  List<T9SmsBody> smsList = (List<T9SmsBody>)request.getAttribute("sbList");
  Map<T9SmsBody,List<T9Sms>> smsMap = (Map<T9SmsBody,List<T9Sms>>)request.getAttribute("DataMap");
%>
...
function loadDataAction(obj){
  ...
  window.location.href = "<%=contextPath %>/t9/core/funcs/sms/act/T9SmsAct/listAllSms.act?pageNo=" + pageNo + "&pageSize=" + pageSize ;
}
...
<table cellscpacing="1" cellpadding="3" width="100%" id="sentSmscon">
  <tr class="TableHeader">
   ...
   int i = 0;
   int k = 0;
     for(T9SmsBody smsBody: smsList) {
       List<T9Sms> smslist = smsMap.get(smsBody);
       int smsSeqId = 0;
