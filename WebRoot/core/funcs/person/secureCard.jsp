...
var pageMgr = null;
function doInit() {
  var url = "<%=contextPath%>/t9/core/funcs/person/act/T9SecureCardAct/getSecureCard.act";
  var cfgs = {
      dataAction: url,
...
function bindUser(recordIndex,keySn){
	...
	var url = "<%=contextPath%>/t9/core/funcs/person/act/T9SecureCardAct/bindUser.act?userId="+$("userName_bind"+recordIndex).value+"&keySn="+keySn;
...
function passwordsyn(recordIndex){
  ...
  var url = "<%=contextPath%>/t9/core/funcs/person/act/T9SecureCardAct/passwordsyn.act?password="+$("password_"+recordIndex).value+"&seqId="+seqId;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
	  alert(rtJson.rtData)
    window.location.reload();
...
function deleteAll(){
  ...
  var requestURLStr = contextPath + "/t9/core/funcs/person/act/T9SecureCardAct";
...
<table id="beSortTable" class="TableList" width="100%">
   <tr class='TableControl'>
     <td colspan='10'>
       ...
       <input type='button' value='解除绑定' class='SmallButtonW' onClick='unBind();' title='解除绑定'>
     </td>
   </tr>
</table>
</div>
