...
function checkUserId() {
  ...
  var url = contextPath + "/t9/core/funcs/person/act/T9PersonAct/selectPerson.act";
  if (seqId) {
    if (userIdOld != userId) {
...
function deptFunc(){
  var url = "<%=contextPath%>/t9/core/funcs/dept/act/T9DeptAct/getDeptSelectFunc.act";
...
function doInit() {
  ...
  if(userIdStr != "admin"){
    var url = contextPath + "/t9/core/funcs/person/act/T9PersonAct/getUserPrivByNo.act";
...
function loadData() {    
  if (seqId) {
    var url = "<%=contextPath%>/t9/core/funcs/person/act/T9PersonAct/getOffDutyPerson.act";
...
function modulePriv(){
  ...
  window.open('<%=contextPath%>/core/funcs/modulepriv/index.jsp?uid='+seqId+'&userName='+encodeURIComponent(userNameFunc) , 'newwindow', 
      'height=500, width=700, top='+height+', left='+width+', toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no');
}
...
function checkByname() {
  ...
  var url = contextPath + "/t9/core/funcs/person/act/T9PersonAct/selectPersonName.act";
  if (seqId) {
    if (bynameOld != byname) {
...
//    }
   if ($("bynameOld") && $("byname")) {
    ...
    var url = "<%=contextPath%>/t9/core/funcs/person/act/T9PersonAct/updatePerson.act?seqId="+seqId+"&deptId="+deptId+"&deptIdStr="+deptIdStr+"&notLoginFlag="+notLoginFlag;
    ...
    if (rtJson.rtState == "0") {
      ...
      if (rtJson.rtData[0].num >= 1) {
        ...
        var url = "<%=contextPath%>/t9/core/funcs/person/act/T9PersonAct/addPerson.act?deptIdLoca="+deptId;
        var rtJson = getJsonRs(url, mergeQueryString($("userInfoForm")));
        if (rtJson.rtState == "0") {
...
function setBindInfo(){
  var url = "<%=contextPath%>/t9/core/funcs/person/act/T9BindUsersAct/bindInfo.act?userId=" + seqId;
  var json = getJsonRs(url);
  if (json.rtState == "0"){
    if (json.rtData != 0){
 ...
 */
function removeBind(){
  var url = "<%=contextPath%>/t9/core/funcs/person/act/T9BindUsersAct/removeBind.act?userId=" + seqId;
  var json = getJsonRs(url);
...
function windowClose(){
    window.parent.document.getElementById("iframe").click();
}
</script>
</head>
<body onload="doInit();">
...
%> 
  <input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.person.data.T9Person"/>
...
<span>
  ...
  <input type="button" value="关 闭" class="BigButton" title="关闭窗口" onclick="windowClose();">
</span>
<%
  }else {
%>
