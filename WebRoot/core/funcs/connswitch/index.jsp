...
<script type="text/javascript">
 function doInit(){
	  var url = contextPath + "/t9/core/funcs/connswitch/act/T9ConnSwitchAct/getConnectingDbms.act";
...
function warm(){
   ...
   if(window.confirm("请确认是否要切换到"+dbms+"数据库，请谨慎！")){
	    var url = contextPath + "/t9/core/funcs/connswitch/act/T9ConnSwitchAct/getSwitchDbms.act";
	    var rtJson = getJsonRs(url,"dbms="+dbms);
	    if(rtJson.rtState == "0"){
		    if(window.confirm("成功切换到"+dbms+",是否进入登陆界面？")){
		    	 window.open('/t9/login.jsp') ;
			  }
	    }else{
	      alert( rtJson.rtMsrg);
	    }
