...
function doInit(){
    var url = contextPath + "/t9/core/funcs/connswitch/act/T9ConnSwitchAct/getMysqlConnInfo.act";
...
function testDbConnect(){
    var url = contextPath + "/t9/core/funcs/connswitch/act/T9ConnSwitchAct/testMysqlConnect.act";
    var param="";
...
function saveDbConnect(){
    if(window.confirm("确认是否要保存Mysql配置信息！")){
      var url = contextPath + "/t9/core/funcs/connswitch/act/T9ConnSwitchAct/saveMysqlConnect.act";
      var param="";
      param="hostName="+$("hostName").value+"&socket="+$("socket").value+"&dbase="+$("dbase").value+"&uid="+$("uid").value+"&pwd="+$("pwd").value;
       var rtJson = getJsonRs(url,param);
        if(rtJson.rtState == "0"){ 
