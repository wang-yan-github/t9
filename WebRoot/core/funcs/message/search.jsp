...
<%
  ...
  ArrayList<T9SmsBody> bodyList = (ArrayList<T9SmsBody>)request.getAttribute("bodySearchList");
%>
...
var change = "<%=change%>";
  function doInit() {
    ...
    var url = "<%=contextPath%>/t9/core/funcs/sms/act/T9SmsAct/search.act";
  ...
  function deleteAllCheckBoxs() {
    ...
    if (rtJson.rtState == "0") {
      alert(rtJson.rtMsrg); 
      window.location.reload();
      //var url = "<%=contextPath %>/t9/core/funcs/sms/act/T9SmsAct/listAllSms.act";
      //getJsonRs(url, null);
    }else {
      alert(rtJson.rtMsrg); 
    } 
...
<table cellscpacing="1" cellpadding="3" width="100%">
  ...
  <%
     for(int i = 0; i < bodyList.size(); i++) {
       //T9PageModel pageModelWant = wantedPageModelList.get(i);
       T9SmsBody sb = bodyList.get(i);
       //T9SmsLogic smsLogicWant = new T9SmsLogic();
       int smsSeqId = 0;
       String userId = "";
