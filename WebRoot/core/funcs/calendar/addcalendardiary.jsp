<%@ page language="java" import="java.util.*,java.text.SimpleDateFormat,t9.core.funcs.person.data.T9Person" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
...
function selectDiaryByDate(date){
  var seqId = '<%=cal%>';
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarDiaryAct/selectDiaryByDate.act?seqId="+seqId+"&date="+date;
...
function selectCalendar(date){
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/selectCalendar.act?date="+date;
  var rtJson = getJsonRs(requestUrl);
  if(rtJson.rtState == "1"){
    alert(rtJson.rtMsrg); 
    return ;
...
<form name="formFile" id="formFile" action="<%=contextPath %>/t9/core/funcs/diary/act/T9DiaryAct/uploadFile.act" method="post" enctype="multipart/form-data" target="commintFrame"></form>
