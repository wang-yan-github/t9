...
<%@page import="t9.core.funcs.picture.data.T9Picture"%>
<%@page import="t9.core.funcs.picture.act.T9Page"%>
...
<%
	...
	T9Page page2=(T9Page)request.getAttribute("page");
...
//var requestURL="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct";
var getPicInfoURL="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct";
var uploadPriv=0;
var managePriv=0;
var rootDir=0;
...
window.onload = function() {
  ...
  var settings = {
    ...
    upload_url: "<%=contextPath %>/t9/core/funcs/picture/act/T9PictureAct/uploadFile.act?seqId=<%=seqId%>&subDir=" + encodePageSubDir,
...
function delFolder(){
  ...
  if(window.confirm(msg)){
	  ...
	  //alert(flag);
	  if(flag=="true"){
		  location.href="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicInfoById.act?ascDescFlag=<%=pageAscDesc %>&field=<%=pageField %>&subDir=" + encodeDelSubDir + "&seqId=<%=seqId%>";
		}
...
function comeBack(){
  ...
  var seqId=prc.seqId;
  if(flag == "true"){
  	location.href="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicInfoById.act?ascDescFlag=<%=pageAscDesc %>&field=<%=pageField %>&subDir=" + encodeReturnSubDir +"&seqId="+seqId;
  }else{
...
function refresh(){
	//alert("aa");
	location.href="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicInfoById.act?seqId=<%=seqId%>&subDir=" + encodePageSubDir + "&ascDescFlag=<%=pageAscDesc %>&field=<%=pageField %>";
}
...
function downLoadFile(picPath,fileName){
	...
	var url ="<%=contextPath%>/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal2.act?path=" + picPath + "/" +fileName + "&fileName="+fileName;
	//alert(url);
...
<body>
<div id="listDiv" >
<form	action="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicInfoById.act?seqId=<%=seqId %>"	method="post" name="form2">
...
<table border="0" cellspacing="1" width="700" align="center" class="TableBlock"
	...
	%>
		...
		%> 
			...
			<a href="<%=contextPath%>/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal2.act?path=<%=URLEncoder.encode(picPath + "/" + picName,"UTF-8") %>&fileName=<%=URLEncoder.encode(picName,"UTF-8")%>"><%=picNameHidden %></a>
	 	...
	 	<script type="text/javascript">
			...
			<a href="<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicInfoById.act?subDir=<%=URLEncoder.encode(subDir+"/"+picName,"UTF-8") %>&seqId=<%=picId %>&ascDescFlag=<%=pageAscDesc %>&field=<%=pageField %>" 
		...
		<script type="text/javascript">
			...
			<a href="<%=contextPath%>/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal2.act?path=<%=URLEncoder.encode(picPath + "/" + picName,"UTF-8") %>&fileName=<%=URLEncoder.encode(picName,"UTF-8")%>"><%=imgFlag%></a><br>
			<%--<a href="javascript:downLoadFile('<%=picPath %>','<%=picName %>')"><%=picNameHidden %></a>--%> 
			
			<%--<a href="javascript:location.href=contextPath+ '/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal2.act?path='+ encodeFilePath + '&fileName=' + encodeFileNameString;"><%=picNameHidden %></a>--%>  
			<%--<a href="javascript:location.href=contextPath+ '/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal2.act?path=<%=URLEncoder.encode(picPath + "/" + picName,"UTF-8") %>&fileName=<%=URLEncoder.encode(picName,"UTF-8") %> '"><%=picNameHidden %></a> --%> 
...
<table class="TableList" width="749" align="center" id="singleUpload" style="display: none">
	<tr>
		...
		<form name="uploadSingleFile" id="uploadSingleFile" method="post" action="<%=contextPath %>/t9/core/funcs/picture/act/T9PictureAct/uploadSingleFile.act?seqId=<%=seqId %>"	enctype="multipart/form-data" >
