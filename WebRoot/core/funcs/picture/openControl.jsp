...
function adapt(flag)
{
  ...
  if (!true_width || !true_height || true_width == "0" || true_height == "0") {
    //从服务器端获取图片宽度和高度
    var url = "<%=contextPath%>/t9/core/funcs/picture/act/T9PictureAct/getPicDimension.act";
...
function down_pic()
{
  ...
  //window.location="<%=contextPath%>/getFile?uploadFileNameServer=<%=subDir%>/" + FILE_ATTR_ARRAY[cur_pic_no][1];
   var downUrl="<%=contextPath%>/t9/core/funcs/office/ntko/act/T9NtkoAct/downFileByLocal.act?path=" + encodeURIComponent(parameterStr) + "&fileName=" + encodeURIComponent(parameterName);
	//alert(downUrl);
