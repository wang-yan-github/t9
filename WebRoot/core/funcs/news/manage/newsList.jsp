...
function loadType(){
	var url =contextPath+'/t9/core/funcs/news/act/T9NewsHandleAct/getnewsType.act';
...
function loadData(pageIndex , showLength ,typeId , ascDesc , field){
  ...
  if(!index){
    ...
    pageIndex = 1;
	  ...
	  var url =contextPath+'/t9/core/funcs/news/act/T9NewsHandleAct/getnewsManagerList.act';
...
function addRow(data , i,pageIndex,showLength){
	...
	var td = "<td>&nbsp;<input type='checkbox' name='deleteFlag' value='" + data.seqId +"'>"+"</td>"
	        ...
	        + "<td nowrap align='center'><a href='/t9/core/funcs/news/manage/newsAdd.jsp?seqId=" + data.seqId + "'> 修改</a>";
...
{   
      ...
      var url =contextPath+'/t9/core/funcs/news/act/T9NewsHandleAct/deleteCheckNews.act';
	  var json = getJsonRs(url,par);
	  if (json.rtState == "0"){
...
function show_reader(){
	 ...
	 URL="/t9/core/funcs/news/manage/showReader.jsp?seqId="+seqId;
...
{ 
	var url =contextPath+'/t9/core/funcs/news/act/T9NewsHandleAct/deleteAllnews.act';
	var json = getJsonRs(url);	
	if (json.rtState == "0"){
		window.location.reload();
	} else{
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big">
       ...
       &nbsp;
       <select id="typeId" name="typeId" class="BigSelect"  onChange="javascript:selChange($F('pageIndex') , $F('pageLen'),$('typeId').value,'n.SEQ_ID');">
       </select>
    </td>
    <td>
