...
function saveNewsByUp(){
 /* var FCK = FCKeditorAPI.GetInstance('Econtent'); //获得表单设计器的顶层对象FCK，该方法定义位置fckeditorapi.js第47行 by dq 090521
  ...
  var url = "/t9/t9/core/funcs/news/act/T9NewsHandleAct/saveNewsByUp.act";
...
function CheckForm()
{
  ...
  if(document.newsForm.format.value=="0"){
    ...
    checkNull = document.getElementById("content");
    if(!checkNull) {
      alert("新闻内容不能为空！");
      return false;
    }
...
function getSysRemind(){ 
  var requestUrl = contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=14"; 
...
function deleteAttachBackHand(attachName,attachId,attrchIndex){ 
  var url= contextPath + "/t9/core/funcs/news/act/T9NewsHandleAct/delFloatFile.act?attachId=" + attachId +"&attachName=" + attachName ; 
  if (seqId) {
    url += "&seqId=" + seqId;
  }
...
function moblieSmsRemind(remidDiv,remind){ 
  var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=14"; 
  var rtJson = getJsonRs(requestUrl); 
  if(rtJson.rtState == "1"){ 
    alert(rtJson.rtMsrg); 
    return ; 
