...
<%
  ...
  deptParent = T9Utility.encodeSpecial(deptParent);
  if (deptParent == null){
    deptParent = "";
  }
  String deptLocal = request.getParameter("deptLocal");
  deptLocal = T9Utility.encodeSpecial(deptLocal);
  if (deptLocal == null){
    deptLocal = "";
  }
...
var deptParentStr = "";
function doInit(){
  var url = "<%=contextPath%>/t9/core/funcs/dept/act/T9DeptAct/getDept.act?treeId=" + treeId;
...
function commitDept(){
  ...
  var url = "<%=contextPath%>/t9/core/funcs/dept/act/T9DeptAct/updateDept.act?treeId=" + treeId;
...
function nextDept(){
  location.href="/t9/core/funcs/dept/deptinput.jsp?deptLocal=" + deptLocal + "&deptParent=" + deptParent + "&treeId=" + treeId;
}

function deleteDept(){
  if(confirm("确认要删除该部门/成员单位吗？这将同时删除所有下级部门和部门中的用户！")){
    var url = "<%=contextPath%>/t9/core/funcs/dept/act/T9DeptAct/deleteDept.act?treeId=" + treeId;
    var rtJson = getJsonRs(url, mergeQueryString($("form1"))); 
...
function SelectUser(TO_ID, TO_NAME){
  var treId = treeId;
  URL = "/t9/core/funcs/dept/userselect.jsp?treeId=" + treId + "&TO_ID=" + TO_ID + "&TO_NAME=" + TO_NAME;
  openDialog(URL,'400', '350');
}

function deptFunc(){
  var url = "<%=contextPath%>/t9/core/funcs/dept/act/T9DeptAct/selectDeptToAttendance.act";
...
<!--  
 ...
 <input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.dept.data.T9Department"/>
   <tr>
