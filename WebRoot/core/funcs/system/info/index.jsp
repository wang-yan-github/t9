...
function doInit() {
	var version;
    var url = '<%=contextPath%>/t9/core/funcs/system/info/act/T9InfoAct/getSystemInfo.act';
    var rtJson = getJsonRs(url);
    if (rtJson.rtState == "0") {
        var data = rtJson.rtData;
        $('sysname').innerHTML = data.softName;
        ...
        $('regUserAmount').innerHTML = userAmount;
	    ...
	    $('unitName').innerHTML = data.regUnitName;
	    ...
	    if (data.regUnitName) {
	      ...
	      //}
		  $('regUnitName').innerHTML = data.unitName;// + noteMsrg;
    ...
    if (rtJson.rtState == "0") {
        ...
        $('sysname').innerHTML = (data['sysname.t9'] || productName) + '&nbsp;20' + ver + '版';
    }else {
        var verArray = (version || "").split('.');
        var ver;
        if (verArray.length > 2) {
        ...
        $('sysname').innerHTML = productName + '&nbsp;20' + ver + '版';
    }**/
}

function reloadRegInfo() {
  var url = '<%=contextPath%>/t9/core/funcs/system/info/act/T9InfoAct/reloadRegInfo.act';
  var rtJson = getJsonRs(url);
  if (rtJson.rtState == "0") {
    alert("重新加载注册信息成功!");
  }
...
function triaReg() {
  $("triaRegTable").show();
  $("triaRegLink").href = orgSecondSite+"?machineCode=" + machineCode;
}
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="<%=imgPath %>/sys_config.gif" WIDTH="22" HEIGHT="20" align="absmiddle"><span class="big3"><%=t9SysInfo %></span>
    </td>
  </tr>
</table>
<table id="triaRegTable" style="width: 80%;display: none;" align="center">
  <tr>
    <td class="">
			<a id="triaRegLink" class="" href="<%=orgSecondSite %>" target="_blank"><span> </span></a>
			&nbsp;&nbsp;&nbsp;&nbsp;参加试用登记的用户，均可获得售前技术支持服务，还可获赠试用期延长和用户数限制增加的注册文件。
    </td>
  </tr>
</table>
<form action="<%=contextPath %>/t9/core/funcs/system/info/act/T9InfoAct/reg.act" enctype="multipart/form-data" id="form1" method="post">
<br>
...
<table class="TableBlock no-top-border" width="80%" align="center">
  ...
  <tr  class="TableData">
    <td nowrap width="150"><b>&nbsp;版本类型：</b></td>
    <td nowrap >
  ...
  <tr class="TableData">
    ...
    <td nowrap ><=fullOrgName %> <a href="<=orgSecondSite %>" target="_blank"><=orgSecondSite %></a></td>
  </tr>
   -->
  <tr  class="TableData">
  ...
  <tr class="TableData">
    <td nowrap width="150"><b>&nbsp;软件注册时间：</b></td>
    <td nowrap id="installTime"></td>
  </tr>
  <tr id="remainDays" class="TableData">
    <td nowrap width="150"><b>&nbsp;有效期：</b></td>
    <td nowrap><span></span>
        <%if(SignProvider.type == null){
            out.println("");
        }else if(SignProvider.type.equals("oem")){
            out.println(SignProvider.expiresDate);
        }else if(SignProvider.type.equals("trail")){
            out.println("30天");
        }else{
            out.println("永久");
  ...
  <tr class="TableData">
    <td nowrap width="150"><b>&nbsp;有效域名：</b></td>
    <td nowrap><%=SignProvider.domain %></td>
  </tr>
  <tr class="TableData">
  ...
  <tr class="TableData">
    <td nowrap  width="150"><b>&nbsp;注册用户单位：</b></td>
    <td nowrap  id="unitName"></td>
  </tr>
  <tr class="TableData" id="regUnitNameTr" >
    <td nowrap width="150"><b>&nbsp;软件开发单位：</b></td>
