<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	// IP 校验
	  ...
	  if(document.form1.OP_EXPORT.checked){  //当点击单选按钮导出时  会提示		  var src=contextPath+"/t9/core/funcs/system/syslog/act/T9GlSyslogAct/SysExport.act";
		  document.form1.action=src;
		  document.form1.submit();
		  return true;
	  }
    if(document.form1.OP_QUERY.checked){
	    $("form1").action = contextPath + "/t9/core/funcs/system/syslog/act/T9GlSyslogAct/getMyGlSysLog.act";
	    document.form1.submit();
	    return true;
	  }
		/* 系统日志导入 
		if(document.form1.OP_IMPORT.checked){
		var src=contextPath+"/t9/core/funcs/system/syslog/act/T9GlSyslogAct/SysImport.act";
		document.form1.action=src;
		document.form1.submit();
		alert("系统开发中、、、");
		return false;
  ...
  // 根据单选按钮来删除
	function doInit3(){
	  ...
	  var tmp = queryParam;
	  var url = contextPath+'/t9/core/funcs/system/syslog/act/T9GlSyslogAct/deleteRadio.act';
	  var json = getJsonRs(url,tmp); 
	
	   if (json.rtState == "0"){
		    var rtData = json.rtData;
		    var cou = rtData; 
				window.location.href="/t9/core/funcs/system/syslog/deletecount.jsp?cou=" + cou; //到deletecount.jsp 页面接收cou
	 		} else{
		    alert("没有符合条件数据删除！！");
		}
	}
	...
	*/
	function doInit(){ 
	  getLogType();
	  //var url = contextPath+'/t9/core/funcs/system/syslog/act/T9SysLogSaveAct/getsysradio.act';
	  // var json = getJsonRs(url); 
	 // document.all.submit="";
	 //$("form1").action='/t9/t9/core/funcs/system/syslog/act/T9SysLogSaveAct/getsysradio.act';
	  //$("form1").submit();
	 /*  xin gong neng */
	...
	function copy_log(){
	 ...
	 // var msg = '确认要存档系统日志吗？';
	  ...
	  //window.location="/t9/core/funcs/system/syslog/copy.jsp";
	  return false;
	}
	// 清空系统日志
	function delete_log(){
	...
	if(window.confirm(msg)){
	   ...
	   var tmp = queryParam;
	   var url = contextPath+'/t9/core/funcs/system/syslog/act/T9GlSyslogAct/deleteAllChecklog.act';
	   var json = getJsonRs(url,tmp); 
	   if (json.rtState == "0"){
	  		 var rtData = json.rtData;
	       var dele = rtData;
	  		 window.location.reload();//返回当前页面
	  	//xin gongneng	window.location="/t9/core/funcs/system/syslog/delete_all.jsp?right="+dele;
	    } else{
	  	      alert(json.rtMsrg);
	  	}
	  }
	...
	function getLogType(){
	  var requestURL = "<%=contextPath%>/t9/core/codeclass/act/T9CodeClassAct/getCodeItemByNo.act"; 
