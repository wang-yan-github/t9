...
function commit(){
  ...
  if(document.getElementById("overwrite").checked == true){
    var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/updateSingleWords.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
    ...
    var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/addSingleWords.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
    var rtJson = getJsonRs(url, mergeQueryString($("form1")));
    if(rtJson.rtState == "0"){
      location = "<%=contextPath %>/core/funcs/system/censorwords/new/submit.jsp?find="+encodeURIComponent(find);
    }else{
...
function commitMore(){
  ...
  if(document.getElementById("overwrite2").checked == true){
    var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/deleteMore2Words.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
  ...
  for(var i = 0; i < censorVal.length; i++){
    ...
    if(document.getElementById("overwrite2").checked == true){
      //alert("2");
      var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/updateMore2Words.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
      var rtJson = getJsonRs(url, mergeQueryString($("form2")));
      if(rtJson.rtState == "0"){
      ...
      //alert("1");
      var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/updateMore1Words.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
      var rtJson = getJsonRs(url, mergeQueryString($("form2")));
      if(rtJson.rtState == "0"){
        location = "<%=contextPath %>/core/funcs/system/censorwords/new/import2.jsp?okCount="+okCount+"&errCount="+errCount;
      }else{
      ...
      var url = "<%=contextPath%>/t9/core/funcs/system/censorwords/act/T9CensorWordsAct/updateMore0Words.act?find="+encodeURIComponent(find)+"&replacement="+encodeURIComponent(replacement);
      //location = url;
...
<input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.system.censorwords.data.T9CensorWords.java"/>
