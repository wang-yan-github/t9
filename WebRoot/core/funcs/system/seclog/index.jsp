...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/core/funcs/seclog/act/T9SeclogAct/getLogListJson.act?"+param;
  var cfgs = {
    dataAction: url,
...
function doExp(){
	  ...
	  var optDesc=$("optDesc").value;
	  var param="userId="+userId+"&opType="+opType+"&startDate="+startDate+"&endDate="+endDate+"&optDesc="+optDesc;
	  var url = contextPath + "/t9/core/funcs/seclog/act/T9SeclogAct/doExport.act?"+param;
	  location.href=url;
...
function doArchive(){
	if(!confirm("归档日志将转存在之外的数据表，原数据表清空，请确认是否要继续归档？")){
	  ...
	  var url = contextPath + "/t9/core/funcs/seclog/act/T9SeclogAct/doArchive.act";
