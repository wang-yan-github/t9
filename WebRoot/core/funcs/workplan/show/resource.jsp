...
<%@page import="t9.core.funcs.workplan.data.T9WorkPerson"%>
...
function checkForm2(planId,name) {
  if (checkForm()) { 
    ...
    var url = contextPath + "/t9/core/funcs/workplan/act/T9WorkPersonAct/addPerson.act";
    var json = getJsonRs(url,pars);
    if (json.rtState == "1") {
      alert(json.rtMsrg);
    }else {
      window.location = contextPath + "/t9/core/funcs/workplan/act/T9WorkPersonAct/selectPerson.act?" + param;
    }
  }
}
function jugeFile(){
...
function deleteId(seqId,name,planId) {
  ...
  var param = encodeURI(param1);
  var url = contextPath + "/t9/core/funcs/workplan/act/T9WorkPersonAct/deletePerson.act?" + param;
  if (window.confirm(msg)) {
  window.location = url;
  } 
}
function selectId(seqId,name) {
  window.location = "<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkPersonAct/selectId.act?seqId=" + seqId + "&name=" + name;
}
function doInit3(){
...
List<T9WorkPerson> list = (List<T9WorkPerson>)request.getAttribute("person");
T9WorkPlan plan = (T9WorkPlan)request.getAttribute("plan");

//创建人，负责人
T9Person personLog = (T9Person)request.getSession().getAttribute(T9Const.LOGIN_USER);
int len = 0;
boolean flag = false;
