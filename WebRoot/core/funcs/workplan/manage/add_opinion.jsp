...
//负责人
  ...
  if (!T9Utility.isNullorEmpty(plan.getCreator())) {
...
<script type="text/javascript">
  ...
  function updateFugle(seqId) {
    window.location = "<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/selectId.act?seqId=" + seqId;
  }
  function deleteDetail(seqId,planId) {
    var msg = "确认要删除该批注吗?";
    if (window.confirm(msg)) {
    window.location = "<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/deleteDetai.act?seqId=" + seqId + "&planId=" + planId;
    }   
  }
  function checkForm() {   
    if (!$("PROGRESS").value) {
  ...
  function checkForm2(planId) {  
    if (checkForm()) {
      ...
      var url = contextPath + "/t9/core/funcs/workplan/act/T9WorkDetailAct/addDetai.act";
      var json = getJsonRs(url,pars);
      if(json.rtState == "1"){
        alert(json.rtMsrg);
      }else {
        window.location = contextPath + "/t9/core/funcs/workplan/act/T9WorkDetailAct/selectDetai.act?seqId=" + planId
      }
    }
  }
  function jugeFile(){
...
<form id="formFile" action="/t9/t9/core/funcs/workplan/act/T9WorkPlanHandleAct/uploadFile.act" method="post" enctype="multipart/form-data" target="commintFrame">
