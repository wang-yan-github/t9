...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/workSelect.act";
   cfgs = {
    dataAction: url,
    container: "giftList",
    paramFunc: getParam,
...
function typeName() {
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanTypeAct/planType.act";
...
function toUser(cellData,recordIndex,columIndex){
  ...
  var requestUrl = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/userName.act" + seqId;
...
function workDetail(seqId) {
  window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/workSelect2.act?seqId=" + seqId,"myOpen","height=650,width=700,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top=90,left=290,resizable=yes");
}
//进度图
function myOpen(seqId) {
  window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/selectDetai2.act?seqId=" + seqId,"myOpen2","status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=250,top=0,resizable=yes");
}
//删除所有自己发布的
function workDeleteAll() {
  var msg = "确认要删除所有工作计划吗?";
  if (window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/deleteWorkAll.act";
...
function updateWork(seqId) {
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/updatePlu2.act?seqId=" + seqId;
  window.location = url;
}
//领导批注
function opinion(seqId) {
  window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/selectDetai.act?seqId=" + seqId,"myOpen","height=550,width=800,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top=90,left=290,resizable=yes");
}
//删除
function workDelete(seqId) {
    var msg = "确认要删除该工作计划吗?";
    if (window.confirm(msg)) {
      var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/deleteWork.act?seqId=" + seqId;
      var rtJson = getJsonRs(url);
      queryGift();
    }
}
...
function updateJix(seqId,pub) {
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/updatePlu.act?seqId=" + seqId + "&pub=" + pub;
  var rtJson = getJsonRs(url);
  queryGift();
}
//结束,生效
function updateJiesu(seqId,pub,time) {
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/updatePlu4.act?seqId=" + seqId + "&pub=" + pub + "&time=" + time;
  var rtJson = getJsonRs(url);
  queryGift();
}
//今日计划
...
function updateShengX(seqId,pub,time) {
  var url = "<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/updateShengX.act?seqId=" + seqId + "&pub=" + pub + "&time=" + time;
  var rtJson = getJsonRs(url);
  queryGift();
}
</script>
