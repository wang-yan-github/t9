...
function cratetime() {
  ...
  function doInit2() {
    ...
    var settings = {
      ...
      upload_url: "<%=contextPath %>/t9/core/funcs/notify/act/T9NotifyHandleAct/fileLoad.act",
...
//    progress.setStatus("Complete.");
  ...
  function checkForm2(chi) {
    ...
    if (checkForm()) {
      ...
      var url = "";
      if(<%=plan.getSeqId()%>){
        url = contextPath + "/t9/core/funcs/workplan/act/T9PlanWorkAct/updateWork3.act?seqId=<%=plan.getSeqId()%>";
      }
      else{
        url = contextPath + "/t9/core/funcs/workplan/act/T9PlanWorkAct/add.act";
      }
...
//判断是否要显示短信提醒 
  ...
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=12"; 
  ...
  function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
    ...
    for(var i = 0 ; i < attachNameArrays.length ; i++) { 
      ...
      if (checkForm()) {
        var pars = $('form1').serialize() ;
        var url = contextPath + "/t9/core/funcs/workplan/act/T9PlanWorkAct/updateWork3.act?seqId=<%=plan.getSeqId()%>";
        var json = getJsonRs(url,pars);
        if(json.rtState == "1") {
          alert(json.rtMsrg);
          return false;
 ...
 function moblieSmsRemind(remidDiv,remind) {
     var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=12";
     var rtJson = getJsonRs(requestUrl); 
     if (rtJson.rtState == "1"){ 
       alert(rtJson.rtMsrg); 
       return ; 
...
<%} %>
<textarea cols="45" name="deptParentDesc" id="deptDesc" rows="2" style="overflow-y: auto;" class="SmallStatic" readonly><%if (!T9Utility.isNullorEmpty(plan.getDeptParentDesc()) && (plan.getDeptParentDesc().equals("0") || plan.getDeptParentDesc().equals("ALL_DEPT"))) {%>
全体部门
<%}%>
</textarea>
...
<form id="formFile" action="<%=contextPath %>/t9/core/funcs/workplan/act/T9WorkPlanHandleAct/uploadFile.act" method="post" enctype="multipart/form-data" target="commintFrame">
