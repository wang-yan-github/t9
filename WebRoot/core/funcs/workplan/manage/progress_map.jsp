...
<%@page import="t9.core.funcs.workplan.data.T9WorkDetail"%>
<%@page import="t9.core.funcs.workplan.data.T9WorkPlan"%>

<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%@page import="t9.core.funcs.workplan.act.T9CreateTime"%>
<%@page import="java.text.SimpleDateFormat"%>
<%@page import="t9.core.funcs.person.data.T9Person" %>
<%
T9WorkPlan plan = (T9WorkPlan)request.getAttribute("plan");
T9WorkDetail detail = new T9WorkDetail();
List<T9WorkDetail> list = (List<T9WorkDetail>)request.getAttribute("list");
List<T9WorkDetail> listP = (List<T9WorkDetail>)request.getAttribute("list2");
...
if (T9Utility.isNullorEmpty(endTime) || endTime.equals("null")) {
...
Date startday = T9Utility.parseDate(statrTime);
Date endday = T9Utility.parseDate(endTime);
Calendar c = Calendar.getInstance(); 
c.setTime(startday); 
int weekthInt = c.get(Calendar.WEEK_OF_YEAR);   
String start = T9CreateTime.getFullTimeStr(T9CreateTime.getStartEnd(Integer.parseInt(statrTime.substring(0,4)),weekthInt)[0]);//本周的第一天

Date startDate = T9Utility.parseDate(start); //到哪一天 
//String end = getFullTimeStr(getStartEnd(2010,weekthInt)[1]); //本周最后一天

c.setTime(endday);
int weekthInt2 = c.get(Calendar.WEEK_OF_YEAR);  
String end2 = T9CreateTime.getFullTimeStr(T9CreateTime.getStartEnd(Integer.parseInt(endTime.substring(0,4)),weekthInt2)[1]);//本周的最后一天天

//得到相差多少天 
int spanDays = T9CreateTime.getIntervalDays(startDate,T9Utility.parseDate(end2)); 
List list2 = T9CreateTime.getDays(startDate, endday, spanDays);
...
int days = T9CreateTime.getIntervalDays(startday, endday);
int week = 0;
if (list2.size()%7 != 0) {
  week = list2.size()/7 + 1;
}
...
<script type="text/javascript">
  function myOpen(seqid) {
    window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/selectDetai.act?seqId=" + seqid,"myOpenT","height=550,width=800,top=90,left=290,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=1");
  }
  function myOpen3(seqId) {
    window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9WorkDetailAct/selectDetaiId.act?seqId=" + seqId,"myOpen3","height=550,width=800,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top=90,left=290,resizable=yes");
  }
  function workSelect2(seqid) {
    window.open("<%=contextPath%>/t9/core/funcs/workplan/act/T9PlanWorkAct/workSelect2.act?seqId=" + seqid,"myOpenN","height=650,width=700,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top=90,left=290,resizable=yes");
  }
  function showOpen(seqid,name) {
    var param1 = "seqId=" + seqid + "&name=" + name;
    var param = encodeURI(param1);
    var url = contextPath + "/t9/core/funcs/workplan/act/T9WorkPersonAct/selectPerson.act?" + param;
  ...
  function showOpen2(i,seqid,name) {
    ...
    if (document.getElementById(i).style.display == "none") {
      ...
      var param1 = "seqId=" + seqid + "&name=" + name;
      var param = encodeURI(param1);
      var requestUrl = contextPath + "/t9/core/funcs/workplan/act/T9WorkPersonAct/selectPerson2.act?" + param;
 ...
 <%} %>
    ...
    for (int k = -1; k < listP.size()-1 ; k++) {
      ...
      if(k==-1){
        ...
        T9WorkDetail detail2 = listP.get(k+1);      
