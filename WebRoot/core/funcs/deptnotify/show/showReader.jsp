...
<script type="text/Javascript" src="/t9/core/js/cmp/select.js"></script>
<script type="text/javascript" src="/t9/core/js/cmp/tab.js"></script>
<script type="text/javascript" src="/t9/core/js/cmp/Menu.js"></script> 
<script type="text/javascript" src="/t9/core/js/cmp/attachMenu.js"></script>
...
function loadData(){
	  var url =contextPath+'/t9/core/funcs/deptnotify/act/T9NotifyShowAct/showReader.act?seqId='+seqId;
...
function addRow(data , i){
    //alert(data.readerDate + "------" + data.userNameStr);
    var rDate = data.readerDate;
    var rDates; // 阅读时间
    if(rDate){rDates = rDate.split(",");}
    //alert(rDates);
    var rName = data.userNameStr;
    var rNames; // 阅读人员
    if(rName){rNames = rName.split(",",rName.length-1);}
    //alert(rNames);
    td1 += "<tr><td class='TableData' align='left'>&nbsp;"+data.deptName+"</td>"
    td1 += "<td class='TableData'>"
    if(data.readerDate && data.userNameStr){
        for( i = 0 ;i < rNames.length-1 ;i ++){
          td1 += "<span title="
          td1 += rDates[i] +">"
          td1 += rNames[i]
          td1 += "</span>,"
     ...
     td1 += "</td>"
     td1 += "<td class='TableData'>&nbsp;"+data.unUser+"</td>";
...
function delete_reader()
{
 ...
 {
  URL= contextPath+"/t9/core/funcs/deptnotify/act/T9NotifyShowAct/deleteReader.act?seqId=" + seqId;
  window.location=URL;
 }
}
</script>
