...
function loadData(pageIndex , showLength , typeId , ascDesc , field){
  ...
  var par = "pageIndex="+pageIndex+"&showLength=" +showLength+"&type="+typeId+"&ascDesc="+ascDesc+"&field="+field+"&portName="+portName;
  if (type) {
    par = par + "&type="+type;
  }
  if(sendTime){
  ...
  var url =contextPath+'/t9/core/funcs/deptnotify/act/T9NotifyShowAct/getnotifyShowList.act';
...
function addRow(data , i){
  ...
  var td =  "<td align='center'><u title='部门：" + data.deptName + "' style='cursor:pointer'>" + data.fromName + "</u></td>"
    ...
    + " <td nowrap><a href=javascript:open_notify('" + data.seqId + "','" + data.format + "'); title='" + data.subjectTitle + "'>" + data.subject;
  if (data.iread == 'no') {
    td += "</a><img src='<%=imgPath%>/email_new.gif' align='absMiddle' border='0'></td>";
  }
  td += "</a></td>";
...
function read_all() {
  ...
  if (window.confirm(msg)) {
    url="/t9/t9/core/funcs/deptnotify/act/T9NotifyShowAct/changeNoReadAll.act?forward=all";
    window.location = url;
  }
}
