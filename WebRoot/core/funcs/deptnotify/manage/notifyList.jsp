...
//	  $('pgSearchInfo').innerHTML = "加载数据中,请稍后.....";
	  ...
	  var url =contextPath+'/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/getnotifyManagerList.act';
...
function addRow(data , i,pageIndex,showLength){
	  ...
	  td = td + "<td><a href=javascript:show_reader('" + data.seqId + "'); title='查阅情况'> 查阅情况</a>&nbsp;<a href='/t9/core/funcs/deptnotify/manage/notifyAdd.jsp?seqId=" + data.seqId + "'> 修改</a>";
      var notifyStatus = data.notifyStatus;
      var publish = data.publish;
      if(notifyStatus=="1"&&publish=="1"){
          td = td + "&nbsp;<a href='/t9/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/changeState.act?seqId="+data.seqId+"&pageIndex="+pageIndex+"&showLength="+showLength+"&operation=1'> 立即生效</a>"
      }else if(notifyStatus=="2"&&publish=="1"){
    	  td = td + "&nbsp;<a href='/t9/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/changeState.act?seqId="+data.seqId+"&pageIndex="+pageIndex+"&showLength="+showLength+"&operation=2'>终止</a>"
      }else if(notifyStatus=="3"&&publish=="1"){
    	  td = td + "&nbsp;<a href='/t9/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/changeState.act?seqId="+data.seqId+"&pageIndex="+pageIndex+"&showLength="+showLength+"&operation=3'>生效</a>"
      }
      td = td + "</td>";
...
{  
  ...
  {   
      ...
      var url =contextPath+'/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/cancelTop.act';
	  var json = getJsonRs(url,par);
	  if (json.rtState == "0"){
		  window.location.reload();
		} else{
