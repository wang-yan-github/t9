...
function doInit(){
  ...
  var url =contextPath+'/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/beforeAddNotify.act';
  ...
  if (json.rtState == "0") {
    ...
    if (rtData.topdays != 0) {
      topdays = rtData.topdays;
      topdaysremind = "最大置顶时间为" + rtData.topdays +"天";
      $('topdaysremindSpan').update(topdaysremind);
      $('topDays2').value = topdays;
    }
  }else{
  ...
  if(seqId) { //是修改    //document.getElementById("editBack").style.display = "";
    ...
    var url = contextPath + "/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/editNotify.act?seqId="+seqId;
    var json = getJsonRs(url);
    if (json.rtState == "0") {
      ...
      UE.getEditor('content').addListener("ready", function () {
          UE.getEditor('content').setContent(rtData.content);
      });
      /**if (!isTouchDevice) {
...
//  var textStr = FORM_HTML;
  var url = contextPath + "/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/saveNotifyByUp.act";
...
function CheckForm() {
  ...
  if (document.notifyForm.top.checked ) {
    if(document.notifyForm.topDays.value && !isInteger(document.notifyForm.topDays.value) ) {
      alert("最大置顶时间应为整数！");
      document.notifyForm.topDays.focus();
      return false;
    }
    if( document.notifyForm.topDays.value && parseInt(document.notifyForm.topDays.value) <0 ) {
      alert("最大置顶时间应为正整数！");
      document.notifyForm.topDays.focus();
      return false;
    }
    if(document.notifyForm.topDays.value && parseInt(document.notifyForm.topDays.value) > topdays ) {
      alert("最大置顶时间不能大于系统设置中的最大置顶时间！");
      document.notifyForm.topDays.focus();
      return false;
    }
  }
...
function _edit(publish,content,format){ 
  var contents = "<form enctype='multipart/form-data' action='"+ contextPath +"/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/addNotify.act'" 
...
function saveNotify(){
  var url = contextPath + "/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/addNotify.act";
  $("notifyForm").action = url;
  $("notifyForm").submit();
}

function goBack() {
  window.location.href = "/t9/core/funcs/deptnotify/manage/notifyList.jsp";
}
...
function getSysRemind(){ 
  var requestUrl =  contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=1"; 
...
function deleteAttachBackHand(attachName,attachId,attrchIndex) {
  var url= contextPath + "/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/delFloatFile.act?attachId=" + attachId +"&attachName=" + attachName ; 
  if (seqId) {
    url += "&seqId=" + seqId;
  }
...
function getManageTopDay(){
	var requestUrl =  contextPath + "/t9/core/funcs/deptnotify/act/T9NotifyHandleAct/getManageTopDays.act"; 
  var rtJson = getJsonRs(requestUrl); 
  if(rtJson.rtState == "1"){ 
    alert(rtJson.rtMsrg); 
    return 0; 
