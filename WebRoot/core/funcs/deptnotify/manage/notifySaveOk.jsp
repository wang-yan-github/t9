...
//	$('msgInfo').update(msg);
 if(publish == '1'){
   $('msg').update('公告通知发布成功');
   url = '/t9/core/funcs/deptnotify/manage/notifyList.jsp';
 }
 if(publish == '2'){
 ...
 if(publish == '0'){
	 $('msg').update('公告通知保存成功');
	  url = '/t9/core/funcs/deptnotify/manage/notifyAdd.jsp';
 }
...
function to0back() {	
  if (top.dispParts) {
	  ...
	  window.location.href = "<%=contextPath%>/core/funcs/deptnotify/manage/notifyAdd.jsp";
  }else {
    window.close();
  }
}
