...
var requestFolderURL="<%=contextPath%>/t9/core/funcs/personfolder/act/T9PersonFolderAct";
function doInit(){
...
function do_action(action){
	...
	case "downFile": //下载
		...
		if(count>1 && window.confirm("一次下载多个文件需要在服务器上做压缩处理，会占用较多服务器CPU资源，确定继续下载吗？\n该操作请不要下载超过128MB的大文件")){
			location.href = "<%=contextPath%>/t9/core/funcs/filefolder/act/T9FileContentAct/batchDownload.act?contentIdStr=" + selects + "&name=";
		}else if(count == 1){
			location.href = "<%=contextPath%>/t9/core/funcs/filefolder/act/T9FileContentAct/batchDownload.act?contentIdStr=" + selects + "&name=";
		}
		break;
