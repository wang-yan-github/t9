{
  ...
  getIt:function(sort,desc){
    var obj = {
        ...
        loader: {
          url: contextPath + "/t9/core/funcs/news/act/T9NewsShowAct/getDeskNewsAllList.act?type=" + sort
        },
        rowRender: function(i, e) {
          function deskNotify(seqId){
            var img = document.getElementById("img_news_"+seqId+"");
            if(img){
              document.getElementById("img_news_"+seqId+"").style.display="none";
            }
          ...
          var icon;
          if (e.iread == "no") {
            icon = $('<img id="img_news_'+e.seqId +'" class="icon"></img>');
