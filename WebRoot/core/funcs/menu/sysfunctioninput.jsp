...
function doInit() {
  ...
  if(seqId){ 
    var url = "<%=contextPath%>/t9/core/funcs/menu/act/T9SysMenuAct/getSysFunction.act";
...
function commitItem(){
  ...
  if (seqId) {
    ...
    url = "<%=contextPath%>/t9/core/funcs/menu/act/T9SysMenuAct/updateSysFunction.act?menuIdStr="+menuIdStr+"&seqId="+seqId+"&menuSort="+menuSort+"&menuId4="+menuId4+"&menuId="+menuId+"&menuSortOld="+menuSortOld+"&menuIdOld="+menuIdOld+"&menuLast="+menuLast+"&menuIdOverAll="+menuIdOverAll+"&icon="+icon;
    ...
    if (rtJson.rtState == "0") {
      ...
      location = "<%=contextPath %>/t9/core/funcs/menu/act/T9SysMenuAct/listSysFunction.act?menuId="+menuIdOld;
    }else {
      alert(rtJson.rtMsrg); 
      $("menuIdDesc").select();
      $("menuIdDesc").focus();
    ...
    url = "<%=contextPath%>/t9/core/funcs/menu/act/T9SysMenuAct/addSysFunction.act";
    rtJson = getJsonRs(url, mergeQueryString($("functionInfoForm")));
    if (rtJson.rtState == "0") {
      //alert(rtJson.rtMsrg);
      document.getElementById("menuId").value = menuId;
      location = "<%=contextPath %>/t9/core/funcs/menu/act/T9SysMenuAct/listSysFunction.act?menuId="+menuId;
...
function goback() {
  window.location.href = "<%=contextPath %>/t9/core/funcs/menu/act/T9SysMenuAct/listSysFunction.act?menuId=" + menuId ;
}
...
<table border="0" width="90%" cellspacing="0" cellpadding="3" class="small">
  ...
  <input type="hidden" name="seqId" id="seqId" value="" />
   ...
   <input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.menu.data.T9SysFunction"/>
  ...
  <table cellscpacing="1" cellpadding="3" width="500" align="center" class="TableList">
   ...
   <tr class="TableLine1">
    <td colspan="2">
      ...
      <b>类型一，此菜单挂接OA系统中的模块：</b>则填写程序路径，格式如：email，则实际对应的路径是：OA安装目录/webroot/t9/email，对应的图片是：OA安装目录/webroot/t9/core/styles/imgs/menuIcon/email.gif<br/><br/>
      <b>类型二，此菜单挂接外部的B/S结构的系统：</b>则填写该系统网址，格式如：<%=orgFirstSite %>，注意：请一定要以http://开头<br/><br/>
   </td>
  </tr> 
  <tr class="TableLine2">
