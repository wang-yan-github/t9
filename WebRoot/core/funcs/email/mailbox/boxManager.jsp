...
<title>邮箱管理</title>
<%
List<T9EmailBox> emboxList = (List<T9EmailBox>)request.getAttribute("boxList");
%>
</head>
<body  topmargin="5" >
<table  width="90%" align="center">
    ...
    <%  for(int i = 0; i < emboxList.size(); i++) {
          T9EmailBox embox = emboxList.get(i);
    %>
    <tr class="TableData">
...
function SetEmailNums(boxId,boxName,cntrlId){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9EmailBoxAct/setDefaultCount.act?boxId=" + boxId + "&defCount=" + defCount;
...
function deleteMisBox(boxId){
  ...
  if(window.confirm(msg)){
	  var url = contextPath + "/t9/core/funcs/email/act/T9EmailBoxAct/deleteBox.act?boxId=" + boxId;
	  var rtJson = getJsonRs(url);
	  if(rtJson.rtState == "0"){
	    alert(rtJson.rtMsrg);
