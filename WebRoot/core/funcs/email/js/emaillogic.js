...
function sendMail(){
  ...
  */
  textStr = UE.getEditor('content').addListener("ready", function () {
      UE.getEditor('content').setContent(content);
  });
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/sendMail.act";
...
function filterPerson() {
  var toId = $('toId').value;
  var copyToId = $('copyToId').value;
  var secretToId = $('secretToId').value;
  
  var sids = "";
  var toIds = secretToId.split(",");
  for (var i = 0 ;i < toIds.length ;i++) {
    var to = toIds[i];
    var flag = false;
    if (to && !findId(toId + "," + copyToId , to)) {
      if (sids) {
        sids += "," + to ;
      } else {
        sids = to;
  ...
  var cids = "";
  var toIds2 = copyToId.split(",");
  for (var i = 0 ;i < toIds2.length ;i++) {
    var to = toIds2[i];
    if (to && !findId(toId , to)) {
      if (cids) {
        cids += "," + to ;
      } else {
        cids = to;
...
*/  
  ...
  var textStr = editor.getContent();
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/saveMailByTime.act";
...
function showSelfBox(){
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getSelfDefBox.act";
...
function sendMailForDbox(){
  ...
  if(window.confirm(msg)) {
    var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/sendMailAll.act";
...
function readMailById(id) {
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/readFlag.act";
...
function readStatus(bodyId,type){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/readStatus.act?bodyId="+ bodyId + "&type=" + type;
  window.open(url);
}
/**
 * 重发邮件
...
function saveMail(type){
  ...
  */
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/saveMail.act";
  document.form1.content.value = textStr;
  if(type == 2){
    $("form1").seqId.value = '';
  }
...
function saveMaiByUp(){
  ...
  */
  var textStr = editor.getContent();
  textStr = textStr.replace(/[\n\r\f]/g,"");
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/saveMailByUp.act";
  document.form1.content.value = textStr;
  $("form1").action = url;
  $("form1").submit();
  if(parent.mail_menu)
...
function updateMail(seqId){
  ...
  */
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/updateMail.act";
  document.form1.content.value = textStr;
  $("form1").action = url;
  $("form1").submit();
}
...
function saveBox(){
  // alert("ssss");
  var url = contextPath +  "/t9/core/funcs/email/act/T9EmailBoxAct/saveBox.act";
...
function updateBox(){
 // alert("ssss");
 var url = contextPath +  "/t9/core/funcs/email/act/T9EmailBoxAct/updateBox.act";
 var rtJson = getJsonRs(url, mergeQueryString($("form1")));
 if(rtJson.rtState == "0"){
   alert(rtJson.rtMsrg);
...
function getBoxById(boxId){
 // alert("ssss");
 var url = contextPath +  "/t9/core/funcs/email/act/T9EmailBoxAct/getBoxById.act?boxId=" + boxId;
 var rtJson = getJsonRs(url);
 if(rtJson.rtState == "0"){
...
function delForShortCut(delType,shortCutType,boxId,msg){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/deletMailForShort.act";
  var rtJson = getJsonRs(url, param);
  if (rtJson.rtState == "0") {
...
function mailStatus(mailBodyId,typesy,emailId){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getMailStatus.act?" + param;
  var rtJson = getJsonRs(url);
  if (rtJson.rtState == "0") {
 ...
 * 
 * url删除邮件后页面加载地址 url = "<%=contextPath%>/t9/core/funcs/email/act/T9InnerEMailAct/listInBox.act?boxId=0";
 * 
 * @return
 */
 function deleteMail (type,reloadurl,id,notParent) {
   ...
   if(window.confirm(msg)) {
     var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/deletM.act?bodyId="+ delete_str + "&deType=" + type ;
     var rtJson = getJsonRs(url);
     if(rtJson.rtState == "1"){
       alert(rtJson.rtMsrg);
     }
...
function getNewDelCount(type,boxId,cntrlId){
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getNewDelCount.act?boxId="+ boxId + "&type=" + type ;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
   ...
   function getNewDelCount2(type,boxId,cntrlId){
     var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getNewDelCount.act?boxId="+ boxId + "&type=" + type + "&queryType=" + $('type').value ;
...
function exportMail(){
  ...
  location = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/exportToExcel.act?emailIds=" + delete_str ;
}
/**
 * 草稿箱
...
function exportMail2(){
  ...
  location = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/exportToExcel2.act?emailIds=" + delete_str ;
}
/**
 * 批量导出邮件
 * 
...
function exportEml(){
  ...
  location = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/exportToEml.act?emailIds=" + delete_str ;
}
...
function exportEmlOut(){
  ...
  location = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/exportToEml2.act?emailIds=" + delete_str ;
}
/**
 * 批量导出邮件
 * 
...
function exportEml2(id){
  location = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/exportToEml.act?emailIds=" + id ;
}
/**
 * 邮件查询
 * 
...
function getSysRemind(remidDiv,remind){ 
  var requestUrl = contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=2"; 
...
function moblieSmsRemind(remidDiv,remind){ 
  var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=2"; 
  var rtJson = getJsonRs(requestUrl); 
    if(rtJson.rtState == "1"){ 
    alert(rtJson.rtMsrg); 
    return ; 
...
function loadLinkMan(bindId){
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getLastLinkMan.act?";
  var rtJson = getJsonRs(url); 
  var menus = new Array();
  if(rtJson.rtState == "0"){
...
function isBoxNameExist(boxName,boxId){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9EmailBoxAct/isBoxNameExist.act?";
...
function isFull(){
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/isFull.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
 ...
 */
function getSpare(){
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getSpareCapacity.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == '0'){
...
function bachDownByList(){
  ...
  var url = contextPath + "/t9/core/funcs/email/act/T9InnerEMailAct/getAttachInfo.act?emailId=" + delete_str ;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == '0'){
    var obj = rtJson.rtData;
...
function bindWebMail(){
  //var rtJson = getJsonRs(url);
  var url = contextPath + "/t9/core/funcs/email/act/T9WebmailAct/listWebmailInfo.act";
