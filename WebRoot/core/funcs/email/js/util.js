...
function checkBoxRender(cellData, recordIndex, columIndex) {
  ...
  html += "<input type=\"checkbox\" name=\"check_diay_search\" value=\""
          + checkValue + "\" onclick=\"checkSelf()\" >";
  return html;
}
...
function WarningMsrg(msrg, cntrlId, type) {
  ...
  msrgDom += "<div class=\"content\" style=\"font-size:12pt\">" + msrg
          + "</div>" + " </td> </tr> </table>";
...
function showTr(cntrlId, dom) {
  try {
    $(cntrlId).style.display = ($(cntrlId).style.display == "none") ? ""
            : "none";
  } catch (e) {
    // alert(e);
    $(cntrlId).setAttribute('style').display = ($(cntrlId)
            .getAttribute('style').display == "none") ? "" : "none";
  }
}
/**
 * 处理附件的显示
...
function showAttach(attrIds, attrNames, cntrId) {
  ...
  for (var i = 0; i <= attrIdArrays.length; i++) {
    ...
    reStr += "<a href=\"javascript:downFile(\'" + key + "\',\'" + value
            + "\');\" title=\"" + value + "\">" + value + "</a><br>";
...
/**
 * 需要挂接菜单，可以直接输出或者注入到dom控件中 菜单的挂接通过函数处理
 * 
 * @param attrIds
 * @param attrNames
 * @param cntrId
...
function showAttach2(attrIds, attrNames, cntrId, menuFun) {
  ...
  for (var i = 0; i <= attrIdArrays.length; i++) {
    ...
    reStr += "<a href=\"javascript:downFile(\'" + key + "\',\'" + value
            + "\');\" title=\"" + value
            + "\" onmouseover='createDiv(event,\"1\")'>" + value + "</a><br>";
    // MODULE=email&amp;YM=1001&amp;ATTACHMENT_ID=216664316&amp;ATTACHMENT_NAME=SoftMgrUninst.exe
  }
  reStr += "</div>";
  if (cntrId) {
...
function downFile(attrId, name) {
  ...
  var url = contextPath
          + '/getFile?uploadFileNameServer=core/funcs/email/attachment/'
          + aPath;
  window.open(url);
}
/**
...
function userRender(cellData, recordIndex, columIndex) {
  ...
  if ((toId && toId != '-1') || copyToIds || secretToIds) {
    ...
    if (toIdArray.length == 1) {
      ...
      return "<center> <span id=\"span_" + recordIndex + "_" + columIndex
              + "\">" + deptName + "</span></center> ";
    } else {
      return "<center> <a href=\"javascript:readStatus(" + emailBodyId
              + ");\">查看</a></center>";
    }
  }
  return "";
...
function userRenderByIn(cellData, recordIndex, columIndex) {
  ...
  if (isWebmail == "1") {
    ...
    var mail = webToUser.substring(webToUser.indexOf("<") + 1, webToUser
            .indexOf(">"));
    var html = "";
    if (name.trim()) {
      if (name.length > 10) {
    ...
    return "<a href=\"javascript:\" title=\"" + webToUser + "\">" + html
            + "</a>";
  } else {
    return cellData;
  }
...
function userRender2(cellData, recordIndex, columIndex, obj) {
  ...
  if (toIdArray.length == 1) {
    ...
    var deptName = getUserNameById(toIdSeq);
    return "&nbsp;&nbsp;<span id=\"span_" + recordIndex + "_" + columIndex
            + "\">" + deptName + "</span>";
  } else {
    return "&nbsp;&nbsp;<a href=\"javascript:readStatus(" + emailBodyId
            + ");\">查看</a>";
  }
}
/**
...
function getUserNameById(userId) {
  var url = contextPath + "/t9/core/funcs/diary/act/T9DiaryAct/getUserName.act";
  var rtJson = getJsonRs(url, "userId=" + userId);
  if (rtJson.rtState == '0') { return rtJson.rtData; }
}
...
function reDeleteMail(type, url) {
  ...
  if (window.confirm(msg)) {
    var url = contextPath
            + "/t9/core/funcs/email/act/T9InnerEMailAct/deletM.act?bodyId="
            + delete_str + "&deType=" + type
            + "&PAGE_START=&BOX_ID=&FIELD=&ASC_DESC=";
    var rtJson = getJsonRs(url);
    if (rtJson.rtState == "0") {
      alert("邮件已恢复！");
    }
    location.reload();
    if (parent.mail_menu) parent.mail_menu.location.reload();
    // history.back();
  }
}
/**
...
function bindReply(json, fck, url) {
  ...
  /*
   * var FCK = FCKeditorAPI.GetInstance(fck);
   * //获得表单设计器的顶层对象FCK，该方法定义位置fckeditorapi.js第47行 by dq 090521 var FORM_MODE =
   * FCK.EditingArea.Mode; //获取编辑区域的常量——源文件模式 var editingAreaFrame =
   * document.getElementById(fck + "___Frame"); var editModeSourceConst =
   * editingAreaFrame.contentWindow.FCK_EDITMODE_SOURCE;//常量FCK_EDITMODE_SOURCE的定义位置_source/fckconstants.js
   * if(FORM_MODE == editModeSourceConst) { FCK.Commands.GetCommand( 'Source'
   * ).Execute(); }
   */
  ...
  var content = fromName
          ...
          + sendTime
          + "的来信中写道：========<br /> "
          + "<div style=\"padding-right: 5px; padding-left: 5px; padding-bottom: 5px; border-left: #000000 2px solid; padding-top: 5px\">"
          + contentHtml
          + "</div>"
          + " <br /> "
          + " =================================================<br /> "
  ...
  editor = UE.getEditor('content').addListener("ready", function() {
...
function bindFw(json, fck, url) {
   ...
   * editingAreaFrame.contentWindow.FCK_EDITMODE_SOURCE;//常量FCK_EDITMODE_SOURCE的定义位置_source/fckconstants.js
   * if(FORM_MODE == editModeSourceConst) { FCK.Commands.GetCommand( 'Source'
   * ).Execute(); } var fromName = getUserNameById(obj.fromId); var sendTime =
   * obj.sendTime; var contentHtml = ""; if(!obj.content){ contentHtml =
   * obj.compressContent; }else{ contentHtml = obj.content; }
   */
  var content = "您好！<br /> "
          + "<br /> "
          + "<br /> "
  ...
  if (!isTouchDevice) {
    ...
    // FCK.EditingArea.Window.document.body.innerHTML = content;
    editor = UE.getEditor('content').addListener("ready", function() {
      UE.getEditor('content').setContent(contentHtml);
    });
  } else {
...
function turan_attach(attachName, attachId, module) {
  var url = contextPath
          + "/t9/core/funcs/email/act/T9InnerEMailAct/transferFolderAct.act?attachId="
          + attachId + "&attachName=" + encodeURIComponent(attachName)
          + "&module=" + module;
  var rtJson = getJsonRs(url);
  if (rtJson.rtState == "0") {
...
function bindCg(json, fck, url, resendUserId) {
  ...
  $('ensize').value = obj.ensize;// 附件大小
  if ($('copyToId').value.trim() || $('secretToId').value.trim()
          || $('toId').value.trim()) {
    bindDesc([{
      cntrlId: "toId",
      dsDef: "PERSON,SEQ_ID,USER_NAME"
  ...
  /*
   * if (isTouchDevice) { $("contentTextarea") && ($("contentTextarea").value =
   * contentHtml); } else { var FCK = FCKeditorAPI.GetInstance(fck);
   * //获得表单设计器的顶层对象FCK，该方法定义位置fckeditorapi.js第47行 by dq 090521 var FORM_MODE =
   * FCK.EditingArea.Mode; //获取编辑区域的常量——源文件模式 var editingAreaFrame =
   * document.getElementById(fck + "___Frame"); var editModeSourceConst =
   * editingAreaFrame.contentWindow.FCK_EDITMODE_SOURCE;//常量FCK_EDITMODE_SOURCE的定义位置_source/fckconstants.js
   * if(FORM_MODE == editModeSourceConst) { FCK.Commands.GetCommand( 'Source'
   * ).Execute(); } //var sendTime = obj.sendTime;
...
function updateFlag(paramArrays) {
  ...
  var rtJson = getJsonRs(contextPath
          + "/t9/core/funcs/email/act/T9InnerEMailAct/flag.act", "queryParam="
          + queryParam);
...
/**
 * type[1收件箱|2发件箱] [1发件箱已删除|2发件箱未读|3发件箱已读|4收件件箱已删除|5收件箱未读new|6收件箱已读] 绑定邮件状态
 * 
 * @return
 */
...
function statusRenderByIn(cellData, recordIndex, columIndex) {
  ...
  // }else{
  // return "<center> <a href=\"javascript:readStatus(" + emailBodyId +
  // ");\"><img src=\"" + imgPath + "/cmp/email/node_dept.gif\"
  // title=\"查看详情\"></a></center> ";
  // }
 ...
 * @return
 */
function showMenu(ctrlid, align, click, offset, duration, timeout, layer,
        showid, maxh, drag) {
  // alert("ss");
  var ctrlobj = $(ctrlid);
  if (!ctrlobj) return;
  ...
  if (is_ie && is_ie < 7) {
    if (!jsmenu['iframe'][layer]) {
      ...
      $('append_parent') ? $('append_parent').appendChild(iframe)
...
function setMenuPosition(showid, align, offset) {
  ...
  if (showobj) {
    ...
    // if(align == 1)
    // menuobj.style.width = ((menuobj.clientWidth < showobj.w ? showobj.w :
    // menuobj.clientWidth)+2) + 'px';
    if (offset < 3) {
      menuobj.style.left = ((showobj.X + menuobj.w > document.body.clientWidth)
              && (showobj.X + showobj.w - menuobj.w >= 0) ? showobj.X
              + showobj.w - menuobj.w : showobj.X)
              - menu_offset['left'] + 'px';
      if (offset == 1)
        ...
        menuobj.style.top = showobj.Y - menu_offset['top'] + 'px';
        var bb = (document.compatMode && document.compatMode != "BackCompat")
                ? document.documentElement : document.body;
        if (parseInt(menuobj.style.top) + menuobj.h > bb.clientHeight
                + bb.scrollTop)
          menuobj.style.top = showobj.Y - menuobj.h + 'px';
      }
    } else if (offset == 3) {
      menuobj.style.left = (document.body.clientWidth - menuobj.clientWidth)
              / 2 + document.body.scrollLeft - menu_offset['left'] + 'px';
      menuobj.style.top = (document.body.clientHeight - menuobj.clientHeight)
              / 2 + document.body.scrollTop - menu_offset['top'] + 'px';
    }
...
function isValidDateStrTime(str) {
  ...
  if (strArray.length != 2) {
    ...
    if (!isValidDateStr(strArray[0]) || strArray[1].match(re1) == null
            || strArray[1].match(re2) != null) { return false; }
  }
}
...
function changeBox(dom) {
  ...
  var boxId = dom.value;
  var url = contextPath
          + "/t9/core/funcs/email/act/T9InnerEMailAct/changMail2OtherBox.act?bodyIds="
          + ids + "&boxId=" + boxId;
  var rtJson = getJsonRs(url);
  if (rtJson.rtState == "0") {
