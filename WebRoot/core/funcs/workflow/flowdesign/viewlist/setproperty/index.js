...
var requestUrl = contextPath + "/t9/core/funcs/workflow/act/T9FlowProcessAct";
var prcsList ; //步骤列表
var prcsNode ;//步骤节点
var flowList; //流程列表
var disAipList;//呈批单
...
var UserHi,UserDesc;
var AutoUsers,mailToDesc,autoDeptOpDesc;
var selectedProc;
var reg = /['"]/g;
var tabs = null;
function doInit(){
...
function displayEditMessage(){
  ...
  if(prcsNode.childFlow != 0){
    ...
    autoDeptOpDesc =  editMessage.rtData.autoDeptOpDesc;
  }else{
    AutoUsers = editMessage.rtData.autoUsers;
    mailToDesc = editMessage.rtData.mailToDesc;
    autoDeptOpDesc =  editMessage.rtData.autoDeptOpDesc;
...
function setFlowDispatchPage(){
  ...
  if (prcsNode.viewPriv) {
    $('viewPriv').value = prcsNode.viewPriv;
  }
}
function setAutoSelectPage(){
  ...
  if(isEdit){
    ...
    if( autoType == '2'
      ...
      $('autoDeptOp').value = prcsNode.autoDeptOp;
      $('autoDeptOpDesc').innerHTML =  autoDeptOpDesc;
      $('deptItemDiv').show();
    }
  }
...
function prcsTypeChange2(){
  ...
  if($F('childFlow') == '0'){
    ...
    document.getElementById("processTbody").style.display = '';
    //$('processTbody').show();
    //$('processTbody2').show();
  }else{
    tabs.setDisable(true , 1);
    ...
    if ($('processTbody')) {
     // alert(document.getElementById("processTbody").style.display);
      document.getElementById("processTbody").style.display = 'none';
     // $('processTbody').hide();
      //$('processTbody2').hide();
    }
    $('childFlowTbody').show();
...
function selPlugin(){
  var url = contextPath + "/t9/core/funcs/workflow/act/T9PluginAct/getPluginList.act" ;
  loc_y=loc_x=200;
