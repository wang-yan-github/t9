...
var reg = /['"]/g;
function loadSelect() {
  var url = "<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowSortAct/getSortTreeJson.act";
...
function doInit() {
   loadSelect();
   if (isAdmin) {
     var url = "<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowSortAct/getDeptList.act";
     var rtJson = getJsonRs(url);
     if (rtJson.rtState == "0") {
   ...
   if(seqId) {
    ...
    var url = "<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowSortAct/getFlowSort.act";
...
function commitItem() {
  ...
  if(seqId) {
    var url = "<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowSortAct/updateFlowSort.act";
    ...
    var url = "<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowSortAct/addFlowSort.act";
    var rtJson = getJsonRs(url, mergeQueryString($("sortflowInfoForm")));
    if (rtJson.rtState == "0") {
      alert(rtJson.rtMsrg);
      loadSelect();
...
<form id="sortflowInfoForm" name="sortflowInfoForm" method="post">
  ...
  <input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.workflow.data.T9FlowSort"/>
  <div align=center>
