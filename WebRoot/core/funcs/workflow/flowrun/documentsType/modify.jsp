...
function doInit(){
    getFlowType();
	var url = "<%=contextPath%>/t9/core/funcs/workflow/act/T9DocumentsTypeAct/getDocumentsTypeDetail.act?seqId=<%=seqId%>";
    ...
    if(data.documentsFont){
      var urls = contextPath + "/t9/core/funcs/workflow/act/T9DocumentsTypeAct/getDocWordById.act?seqId="+data.documentsFont;
...
function setModule(selected2) {
  var url = contextPath + "/t9/subsys/inforesouce/docmgr/act/T9DocAct/getDocModule.act";  
...
function getFlowType() {
  var url = contextPath + "/t9/subsys/inforesouce/docmgr/act/T9DocAct/getFlowType.act";  
  var json = getJsonRs(url);
  if (json.rtState == "0") {
    var type = json.rtData;
    if (type.length > 0) {
...
<br>
<form action="<%=contextPath%>/t9/core/funcs/workflow/act/T9DocumentsTypeAct/updateDocumentsTypeInfo.act"  method="post" name="form1" id="form1" onsubmit="">
