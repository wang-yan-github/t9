...
<%@ page import="t9.core.funcs.person.data.T9Person,t9.core.global.T9SysProps,t9.core.funcs.workflow.act.T9WorkHandleAct" %>
<%
...
String noCancle = T9Utility.null2Empty(request.getParameter("noCancle"));
...
T9WorkHandleAct act = new T9WorkHandleAct();
%>
...
window.UEDITOR_HOME_URL = "/t9/core/js/cmp/ueditor/ueditor/";//编辑器项目路径  
</script> 
<script type="text/javascript" src="<%=contextPath %>/core/js/cmp/ueditor/ueditor/ueditor.config.js"></script>
<script type="text/javascript" src="<%=contextPath %>/core/js/cmp/ueditor/ueditor/ueditor.all.js"></script>
<link rel=stylesheet href="<%=contextPath %>/core/js/cmp/ueditor/ueditor/themes/default/css/ueditor.css">
...
<script type="text/javascript">
var upload_limit=1,limit_type="<%=T9SysProps.getLimitUploadFiles()%>";
var oa_upload_limit="<%=T9SysProps.getLimitUploadFiles()%>";
var attachUrl = "<%=T9SysProps.getAttachPath()%>";//附件路径
var swfupload;
...
function checkClose(){
  if (mouse_is_out) {
    if (isNew == 1 ) {
      <%=T9Utility.isNullorEmpty(noCancle) ? "cancelRun(true);" : "" %>
    } else {
      mouse_is_out = false;
...
function loadAIP(seq_id) {
  var rtJson = getJsonRs( contextPath + "/t9/core/funcs/workflow/act/T9FlowPrintAct/loadAip.act", "seq_id="+seq_id);
  if (rtJson.rtState == "0") {
...
function aip_upload()
{
  ...
  if(obj.IsSaved())
    ...
    try{  
        ...
        URL = "http://<%=host %>/t9/core/funcs/workflow/act/T9FlowPrintAct/uploadAipFile.act";
...
function getAipData( flag) {
  ...
  if (nodes) {
    var rtJson = getJsonRs(contextPath + "/t9/core/funcs/workflow/act/T9FlowPrintAct/loadAutoFlag.act", "runId=<%=runId %>&flowId=<%=flowId %>&nodes=" + encodeURIComponent(nodes));
    if (rtJson.rtState == "0") {
      var data=rtJson.rtData;
...
if (!dispAipFile) {
  ...
  success = "1";
  var URL =  "http://<%=host %>/t9/core/funcs/workflow/act/T9FlowPrintAct/getAipFile.act?module=workflow&attachmentId=" + dispAipFile;
...
<form style="display:none" action="<%=contextPath %>/t9/core/funcs/workflow/act/T9AttachmentAct/uploadImg.act" name="workFlowForm" target="returnPage" id="workFlowForm"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %>/t9/core/funcs/workflow/act/T9AttachmentAct/uploadFile.act" name="formFile" target="returnPage" id="formFile"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %>/t9/core/funcs/workflow/act/T9AttachmentAct/uploadFile.act" name="ATTACHMENT1_formFile" target="returnPage" id="ATTACHMENT1_formFile"  method="post" enctype="multipart/form-data">
<input type="hidden"  name="flowId" value="<%=flowId %>"/>
<input type="hidden"  name="runId" value="<%=runId %>"/>
<input type="hidden" name="prcsId" value="<%=prcsId %>"/>
<input type="hidden"  name="flowPrcs" value="<%=flowPrcs %>"/>
    ...
    if (sealForm == 1 ) {
      ...
      if(typeof seal_id == "undefined") {
        show_seal(item,"addSeal");
    	  ...
    	  var URL = "http://"+ host +"/t9/core/funcs/workflow/act/T9FlowFormViewAct/getSeal.act?id=" + seal_id; 
