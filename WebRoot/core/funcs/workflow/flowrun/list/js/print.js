...
var isPrint = true;
function doInit(){
  var url = contextPath + "/t9/core/funcs/workflow/act/T9FlowFormPrintAct/getFormPrintMsg.act";
  var json = getJsonRs(url , "runId=" + runId +"&flowId=" + flowId + "&flowView=" + flowView + "&isAutoPrint=" + isAutoPrint);
  //alert(rsText);
  if(json.rtState == '0'){
...
function addViewUser(u){
  var tr = new Element('tr' , {'class' : 'TableData'});
  $('viewTbody').appendChild(tr);
  var td = new Element('td',{'align' : 'left','width' : '40%'});
  tr.appendChild(td);
  td.innerHTML = u.userName;
  td = new Element('td',{'align' : 'left','width' : '30%'});
  tr.appendChild(td);
  td.innerHTML = u.status;
  td = new Element('td',{'align' : 'left','width' : '30%'});
  tr.appendChild(td);
  td.innerHTML = u.time;
}
...
function LoadSignDataSign(data,count,content,version)
{
  ...
  try {
    ...
    if(version==0)
      vDWebSignSeal.SetSealSignData (strObjectName,fullOrgName);
    else
