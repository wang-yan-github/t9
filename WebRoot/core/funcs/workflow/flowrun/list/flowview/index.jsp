...
function doInit(){
  var url = contextPath + "/t9/core/funcs/workflow/act/T9MyWorkAct/getWorkMsg.act";
  ...
  if(json.rtState == '0'){
    ...
    if (window.navigator.appVersion.match(/9./i)!='9.') {
      jso.push({title:"图形视图",imgUrl:imgPath + "/asset.gif", contentUrl:"<%=contextPath %>/core/funcs/workflow/flowrun/list/flowview/viewgraph.jsp?runId=<%=runIdStr%>&flowId=<%=flowIdStr%>", useIframe:true});
       ...
       var requestURL = contextPath + "/t9/core/funcs/workflow/act/T9MyWorkAct";
	...
	if(type == 1){
      if (window.navigator.appVersion.match(/9./i)!='9.') {
        jso.push({title:"流程设计图",imgUrl:imgPath + "/workflow.gif", contentUrl:"<%=contextPath %>/core/funcs/workflow/flowrun/list/viewgraph/index.jsp?flowId=<%=flowIdStr%>" , useIframe:true});
         ...
         jso.push({title:"流程设计图",useTextContent:true,imgUrl:imgPath + "/asset.gif", contentUrl:"<%=contextPath %>/t9/core/funcs/workflow/act/T9FlowProcessAct/getProcessList1.act?flowId=<%=flowIdStr%>&type=1", useIframe:true});
       }
    }
