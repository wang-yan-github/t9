...
function addUser(i , userFilter , childFlow , parentFlowId , backPrcs){
  ...
  var actionUrl = contextPath + "/core/funcs/workflow/flowrun/list/turn/userSelect/FlowUserSelect.jsp?prcsId=" + i + "&userFilter=" + userFilter + "&childFlow=" + childFlow + "&parentFlowId=" + parentFlowId+ "&backPrcs=" + backPrcs;
  var attrs = "dialogLeft=480px;dialogTop=350px;dialogWidth=520px;dialogHeight=400px;status=no;resizable=no";
  var chooseType;
  //var chooseType = window.showModalDialog(actionUrl,window,attrs); 
  //IE浏览器
  if(!!window.ActiveXObject || "ActiveXObject" in window){
  return window.showModalDialog(actionUrl, self, attrs);
  }{ //其他浏览器
	  openWindow(actionUrl,'',520,400);
  }
}
function setAction(){
...
function myClearUser() {
  $('VIEW_USER_ID').value = "";
  $('VIEW_USER_NAME').innerHTML = "";
}
...
<tbody id="viewList" style="display:none">
   <tr>
      <td nowrap class="TableContent"  colspan="2"><span style="margin:3px 0 3px 3px;font-weight:bold" >增加传阅人：</span><br />
        <input type="hidden" id="VIEW_USER_ID" name="VIEW_USER_ID" value="">
        <textarea cols=40 id="VIEW_USER_NAME"  name="VIEW_USER_NAME" rows=5 class="BigStatic" wrap="yes" readonly></textarea>
        <a href="javascript:;" class="orgAdd" onClick="selectUser(['VIEW_USER_ID', 'VIEW_USER_NAME'])">选择</a>
        <a href="javascript:;" class="orgClear" onClick="myClearUser()">清空</a>
   </tr>
</tbody>
    <tr id="need_remind">
      <td colspan="2">
