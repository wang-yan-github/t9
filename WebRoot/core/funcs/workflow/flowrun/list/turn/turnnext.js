var requestUrl = contextPath + "/t9/core/funcs/workflow/act/T9WorkTurnAct";
...
function doInit(){
  ...
  if(json.rtState == '0'){
    ...
    viewPriv = data.flowRun.viewPriv;
    if (viewPriv) {
      $('VIEW_USER_ID').value = data.flowRun.viewUser;
      $('VIEW_USER_NAME').innerHTML = data.flowRun.viewUserName;
    }
...
function changePrcs(){
 ...
 function setViewPrivShow() {
   if (viewPriv) {
     $('viewList').show();
   }
 }
 function setViewPrivHide() {
   if (viewPriv) {
     $('viewList').hide();
   }
 }
...
function getPrcsText(tmp) {
 ...
 if(tmp.prcsUser){
   ...
   if (prcsUser.charAt(prcsUser.length - 1) != ',') {
     prcsUser += ",";
   }
   if(tmp.userLock && tmp.isAutoSelect){
