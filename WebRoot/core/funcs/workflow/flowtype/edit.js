var requestUrl = contextPath + "/t9/core/funcs/workflow/act/T9FlowTypeAct"

function doInit(){
  if(flowId){
    //t9/t9/core/funcs/workflow/act/T9FlowTypeAct/getEditMessage.act?flowId=1    //设置显示，取分类，flow json对象，所属表单对象 ,以及部门名称， 下面是工作数量，已删除工作数量，决定是否取表单列表
	//$('#fsUploadRow').remove();
    ...
    if(json.rtState == '0'){
      ...
      var attrIds = flowType.attachmentId;
      var attrNames = flowType.attachmentName;
      ...
      if($('attachLink_0')){document.getElementById('add_image').style.visibility="hidden";}
      
      if (flowType.viewPriv == '1') {
        $('VIEW_PRIV').checked = true;
      }
      
      if (flowType.viewUser) {
        $('VIEW_USER_ID').value = flowType.viewUser;
        $('VIEW_USER_NAME').innerHTML = json.rtData.viewUserName;
      }
...
function delFlowType(){
  if(confirm("确定删除吗？\n这将删除以下数据：\n1,流程描述与步骤设置。\n2,依托于该流程的所有工作。")){
    var url =  contextPath + "/t9/core/funcs/workflow/act/T9FlowTypeAct/delFlowType.act";
    var json = getJsonRs(url, "flowId=" + flowId);
    if(json.rtState == '0'){
     // alert(json.rtMsrg);
...
function clear(){
  if(confirm("确定清空依托于该流程的所有工作数据吗?")) {
    var url =  contextPath + "/t9/core/funcs/workflow/act/T9FlowTypeAct/empty.act";
    var json = getJsonRs(url, "flowId=" + flowId);
    if(json.rtState == '0'){
      //alert(json.rtMsrg);
      myAlert(json.rtMsrg , 150 ,20);
