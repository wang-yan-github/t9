...
<%@page import="t9.core.funcs.workflow.util.T9WorkFlowUtility"%>
<HTML>
<HEAD>
<TITLE>表单智能设计器</TITLE>
<link rel="stylesheet" href = "<%=cssPath%>/style.css">
<link rel=stylesheet href="<%=contextPath %>/core/js/cmp/ueditor/ueditor/themes/default/css/ueditor.css" type="text/css"/>
...
<script type="text/javascript" src="<%=contextPath %>/core/js/cmp/ueditor/ueditor/form-design/design-plugin.js"></script>
<META http-equiv=Content-Type content="text/html; charset=UTF-8">
<SCRIPT language=JavaScript>
var itemMax = 0 ;
var formId = 0 ;
//var flag = "true"; 
var isSave2DataTable = <%=T9WorkFlowUtility.isSave2DataTable() %>;
self.moveTo(0,0);
...
function send(flag)
{
  if(CheckForm())
  {
   ...
   /// var url = "";
    ...
    if (!flag) {
      if(seqId != null && seqId != ""){
        url = "<%=contextPath%>/t9/core/funcs/workflow/act/T9FlowFormAct/updateForm.act";
      }else{
        url = "<%=contextPath%>/t9/core/funcs/workflow/act/T9FlowFormAct/insertForm.act";
      }
    } else {
      url = "<%=contextPath%>/t9/core/funcs/workflow/act/T9FormVersionAct/genFormVersion.act";
    }
...
function loadData(){
  var url = "<%=contextPath%>/t9/core/funcs/workflow/act/T9FlowFormAct/getFormView.act?";
...
function td_textfield() {
    var designer = UE.getEditor('editor'); 
	designer.execCommand('mini-textbox');
	alert(1);
	
  //var FCK = FCKeditorAPI.GetInstance('FORM_CONTENT'); //获得表单设计器的顶层对象FCK，该方法定义位置fckeditorapi.js第47行 by dq 090521
  //FCK.Focus();
  //FCK.Commands.GetCommand("NTextField").Execute(); //仿照fcktoolbarbutton.js第71行的写法 by dq 090521
  /**UE.registerUI('textDialog',function(editor, uiName){
	    //创建dialog
        var dialog = new UE.ui.Dialog({ 
	        //指定弹出层中页面的路径，这里只能支持页面,因为跟addCustomizeDialog.js相同目录，所以无需加路径
	        iframeUrl:'editor/plugins/NTextField/ntextfield.jsp',
	        //需要指定当前的编辑器实例
	        editor:editor,
	        //指定dialog的名字
	        name:uiName,
...
//--- 数据获取控件（新） ---
function td_mobile_seal() {
  var FCK = FCKeditorAPI.GetInstance('FORM_CONTENT');
  FCK.Focus();
  FCK.Commands.GetCommand("NMobileSeal").Execute(); 
}
//--- 流程数据获取控件 ---
...
//--- 图片上传控件 ---
function td_imgupload() {
  var FCK = FCKeditorAPI.GetInstance('FORM_CONTENT');
  FCK.Focus();
  FCK.Commands.GetCommand("NImgupload").Execute(); 
}
...
<TABLE class=TableBlock height="100%" width="100%" align=center>
  ...
  <TR bgColor=#dddddd>
    ...
    <script id="editor" type="text/plain" style="height:550px;"></script>
    <SCRIPT language=JavaScript>
    createUeditor();
    </script>
<%
      ...
      <TABLE class=TableBlock width=120 align=center border=0>
          ...
          <TR class=TableData>
            ...
            <!--  <BUTTON style="WIDTH: 120px; TEXT-ALIGN: left" onclick=module_select()>
              ...
              <BUTTON style="WIDTH: 120px; TEXT-ALIGN: left" onclick=td_flow_data_fetch()>
               <IMG height=20 src="image/data.gif" width=20 align=absMiddle>流程数据控件</BUTTON> <br>
               <BUTTON style="WIDTH: 120px; TEXT-ALIGN: left" onclick=td_imgupload()>
               <IMG height=20 src="image/imgupload.gif" width=20 align=absMiddle>图片上传控件</BUTTON><br>
               <BUTTON style="WIDTH: 120px; TEXT-ALIGN: left" onclick=td_mobile_seal()>
               <IMG height=20 src="image/seal.gif" width=20 align=absMiddle>移动签章控件</BUTTON>
             </TD>
           </TR>
         </TBODY>
       </TABLE><BR>
       <TABLE class=TableBlock width=120 align=center border=0>
        ...
        <FORM id="form1" name='form1' >
          <input type="hidden" id="dtoClass" name="dtoClass" value="t9.rad.flowform.data.T9FlowFormType"/>
