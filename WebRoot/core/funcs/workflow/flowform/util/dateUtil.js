...
function getIp(){
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getIpAdd.act"; 
  var rtJson = getJsonRs(url); 
  return rtJson.rtData.ip;
}
function runName(){ //工作名称/文号
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getRunName.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function getUser(){//当前用户姓名
  var url = contextPath + "/t9/core/funcs/person/act/T9PersonAct/getLoginUser.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function beginTime(){ //流程开始日期
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getBeginTime.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function runId(){ //流水号
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getRunId.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function endTime(){ //流程开始日期
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getEndTime.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function deptNameLong(){//当前用户部门（长名称）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptNameLong.act?";
  var rtJson = getJsonRs(url);
  var result = "";
  if(rtJson.rtState == "0"){
...
function deptNameShort(){//当前用户部门（短名称）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptNameShort.act?";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function getUserPriv(domId){    //权限列表
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getSelectData.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function getPerson(domId){    //人员列表
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getPerson.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function getDepartment(domId){    //部门列表
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDepartment.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptParent(domId){   //部门主管（上级部门）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptParent.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptLocalInput(){ //部门主管（本部门 input）
  var url =contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptLocalInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function deptParentInput(){//部门主管（上级部门  input）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptParentInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    return rtJson.rtData[0].userName; 
  }else{
...
function deptFirstInput(){ //部门主管（ 一级部门 input）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptFirstInput.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    return rtJson.rtData[0].userName; 
  }else{
...
function deptFirst(domId){   //部门主管（一级部门）
  var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptFirst.act";
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
  var select = document.getElementById(domId);
  var len = select.options.length;
...
function deptLocal(domId){ //本部门主管
  ...
  // var url = "/t9/core/funcs/person/act/T9UserPrivAct/getAutoData.act?seqId=" + seqId;
   var url = contextPath + "/t9/core/funcs/workflow/util/T9FlowFormUtilAct/getDeptLocal.act";
   var rtJson = getJsonRs(url);
   if(rtJson.rtState == "0"){
...
function data_fetch(obj,run_id,item_str){
  ...
  var URL= contextPath + "/t9/core/funcs/workflow/act/T9FormDataFetch/dataFetch.act";
