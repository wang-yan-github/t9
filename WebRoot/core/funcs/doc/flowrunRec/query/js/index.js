...
function flowNameRender(cellData, recordIndex, columIndex){
  ...
  var html = "<a href='javascript:;'  onclick='formView2(" + runId + "," + flowId + ")'>" + cellData + "</a>";
  return html;
}
/**
 * 流程状态描画器
...
function loadFlowType(){
  var url = contextPath+moduleSrcPath+'/act/T9WorkQueryAct/getFlowTypeJson.act?sortId=' + sortId;
...
function restore() {
  var par = arguments[2];
  var url = contextPath +moduleSrcPath+"/act/T9FlowManageAct/restore.act";
...
function exportZip() {
  ...
  var url = contextPath +moduleSrcPath+"/act/T9FlowExportAct/exportFlowZip.act?runIdStr=" + runIds;
  window.open(url);
}
function delWorkFlow() {
  ...
  var url = contextPath +moduleSrcPath+"/act/T9FlowManageAct/delWorkFlow.act";
...
function endWorkFlow() {
 ...
 var url = contextPath +moduleSrcPath+"/act/T9FlowManageAct/endWorkFlow.act";
 var json = getJsonRs(url, "runIdStr=" + runIds) ;
 if (json.rtState == '0') {
   alert(json.rtMsrg); 
   pageMgr.search();
...
function focus(event , ee, par) {
  if(window.confirm(tooltipMsg6)) {
    var url = contextPath + "/t9/core/funcs/doc/flowrunRec/act/T9FlowManageAct/focus.act";
    var json = getJsonRs(url, par) ;
    if (json.rtState == '0') {
      alert(json.rtMsrg); 
      pageMgr.search();
...
function calFocus(event , ee, par) {
  if(window.confirm(tooltipMsg7)) {
    var url = contextPath +moduleSrcPath+"/act/T9FlowManageAct/calFocus.act";
    var json = getJsonRs(url, par) ;
    if (json.rtState == '0') {
      alert(json.rtMsrg); 
      pageMgr.search();
