...
function loadFlowType(){
  var url = contextPath+'/t9/core/funcs/doc/act/T9FlowTypeAct/getFlowTypeJson.act?sortId=' + sortId;
...
function loadData(pageIndex , showLength , flowId , opFlag){
  ...
  var date1 = new Date();
  var url =contextPath+'/t9/core/funcs/doc/flowrunRec/act/T9MyWorkAct/getMyWorkList.act?sortId=' + sortId;
...
function addRow(data , i , isLast , title){
  ...
  titlestr =  "<a href='javascript:;' onclick='formView2("+ data.runId +"," + data.flowId +")'>" + titlestr + "</a>";
  var prcsName = "";
  if(type == 3){
  ...
  if(type != 3){
    ...
    var url = contextPath + "/t9/core/funcs/doc/act/T9FlowExportAct/exportFlowZip.act?runIdStr=" + data.runId;
  ...
  if(type != 3){
    ...
    +  "<td align=center title=\"" + data.title + "\">" + titlestr +"</td>"
...
function exportAction(){
  var par = arguments[2]; 
  var url = contextPath + "/t9/core/funcs/doc/act/T9FlowExportAct/exportFlowZip.act?" + par;
  window.open(url);
}
function delAction(){
...
function deleteRun(runId) {
  if (window.confirm(tooltipMsg5)) {
    var url = contextPath+'/t9/core/funcs/doc/flowrunRec/act/T9MyWorkAct/delRun.act';
...
function callBack(par){
  if(window.confirm(tooltipMsg3)) {
    var url = contextPath+'/t9/core/funcs/doc/act/T9MyWorkAct/callBack.act';
    var json = getJsonRs(url , par);
    if (json.rtState == '0') {
      alert(json.rtMsrg);
      location.reload();
...
function stop(par){
  if(window.confirm(tooltipMsg4)) {
    var url = contextPath+'/t9/core/funcs/doc/act/T9FreeFlowTypeAct/stop.act';
    var json = getJsonRs(url , par);
    if (json.rtState == '0') {
      alert(json.rtMsrg);
      location.reload();
