...
<%@ page import="t9.core.funcs.person.data.T9Person,t9.core.funcs.doc.util.T9WorkFlowUtility" %>
<%
...
var reg = /['"]/g;
function loadSelect() {
  var url = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/getSortName.act";
...
function doInit() {
  //loadSelect();
   if (isAdmin) {
     var url = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/getDeptList.act";
     var rtJson = getJsonRs(url);
     if (rtJson.rtState == "0") {
   ...
   // for(var i=0; i<sortParentSel.options.length; i++) {
      ...
      //}
    //}
    var url = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/getFlowSort.act";
...
function commitItem() {
  ...
  if(seqId) {
    var url = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/updateFlowSort.act";
    var rtJson = getJsonRs(url, mergeQueryString($("sortflowInfoForm")));
    if (rtJson.rtState == "0") {
      alert(rtJson.rtMsrg);
      window.location = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/listFlowSort.act";
    }else {
      alert(rtJson.rtMsrg); 
    }    
  }else {
    var url = "<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/addFlowSort.act";
    var rtJson = getJsonRs(url, mergeQueryString($("sortflowInfoForm")));
    if (rtJson.rtState == "0") {
      alert(rtJson.rtMsrg);
      loadSelect();
...
<form id="sortflowInfoForm" name="sortflowInfoForm" method="post">
  ...
  <input type="hidden" id="dtoClass" name="dtoClass" value="t9.core.funcs.doc.data.T9DocFlowSort"/>
  <div align=center>
  ...
  <tr class="TableControl">
    <td colspan="2" align="center">
     <% if (!isAdmin) { %>
      ...
      <input type="button" value="返回" class="SmallButtonW" onclick="location='<%=contextPath %><%=moduleSrcPath %>/act/T9FlowSortAct/listFlowSort.act'">
    </td>
  </tr>
