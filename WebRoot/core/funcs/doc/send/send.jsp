<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
     ...
     <%@ page import="t9.core.funcs.doc.util.T9DocUtility"  %>
<%
...
function doInit() {
  ...
  var url2 = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/getRemindType.act";
  ...
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/getSendMessage.act?runId="+ runId;
  cfgs = {
      dataAction: url,
...
function cancel(seqId , status){
  if (confirm("确认收回此公文！")) {
    var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/cancel.act?seqId=" + seqId + "&status=" + status;
...
if (confirm("确认重新发送此公文！")) {
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/resend.act?seqId=" + seqId + "&status=" + status;
  var json = getJsonRs(url);
  if (json.rtState == '0') {
    pageMgr.search();
  }
...
function send() {
  ...
  getOut($('deptId'));
  <% if (T9DocUtility.usingEsb()) { %>
  getOut($('deptId2'));
  <% } %>
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/sendDoc.act";
...
if (confirm("确认收回所有已发送公文！")) {
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocSendAct/cancelAll.act?runId=" + runId;
  var json = getJsonRs(url);
  if (json.rtState == '0') {
    pageMgr.search();
  }
