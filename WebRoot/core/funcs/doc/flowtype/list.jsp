...
function clear(){
  ...
  {
    var url =  contextPath + "<%=moduleSrcPath %>/act/T9FlowTypeAct/empty.act";
...
function del(){
  ...
  if(confirm("确定删除吗？\n这将删除以下数据：\n1,流程描述与步骤设置。\n2,依托于该流程的所有工作。"))
  {
    var url =  contextPath + "<%=moduleSrcPath %>/act/T9FlowTypeAct/delFlowType.act";
    var json = getJsonRs(url, "flowId=" + flowId);
    if(json.rtState == '0'){
      alert(json.rtMsrg);
      parent.leftFrame.location.reload();
...
function exportFlowType(){
  var flowId = arguments[2];
  window.open(contextPath + "<%=moduleSrcPath %>/act/T9FlowExportAct/exportFlow.act?flowId=" + flowId);
}
function inport(){
  var flowId = arguments[2];
...
function doInit(){
  var url = contextPath + "<%=moduleSrcPath %>/act/T9FlowTypeAct/getFlowTypeBySort.act";
