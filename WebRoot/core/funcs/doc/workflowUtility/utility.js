...
function formViewByRunId(runId , flowName) {
  var url = contextPath  + moduleSrcPath + "/act/T9FlowRunAct/getFlowIdByRunId.act";
  var  par = 'runId=' + runId;
...
function playAction(){
  ...
  var param = "attachmentName=" + encodeURIComponent(attachName) + "&attachmentId=" + attachId + "&moudle=doc&mediaType=" + mediaType + "&video=" + video;
...
function getSortIdByName(sortName){
  var url = contextPath  + moduleSrcPath + "/act/T9FlowSortAct/getSortId.act";
...
function getSortIdsByName(sortName){
  if (sortName) {
    var url = contextPath  + moduleSrcPath + "/act/T9FlowSortAct/getSortIds.act";
    var json = getJsonRs(url , "sortName=" + sortName);
    if(json.rtState == "0"){
      return json.rtData;
    }
