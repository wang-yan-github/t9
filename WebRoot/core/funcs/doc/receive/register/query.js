...
function opRender(cellData, recordIndex, columIndex){
  ...
  var runId = this.getCellData(recordIndex,"runId");
  var isDel = this.getCellData(recordIndex,"isDel");
  ...
  if (isDel == '1') {
    result += "&nbsp;<a  href='javascript:void(0)' onclick='restoreDel("+seqId+")'>恢复</a>";
  } else {
...
function restoreDel(seqId) {
  if(!confirm("确认恢复删除的工作！")) {
    return ;
  }
 var url = contextPath + "/t9/core/funcs/doc/receive/act/T9DocRegisterAct/restoreDelRegister.act";
 var json = getJsonRs(url, "seqId=" + seqId) ;
 if (json.rtState == '0') {
   alert(json.rtMsrg); 
   pageMgr.search();
...
function query() {
  if (pageMgr == null) {
    var url = contextPath + "/t9/core/funcs/doc/receive/act/T9DocRegisterAct/queryRegList.act?type=" +type ;
    cfgs = {
        dataAction: url,
        container: "container",
        moduleName:"doc",
        ...
        colums:[
                ...
                {type:"data", name:"isDel", text:"登记状态", width:"5%" , render:delRender},
...
function delRender (cellData, recordIndex, columIndex) {
  if (cellData == '1') {
    return "已删除";
  } else {
    return "未删除";
