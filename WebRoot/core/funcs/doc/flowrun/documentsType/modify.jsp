...
function doInit(){
    getFlowType();
	var url = "<%=contextPath%><%=moduleSrcPath %>/act/T9DocumentsTypeAct/getDocumentsTypeDetail.act?seqId=<%=seqId%>";
    ...
    if(data.documentsFont){
      var urls = contextPath + "<%=moduleSrcPath %>/act/T9DocumentsTypeAct/getDocWordById.act?seqId="+data.documentsFont;
...
function setModule(selected2) {
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocAct/getDocModule.act";  
...
function checkForm(){
  ...
  var typeNo = $("typeNo").value;
  if(typeNo && !isInteger(typeNo)){
    alert("排序号只能是整型！");
    $("typeNo").focus();
    return (false);
  }
...
function getFlowType() {
  var url = contextPath + "/t9/core/funcs/doc/send/act/T9DocAct/getFlowType.act";  
  var json = getJsonRs(url);
  if (json.rtState == "0") {
    var type = json.rtData;
    if (type.length > 0) {
...
<br>
<form action="<%=contextPath%><%=moduleSrcPath %>/act/T9DocumentsTypeAct/updateDocumentsTypeInfo.act"  method="post" name="form1" id="form1" onsubmit="">
  <table class="TableBlock" width="80%" align="center">
    <tr>
      ...
      <td class="TableData">
        <input type="text" name="typeNo" id="typeNo" class="BigInput" size="15">
      </td>
