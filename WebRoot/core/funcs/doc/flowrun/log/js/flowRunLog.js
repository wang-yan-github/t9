...
function loadFlowPrcs(cntrlId){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/getFlow.act?sortId=' + sortId;
...
function exportExecl(cntrlId){
  ...
  var param = form.serialize();
  location = contextPath +moduleSrcPath+"/act/T9WorkLogAct/export.act?" + param;
}
/**
 * 消息提示
 * @param msrg
...
function getPrcsName(flowId,flowPrcs){
  var url = contextPath+moduleSrcPath+'/act/T9WorkLogAct/getPrcsName.act';
...
function doDelete(){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkLogAct/deleteLog.act?seqId=' + ids;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    alert("日志删除成功！");
  }else{
...
function doDeleteBySearch(cntrlId){
  ...
  var param = form.serialize();
  var url = contextPath+moduleSrcPath+'/act/T9WorkLogAct/deleteLogBySearch.act?' + param;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function loadFlowType(){
  var url = contextPath+moduleSrcPath+'/act/T9FlowTypeAct/getFlowTypeJson.act?checkType=1&sortId=' + sortId;
