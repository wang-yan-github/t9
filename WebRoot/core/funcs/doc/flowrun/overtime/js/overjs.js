...
function doInit(flag){
  ...
  loadFlowType() ;
  var url = contextPath+moduleSrcPath+'/act/T9WorkOverTimeAct/getWorkOverTimeList.act';
  cfgs = {
      dataAction: url,
...
function exportCsv() {
  var url = contextPath+moduleSrcPath+'/act/T9WorkOverTimeAct/exportCsv.act?' + getParam();
  window.open(url);
}