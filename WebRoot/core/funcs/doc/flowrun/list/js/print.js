...
var isPrint = true;
function doInit(){
  var url = contextPath +moduleSrcPath+"/act/T9FlowFormPrintAct/getFormPrintMsg.act";
...
function createDiv(event , node , attachId , attachName , ext){
  ...
  var readpdf = { attachmentId:attachId , attachmentName: attachName ,name:'<div  style="padding-top:5px;margin-left:10px">阅读<div>',action:playAction,extData: ""};
  ...
  if (isPdf(attachName)) {
    ...
    menuD.push( down );
    menuD.push( save );
  } else if ( isMedia(attachName) || isVideo(attachName)){
    menuD.push( play );
    menuD.push( down );
    menuD.push( save );
