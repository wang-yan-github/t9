...
function loadAIP(seq_id) {
  var rtJson = getJsonRs( contextPath + "<%=moduleSrcPath %>/act/T9FlowPrintAct/loadAip.act", "seq_id="+seq_id);
  if (rtJson.rtState == "0") {
...
function aip_upload()
{
  ...
  if(obj.IsSaved())
    ...
    try{  
        ...
        URL = "http://<%=host %><%=moduleSrcPath %>/act/T9FlowPrintAct/uploadAipFile.act";
...
function getAipData( flag) {
  ...
  if (nodes) {
    var rtJson = getJsonRs(contextPath + "<%=moduleSrcPath %>/act/T9FlowPrintAct/loadAutoFlag.act", "runId=<%=runId %>&flowId=<%=flowId %>&nodes=" + encodeURIComponent(nodes));
    if (rtJson.rtState == "0") {
      var data=rtJson.rtData;
...
if (!dispAipFile) {
  ...
  success = "1";
  var URL =  "http://<%=host %><%=moduleSrcPath %>/act/T9FlowPrintAct/getAipFile.act?module=workflow&attachmentId=" + dispAipFile;
...
<form style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/T9AttachmentAct/uploadFile.act" name="workFlowForm" target="returnPage" id="workFlowForm"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/T9AttachmentAct/uploadFile.act" name="formFile" target="returnPage" id="formFile"  method="post" enctype="multipart/form-data">
...
<form  style="display:none" action="<%=contextPath %><%=moduleSrcPath %>/act/T9AttachmentAct/uploadFile.act" name="ATTACHMENT1_formFile" target="returnPage" id="ATTACHMENT1_formFile"  method="post" enctype="multipart/form-data">
<input type="hidden"  name="flowId" value="<%=flowId %>"/>
<input type="hidden"  name="runId" value="<%=runId %>"/>
<input type="hidden" name="prcsId" value="<%=prcsId %>"/>
<input type="hidden"  name="flowPrcs" value="<%=flowPrcs %>"/>
    ...
    if (sealForm == 1 ) {
      ...
      if(typeof seal_id == "undefined") {
        show_seal(item,"addSeal");
    	  ...
    	  var URL = "http://"+ host +"<%=moduleSrcPath %>/act/T9FlowFormViewAct/getSeal.act?id=" + seal_id; 
