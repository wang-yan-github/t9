...
function loadFlowPrcs(cntrlId){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/getFlow.act';
...
function loadFlowType(cntrlId){
  var url = contextPath+moduleSrcPath+'/act/T9FlowTypeAct/getFlowTypeJsonByManager.act?checkType=1&sortId=' + sortId;
...
function destroyBySearch(cntrlId){
  ...
  var param = form.serialize();
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/destroyBysearch.act?' + param;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
...
function recoverBysearch(cntrlId){
  ...
  var param = form.serialize();
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/recoverBysearch.act?' + param;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    var count = rtJson.rtData;
...
function destroy(){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/destroy.act?runId=' + ids;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    alert(tooltipMsg7);
  }else{
...
function destroyById(runId){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/destroy.act?runId=' + runId;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
...
function recover(){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/recover.act?runId=' + ids;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    alert("还原成功！");
  }else{
...
function recoverById(runId){
  ...
  var url = contextPath+moduleSrcPath+'/act/T9WorkDestroyAct/recover.act?runId=' + runId;
  var rtJson = getJsonRs(url);
  if(rtJson.rtState == "0"){
    location.reload();
  }else{
