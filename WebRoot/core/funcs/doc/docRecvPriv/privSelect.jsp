...
function submit(){
  if(changeValue == 0){
    ...
    var url = "<%=contextPath %>/t9/core/funcs/doc/act/T9DocRecvPrivAct/addDeptPriv.act?deptIdStr="+$('deptStr').value+"&userIdStr="+$('userStr').value;
    ...
    if (!$('privStr').value) {
	  ...
	  var url = "<%=contextPath %>/t9/core/funcs/doc/act/T9DocRecvPrivAct/addAllPriv.act?privIdStr="+$('privStr').value;
	  var rtJson = getJsonRs(url);
	  if(rtJson.rtState == "0"){
		  alert("全局权限设置成功！");
		  window.opener.location.reload();
