...
var parentStr = "";
function doInit(){
  var url = "<%=contextPath%>/t9/core/funcs/doc/send/act/T9SubjectTermAct/getDept.act?treeId=" + treeId;
...
function commitDept(){
  if(checkForm()&& checkWord()){ 
    var url = "<%=contextPath%>/t9/core/funcs/doc/send/act/T9SubjectTermAct/updateWord.act?treeId=" + treeId;
...
function deleteDept(){
  if(confirm("确认要主题词类别/主题词吗？这将同时删除所有下级类别和和其中的主题词！")){
    var url = "<%=contextPath%>/t9/core/funcs/doc/send/act/T9SubjectTermAct/deleteDept.act?treeId=" + treeId;
    var rtJson = getJsonRs(url, mergeQueryString($("form1"))); 
    if(rtJson.rtState == '0'){
  	  var curTree = parent.wordListTree.tree;
...
function nextWord(){
  location.href="/t9/core/funcs/doc/docword/wordInput.jsp?treeId="+treeId;
}

function deptFunc(){
  var url = "<%=contextPath%>/t9/core/funcs/doc/send/act/T9SubjectTermAct/selectWordToAttendance.act";
...
function checkWord(){
  var rtJson = getJsonRs(contextPath + "/t9/core/funcs/doc/send/act/T9SubjectTermAct/getAjaxCheck.act", mergeQueryString($("form1")));
  if(rtJson.rtState=="0"){
