...
window.getCalendar = function (index){
  ...
  var URL = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/selectCalendarToDisk.act?date=" + date + "&index=" + index ;
...
window.updateStatus = function (seqId,status){
  var URL = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/updateStatusById.act?seqId="+seqId+"&status="+status;
  var rtJson = getJsonRs(URL);
  if(rtJson.rtState == "1"){
    alert(rtJson.rtMsrg); 
    return ;
...
window.getAffair = function (index,calendarRecord){
  ...
  var currMonth = currMonth+1;
  var currYear = currDate.getFullYear(); 
  var URl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9AffairAct/selectAffairToDisk.act?year="+currYear+"&month="+currMonth+"&day="+currDay+"&index="+index+"&calendarRecord="+calendarRecord;
...
window.task = function (){
  var URl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9TaskAct/selectTask.act";
  var rtJsons = getJsonRs(URl);
  if(rtJsons.rtState == "1"){
    alert(rtJsons.rtMsrg); 
    return ;
