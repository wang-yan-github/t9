...
function doInit() {
  var url =  contextPath + "/t9/core/funcs/person/act/T9UserPrivAct/getUserPrivList.act";
  var cfgs = {
    dataAction: url,
...
function getCountFunc(cellData, recordIndex, columIndex){
  ...
  var count3 = "";
  var seqId = this.getCellData(recordIndex,"seqId");
  var url = "<%=contextPath %>/t9/core/funcs/person/act/T9UserPrivAct/getAllUsers.act";
  var rtJsonAll = getJsonRs(url, "seqId=" + seqId);
  if (rtJsonAll.rtState == "0") {
    count1 = rtJsonAll.rtData ;
  }
  var url = "<%=contextPath %>/t9/core/funcs/person/act/T9UserPrivAct/getNotLoginUser.act";
  var rtJson = getJsonRs(url, "seqId=" + seqId);
  if (rtJson.rtState == "0") {
    count2 = rtJson.rtData ;
  }
  var url = "<%=contextPath %>/t9/core/funcs/person/act/T9UserPrivAct/getOtherUser.act";
...
function doDetail(seqId, privName){
  var URL = "/t9/core/funcs/userpriv/showUsers.jsp?seqId="+seqId+"&privName="+encodeURIComponent(privName);
...
function doDelete(seqId){
  ...
  var url = "<%=contextPath %>/t9/core/funcs/person/act/T9UserPrivAct/getUserPrivNo.act";
  var rtJson = getJsonRs(url, "seqId=" + seqId);
  if (rtJson.rtState == "0") {
    if(rtJson.rtData > "0"){
      location = "<%=contextPath %>/core/funcs/userpriv/deletepriv.jsp";
     }else{
       var urls = "<%=contextPath %>/t9/core/funcs/person/act/T9UserPrivAct/deleteUserPriv.act";
