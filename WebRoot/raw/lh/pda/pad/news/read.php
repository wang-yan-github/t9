...
<? } ?>
   ...
   oImg.each(function(){
	...
	tiScroll_1_main.getOIScroll().refresh();
   function preLoadImage(obj, url)
   {
      ...
      { 
         ...
         setTimeout(function(){tiScroll_1_main.getOIScroll().refresh()},100);//延时触发 修复滚动条高度问题 by JinXin @ 2012/9/7
         return; //直接返回，不用再处理onload事件
      }
      img.onload = function () 
      {
         obj.empty().append(img);
         setTimeout(function(){tiScroll_1_main.getOIScroll().refresh()},100);
			tiScroll_1_main.getOIScroll().refresh();
      };
