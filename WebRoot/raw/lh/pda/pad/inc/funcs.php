<?
   include_once("pda/pad/config.php");
   ...
   //默认每页显示数据条数
   $PAGE_SIZE = Ag("iPad") ? $fix_for_pad['list-show-num'] : 10;
   ...
   function buildSiderHead($data)
   {
      if(!is_array($data))
         return;
      
      $html = '<div id="siderheader" class="header">';
      foreach($data as $k => $v)
      {
         $style = "";
         if(isset($v['display'])){
         ...
         $html.= '<div id="siderheader_'.$k.'"'.$style.'>';
         ...
         if($v["r"])
         {
            ...
            $v["r"]["event"] == "" ? $levent = "" : $levent = "onclick='".$v["r"]["event"]."'"; 
				$classPrefix = $rclass == " icon-group" ? 'rbtn' : 'combtn rbtn';
            $html.=  '<span class="'.$classPrefix.$rclass.'" '.$levent.$style.'>
   ...
   // 2012/2/23 0:17:54 lp 生成页面导航
   function buildSiderFoot($data)
   {
      if(!is_array($data))
         return;
      
      $html = '<div id="siderfooter" class="footer">';
      foreach($data as $k => $v)
      {
         $style = "";
         if(isset($v['display'])){
         ...
         $html.= '<div id="siderfooter_'.$k.'"'.$style.'>';
         
         if($v["l"])
         { 
            $v["l"]["class"] == "" ? $lclass = "" : $lclass = " ".$v["l"]["class"];
            $v["l"]["event"] == "" ? $levent = "" : $levent = "onclick='".$v["l"]["event"]."'";   
				$classPrefix = $lclass == " icon-group" ? 'lbtn' : 'lcbtn';
            $html.=  '<span class="'.$classPrefix.$lclass.'" '.$levent.'>
                        <span>'._($v["l"]["title"]).'</span>
                     </span>';
         }
   ...
   // 2012/2/23 0:17:54 lp 生成页面导航
   function buildMainHead($data)
   {
      if(!is_array($data))
         return;
      
      $html = '<div id="mainheader" class="header">';
      foreach($data as $k => $v)
      {
         $style = "";
         if(isset($v['display'])){
         ...
         $html.= '<div id="mainheader_'.$k.'"'.$style.'>';
   ...
   // 2012/7/30 0:22:48 lp 生成页面Loading效果
   function buildSideProLoading()
   {
      return '<div id="sideoverlay" class="overlay"></div>
   ...
   function buildMainProLoading()
   {
      return '<div id="mainoverlay" class="overlay"></div>
               <div id="main-ui-loader" class="ui-loader loading">
                  <span class="ui-icon ui-icon-loading"></span>
                  <h1>'._("加载中...").'</h1>   
               </div>';   
   }
