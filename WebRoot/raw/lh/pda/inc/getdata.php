...
}else if($STYPE=="email")
{
   if($A=="GetNew")
   {
      ...
      if($count == 0)
      {
         ...
         $query = "SELECT EMAIL_ID,FROM_ID,SUBJECT,READ_FLAG,from_unixtime(SEND_TIME) as SEND_TIME,IMPORTANT,ATTACHMENT_ID,ATTACHMENT_NAME,USER.USER_NAME from EMAIL,EMAIL_BODY left join USER on EMAIL_BODY.FROM_ID=USER.USER_ID where EMAIL_BODY.BODY_ID=EMAIL.BODY_ID and BOX_ID=0 and TO_ID='$LOGIN_USER_ID' and SEND_FLAG='1' and (DELETE_FLAG='' or DELETE_FLAG='0' or DELETE_FLAG='2') AND EMAIL_ID > $LASTEDID order by SEND_TIME DESC";   
      }
   }else{
      $query = "SELECT EMAIL_ID,FROM_ID,SUBJECT,READ_FLAG,from_unixtime(SEND_TIME) as SEND_TIME,IMPORTANT,ATTACHMENT_ID,ATTACHMENT_NAME,USER.USER_NAME from EMAIL,EMAIL_BODY left join USER on EMAIL_BODY.FROM_ID=USER.USER_ID where EMAIL_BODY.BODY_ID=EMAIL.BODY_ID and BOX_ID=0 and TO_ID='$LOGIN_USER_ID' and SEND_FLAG='1' and (DELETE_FLAG='' or DELETE_FLAG='0' or DELETE_FLAG='2') order by SEND_TIME desc limit ".$CURRITERMS.",$PAGE_SIZE"; 
      $count = resultCountByROW($query);
   ...
   while($ROW=mysql_fetch_array($cursor))
   {
      ...
      $RETURN .= '<li class="'.$fix_for_pad['list-li-style'].$Class.'" q_id="'.$EMAIL_ID.'">
         ...
         if($count_calendar > 0)
         {
            ...
            while($ROW=mysql_fetch_array($cursor))
            {
               ...
               $RETURN .= '<li class="'.$fix_for_pad['list-li-style'].'" q_id="'.$CAL_ID.'" >
                                 ...
                                 <p class="w100 grapc">'.$CAL_LEVEL_DESC." ".$CONTENT.'</p>
                                 <span class="ui-icon-rarrow"></span>
                           </li>'; 
            }
         }
         ...
         if($count_affair > 0)
         {
            ...
            while($ROW=mysql_fetch_array($cursor))
            {
             ...
             $RETURN .= '<li class="'.$fix_for_pad['list-li-style'].'">
                                 ...
                                 <p class="w100 grapc">'._("周期性事务：").$CONTENT.'</p>
                           </li>';    
            }
         }
...
}else if($STYPE=="file_folder")
{
   ...
   if($A == "getPsnList")
   {
      ...
      while($ROW=mysql_fetch_array($cursor))
      {
         ...
         <p class="grapc">'._("单位").':'.$DEPT_NAME.'</p>
         <span class="ui-icon-rarrow"></span>
         </li>';
      }
      echo $RETURN;
   ...
   $WHERE_STR = " WHERE FLOW_RUN_PRCS.RUN_ID=FLOW_RUN.RUN_ID and FLOW_RUN.FLOW_ID=FLOW_TYPE.FLOW_ID and USER_ID='$LOGIN_USER_ID' and DEL_FLAG=0 and PRCS_FLAG<'3'";
   ...
   while($ROW=mysql_fetch_array($cursor))
   {
      ...
      $RETURN.='<li class="'.$fix_for_pad['list-li-style'].$Class.'" q_id="'.$CREATE_TIME.'" q_run_id="'.$RUN_ID.'"  q_flow_id="'.$FLOW_ID.'"  q_prcs_id="'.$PRCS_ID.'"  q_flow_prcs="'.$FLOW_PRCS.'" q_op_flag="'.$OP_FLAG.'">
            ...
            <p class="grapc">'.$PRCS_NAME.' '.$OP_FLAG_DESC.'</p>
            <span class="ui-icon-rarrow"></span>
      </li>';
   }     
   echo $RETURN;
      ...
      $MSG_NUM = count($MSG_LIST);
		...
		foreach($MSG_LIST as $k => $v)
		{
	      if($USER_ARRAY[$v[0]]['NAME']=="") continue;
			...
			$str .= 			'<p class="grapc">'.strip_tags($v[3]).'&nbsp;</p>';
