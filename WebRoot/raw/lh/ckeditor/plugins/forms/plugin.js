...
CKEDITOR.plugins.add('forms', {
  init : function(a) {
    ...
    var c = function(e, f, g) {
      ...
      a.ui.addButton(e, {
        label : b.common[e.charAt(0).toLowerCase() + e.slice(1)],
        command : f
      });
    ...
    if (a.addMenuItems)
      a.addMenuItems( {
        ...
        checkbox : {
          label : b.checkboxAndRadio.checkboxTitle,
          command : 'checkbox',
          group : 'checkbox'
        },
        radio : {
          label : b.checkboxAndRadio.radioTitle,
          command : 'radio',
          group : 'radio'
        },
        textfield : {
          label : b.textfield.title,
          command : 'textfield',
          group : 'textfield'
        },
        hiddenfield : {
          label : b.hidden.title,
          command : 'hiddenfield',
          group : 'hiddenfield'
        },
        imagebutton : {
        ...
        button : {
          label : b.button.title,
          command : 'button',
          group : 'button'
        },
        select : {
          label : b.select.title,
          command : 'select',
          group : 'select'
        },
        textarea : {
          label : b.textarea.title,
          command : 'textarea',
          group : 'textarea'
        }
      });
      ...
      a.contextMenu.addListener(function(e) {
        if (e) {
          ...
          if (f == 'input') {
            ...
            if (g == 'text' || g == 'password')
              return {
                textfield : CKEDITOR.TRISTATE_OFF
              };
            if (g == 'button' || g == 'submit' || g == 'reset')
              return {
          ...
          if (f == 'img'
              && e.getAttribute('_cke_real_element_type') == 'hiddenfield')
            return {
...
if (CKEDITOR.env.ie)
  CKEDITOR.dom.element.prototype.hasAttribute = function(a) {
    var d = this;
