...
function delete_all(){
  ...
  if(!window.confirm("确认要删除已选中的培训计划吗？")) {
	...
	var requestURLStr = contextPath + "/t9/subsys/oa/training/act/T9HrTrainingPlanAct";
...
function getSysRemind(remidDiv,remind){ 
  var requestUrl = contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=61"; 
...
function moblieSmsRemind(remidDiv,remind){ 
  var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=61"; 
  var rtJson = getJsonRs(requestUrl); 
    if(rtJson.rtState == "1"){ 
    alert(rtJson.rtMsrg); 
    return ; 
