...
function printMail() {
  ...
  if(window.confirm(msg)) {
    URL = "<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/updatePrint.act?printStr=" + printStr
    window.open(URL);
  }
}
...
function reimbursemenMail() {
  ...
  msg = '确认要报销所选记录？';
  if (window.confirm(msg)) {
    URL = "<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/expenseOut.act?printStr=" + printStr
    window.open(URL);
  }
}
 //作废
function makeWaste() {
  ...
  msg = '确认要作废所选记录？';
  if(window.confirm(msg)) {
    URL = "<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/makeWaste.act?printStr=" + printStr
    window.open(URL);
  }
}
//时间
...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/expenseSelect2.act";
   cfgs = {
    dataAction: url,
    container: "giftList",
    paramFunc: getParam,
...
function chargeDetail(seqId) {
  ...
  window.open("<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/selectExpense.act?seqId=" + seqId,"","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=820,height=650,left="+myleft+",top=50");
}
function editCharge(seqId) {
  myleft = (screen.availWidth-800)/2;
  window.open("<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/selectCharge.act?seqId=" + seqId,"","status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,width=850,height=600,left="+myleft+",top=50");
}
function delCharge(seqId) {
  URL = "<%=contextPath%>/t9/subsys/oa/finance/act/T9ChargeExpenseAct/delCharge.act?seqId=" + seqId;
