...
function doForm() {
  var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/showDetail.act?seqId=" + seqId;
...
function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
  ...
  if (checkForm()) {
    ...
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/updateVote.act";
...
function checkForm2() {
  if (checkForm()) {
    ...
    var pars = $('form1').serialize() ;
    var requestURL = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/updateVote.act";
...
function checkForm3() {
  ...
  if (checkForm()) {
    ...
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/selectId.act?seqId=" + seqId; //查询投票项
    var json = getJsonRs(url);
    if (json.rtState == '1') { 
      alert(json.rtMsrg); 
      return ; 
...
function moblieSmsRemind(remidDiv,remind) {
  var requestUrl = "<%=contextPath%>/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=11";
...
function getSysRemind(){ 
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=11"; 
  var rtJson = getJsonRs(requestUrl); 
  if (rtJson.rtState == "1") { 
    alert(rtJson.rtMsrg); 
    return ; 
...
<input type="hidden" name="dtoClass" value="t9.subsys.oa.vote.data.T9VoteTitle">
...
<form id="formFile" action="<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/uploadFile.act" method="post" enctype="multipart/form-data" target="commintFrame">
