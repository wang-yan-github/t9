...
function doInit(){
  //outVote();
  var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/selectVote.act";
   cfgs = {
    dataAction: url,
    container: "giftList",
    afterShow:insertTr,
  ...
  if(total <= 0){
   ...
   // if (userPriv == "1") {
     // var table = new Element('table',{"class":"TableList","width":"100%"}).update("<tr class='TableControl'><td width='100%' colspan='12'>&nbsp;"
      ...
      //    + "<a href='javascript:onClonVote();' title='复制投票，但不复制数据'><img src='/t9/core/funcs/filefolder/images/copy.gif' align='absMiddle'>克隆</a>&nbsp;"
...
function onClearVote() {
  ...
  if(window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/clearVote.act?seqIds=" + seqIdStr;
...
function onClonVote() {
  ...
  msg = '确认要克隆所选投票数据？';
  if(window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/clonVote.act?seqIds=" + seqIdStr;
    var json = getJsonRs(url);
    window.location.reload();
  }
}
...
function updateNoTopVote() {
  ...
  msg = '确认要取消置顶所选投票数据？';
  if(window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/updateNoTopVote.act?seqIds=" + seqIdStr;
    var json = getJsonRs(url);
    window.location.reload();
  }
}
...
function onAllDelete() {
  ...
  if(window.prompt(msg,"") == "OK"){
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/deleteAllVote.act";
    var json = getJsonRs(url);
    window.location.reload();
  }
}
...
function onDelete() {
  ...
  msg = '确认要删除所选记录？';
  if(window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/deleteVote.act?seqIds=" + seqIdStr;
    var json = getJsonRs(url);
    window.location.reload();
  }
}
...
function toStr(cellData,recordIndex,columIndex){
  ...
  if (toId != "0" && toId != "ALL_DEPT" && toId != "") {
    var str = "?seqId=" + toId + "&tableName=DEPARTMENT&tdName=DEPT_NAME";
    var requestUrl = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/strString.act" + str;
...
function showSubject(seqId) {
  var URL = "/t9/subsys/oa/vote/manage/showVote.jsp?seqId=" + seqId;
...
function updateBeginDate(seqId,tdName,dayTime) {
  ...
  if (numSeqId1 > 0 || numSeqId2 > 0) {
    var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/updateBeginDate.act?seqId=" + seqId + "&tdName=" + tdName + "&dayTime=" + dayTime;
    var json = getJsonRs(url);
    window.location.reload();
  } else {
    alert("请先添加投票项或者子投票！");
...
function updatePublish(seqId,publish) {
  ...
  if (window.confirm(msg)) {
    ...
    var numSeqId2 = itemSeqId(seqId);
    if (numSeqId1 > 0 || numSeqId2 > 0) {
      var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/updatePublish.act?seqId=" + seqId + "&publish=" + publish;
      var json = getJsonRs(url);
      window.location.reload();
    } else {
      alert("请先添加投票项或者子投票！");
...
function itemSeqId(seqId) {
  var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/selectId.act?seqId=" + seqId; 
  var json = getJsonRs(url);
  if (json.rtState == '1') { 
    alert(json.rtMsrg); 
    return ; 
...
var url = "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteTitleAct/selectId2.act?seqId=" + seqId;
var json = getJsonRs(url);
if (json.rtState == '1') { 
  alert(json.rtMsrg); 
  return ; 
