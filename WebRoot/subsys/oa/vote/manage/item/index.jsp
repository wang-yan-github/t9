...
var seqId = "<%=seqId%>";
function doInit () {
  var url= "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteItemAct/selectItemByVoteId.act?voteId="+seqId;
  ...
  var itemList = rtJson.rtData;
    ...
    tr.update("<form name='form0' method='post'id='form0' action='#'><td colspan='4'> 添加项目："
        +"<input type='text' name='itemName' id='itemName0'  value=''>"
...
function addItem(){
  ...
  var url= "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteItemAct/addItem.act?"+queryParam;
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
...
function updateItem(seqId){
  ...
  var url= "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteItemAct/updateItem.act?itemName="+encodeURIComponent(itemName) + "&seqId="+seqId;
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
...
function deleteItem(seqId){
  if(window.confirm("您真的要删除该条选项吗？\n\n注意：该操作不可恢复!")){
    var url= "<%=contextPath%>/t9/subsys/oa/vote/act/T9VoteItemAct/deleteItemById.act?seqId="+seqId;
    var rtJson = getJsonRs(url); 
    if(rtJson.rtState == "1"){
  	alert(rtJson.rtMsrg); 
  	return ; 
