<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.*,java.text.SimpleDateFormat,t9.core.funcs.person.data.T9Person,t9.core.funcs.calendar.data.*"%>
<%@ include file="/core/inc/header.jsp" %>
<%
  String seqId = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("seqId")));
  String subject = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("subject")));
  String createUserId = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("createUserId")));
...
function getMsg(){
  var url = contextPath + "/t9/subsys/oa/netmeeting/text/act/T9NetmeetingAct/getMsg.act?seqId=" + <%=seqId%> + "&count=" + count;
...
function submitSay(){
	...
	var url = contextPath + "/t9/subsys/oa/netmeeting/text/act/T9NetmeetingAct/setMsg.act?seqId=" + <%=seqId%> 
...
function stopMeeting(){
	if(window.confirm('确认要结束会议吗？\n结束后可以在文本会议管理中继续该会议')){
		window.clearInterval(timer);
	  var url = contextPath + "/t9/subsys/oa/netmeeting/text/act/T9NetmeetingAct/stopMeeting.act?seqId=<%=seqId%>";
	  var rtJson = getJsonRs(url);
	  if (rtJson.rtState == "0") {
