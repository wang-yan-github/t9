...
<title>图书详细资料 </title>
<%	
T9BookInfo book = (T9BookInfo)request.getAttribute("aBook");
List<T9BookManage> daipi = (List<T9BookManage>)request.getAttribute("daipi");
List<T9BookManage> weihuan = (List<T9BookManage>)request.getAttribute("weihuan");  
%>
    ...
    <td class="TableData" rowspan="6" width="100">
        <%
          if("".equals(book.getAttachmentId()) || null == book.getAttachmentId()){%>
            <center>暂无封面</center> 
         <% }else{%>
           <a href="<%=contextPath %>/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?attachmentName=<%=book.getAttachmentName() %>&attachmentId=<%=book.getAttachmentId() %>&module=book&directView=1">
           <img src="<%=contextPath %>/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?attachmentName=<%=book.getAttachmentName() %>&attachmentId=<%=book.getAttachmentId() %>&module=book" width='100' border=1 alt="文件名:<%=book.getAttachmentName() %>"></img>
           </a>
         <%} %>          
    </td>
   </tr>
