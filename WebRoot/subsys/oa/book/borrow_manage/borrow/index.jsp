  ...
  <%@ page  import="t9.subsys.oa.book.data.*"%> 
  <%@ page import=" java.net.URLEncoder;" %>
<html>
<head>
<title>借书管理</title>
  <%
    List<T9BookManage> bmanages = (List<T9BookManage>)request.getAttribute("bmanages");
    List<T9BookManage> rmanages = (List<T9BookManage>)request.getAttribute("rmanages");
    List<T9BookManage> regManages = (List<T9BookManage>)request.getAttribute("regManages");
  	T9Page pages = (T9Page)request.getAttribute("page");
  %>
  ...
  <script type="text/javascript">
      ...
      function goPage(currNo){
        var url = contextPath + "/t9/subsys/oa/book/act/T9BookRuleAct/index.act?currNo="+currNo;     
        window.location.href=url;   
      }
      function returnBook(borrowId, bookNo){
       var url = contextPath +"/t9/subsys/oa/book/act/T9BookRuleAct/returnBookByAdmin.act?bookNo="+bookNo +"&borrowId=" +borrowId;
       window.location.href=url;  
      }
  </script>
</head>
  ...
  if(bmanages!=null && bmanages.size()>0){%>
    <table class="TableList" width="95%" align="center">
	      ...
	      <td nowrap align="center">
	      	<a href="<%=contextPath%>/t9/subsys/oa/book/act/T9BookRuleAct/agreeBorrOrNot.act?seqId=<%=bmanages.get(i).getSeqId()%>&flag=1&bookNo=<%=URLEncoder.encode(bmanages.get(i).getBookNo(), "utf-8") %>&toId=<%=bmanages.get(i).getBuserId() %>">
	      	同意</a>
	      	<a href="<%=contextPath%>/t9/subsys/oa/book/act/T9BookRuleAct/agreeBorrOrNot.act?seqId=<%=bmanages.get(i).getSeqId()%>&flag=0&bookNo=<%= URLEncoder.encode(bmanages.get(i).getBookNo(),"utf-8") %>&toId=<%=bmanages.get(i).getBuserId() %>">
	      	退回</a>
	      </td>
      </tr>
		   <%}%>		  
    ...
    <table class="TableList" width="95%" align="center">
		  <tr class="TableHeader">
		    ...
		    <tr class="TableLine1">
		      ...
		      <td nowrap align="center">
		      <a href="<%=contextPath%>/t9/subsys/oa/book/act/T9BookRuleAct/agreeReturnOrNot.act?seqId=<%=rmanages.get(i).getSeqId()%>&flag=1&bookNo=<%= URLEncoder.encode(rmanages.get(i).getBookNo(),"utf-8") %>&toId=<%=rmanages.get(i).getBuserId() %>">同意</a>
		      <a href="<%=contextPath%>/t9/subsys/oa/book/act/T9BookRuleAct/agreeReturnOrNot.act?seqId=<%=rmanages.get(i).getSeqId()%>&flag=0&bookNo=<%= URLEncoder.encode(rmanages.get(i).getBookNo(), "utf-8") %>&toId=<%=rmanages.get(i).getBuserId() %>">退回</a>
