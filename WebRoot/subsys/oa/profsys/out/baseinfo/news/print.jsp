...
<%
List<T9Project> list = (List<T9Project>)request.getAttribute("project");
...
function exportExcel(){
  var url = "<%=contextPath%>/t9/subsys/oa/profsys/act/out/T9ProjectAct/exportXls.act?printStr=" + '<%=printStr%>';
  window.location = url;
}
function doInt() {
  pagesetupNull();
...
for(int i = 0; i < o.length; i++){
  if (!T9Utility.isNullorEmpty(o[i].toString())) {
    newCountry = newCountry + o[i]  + "、";
  } else {
    newCountry = newCountry + o[i]  + "";
  }
}
if (!T9Utility.isNullorEmpty(newCountry)) {
