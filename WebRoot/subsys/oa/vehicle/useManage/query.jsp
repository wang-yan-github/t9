...
function doInit(){
  var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/useManage.act?vuStatus=" + vuStatus;
   cfgs = {
    dataAction: url,
    container: "giftList",
    colums: [
...
function deleteUsage(vuId) {
  ...
  if (window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/deleteVehicle.act?seqId=" + vuId;
    //window.location = url;
...
function usageDetail(vuId) {
  window.open('<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/selectDetail.act?seqId=' + vuId,'','height=500,width=500,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=280,top=150,resizable=yes');
}
//车辆情况
...
function updateVehicle(vuId) {
  var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/selectId.act?seqId=" + vuId
...
function checkup(seqId,status) {
  var msg = '确认要批准该记录吗？';
  if (window.confirm(msg)) {
    var url = '<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/updateStatusId.act?seqId=' + seqId + '&status=' + status;
    var json=getJsonRs(url);
    if (json.rtState == '1') { 
      alert(json.rtMsrg); 
      return ; 
...
function autoUsaBack(){
  var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleAct/getAutoUsageBack.act";
  var json=getJsonRs(url);
  if (json.rtState == '1') { 
    alert(json.rtMsrg); 
    return ; 
