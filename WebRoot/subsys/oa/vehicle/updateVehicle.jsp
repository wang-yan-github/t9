...
<%@page import="t9.subsys.oa.vehicle.data.T9VehicleUsage"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%
T9VehicleUsage usage = (T9VehicleUsage)request.getAttribute("usage");
%>
<html>
<head>
<title></title>
...
function checkForm2() {
  if (checkForm()) {
    ...
    var requestURL="<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/editVehicle.act";
...
//车牌号
function selectNum() {
  var requestURL="<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleAct/selectUseVehicle.act";
...
//调度人员名称
function doName() {
  var requestURLs = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleOperatorAct/selectManagerPerson.act"; 
...
//调度人员名称
function doUserName() {
  var requestURLs = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleOperatorAct/selectPerson.act"; 
  var rtJson = getJsonRs(requestURLs); 
  var prcsJsons = rtJson.rtData; 
  if(rtJson.rtState == '1') {
    alert(rtJson.rtMsrg); 
...
function moblieSmsRemind(remidDiv,remind) {
   var requestUrl = "<%=contextPath%>/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=9";
...
function getSysRemind(){ 
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=9"; 
  var rtJson = getJsonRs(requestUrl); 
  if (rtJson.rtState == "1") { 
    alert(rtJson.rtMsrg); 
    return ; 
...
<input type="hidden" name="dtoClass" value="t9.subsys.oa.vehicle.data.T9VehicleUsage">
...
<table class="TableBlock" align="center" width="90%">
    ...
    <tr>
      ...
      <td class="TableData">
      <%if(T9Utility.isNullorEmpty(usage.getDeptManager())) {%>
