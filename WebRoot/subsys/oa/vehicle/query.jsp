...
var status = "<%=status%>";
function doInit(){
  var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/vehicleSelect.act?status=" + status;
   cfgs = {
    dataAction: url,
    container: "giftList",
    colums: [
...
function updateStatus(vuId) {
  var msg = '确认要结束吗？';
  if (window.confirm(msg)) {
    var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/updateStatus.act?seqId=" + vuId;
    var json=getJsonRs(url);
    if (json.rtState == '1') { 
      alert(json.rtMsrg); 
      return ; 
