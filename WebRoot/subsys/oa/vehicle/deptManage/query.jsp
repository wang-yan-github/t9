...
function doInit(){
  var url = "<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/vehicleDept.act?dmerStatus=" + dmerStatus;
   cfgs = {
    dataAction: url,
    container: "giftList",
    colums: [
...
function usageDetail(seqId) {
  window.open('<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/selectDetail.act?seqId=' + seqId,'','height=500,width=500,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=280,top=150,resizable=yes');
}
//车辆情况
...
function checkup(seqId,dmerStatus) {
  ...
  if (window.confirm(msg)) {
    var url = '<%=contextPath%>/t9/subsys/oa/vehicle/act/T9VehicleUsageAct/updateDmerStatus.act?seqId=' + seqId + '&dmerStatus=' + dmerStatus;
