...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/subsys/oa/asset/act/T9CpCptlRecordAct/listSelect.act?" + param;
    cfgs = {
    dataAction: url,
    container: "giftList",
    colums: [
...
function updateCptl(seqId) {
  var URL = "<%=contextPath%>/t9/subsys/oa/asset/act/T9CpCptlRecordAct/getAssetId.act?seqId=" + seqId;
...
function delDetail(seqId) {
  if (window.confirm("确认要删除该资产使用记录吗？")) {
    var url = "<%=contextPath%>/t9/subsys/oa/asset/act/T9CpCptlRecordAct/getDelete.act?seqId=" + seqId;
...
function toUser(cellData,recordIndex,columIndex){
  ...
  if (cpreUser != "" || cpreUser != null) {
    ...
    var requestUrl = "<%=contextPath%>/t9/subsys/oa/asset/act/T9CpCptlInfoAct/userName.act" + seqId;
...
function toDept(cellData,recordIndex,columIndex){
  ...
  if (useDept != "" || useDept != null) {
    ...
    var requestUrl = "<%=contextPath%>/t9/subsys/oa/asset/act/T9CpCptlInfoAct/userDept.act" + seqId;
    var rtJson = getJsonRs(requestUrl);
    var userList = rtJson.rtData;
    return userList;
  } else {
