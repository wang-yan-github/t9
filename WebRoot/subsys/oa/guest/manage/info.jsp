...
function selectGuest(){
  var url = "<%=contextPath%>/t9/subsys/oa/guest/act/T9GuestAct/queryGuest.act";
   cfgs = {
    dataAction: url,
    container: "orgDiv",
    colums: [
...
function toDept(cellData, recordIndex, columInde){
  ...
  var requestURL = "<%=contextPath%>/t9/subsys/oa/guest/act/T9GuestAct/getDeptNameBySeqIds.act?deptId="+deptId; 
...
function delGuest(seqId){
 if(window.confirm("确认要删除该记录吗？")){
   var requestURL = "<%=contextPath%>/t9/subsys/oa/guest/act/T9GuestAct/delGuest.act?seqId="+seqId; 
   var json=getJsonRs(requestURL); 
   if (json.rtState == '1') { 
     alert(json.rtMsrg); 
     return ; 
