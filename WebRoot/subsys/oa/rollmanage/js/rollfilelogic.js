...
function checkSelectBox(extValue){
  var url = contextPath + "/t9/subsys/oa/rollmanage/act/T9RmsRollAct/getRmsRollSelect.act";
  var rtJson = getJsonRs(url);
...
function checkSelectBox2(extValue){
  var url = contextPath + "/t9/subsys/oa/rollmanage/act/T9RmsRollAct/getRmsRollSelect2.act";
  var rtJson = getJsonRs(url);
  var selectObj =  $('rollId');
  //selectObj.length = 2;
  if(rtJson.rtState == "0"){
...
//所属案卷function rmsRollNameFunc(cellData, recordIndex, columIndex){
	var str = "";
	var url = contextPath + "/t9/subsys/oa/rollmanage/act/T9RmsRollAct/getRmsRollDetail.act";
...
function lendFile(seqId,rollId){
	var msg='确认要借阅该项文件么？';
	if(window.confirm(msg)) {
		var requestURLStr = contextPath + "/t9/subsys/oa/rollmanage/act/T9RmsLendAct";
