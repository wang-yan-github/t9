...
<%
	...
	String subject = request.getParameter("SUBJECT") == null ? "" : T9Utility.encodeSpecial(request.getParameter("SUBJECT"));// 标题包含文字
	String contentNo = request.getParameter("CONTENT_NO") == null ? "" : T9Utility.encodeSpecial(request.getParameter("CONTENT_NO"));// 排序号
	String key1 = request.getParameter("KEY1") == null ? "" : T9Utility.encodeSpecial(request.getParameter("KEY1"));// 关键字
	String key2 = request.getParameter("KEY2") == null ? "" : T9Utility.encodeSpecial(request.getParameter("KEY2"));// 关键字
	String key3 = request.getParameter("KEY3") == null ? "" : T9Utility.encodeSpecial(request.getParameter("KEY3"));// 关键字

	String attachmentDesc = request.getParameter("ATTACHMENT_DESC") == null ? "" : T9Utility.encodeSpecial(request.getParameter("ATTACHMENT_DESC"));// 附件说明包含文字
	String attachmentName = request.getParameter("ATTACHMENT_NAME") == null ? "" : T9Utility.encodeSpecial(request.getParameter("ATTACHMENT_NAME"));// 附件文件名包含文字
	String attachmentData = request.getParameter("ATTACHMENT_DATA") == null ? "" : T9Utility.encodeSpecial(request.getParameter("ATTACHMENT_DATA"));// 附件内容包含文字

	String sendTimeMin = request.getParameter("sendTime_Min") == null ? "" : T9Utility.encodeSpecial(request.getParameter("sendTime_Min"));// 最小日期
	String sendTimeMax = request.getParameter("sendTime_Max") == null ? "" : T9Utility.encodeSpecial(request.getParameter("sendTime_Max"));// 最大日期
%>
...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/subsys/oa/confidentialFile/act/T9ConfidentialContentAct/getGlobalFileListJson.act?" + param;
  var cfgs = {
      dataAction: url,
