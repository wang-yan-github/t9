...
function allQueryGift(){
  ...
  var url = "<%=contextPath%>/t9/subsys/oa/giftProduct/instock/act/T9GiftInstockAct/queryAllGift.act";
   cfgs = {
    dataAction: url,
    container: "giftList",
    paramFunc: getParam,
...
function delGift(seqId){
  ...
  if(window.confirm(msg)){
    var url="<%=contextPath%>/t9/subsys/oa/giftProduct/instock/act/T9GiftInstockAct/delGiftInstock.act?seqIds="+seqId;
...
function delete_arrang(){
  ...
  msg='确认要删除所选办公用品吗？';
  if(window.confirm(msg)){
    var url="<%=contextPath%>/t9/subsys/oa/giftProduct/instock/act/T9GiftInstockAct/delGiftInstock.act?seqIds="+delete_str;
    var json = getJsonRs(url); 
    //alert(rsText);
    if(json.rtState == '1'){ 
      alert(json.rtMsrg); 
