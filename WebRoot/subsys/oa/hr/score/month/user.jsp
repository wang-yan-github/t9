...
function doInit(){	
  ...
  var url= "<%=contextPath%>/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/getScoreOnline.act?flowId=" + flowId + "&userId="+userId+"&currPage="+currPage;
...
function loadData(){
  var url = "<%=contextPath%>/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getScoreFlowMonthData.act?groupId="+groupId;
...
function scoreData(userId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getScoreShow.act?userId=" + userId +"&year=" + year + "&month=" +month; 
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "0"){ 
...
function getOperationFlag(userId, roleId){
  ...
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getOperationFlag.act?userId=" + userId +"&groupId=" + groupIdStr + "&year=" + year + "&month=" + month; 
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "0"){ 
    var data = rtJson.rtData;
    return data;
...
function groupIdFunc(roleId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getGroupId.act?roleId=" + roleId; 
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "0"){ 
    var data = rtJson.rtData;
    return data;
...
function getPageEnd(seqId){
  var URL = "/t9/subsys/oa/examManage/examOnline/queryExam/scoreIndex.jsp?flowId=" + seqId;
  window.location.href = URL;
}


function getGroupFlag(groupId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getGroupFlag.act?groupId=" + groupId; 
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "0"){ 
    var data = rtJson.rtData;
    return data;
...
function getCheckEnd(groupId, userId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getCheckEnd.act?groupId=" + groupId + "&userId=" + userId+"&year=" + year + "&month=" +month; 
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "0"){ 
    var data = rtJson.rtData;
    return data;
