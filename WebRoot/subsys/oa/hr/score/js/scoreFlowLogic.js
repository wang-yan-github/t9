...
function moblieSmsRemind(remidDiv,remind) {
  var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=15";
...
function getSysRemind(){ 
  var requestUrl = contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=15"; 
  var rtJson = getJsonRs(requestUrl); 
  if (rtJson.rtState == "1") { 
    alert(rtJson.rtMsrg); 
    return ; 
...
function deleteSingle(seqId){
  ...
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/deleteSingle.act";
...
function excelReport(seqId,groupId){
  ...
  window.location.href = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/excelExport.act?" + param;
}

//导出分数明细
function excelDetail(seqId,groupId){
  var param = "seqId=" + seqId + "&groupId=" + groupId;
  window.location.href = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/excelDetail.act?" + param;
}
...
function updateBeginDate(seqId,tdName,dayTime) {
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/updateBeginDate.act?seqId=" + seqId + "&tdName=" + tdName + "&dayTime=" + dayTime;
...
function getGroupName(seqId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/getGroupName.act";
  var rtJson = getJsonRs(url, "seqId=" + seqId);
  if (rtJson.rtState == "0") {
...
function getFlowTitleName(seqId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreFlowAct/getFlowTitleName.act";
  var rtJson = getJsonRs(url, "seqId=" + seqId);
  if (rtJson.rtState == "0") {
    return  rtJson.rtData ;
  } else {
 ...
 */
function getGroupName(seqId){
  var url = contextPath + "/t9/subsys/oa/hr/score/act/T9ScoreShowAct/getScoreGroupName.act";
  var rtJson = getJsonRs(url, "seqId=" + seqId);
  if (rtJson.rtState == "0") {
    return  rtJson.rtData ;
  } else {
