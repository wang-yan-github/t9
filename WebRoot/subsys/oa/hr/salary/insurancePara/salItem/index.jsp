  ...
  <%@ page  import="java.lang.*"%>
	...
	<head>
	<%
	List<T9SalItem> SalItemList = (List<T9SalItem>)request.getAttribute("SalItemList");
...
<script type="text/javascript">
	...
	function showFormul(slaItemId){
		  URL="<%=contextPath%>/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/findBJInfo.act?slaItemId="+ slaItemId;
		  myleft=(screen.availWidth-650)/2;
		  window.open(URL,"formulEdit","height=450,width=750,status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,top=150,left="+myleft+",resizable=yes");
		
		// var url  = contextPath + "/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/findBJInfo.act?slaItemId="+ slaItemId;
...
function delSalItemInfo(deleteSeqId){
   ...
   if(!window.confirm(msg)){
     ...
     if(deleteSeqId){
       window.location.href = contextPath + "/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/delSlaItemInfo.act?HiddenId="+deleteSeqId;  
       return true;
      }
    }
 }
...
function delete_all(){
	   ...
	   if(!window.confirm(msg)){
	       ...
	       window.location.href = contextPath + "/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/delAllSlaItemInfo.act";  
	       return true;
	    }
	 }
function updateSalItemInfo(slaItemId){
	 var url  = contextPath + "/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/findSlaItemInfo.act?slaItemId="+ slaItemId;
	  window.location.href=url;
}
</script>
</head>
...
<div  align="center">
<form enctype="multipart/form-data" action="<%=contextPath%>/t9/subsys/oa/hr/salary/insurancePara/salItem/act/T9SalaryItemAct/addSalaryItemAct.act" method="post" name="form1" id="form1">
...
<%for(int i=0; i<SalItemList.size()&&SalItemList.size()>0; i++){ %>
    <tr class="TableData">
      ...
      <%if(!T9Utility.isNullorEmpty(SalItemList.get(i).getIscomputer())&&SalItemList.get(i).getIscomputer().equals("1")){ %>
