...
function doOnload() {
  ...
  var url = "<%=contextPath %>/t9/subsys/oa/hr/setting/act/T9HrCodeAct/selectChildCode.act?seqId="+seqId+"&parentNo="+encodeURIComponent(codeNo);
...
function delCode(seqId){
  if(confirm("确认删除！")) {
    var url = "<%=contextPath %>/t9/subsys/oa/hr/setting/act/T9HrCodeAct/deleteCode.act?seqId="+seqId;
    var rtJson = getJsonRs(url );
    if (rtJson.rtState != "0") {
      alert(rtJson.rtMsrg);
    }
