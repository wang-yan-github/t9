...
<%@ page  import="t9.subsys.oa.hr.manage.staff_contract.data.*"%> 
<%@ page  import="java.util.List"%>
<html>
<head>
<%
List<T9HrStaffContract> contractList = (List<T9HrStaffContract>)request.getAttribute("contractXXInfoList");
T9HrStaffContract contract = contractList.get(0);
...
String removeOrnot = "";
String passOrnot = "";
if(!T9Utility.isNullorEmpty(contract.getRemoveOrNot())){
	removeOrnot = contract.getRemoveOrNot();
}
if(!T9Utility.isNullorEmpty(contract.getPassOrNot())){
	passOrnot = contract.getPassOrNot();
}
if(!T9Utility.isNullorEmpty(contract.getContractType())){//合同类型
	contractType = contract.getContractType();
}
if(!T9Utility.isNullorEmpty(contract.getStatus())){//合同状态
	contractState = contract.getStatus();
}
if(!T9Utility.isNullorEmpty(contract.getContractSpecialization())){//合同属性
...
if(!T9Utility.isNullorEmpty(makeContract) && makeContract!="null"){
...
function CheckForm(){
	...
	if($('PASS_OR_NOT1').checked && document.form1.PROBATION_END_DATE.value!="" && document.form1.TRAIL_OVER_TIME.value!="" && document.form1.TRAIL_OVER_TIME.value > document.form1.PROBATION_END_DATE.value)
	 ...
	 if($('PASS_OR_NOT1').checked && document.form1.PROBATION_END_DATE.value!="" && document.form1.PROBATION_EFFECTIVE_DATE.value!="" && document.form1.PROBATION_END_DATE.value > document.form1.PROBATION_EFFECTIVE_DATE.value)
	   ...
	   document.form1.action = contextPath +"/t9/subsys/oa/hr/manage/staff_contract/act/T9NewContractInfoAct/updateContractInfo.act?seqid="+seqId;
...
function showAttachment(){
	var attachmentId = "<%=T9Utility.null2Empty(contract.getAttachmentId())%>";
	var attachmentName = "<%= T9Utility.encodeSpecial(T9Utility.null2Empty(contract.getAttachmentName()))%>";
...
function deleteAttachBackHand(attachName,attachId,attrchIndex) { 
  var url = "<%=contextPath%>/t9/subsys/oa/hr/manage/staff_contract/act/T9NewContractInfoAct/delFloatFile.act?delAttachId=" + attachId + "&seqId=<%=contract.getSeqId()%>";
