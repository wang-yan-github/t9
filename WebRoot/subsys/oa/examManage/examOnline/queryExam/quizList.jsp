...
function getQuiz(seqId){
  var url= "<%=contextPath%>/t9/subsys/oa/examManage/act/T9ExamQuizAct/getQuizById.act?seqId=" + seqId ;
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
...
function getData(){
  var flowId = '<%=flowId%>';
  var paperTimes = '<%=paperTimes%>';
  var url= "<%=contextPath%>/t9/subsys/oa/examManage/act/T9ExamDataAct/getDataByFlowId.act?flowId=" + flowId + "&paperTimes="+paperTimes+"&userId=<%=userId%>";
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
