...
function doInitFlow(){
  checkTitle(paperId);
  var url = "<%=contextPath%>/t9/subsys/oa/examManage/act/T9ExamFlowAct/selectQIZ.act?paperId=" + paperId + "&flowId=" + flowId;
   cfgs = {
    dataAction: url,
    container: "giftList",
    colums: [
...
function toCount(cellData,recordIndex,columInde) {
  countNum = 0;
  var url = "<%=contextPath%>/t9/subsys/oa/examManage/act/T9ExamFlowAct/showCount.act?flowId=" + flowId;
...
function toshowFalse(cellData, recordIndex,columInde) {
  count = 0;
  var url = "<%=contextPath %>/t9/subsys/oa/examManage/act/T9ExamFlowAct/showCountFalse.act?recordIndex=" + recordIndex + "&flowId=" + flowId;
  var json = getJsonRs(url);
  if(json.rtState == "1") {
    alert(json.rtState);
    return;
...
function checkTitle(paperId) {
  var url = "<%=contextPath %>/t9/subsys/oa/examManage/act/T9ExamFlowAct/showTitle.act?paperId=" + paperId;
  var json = getJsonRs(url);
  if(json.rtState == "1") {
    alert(json.rtState);
    return;
