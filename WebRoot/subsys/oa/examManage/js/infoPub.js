...
function moblieSmsRemind(remidDiv,remind) {
  var requestUrl = contextPath + "/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=36";
...
function getSysRemind(){ 
  var requestUrl = contextPath + "/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=36"; 
  var rtJson = getJsonRs(requestUrl); 
  if (rtJson.rtState == "1") { 
    alert(rtJson.rtMsrg); 
    return ; 
...
function checkForm2() {
  if (checkForm()) {
    ...
    var requestURL = contextPath + "/t9/subsys/oa/examManage/act/T9ExamFlowAct/add.act";
...
function checkForm3() {
  if (checkForm()) {
    var pars = $('form1').serialize() ;
    var requestURL = contextPath + "/t9/subsys/oa/examManage/act/T9ExamFlowAct/updateFlow.act";
    var json = getJsonRs(requestURL,pars);
    if (json.rtState == '1') { 
      alert(json.rtMsrg); 
      return ; 
...
function deleteVote(seqId) {
  ...
  if (window.confirm(msg)) {
    var requestUrl = contextPath + "/t9/subsys/oa/examManage/act/T9ExamFlowAct/deleteFlow.act?seqId=" + seqId;
    var rtJson = getJsonRs(requestUrl);
...
function excelReport(seqId,paperId) {
  window.location.href = contextPath + "/t9/subsys/oa/examManage/act/T9ExamFlowAct/excelReport.act?seqId=" + seqId + "&paperId=" + paperId;
}
//查卷
...
function updateStatus(seqId) {
  var msg ="确认要终止该考试信息！";
  if (window.confirm(msg)) {
    var requestUrl = contextPath + "/t9/subsys/oa/examManage/act/T9ExamFlowAct/updateStatus.act?seqId=" + seqId;
    var rtJson = getJsonRs(requestUrl);
    window.location.reload();
  }
}
