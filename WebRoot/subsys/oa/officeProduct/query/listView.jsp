...
<head>
<%
  String proName = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("proName")));
  String proDesc = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("proDesc")));
  String proCode = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("proCode")));
  String officeDepository = T9Utility.encodeSpecial(T9Utility.null2Empty(request.getParameter("officeDepository")));
...
//申请处理   /t9/subsys/oa/officeProduct/personal/record.jsp
function applicationSingle(proId,typeDepository,typeId){
	
	location.href = contextPath + "/t9/subsys/oa/officeProduct/person/act/T9PersonalOfficeRecordAct/OfficeDepositoryInfo.act?PRO_ID=" +proId+"&DEPOSITORY="+typeDepository+"&TYPE_ID="+typeId;
}
...
function doInit(){
  ...
  var url = "<%=contextPath%>/t9/subsys/oa/officeProduct/query/act/T9OfficeQueryAct/getOfficeProductsListJson.act?"+param;
 //alert(url);
    var cfgs = {
      dataAction: url,
