...
var requestUrl = "<%=contextPath%>/t9/subsys/oa/meeting/act/T9MeetingAct";
var fckContentStr = "";
//附件上传
...
function my_query(){
	...
	var url = "<%=contextPath %>/t9/subsys/oa/meeting/act/T9MeetingEquipmentAct/getEquipmentById.act?mRoom=" + mRoom;
...
function moblieSmsRemind(remidDiv,remind) {
   var requestUrl = "<%=contextPath%>/t9/core/funcs/mobilesms/act/T9MobileSelectAct/isShowSmsRmind.act?type=8";
...
function getSysRemind(){ 
  var requestUrl = "<%=contextPath%>/t9/core/funcs/calendar/act/T9CalendarAct/getSysParaRemind.act?type=8"; 
  var rtJson = getJsonRs(requestUrl); 
  if (rtJson.rtState == "1") { 
    alert(rtJson.rtMsrg); 
    return ; 
...
function checkForm(){
  ...
  if(document.form1.RD.item(1).checked){
    ...
    if(leaveDate1.value == ""){  
     ...
     document.form1.action="<%=contextPath %>/t9/subsys/oa/meeting/act/T9MeetingAct/addMeetingInfo.act";
  }
  ...
  //周期性会议  if(document.form1.RD.item(0).checked){
    ...
    document.form1.action = "<%=contextPath %>/t9/subsys/oa/meeting/act/T9MeetingAct/addCycleMeetingInfo.act";
  }
	return true;
}
...
<form id="formFile" action="<%=contextPath%>/t9/subsys/oa/meeting/act/T9MeetingAct/uploadFile.act" method="post" enctype="multipart/form-data" target="commintFrame">
