...
T9Person loginUser = (T9Person)request.getSession().getAttribute(T9Const.LOGIN_USER);
  ...
  boolean useResInfoSubSys = "1".equals(T9SysProps.getString("useInfoResSubsys"));
%>
...
<script type="text/javascript">
  ...
  //同时，可以设置初始的菜单状况，打开初始文档等等。
   ...
   var URL = contextPath + "/t9/core/funcs/office/ntko/act/T9NtkoAct/upload.act?" + param;
...
<body onload="doInit()">
<form name="form1" id="form1" method="post" action="<%=contextPath %>/t9/core/funcs/office/ntko/act/T9NtkoAct/updateFile.act" enctype="multipart/form-data">
...
<div style="z-index:-1;"> 
<object  id="TANGER_OCX" classid="clsid:C9BC4DFF-4248-4a3c-8A49-63A7D317F404" codebase="<%=contextPath %>/core/cntrls/OfficeControl.cab#version=5,0,1,1" width="99%" height="800">
 ...
 */
 function saveAjax(ids, attachmentId, attachmentName, moudle, zhaiYao){
   ...
   var rtJson = getJsonRs(contextPath + "/t9/subsys/inforesouce/act/T9FileMetaSaveAct/saveFileMeta.act", queryParam);
   if (rtJson.rtState == "0") {
     alert("保存成功!");
   }else {
     alert("保存失败!");
 ...
 **/
 function kengine(){
  // var par = 'attachmentId='+attachmentId; \t9\subsys\inforesouce\act\T9FileMetaSaveAct
   var url = contextPath+ "/t9/subsys/inforesouce/act/T9FileMetaSaveAct/getKengine.act?attachmentId="+attachmentId;
