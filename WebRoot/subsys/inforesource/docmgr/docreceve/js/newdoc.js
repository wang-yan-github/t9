function parseObj(docType, id){
  ...
  function showAttach(attrIds,attrNames,cntrId){
    ...
    for(var i = 0 ; i <= attrIdArrays.length; i++){
      ...
      attachMenuUtil("showAtt","<%=T9DocConst.MODULE%>",null,$('attachmentName').value ,$('attachmentId').value,false);
    }
    reStr += "</div>";
    if(cntrId){
...
//浮动菜单文件的删除 
  ...
  var url= contextPath + "/t9/subsys/inforesouce/docmgr/act/T9DocReceiveAct/delFloatFile.act?attachId=" + attachId +"&attachName=" + attachName ; 
  if (seqId) {
    url += "&seqId=" + seqId;
  }
  ...
  function getMaxOrderNo(typeId){
    var url = contextPath +"/t9/subsys/inforesouce/docmgr/act/T9DocReceiveAct/getMaxOrderNo.act?typeId=" + typeId;
