...
/**
  ...
  */
  ...
  function Search(currNo2,pageSize){      
    var keyword = $("#searchText").val(); 
      ...
      $.ajax({
              ...
              type: 'post',
              dataType: 'json',
              url: contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getFullTextDocList.act?nStartPage="+(currNo2-1)+"&nPageSize="+defaultPageSize,
              data: { q: keyword},
              timeout:TIME_OUT,
              success: function (data) {
                if(data){
   ...
   */
    function getArticleTags(fileId){   
      var keyword = "";
          $.ajax({
            url:contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getArticleTags.act?fileId=" + fileId ,
            type:"post",  
            async:false,  
            timeout:TIME_OUT,
            success:function(data){
    ...
    //点击树节点后要显示的文件列表-------------------------------Begin-------------qwx----------------------//  
     ...
     /**
      ...
      function relationWordAjax(word){  
        ...
        $.ajax({
          url:contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getFullText.act",
          type:"get",  
   ...
   function findNameAndMoudle(fileId){
     $.ajax({
       url:contextPath + "/t9/subsys/inforesouce/act/T9AFileMateAct/findFileNameAndMoudle.act",
       type:"get",             
   ...
   function zhaiYao(fileId, len){//通过sign_files表 中的 file_id查询摘要内容(ABSTRACT)
     ...
     var vale= "";
     $.ajax({
      url:contextPath + "/t9/subsys/inforesouce/act/T9FileMetaSaveAct/getzhaiYao.act",
      type:"post",
      data:"fileId="+fileId,  
      async:false, 
      timeout:TIME_OUT,
 ...
 //选中文本类型||图片类型||视频类型
   function findTypeMenu(typemenu){   
     ...
     $.ajax({
       url:contextPath + "/t9/subsys/inforesouce/act/T9MateTreeAct/findMymoth.act?typemenu="+typemenu,
       type:"post",
       success:function(data){   
         var obj = eval("(" + data +")");
         window.trData = obj;
