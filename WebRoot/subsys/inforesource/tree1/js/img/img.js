...
function doSeachAllImage(currNo){  
  $(".next-page").html("");  
  $.ajax({
    url:contextPath + "/t9/subsys/inforesouce/act/T9ManageImgAct/findAllImages.act",
    type:"post",             
    data:"currNo="+ currNo,   
 ...
 */
function tomanage(){ 
  var url = contextPath + "/t9/subsys/inforesouce/act/T9MateShowAct/toManage.act?ftype=2";
...
function findTypeMenu(){
  $.ajax({
    url:contextPath + "/t9/subsys/inforesouce/act/T9MateTreeAct/findMymoth.act?typemenu=2",
    type:"post",
    success:function(data){   
      var obj = eval("(" + data +")");
      window.trData = obj;
...
function returnAmate(newsId){ 
  var temp = "";
  $.ajax({
    url:contextPath + "/t9/subsys/inforesouce/act/T9ManageImgAct/findAmate.act",
    type:"post",
