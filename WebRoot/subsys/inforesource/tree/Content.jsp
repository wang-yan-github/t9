 ...
 */
	 function ajax(){
	   var url = contextPath + "/t9/subsys/inforesouce/act/T9MateNodeAct/findTagNameAjax.act";//
...
function modAjax(modules, treeNode){   
  ...
  $.ajax({
		url:contextPath + "/t9/subsys/inforesouce/act/T9FileMetaSaveAct/findFileList.act" ,
	  type:"post",
...
function getRelationArticleTags(fileId){
  $.ajax({
		url:contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getRelationArticleList.act?fileId="+ fileId,
	  type:"post",	
		success:function(data){	
    $("#Related-bg").empty();
    $("#page-b").empty();
...
function getUrl(fName, file_id, moudle){
  ...
  function articleListByKeyIDs(keyIds,currNo2){
    $(".hiddiv").attr("id","tab-a-con");
	  $.ajax({
	    url:contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getArticleListByKeyIDs.act?KeyIDs=" + keyIds +"&nStartPage=" + (currNo2-1) +"&nPageSize=8",
	    type:"post",
	    success:function(data){
	       $("#dataa").empty();	       
	       var toJson = eval("("+data+")");
	       if(toJson){
	          ...
	          if(len>0){
	           setLen(len);
		          var temp = "";
		          for(var j=0; j<len; j++){
			          var tName = rtdata[j].FILE_PATH;	//D:\t9\attach\email\1109\681b8d81a339a1a3b4e22e0661d8a55f_文档 7153.docx
			          var subtitle = "";		          
			          var moudle = tName.substring(tName.indexOf("\\attach\\")+8,  tName.length);			          
  ...
  **/
  ...
  function getFullText(data){
     $.ajax({
       url:contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getFullText.act?q="+data+"&limit="+10,
       type:"post",
       success:function(data){
  ...
  *///高级搜索引擎   在文本框中输入字 会自动弹出相关的词
  $(document).ready(function () {
    var url = contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getFullText.act";
    try{
   ...
   function Search(currNo2,pageSize){
          var keyword = $("#searchDate").val();
            ...
            $.ajax({
                    ...
                    type: 'GET',
                    dataType: 'json',
                    url: contextPath + "/t9/subsys/inforesouce/act/T9OutURLAct/getFullTextDocList.act?nStartPage="+(currNo2-1)+"&nPageSize="+pageSize,
