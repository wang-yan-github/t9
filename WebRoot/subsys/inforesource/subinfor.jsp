...
<%@ page  import="t9.subsys.inforesouce.data.*"%>
<html>
<head>
<title>讨论区设置</title>
...
<link rel="stylesheet" type="text/css" href="/t9/rad/dsdef/css/tableList.css" />
 ...
 -->
<%
List<T9MateType> matas = (List<T9MateType>)request.getAttribute("mates");
...
function deleteBookType(seqId){
  ...
  if(!window.confirm(msg)){
    ...
    if(seqId && seqId!="null"){
     // document.form1.action = contextPath + "/t9/subsys/inforesouce/act/T9MateTypeAct/deleteSubMata.act";  
     // document.form1.submit(); 
      window.location.href = contextPath + "/t9/subsys/inforesouce/act/T9MateTypeAct/deleteSubMata.act?seqid="+ seqId;
      return true;
    }
  }
}
   	...
   	if(matas!=null && matas.size()>0){
   	  ...
   	  %>
	      ...
	      <td nowrap align="center">
	      	<%
	      		if("1".equalsIgnoreCase(matas.get(i).getElementId())){%>
	      		  <a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateElementAct/selectvalue.act?seqid=<%=matas.get(i).getSeqId()%>&&number=<%=matas.get(i).getNumberId()%>">值域定义</a> 	    		
	      		<%}else if("2".equalsIgnoreCase(matas.get(i).getElementId())){%>
	      		  <a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateTypeAct/findSubMata.act?seqid=<%=matas.get(i).getSeqId()%>">子元素定义</a>	   
	      		<%}else if("3".equalsIgnoreCase(matas.get(i).getElementId())){%>
	      		  <a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateElementAct/selectvalue.act?seqid=<%=matas.get(i).getSeqId()%>&&number=<%=matas.get(i).getNumberId()%>">值域定义</a> 	 
	      		  <a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateTypeAct/findSubMata.act?seqid=<%=matas.get(i).getSeqId()%>">子元素定义</a>
	      		<%}%>	   
	      		<a href="javascript:deleteBookType(<%=matas.get(i).getSeqId()%>);">删除</a>	
	      		<!--<a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateTypeAct/deleteSubMata.act?seqid=<%=matas.get(i).getSeqId()%>">删除</a>	  
	      		--><a href="<%=contextPath%>/t9/subsys/inforesouce/act/T9MateTypeAct/editAMeta.act?seqid=<%=matas.get(i).getSeqId()%>&pId=<%=matas.get(i).getParentId()%>">编辑</a>	        	         
